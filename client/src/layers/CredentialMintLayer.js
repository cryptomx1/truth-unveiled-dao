...
export class CredentialMintLayer {
    ...
    STORAGE_KEY = 'credential_mint_history';
    PATH_B_THRESHOLD = 10; // 10% failure rate
    ISSUER_DID = 'did:civic:system:credential_minter';
    constructor() {
        this.localSaveLayer = new LocalSaveLayer();
        this.loadMintHistory();
    }
    // Main credential minting function
    async mintCredential(did, credentialType, metadata, issuerDID) {
        ...
        try {
            ...
            return this.createFailureResult(error instanceof Error ? error.message : 'Unknown mint error', startTime);
        }
    }
    // Revoke credential by ZKP hash
    async revokeCredential(zkHash, reason) {
        const startTime = Date.now();
        try {
            // Find credential in history
            const historyIndex = this.mintHistory.findIndex(entry => entry.credentialZkHash === zkHash && !entry.revoked);
            if (historyIndex === -1) {
                return {
                    success: false,
                    zkHash,
    ...
    async generateZKPHash(did, credentialType, metadata) {
    ...
    validateMintInput(did, credentialType, metadata) {
    ...
    updateMetrics(operationTime, operation) {
    ...
    async activatePathBFallback(did, credentialType, metadata, issuerDID) {
        try {
    ...
    mintGovernanceCredential(did, role, permissions) {
    ...
    mintIdentityCredential(did, attestations) {
    ...
    mintVaultCredential(did, vaultAccess, exportRights) {
