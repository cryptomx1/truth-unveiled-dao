...
import { CredentialMintLayer } from './CredentialMintLayer';
...
export class VaultExportNode {
    ...
    STORAGE_KEY = 'vault_export_history';
    PATH_B_THRESHOLD = 10; // 10% sync failure rate
    MAX_BUNDLE_SIZE = 5 * 1024 * 1024; // 5MB limit
    SYSTEM_DID = 'did:civic:system:vault_export';
    constructor() {
    ...
    async exportVaultData(exporterDID, userRole, filters, overrideFlag) {
        ...
        try {
            ...
            return this.createFailureResult(error instanceof Error ? error.message : 'Unknown export error', startTime);
        }
    }
    // Validate export permissions based on role
    validateExportPermissions(exporterDID, userRole, filters, overrideFlag) {
        // Validate DID format
    ...
    // Apply filters and role-based scoping
    applyFiltersAndScoping(mintHistory, exporterDID, userRole, filters) {
        ...
        if (filters.credentialTypes && filters.credentialTypes.length > 0) {
            filteredEntries = filteredEntries.filter(entry => filters.credentialTypes.includes(entry.credentialType));
        }
    ...
    async createExportBundle(exporterDID, userRole, filters, entries) {
    ...
    async generateBundleCID(bundleId, entries) {
    ...
    async generateCIDSignature(bundleCID) {
    ...
    determineExportScope(userRole, filters) {
        if (userRole === 'Citizen')
            return 'own';
        if (userRole === 'Moderator')
            return 'jurisdiction';
        if (userRole === 'Governor' && !filters.targetDID)
            return 'global';
        return 'own';
    }
    // Activate Path B export fallback
    async activatePathBExport(exporterDID, userRole, filters, entries, startTime) {
        try {
    ...
    async createMockBundle(exporterDID, userRole, filters, entries) {
    ...
    updateMetrics(exportTime, entryCount, bundleSize) {
    ...
    async exportByCredentialType(exporterDID, userRole, credentialTypes, overrideFlag) {
        const filters = {
            credentialTypes,
            excludeRevoked: true,
    ...
    async exportByTimestampRange(exporterDID, userRole, startDate, endDate, overrideFlag) {
        const filters = {
            timestampRange: {
                start: startDate,
                end: endDate
    ...
    async exportByDID(exporterDID, userRole, targetDID, overrideFlag) {
        const filters = {
            targetDID,
            excludeRevoked: true,
            requireZKPValidation: true
