...
const DeckTutorialPage = () => {
    const { deckId } = useParams();
    return (<TutorialInterface deckId={deckId || 'unknown'} moduleId="overview" title={`Deck ${deckId} Tutorial`} type="deck"/>);
};
const ModuleTutorialPage = () => {
    const { moduleId } = useParams();
    return (<TutorialInterface deckId="module-tutorial" moduleId={moduleId || 'unknown'} title={`Module ${moduleId} Tutorial`} type="module"/>);
};
const TutorialInterface = ({ deckId, moduleId, title, type }) => {
    ...
    const initializeTutorialAudio = async () => {
        ...
        try {
            ...
            const cid = await ttsAgent.generateNarrationWithIPFS(deckId, moduleId, tutorialContent, 'encouraging');
            if (cid) {
    ...
    const getTutorialContent = (deckId, moduleId, type) => {
        const deckNames = {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 p-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 mb-6">
          ...
          {!isLoading && !error && audioElement && (<>
              ...
              <div className="flex items-center justify-center gap-4">
                ...
                <Button onClick={handleReplay} variant="outline" size="lg" className="w-12 h-12 rounded-full">
