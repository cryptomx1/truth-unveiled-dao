...
import { CIDTier, validateCIDAccess } from '../access/CIDTierMap';
 ...
 */
const AccessDeniedCard = ({ requiredTier, currentTier, deckName }) => {
    useEffect(() => {
    ...
    return (<div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-8 max-w-md w-full text-center">
        ...
        <button onClick={() => window.location.href = '/onboarding-required'} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium transition-colors" aria-label="Go to onboarding to upgrade access tier">
          Upgrade Access
        </button>
      </div>
    </div>);
 ...
 */
const DeckOverview = ({ deckId, userSession }) => {
    ...
    if (!validateCIDAccess(userSession.cid, deckConfig.requiredTier)) {
        return (<AccessDeniedCard requiredTier={deckConfig.requiredTier} currentTier={userSession.tier} deckName={deckConfig.name}/>);
    }
    useEffect(() => {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {deckConfig.modules.map((module) => {
            ...
            return (<div key={module.id} className={`
                ...
                }} role="button" tabIndex={canAccess ? 0 : -1} aria-label={`${canAccess ? 'Open' : 'Locked'} ${module.name} module`}>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-semibold text-white">{module.name}</h3>
                  {canAccess ? (<span className="text-green-400 text-xl">🟢</span>) : (<span className="text-red-400 text-xl">🔒</span>)}
                </div>
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  {module.truthPointsRequired && (<span className={`
        ...
        <div className="mt-12 flex justify-center">
          <button onClick={() => window.location.href = '/command'} className="bg-slate-700 hover:bg-slate-600 text-white px-6 py-3 rounded-lg font-medium transition-colors" aria-label="Return to Civic Command Center">
 ...
 */
const ModuleView = ({ deckId, moduleId, userSession, subpageId }) => {
    const deckConfig = DECK_CONFIGS[deckId];
    ...
    if (!deckConfig || !moduleConfig) {
        ...
        <div className="text-center text-white">
          ...
          <button onClick={() => window.location.href = '/command'} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium transition-colors mt-4">
    ...
    if (!validateCIDAccess(userSession.cid, moduleConfig.requiredTier)) {
        return (<AccessDeniedCard requiredTier={moduleConfig.requiredTier} currentTier={userSession.tier} deckName={`${deckConfig.name} - ${moduleConfig.name}`}/>);
    }
    ...
    return (<div className="min-h-screen bg-slate-900">
      ...
      <div className="bg-slate-800 border-b border-slate-700 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <button onClick={() => window.location.href = `/deck/${deckId}`} className="bg-slate-700 hover:bg-slate-600 text-white px-4 py-2 rounded-lg font-medium transition-colors" aria-label={`Return to ${deckConfig.name} deck overview`}>
            ← Back to Deck
          </button>
        </div>
      </div>
...
const DeckViewRouter = () => {
    ...
    const getDeckIdFromNumber = (deckNum) => {
        const deckMapping = {
            '1': 'wallet-overview',
            '2': 'governance',
            '3': 'education',
    ...
    return (<Switch>
      ...
      <Route path="/deck/:deckId/:moduleId">
        {(params) => {
            ...
            if (deckConfig) {
                ...
                if (moduleExists) {
                    return (<ModuleView deckId={mappedDeckId} moduleId={moduleId} userSession={userSession}/>);
                }
            }
            ...
            return (<div className="min-h-screen bg-slate-900 flex items-center justify-center">
              <div className="text-center text-white">
                ...
                <button onClick={() => window.location.href = '/command'} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium transition-colors">
                  Return to Command Center
                </button>
              </div>
            </div>);
      ...
      <Route path="/deck/:deckId">
        {(params) => {
            ...
            return (<DeckOverview deckId={mappedDeckId} userSession={userSession}/>);
        }}
      </Route>
