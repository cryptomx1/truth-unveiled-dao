...
const TutorialPage = ({ deckId }) => {
    const [tutorialInjector, setTutorialInjector] = useState(null);
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 relative">
      {/* Background Pattern */}
        ...
        <div className="w-full max-w-4xl px-6">
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 p-8 mb-8">
            ...
            <div className="flex justify-center space-x-4">
              <button onClick={() => window.history.back()} className="px-6 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition-colors">
                ← Back to Deck
              </button>
              <button onClick={() => window.location.href = '/command'} className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors">
                Command Center
              </button>
            </div>
          </div>
      ...
      <DeckNarrationOverlay deckId={deckId} deckName={deckName} isVisible={true} position="bottom-right" autoPlay={true} className="mb-8 mr-8"/>
