...
export class LegacyVaultExporter {
     ...
     */
    createVaultSnapshot(did, startDate, endDate) {
     ...
     */
    createLegacyArchive(did, periodDays = 90) {
        ...
        for (let i = 0; i < weeklyIntervals; i++) {
            ...
            const snapshot = this.createVaultSnapshot(did, weekStart.toISOString(), weekEnd.toISOString());
     ...
     */
    validateArchiveIntegrity(archive) {
        const errors = [];
        const warnings = [];
        ...
        const sortedSnapshots = [...archive.snapshots].sort((a, b) => new Date(a.snapshotTimestamp).getTime() - new Date(b.snapshotTimestamp).getTime());
