 ...
 */
import { zkpProofLedger, TruthCoinPillar, recordZKPMintAttempt } from './ZKPProofLedger';
/**
 * Test function to validate ZKP Proof Ledger functionality
 * Runs comprehensive tests on all ledger operations
 */
export async function runZKPLedgerTests() {
    const results = [];
    let testsRun = 0;
    let testsPassed = 0;
    let testsFailed = 0;
    ...
    runTest('Duplicate Hash Prevention', () => {
        const result = zkpProofLedger.recordProof('0x1234567890123456789012345678901234567890', TruthCoinPillar.GOVERNANCE, 'zkp_governance_civic_participation_proof_a1b2c3', // Existing hash
        'pending');
        return !result; // Should return false for duplicate
    });
    // Test 5: New proof recording
    runTest('New Proof Recording', () => {
        const result = zkpProofLedger.recordProof('0x7890123456789012345678901234567890123456', TruthCoinPillar.PEACE, 'zkp_peace_conflict_resolution_proof_test123', 'pending', undefined, 500);
        return result; // Should return true for new proof
    });
    // Test 6: Performance validation (write < 100ms)
    runTest('Write Performance', () => {
        const startTime = Date.now();
        zkpProofLedger.recordProof('0x8901234567890123456789012345678901234567', TruthCoinPillar.JOURNALISM, 'zkp_journalism_investigative_report_perf_test', 'approved', undefined, 750);
    ...
    runTest('Status Update', () => {
        ...
        if (!pendingProof)
            return false;
        const result = zkpProofLedger.updateProofStatus(pendingProof.id, 'approved', 'Commander override approval', true);
        return result;
    });
    ...
    runTest('Integration Function', () => {
        const result = recordZKPMintAttempt('0x9012345678901234567890123456789012345678', TruthCoinPillar.SCIENCE, 'zkp_science_research_contribution_integration_test', true, 650, undefined);
        return result;
    });
    // Final statistics
 ...
 */
export async function quickLedgerTest() {
    ...
    const testResult = zkpProofLedger.recordProof('0xTEST1234567890123456789012345678901234567', TruthCoinPillar.HEALTH, 'zkp_health_quick_test_' + Date.now(), 'approved', undefined, 425);
