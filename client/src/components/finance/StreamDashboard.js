...
import { Search, Filter, Play, Pause, Square, Clock, Wallet, TrendingUp, Eye, Volume2 } from 'lucide-react';
...
const StreamDashboard = () => {
    ...
    const generateMockStreams = useCallback(() => {
        const mockStreams = [
            {
                id: 'stream_001',
                sender: '0x1a2b...c3d4',
    ...
    const filteredStreams = useMemo(() => {
        ...
        if (dashboardState.searchTerm) {
            ...
            filtered = filtered.filter(stream => stream.id.toLowerCase().includes(term) ||
    ...
    return (<div className="container mx-auto p-6 space-y-6" role="main" aria-label="Stream Dashboard">
      {/* Header */}
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" size="sm" onClick={() => setTtsEnabled(!ttsEnabled)} className="flex items-center gap-2" aria-label={`TTS narration ${ttsEnabled ? 'enabled' : 'disabled'}`}>
            <Volume2 className="h-4 w-4"/>
            {ttsEnabled ? 'TTS On' : 'TTS Off'}
          </Button>
          
          <Button onClick={loadStreamData} disabled={dashboardState.isLoading} size="sm" aria-label="Refresh stream data">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input placeholder="Search by stream ID, address, or campaign..." value={dashboardState.searchTerm} onChange={(e) => handleSearch(e.target.value)} className="pl-10" aria-label="Search streams"/>
              </div>
            </div>
            
            <div className="flex items-center gap-2">
              <Filter className="h-4 w-4 text-muted-foreground"/>
              <select value={dashboardState.statusFilter} onChange={(e) => handleStatusFilter(e.target.value)} className="px-3 py-2 border border-input bg-background rounded-md text-sm" aria-label="Filter by status">
      ...
      <Card>
        ...
        <CardContent>
          {dashboardState.isLoading ? (<div className="flex items-center justify-center py-8">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>
              <span className="ml-2">Loading streams...</span>
            </div>) : filteredStreams.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
              No streams found matching your criteria
            </div>) : (<div className="space-y-3">
              {filteredStreams.map((stream) => (<div key={stream.id} className="border rounded-lg p-4 space-y-3 hover:bg-accent/50 transition-colors" role="article" aria-label={`Stream ${stream.id}`}>
                  {/* Stream Header */}
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      {stream.cid && (<Badge variant="outline" className="text-xs">
                          CID: {stream.cid.slice(0, 8)}...
                        </Badge>)}
                    </div>
                    
                    <div className="flex items-center gap-2">
                      {stream.status === 'streaming' && (<Button size="sm" variant="outline" onClick={() => handleStreamAction(stream.id, 'pause')} aria-label={`Pause stream ${stream.id}`}>
                          <Pause className="h-3 w-3"/>
                        </Button>)}
                      {stream.status === 'paused' && (<Button size="sm" variant="outline" onClick={() => handleStreamAction(stream.id, 'resume')} aria-label={`Resume stream ${stream.id}`}>
                          <Play className="h-3 w-3"/>
                        </Button>)}
                      {(stream.status === 'streaming' || stream.status === 'paused') && (<Button size="sm" variant="destructive" onClick={() => handleStreamAction(stream.id, 'cancel')} aria-label={`Cancel stream ${stream.id}`}>
                  ...
                  {/* Progress Bar */}
                  {stream.status !== 'cancelled' && (<div className="space-y-1">
                  ...
                  {/* Metadata */}
                  {stream.metadata && (<div className="text-xs space-y-1">
                      <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    {stream.endTime && (<span>Ended {formatTimeAgo(stream.endTime)}</span>)}
                  </div>
                </div>))}
            </div>)}
        </CardContent>
