...
export function CivicTreasuryWithdrawCard() {
    ...
    const handleWithdrawalRequest = async () => {
        ...
        withdrawalRequest.verificationHash = zkpReceiptGenerator.generateWithdrawalHash(withdrawalRequest.id, amount, recipientAddress, withdrawPurpose);
        ...
        setTimeout(() => {
            setWithdrawalRequests(prev => prev.map(req => req.id === withdrawalRequest.id
                ? { ...req, status: 'approved' }
                : req));
    ...
    return (<Card className="w-full max-w-4xl mx-auto" aria-label="Civic Treasury Withdrawal Interface">
      <CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="amount" type="number" placeholder={`Min: ${currentTier.minimumWithdrawal.toLocaleString()}`} value={withdrawAmount} onChange={(e) => setWithdrawAmount(e.target.value)} min={currentTier.minimumWithdrawal} max={Math.min(currentTier.dailyLimit, availableBalance)}/>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={withdrawPurpose} onValueChange={setWithdrawPurpose}>
                ...
                <SelectContent>
                  {withdrawalPurposes.map((purpose) => (<SelectItem key={purpose} value={purpose}>
          ...
          <div className="space-y-2">
            ...
            <Input id="recipient" placeholder="Treasury recipient address or CID" value={recipientAddress} onChange={(e) => setRecipientAddress(e.target.value)}/>
          </div>

          <Dialog>
            <DialogTrigger asChild>
              <Button className="w-full" disabled={!withdrawAmount || !withdrawPurpose || !recipientAddress} onClick={() => {
            const amount = parseInt(withdrawAmount);
            const validation = validateWithdrawal(amount);
            if (validation.valid) {
            ...
            {confirmWithdrawal && (<DialogContent className="sm:max-w-md">
                <DialogHeader>
                ...
                <div className="space-y-4">
                  ...
                  {currentTier.verificationRequired && (<Alert>
                  ...
                  <div className="flex gap-2 pt-2">
                    <Button variant="outline" size="sm" className="flex-1" onClick={() => setConfirmWithdrawal(null)}>
                      Cancel
                    </Button>
                    <Button size="sm" className="flex-1" onClick={handleWithdrawalRequest}>
                      Confirm & Submit
                    </Button>
                  </div>
                </div>
        ...
        {withdrawalRequests.length > 0 && (<div className="space-y-3">
            <Separator />
            <h3 className="font-medium text-sm">Recent Withdrawal Requests</h3>
            <div className="space-y-3">
              {withdrawalRequests.slice(0, 5).map((request) => (<div key={request.id} className="border rounded-lg p-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {request.verificationHash && (<Button size="sm" variant="outline" className="text-xs h-6" onClick={() => setShowReceipt(request.id)}>
                        View Receipt
                      </Button>)}
                  </div>
                </div>))}
      ...
      {/* ZKP Receipt Modal */}
      {showReceipt && (<Dialog open={!!showReceipt} onOpenChange={() => setShowReceipt(null)}>
          <DialogContent className="sm:max-w-lg">
            ...
            <div className="space-y-4">
              {(() => {
                ...
                return (<div className="bg-gray-50 p-4 rounded-lg font-mono text-xs space-y-2">
