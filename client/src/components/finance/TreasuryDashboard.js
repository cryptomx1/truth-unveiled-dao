...
import { DollarSign, Building, Shield, TrendingUp, Users, Lock, CheckCircle, Clock } from "lucide-react";
import { useTreasuryEngine } from "./TreasuryEngine";
export function TreasuryDashboard() {
    const { allocation, verifiedOrgs, pendingOrgs, stakingPools, stats, allocateToOrg, stakeTruthPoints } = useTreasuryEngine();
    const [selectedOrg, setSelectedOrg] = useState("");
    const [allocationAmount, setAllocationAmount] = useState("");
    const [allocationPurpose, setAllocationPurpose] = useState("");
    const [stakingAmount, setStakingAmount] = useState("");
    const handleAllocate = () => {
        if (!selectedOrg || !allocationAmount || !allocationPurpose)
            return;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <Card>
              <CardContent className="p-4">
                ...
                <Progress value={(allocation.marketing.allocated / allocation.marketing.amount) * 100} className="mt-2"/>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <Progress value={(allocation.ngoRewards.allocated / allocation.ngoRewards.amount) * 100} className="mt-2"/>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <Progress value={(allocation.govRewards.allocated / allocation.govRewards.amount) * 100} className="mt-2"/>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <Progress value={(allocation.reserve.allocated / allocation.reserve.amount) * 100} className="mt-2"/>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="space-y-3 mb-4">
                {verifiedOrgs.map((org) => (<div key={org.id} className={`p-4 border rounded-lg cursor-pointer transition-colors ${selectedOrg === org.id
                ? 'border-blue-500 bg-blue-50'
                : 'border-gray-200 hover:border-gray-300'}`} onClick={() => setSelectedOrg(org.id)}>
              ...
              {pendingOrgs.length > 0 && (<div>
                  <h4 className="font-medium mb-2 flex items-center gap-2">
                    <Clock className="h-4 w-4 text-yellow-600"/>
                    Pending Verification
                  </h4>
                  <div className="space-y-2">
                    {pendingOrgs.map((org) => (<div key={org.id} className="p-3 border border-yellow-200 bg-yellow-50 rounded">
            ...
            <div>
              ...
              {selectedOrgData ? (<div className="space-y-4">
                  ...
                  {/* Staking Information */}
                  {selectedStakingData && (<div className="p-4 bg-blue-50 rounded-lg">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <Input id="allocationAmount" type="number" placeholder="1000000" value={allocationAmount} onChange={(e) => setAllocationAmount(e.target.value)}/>
                    </div>
                    <div>
                      <Label htmlFor="allocationPurpose">Purpose</Label>
                      <Textarea id="allocationPurpose" placeholder="Describe the purpose of this allocation..." value={allocationPurpose} onChange={(e) => setAllocationPurpose(e.target.value)} rows={3}/>
                    </div>
                    <Button onClick={handleAllocate} disabled={!allocationAmount || !allocationPurpose} className="w-full">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <Input id="stakingAmount" type="number" placeholder="500000" value={stakingAmount} onChange={(e) => setStakingAmount(e.target.value)}/>
                    </div>
                    <Button onClick={handleStaking} disabled={!stakingAmount} variant="outline" className="w-full">
