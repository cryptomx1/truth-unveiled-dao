...
import TruthCoinClaimEngine from './TruthCoinClaimEngine';
import GenesisFusionEngine from './GenesisFusionEngine';
export default function WalletFinancePage() {
    ...
    const [guardianPillars, setGuardianPillars] = useState([]);
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [selectedClaimType, setSelectedClaimType] = useState('referral');
    // Mock user data
    const currentUser = {
        id: 'user_demo_123',
        tier: 'Governor',
    ...
    const handleSubmitClaim = async () => {
        ...
        try {
            ...
            const newClaim = await claimEngine.submitClaim(selectedClaimType, currentUser.id, currentUser.tier, 'wallet-finance');
    ...
    return (<div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <TTSToggle deckId="wallet-finance" moduleId="dashboard" content="Finance center dashboard with claim submission, fusion tracking, and reward analytics"/>
      </div>
      ...
      <Tabs defaultValue="submit" className="w-full">
          ...
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                ...
                <TTSToggle deckId="wallet-finance" moduleId="submit-claims" content="Submit TruthCoin claims for referrals, governance, civic duty, and education activities"/>
              </div>
            </CardHeader>
            ...
            { type: 'education', label: 'Education', base: 125, icon: '📚' }
        ].map((claimType) => (<Card key={claimType.type} className={`cursor-pointer transition-all ${selectedClaimType === claimType.type
                ? 'ring-2 ring-blue-500 bg-blue-50 dark:bg-blue-950'
                : 'hover:bg-gray-50 dark:hover:bg-gray-900'}`} onClick={() => setSelectedClaimType(claimType.type)}>
              ...
              <Button onClick={handleSubmitClaim} disabled={isSubmitting} size="lg" className="w-full sm:w-auto">
          ...
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                ...
                <TTSToggle deckId="wallet-finance" moduleId="claim-history" content="Review your TruthCoin claim history with status tracking and reward details"/>
              </div>
            </CardHeader>
            <CardContent>
              {claims.length > 0 ? (<div className="space-y-4">
                  {claims.map((claim) => (<div key={claim.claimId} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 border rounded-lg">
          ...
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                ...
                <TTSToggle deckId="wallet-finance" moduleId="fusion-center" content="Track Genesis badge fusion progress across civic pillars with guardian assignments"/>
              </div>
            </CardHeader>
            <CardContent className="space-y-6">
              {userBadge ? (<>
                ...
                return (<div key={pillar.pillarId} className={`p-3 border rounded-lg text-center ${isCompleted
          ...
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                ...
                <TTSToggle deckId="wallet-finance" moduleId="analytics" content="View comprehensive analytics for claims, rewards, and system performance"/>
              </div>
            </CardHeader>
            <CardContent>
