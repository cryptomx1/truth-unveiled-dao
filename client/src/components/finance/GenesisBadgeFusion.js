...
import { AlertCircle, Coins, Crown, Sparkles, Star, Shield, Award, Target, TrendingUp } from 'lucide-react';
import { TruthCoinClaimEngine } from './TruthCoinClaimEngine';
const GenesisBadgeFusion = () => {
    ...
    // Claim form state
    const [claimType, setClaimType] = useState('genesis_fusion');
    const [pillarCount, setPillarCount] = useState(3);
    const [hasZKPProof, setHasZKPProof] = useState(true);
    const [selectedWallet, setSelectedWallet] = useState('cid:wallet:test_user_123');
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <Input type="number" min="1" max="8" value={pillarCount} onChange={(e) => setPillarCount(parseInt(e.target.value) || 1)} className="mt-1"/>
                ...
                <div className="flex items-center space-x-2">
                  <input type="checkbox" id="zkp-proof" checked={hasZKPProof} onChange={(e) => setHasZKPProof(e.target.checked)} className="rounded border-gray-300"/>
                  <label htmlFor="zkp-proof" className="text-sm">
                    Include ZKP Proof (+10% eligibility bonus)
                  </label>
                </div>

                <Button onClick={handleSubmitClaim} disabled={isProcessing} className="w-full">
                  {isProcessing ? (<>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {claimType === 'genesis_fusion' && pillarCount >= 3 && (<div className="bg-purple-50 border border-purple-200 p-4 rounded-lg">
        ...
        <TabsContent value="history" className="space-y-4">
          <Card>
            ...
            return (<div key={claim.claimId} className="border rounded-lg p-4">
                      ...
                      {claim.transactionHash && (<div className="mt-2 text-xs text-gray-500">
                          Transaction: {claim.transactionHash.substring(0, 20)}...
                        </div>)}
                    </div>);
        })}
                
                {recentClaims.length === 0 && (<div className="text-center py-8 text-gray-500">
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {fusionHistory.map((fusion) => (<div key={fusion.fusionId} className="border rounded-lg p-4 bg-gradient-to-r from-purple-50 to-blue-50">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {Object.entries(fusion.guardianAssignments).map(([pillar, guardian]) => (<Badge key={pillar} variant="secondary" className="text-xs">
                            {pillar}: {guardian}
                          </Badge>))}
                      </div>
                    </div>
                ...
                {fusionHistory.length === 0 && (<div className="text-center py-8 text-gray-500">
        ...
        <TabsContent value="analytics" className="space-y-4">
          <Card>
            ...
            <CardContent>
              {metrics && (<div className="grid grid-cols-2 md:grid-cols-3 gap-6">
