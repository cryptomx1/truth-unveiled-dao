...
export function TruthPointUtilityCard() {
    ...
    const handleUtilityAction = async (action) => {
        ...
        // Log usage to ledger
        const usageEntry = usageLedger.logUsage(`did:civic:user_${Date.now()}`, action.cost, action.name, action.category, action.id);
    ...
    return (<Card className="w-full max-w-4xl mx-auto" aria-label="TruthPoint Utility Interface">
      <CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {utilityActions.map((action) => (<div key={action.id} className={`border rounded-lg p-4 space-y-3 transition-all ${action.available
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="text-right">
                  ...
                  {action.cooldown && (<div className="text-xs text-gray-500">{action.cooldown}</div>)}
                </div>
              </div>
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <Button size="sm" className="w-full" disabled={!action.available} onClick={() => setConfirmAction(action)}>
                    {action.available ? 'Activate' : 'Insufficient TP'}
                  </Button>
                </DialogTrigger>
                
                {confirmAction?.id === action.id && (<DialogContent className="sm:max-w-md">
                    <DialogHeader>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-2 pt-2">
                        ...
                        <Button size="sm" className="flex-1" onClick={() => handleUtilityAction(confirmAction)}>
                          Confirm & Spend
                        </Button>
                      </div>
                    </div>
        ...
        {recentUsage.length > 0 && (<div className="space-y-3">
            ...
            <div className="space-y-2">
              {recentUsage.map((usage) => (<div key={usage.id} className="flex items-center justify-between text-sm bg-gray-50 p-2 rounded">
                  <div className="flex items-center gap-2">
