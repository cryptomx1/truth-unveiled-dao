...
import { Building2, Users, CheckCircle, Clock, AlertCircle, TrendingUp } from 'lucide-react';
...
const MunicipalPilotDashboard = () => {
    const [selectedTab, setSelectedTab] = useState('overview');
    const [showWizard, setShowWizard] = useState(false);
    const [selectedNode, setSelectedNode] = useState(null);
    // Mock municipal nodes data
    const [municipalNodes, setMunicipalNodes] = useState([
        {
            id: 'austin-tx-001',
    ...
    if (showWizard) {
        return (<DIDVerificationWizard onComplete={() => {
                setShowWizard(false);
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-4">
              <Button onClick={handleStartOnboarding} className="bg-blue-600 hover:bg-blue-700 text-white" aria-label="Start municipal onboarding process">
                Start Onboarding
              </Button>
              <Button variant="outline" onClick={() => window.location.href = '/command'} className="border-slate-600 text-slate-300 hover:bg-slate-800">
                Command Center
              </Button>
            </div>
          </div>
        ...
        <Tabs value={selectedTab} onValueChange={setSelectedTab}>
          ...
          <TabsContent value="nodes" className="mt-6">
            <Card className="bg-slate-800 border-slate-700">
              ...
              <CardContent>
                <div className="space-y-4">
                  {municipalNodes.map((node) => (<div key={node.id} className="flex items-center justify-between p-4 bg-slate-700 rounded-lg border border-slate-600 cursor-pointer hover:bg-slate-600 transition-colors" onClick={() => handleNodeSelection(node.id)} role="button" tabIndex={0} aria-label={`Select ${node.name} municipal node`}>
          ...
          <TabsContent value="map" className="mt-6">
            <PilotRegionMap nodes={municipalNodes} selectedNode={selectedNode} onNodeSelect={handleNodeSelection}/>
          </TabsContent>
        </Tabs>

        {/* CID Binder Integration */}
        {selectedNode && (<div className="mt-8">
            <MunicipalCIDBinderComponent nodeId={selectedNode} onClose={() => setSelectedNode(null)}/>
          </div>)}
      </div>
    </div>);
};
