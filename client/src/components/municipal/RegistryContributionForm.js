...
import { X, Shield, FileText, Lock, CheckCircle, AlertCircle, Upload } from 'lucide-react';
const RegistryContributionForm = ({ onSubmit, onCancel, currentUser }) => {
    ...
    const completedFields = requiredFields.filter(field => formData[field].trim().length > 0);
    ...
    return (<div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <CardContent className="space-y-6">
          ...
          {/* Validation Errors */}
          {validationErrors.length > 0 && (<div className="bg-red-50 border border-red-200 p-4 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input id="title" value={formData.title} onChange={(e) => handleInputChange('title', e.target.value)} placeholder="Enter policy title..." className="mt-1"/>
              </div>
              <div>
                <Label htmlFor="category">Policy Category *</Label>
                <select id="category" value={formData.category} onChange={(e) => handleInputChange('category', e.target.value)} className="w-full p-2 border rounded-md mt-1">
            ...
            <div>
              ...
              <Textarea id="description" value={formData.description} onChange={(e) => handleInputChange('description', e.target.value)} placeholder="Provide a comprehensive description of the proposed policy..." rows={4} className="mt-1"/>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Textarea id="impactAssessment" value={formData.impactAssessment} onChange={(e) => handleInputChange('impactAssessment', e.target.value)} placeholder="Describe the expected impact on the community..." rows={3} className="mt-1"/>
              </div>
              <div>
                <Label htmlFor="implementation">Implementation Plan *</Label>
                <Textarea id="implementation" value={formData.implementation} onChange={(e) => handleInputChange('implementation', e.target.value)} placeholder="Outline the implementation strategy..." rows={3} className="mt-1"/>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input id="budget" value={formData.budget} onChange={(e) => handleInputChange('budget', e.target.value)} placeholder="e.g., $500,000 over 2 years" className="mt-1"/>
              </div>
              <div>
                <Label htmlFor="timeline">Timeline</Label>
                <Input id="timeline" value={formData.timeline} onChange={(e) => handleInputChange('timeline', e.target.value)} placeholder="e.g., 18 months from approval" className="mt-1"/>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Textarea id="stakeholders" value={formData.stakeholders} onChange={(e) => handleInputChange('stakeholders', e.target.value)} placeholder="List involved departments, organizations, and community groups..." rows={3} className="mt-1"/>
              </div>
              <div>
                <Label htmlFor="legalBasis">Legal Basis</Label>
                <Textarea id="legalBasis" value={formData.legalBasis} onChange={(e) => handleInputChange('legalBasis', e.target.value)} placeholder="Reference relevant laws, ordinances, or authority..." rows={3} className="mt-1"/>
              </div>
            </div>

            <div>
              <Label htmlFor="publicBenefit">Public Benefit Statement *</Label>
              <Textarea id="publicBenefit" value={formData.publicBenefit} onChange={(e) => handleInputChange('publicBenefit', e.target.value)} placeholder="Explain how this policy serves the public interest..." rows={3} className="mt-1"/>
            </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t">
              ...
              <div className="flex gap-2">
                ...
                <Button type="submit" disabled={isSubmitting || completionPercentage < 100} className="flex items-center gap-2">
                  {isSubmitting ? (<>
