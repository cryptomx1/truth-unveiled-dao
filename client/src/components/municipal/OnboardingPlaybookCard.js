...
export const OnboardingPlaybookCard = () => {
    ...
    const overallProgress = Math.round(playbookStages.reduce((sum, stage) => sum + stage.completionRate, 0) / playbookStages.length);
    return (<Card className="bg-slate-800 border-slate-700">
      ...
      <CardContent>
        <div className="space-y-4">
          {playbookStages.map((stage, index) => (<div key={stage.id} className="flex items-start gap-4 p-4 rounded-lg border border-slate-600 bg-slate-700/50">
              {/* Stage Icon and Progress */}
              <div className="flex-shrink-0 flex flex-col items-center">
                {getStatusIcon(stage.status)}
                {index < playbookStages.length - 1 && (<div className="w-px h-8 bg-slate-600 mt-2"/>)}
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                {stage.completionRate > 0 && stage.completionRate < 100 && (<div className="mb-3">
                ...
                <div className="space-y-1">
                  ...
                  {stage.requirements.map((requirement, reqIndex) => (<div key={reqIndex} className="flex items-start gap-2 text-xs">
              ...
              {stage.status === 'current' && (<div className="flex-shrink-0">
