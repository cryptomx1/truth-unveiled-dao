...
import { CheckCircle, Link, X, RefreshCw, AlertCircle, Copy } from 'lucide-react';
import { MunicipalCIDBinder } from './MunicipalCIDBinder';
export const MunicipalCIDBinderComponent = ({ nodeId, onClose }) => {
    ...
    const handleCreateCredential = async () => {
        try {
            ...
            await binder.createMunicipalCredential(nodeId, `Municipal Node ${nodeId}`, 'Demo Jurisdiction', btoa(JSON.stringify({
    ...
    return (<Card className="bg-slate-800 border-slate-700">
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={onClose} className="border-slate-600 text-slate-300 hover:bg-slate-700">
            <X className="w-4 h-4"/>
          </Button>
        </div>
      ...
      <CardContent className="space-y-6">
        {/* Status Alerts */}
        {error && (<Alert className="border-red-500 bg-red-900/20">
            <AlertCircle className="h-4 w-4"/>
            <AlertDescription className="text-red-300">{error}</AlertDescription>
          </Alert>)}

        {success && (<Alert className="border-green-500 bg-green-900/20">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4 p-4 bg-slate-700 rounded-lg">
            ...
            <div className="flex gap-2">
              ...
              {!bindingStatus.credentialExists && (<Button onClick={handleCreateCredential} disabled={isBinding} size="sm" className="bg-blue-600 hover:bg-blue-700">
          ...
          <div className="flex items-center gap-4 p-4 bg-slate-700 rounded-lg">
            ...
            <div className="flex gap-2">
              ...
              {bindingStatus.credentialExists && !bindingStatus.federationBound && (<Button onClick={handleBindToFederation} disabled={isBinding} size="sm" className="bg-purple-600 hover:bg-purple-700">
          ...
          <div className="flex items-center gap-4 p-4 bg-slate-700 rounded-lg">
            ...
            <div className="flex gap-2">
              ...
              {bindingStatus.credentialExists && !bindingStatus.verificationComplete && (<Button onClick={handleVerifyCredential} disabled={isVerifying} size="sm" className="bg-green-600 hover:bg-green-700">
        ...
        {/* Credential Details */}
        {credential && (<div className="p-4 bg-slate-700 rounded-lg border border-slate-600">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-white">Credential Details</h3>
              <Button onClick={handleCopyCredentialData} variant="outline" size="sm" className="border-slate-500 text-slate-300 hover:bg-slate-600">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <Badge className={credential.verificationStatus === 'verified' ? 'bg-green-100 text-green-800' :
        ...
        {binding && (<div className="p-4 bg-slate-700 rounded-lg border border-slate-600">
        ...
        <div className="flex justify-between pt-4 border-t border-slate-600">
          <Button variant="outline" onClick={loadBindingStatus} className="border-slate-600 text-slate-300 hover:bg-slate-700">
            <RefreshCw className="w-4 h-4 mr-2"/>
            Refresh Status
          </Button>
          
          {progress === 100 && (<Button onClick={onClose} className="bg-green-600 hover:bg-green-700 text-white">
