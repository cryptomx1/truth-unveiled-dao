...
import { CheckCircle, Shield, Users, FileText, ArrowRight, ArrowLeft, X } from 'lucide-react';
export const DIDVerificationWizard = ({ onComplete, onCancel }) => {
    ...
    const updateTrustedContact = (index, field, value) => {
        const updated = formData.trustedContacts.map((contact, i) => i === index ? { ...contact, [field]: value } : contact);
        updateFormData('trustedContacts', updated);
    };
    const removeTrustedContact = (index) => {
    ...
    const nextStep = () => {
        if (validateCurrentStep() && currentStep < steps.length - 1) {
            const updatedSteps = steps.map((step, index) => index === currentStep ? { ...step, completed: true } : step);
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            ...
            <div>
              ...
              <Input id="entity-name" value={formData.name} onChange={(e) => updateFormData('name', e.target.value)} placeholder="e.g., Austin City Council, Portland Township Authority" className="mt-2 bg-slate-700 border-slate-600 text-white" aria-required="true"/>
            </div>
            
            <div>
              <Label htmlFor="jurisdiction" className="text-white">Jurisdiction *</Label>
              <Input id="jurisdiction" value={formData.jurisdiction} onChange={(e) => updateFormData('jurisdiction', e.target.value)} placeholder="e.g., Austin, Texas, USA" className="mt-2 bg-slate-700 border-slate-600 text-white" aria-required="true"/>
            </div>
            ...
            <div>
              ...
              <Input id="official-email" type="email" value={formData.officialEmail} onChange={(e) => updateFormData('officialEmail', e.target.value)} placeholder="clerk@cityname.gov" className="mt-2 bg-slate-700 border-slate-600 text-white" aria-required="true"/>
            </div>

            <div>
              <Label className="text-white">Verification Documents *</Label>
              <div className="mt-2 space-y-2">
                <Button variant="outline" className="w-full border-slate-600 text-slate-300 hover:bg-slate-700" onClick={() => {
                ...
                {formData.verificationDocuments.map((doc, index) => (<Badge key={index} className="bg-green-100 text-green-800">
                    {doc}
                  </Badge>))}
              </div>
            </div>
            ...
            <div>
              ...
              <Textarea id="public-key" value={formData.publicKey} onChange={(e) => updateFormData('publicKey', e.target.value)} placeholder="-----BEGIN PUBLIC KEY-----" className="mt-2 bg-slate-700 border-slate-600 text-white" rows={4}/>
            </div>
          </div>);
            case 2: // Trusted Network
                return (<div className="space-y-6">
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <Button onClick={addTrustedContact} variant="outline" size="sm" className="border-slate-600 text-slate-300 hover:bg-slate-700">
                  <Users className="w-4 h-4 mr-2"/>
                  Add Contact
                </Button>
              </div>
              
              {formData.trustedContacts.map((contact, index) => (<Card key={index} className="bg-slate-700 border-slate-600">
                  <CardContent className="p-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        <Label className="text-white text-sm">Name</Label>
                        <Input value={contact.name} onChange={(e) => updateTrustedContact(index, 'name', e.target.value)} className="mt-1 bg-slate-600 border-slate-500 text-white" placeholder="Official Name"/>
                      </div>
                      <div>
                        <Label className="text-white text-sm">Role</Label>
                        <Input value={contact.role} onChange={(e) => updateTrustedContact(index, 'role', e.target.value)} className="mt-1 bg-slate-600 border-slate-500 text-white" placeholder="e.g., Mayor, Clerk"/>
                      </div>
                      <div className="flex items-end gap-2">
                        <div className="flex-1">
                          <Label className="text-white text-sm">Email</Label>
                          <Input type="email" value={contact.email} onChange={(e) => updateTrustedContact(index, 'email', e.target.value)} className="mt-1 bg-slate-600 border-slate-500 text-white" placeholder="official@gov.domain"/>
                        </div>
                        <Button variant="outline" size="sm" onClick={() => removeTrustedContact(index)} className="border-red-500 text-red-400 hover:bg-red-900">
                          <X className="w-4 h-4"/>
                        </Button>
                      </div>
                    </div>
              ...
              {formData.trustedContacts.length === 0 && (<p className="text-slate-400 text-center py-8">
            ...
            <div className="text-center">
              ...
              {!formData.zkpStub ? (<Button onClick={generateZKPStub} className="bg-blue-600 hover:bg-blue-700 text-white">
                  Generate ZKP Stub
                </Button>) : (<div className="bg-slate-700 p-4 rounded-lg border border-slate-600">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" onClick={onCancel} className="border-slate-600 text-slate-300 hover:bg-slate-800" aria-label="Cancel verification wizard">
          ...
          <div className="flex justify-between mt-6">
            {steps.map((step, index) => (<div key={step.id} className={`flex flex-col items-center ${index <= currentStep ? 'text-blue-400' : 'text-slate-500'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step.completed
                ? 'bg-green-600 border-green-600 text-white'
                : index === currentStep
                    ? 'border-blue-400 bg-blue-400 text-white'
                    : 'border-slate-500 text-slate-500'}`}>
                  {step.completed ? (<CheckCircle className="w-4 h-4"/>) : (<span className="text-sm">{index + 1}</span>)}
                </div>
        ...
        <div className="flex justify-between mt-8">
          <Button variant="outline" onClick={previousStep} disabled={currentStep === 0} className="border-slate-600 text-slate-300 hover:bg-slate-800 disabled:opacity-50">
            <ArrowLeft className="w-4 h-4 mr-2"/>
            Previous
          </Button>
          
          {currentStep === steps.length - 1 ? (<Button onClick={submitVerification} disabled={!validateCurrentStep() || isSubmitting} className="bg-green-600 hover:bg-green-700 text-white disabled:opacity-50">
              {isSubmitting ? 'Submitting...' : 'Submit Verification'}
            </Button>) : (<Button onClick={nextStep} disabled={!validateCurrentStep()} className="bg-blue-600 hover:bg-blue-700 text-white disabled:opacity-50">
              Next
