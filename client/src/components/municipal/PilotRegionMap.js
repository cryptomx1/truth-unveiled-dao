...
export const PilotRegionMap = ({ nodes, selectedNode, onNodeSelect }) => {
    ...
    const getRegionHealthColor = (score, activeNodes) => {
        if (activeNodes === 0)
            return '#6B7280'; // gray
        if (score >= 80)
            return '#10B981'; // green
          ...
          <div className="relative">
            ...
            <div className="w-full h-96 bg-slate-700 rounded-lg border border-slate-600 relative overflow-hidden">
              ...
              <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 60" xmlns="http://www.w3.org/2000/svg">
                <path d="M15 50 L15 20 L25 15 L40 15 L50 10 L75 12 L85 20 L85 45 L80 55 L70 55 L60 50 L40 52 L25 50 Z" fill="rgba(71, 85, 105, 0.3)" stroke="rgba(71, 85, 105, 0.6)" strokeWidth="0.5"/>
              </svg>

              {/* Regional Nodes */}
              {regionClusters.map((cluster) => (<div key={cluster.id} className="absolute transform -translate-x-1/2 -translate-y-1/2 cursor-pointer" style={{
                left: `${cluster.coordinates.x}%`,
                top: `${cluster.coordinates.y}%`
            }} onClick={() => handleRegionClick(cluster)} onMouseEnter={() => handleNodeHover(cluster.id)} onMouseLeave={() => handleNodeHover(null)} role="button" tabIndex={0} aria-label={`Select ${cluster.name} region with ${cluster.nodes.length} nodes`}>
                  {/* Node Circle */}
                  <div className={`w-6 h-6 rounded-full border-2 border-white transition-all duration-200 ${hoveredRegion === cluster.id ||
                ...
                backgroundColor: getRegionHealthColor(cluster.averageGovernanceScore, cluster.activeNodes)
                  ...
                  {/* Hover Info */}
                  {hoveredRegion === cluster.id && (<div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 bg-slate-900 border border-slate-600 rounded-lg p-3 min-w-48 z-10 shadow-xl">
      ...
      <Card className="bg-slate-800 border-slate-700">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {regionClusters.map((cluster) => (<div key={cluster.id} className={`p-4 bg-slate-700 border rounded-lg cursor-pointer transition-colors ${selectedNode && cluster.nodes.some(n => n.id === selectedNode)
                ? 'border-blue-400 bg-slate-600'
                : 'border-slate-600 hover:border-slate-500'}`} onClick={() => cluster.nodes.length > 0 && handleRegionClick(cluster)} role="button" tabIndex={0} aria-label={`View details for ${cluster.name}`}>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-white">{cluster.name}</h3>
                  <Badge className={cluster.activeNodes > 0
                ...
                {/* Node List */}
                {cluster.nodes.length > 0 && (<div className="mt-3 pt-3 border-t border-slate-600">
                    <div className="space-y-2">
                      {cluster.nodes.slice(0, 2).map((node) => (<div key={node.id} className="flex items-center gap-2 text-xs">
                          {node.status === 'active' ? (<CheckCircle className="w-3 h-3 text-green-400"/>) : node.status === 'verifying' ? (<Clock className="w-3 h-3 text-yellow-400"/>) : (<AlertCircle className="w-3 h-3 text-slate-400"/>)}
                          <span className="text-slate-300 truncate">{node.name}</span>
                        </div>))}
                      {cluster.nodes.length > 2 && (<p className="text-xs text-slate-400">
