...
import { TrendingUp, TrendingDown, Minus, Shield, AlertTriangle, CheckCircle, Clock, Users, Activity } from 'lucide-react';
import TTSToggle from '@/components/tts/TTSToggle';
import TrustSentimentAggregator from './TrustSentimentAggregator';
export const TrustPublicMetricCard = ({ deckId, showOverall = false, compact = false, className = '' }) => {
    const [deckMetrics, setDeckMetrics] = useState(null);
    ...
    const getTierBreakdownSummary = (metrics) => {
    ...
    if (showOverall && overallMetrics) {
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <TTSToggle deckId="trust_metrics" moduleId="overall" content={cardContent} size="sm" variant="outline"/>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Progress value={Math.max(0, Math.min(100, (overallMetrics.overallSentiment + 100) / 2))} className="h-2"/>
          </div>
          ...
          {/* System Health Badge */}
          <div className="flex items-center justify-between">
            <Badge variant={overallMetrics.systemHealth === 'excellent' ? 'default' :
                overallMetrics.systemHealth === 'good' ? 'secondary' : 'destructive'} className="capitalize">
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <CardTitle className="text-lg flex items-center gap-2">
                ...
                Trust Metrics
                {deckMetrics.volatilityFlag && (<AlertTriangle className="h-4 w-4 text-orange-600"/>)}
              </CardTitle>
              <CardDescription>
                {deckMetrics.deckId.replace(/_/g, ' ')} sentiment analysis
              </CardDescription>
            </div>
            <TTSToggle deckId="trust_metrics" moduleId={deckId} content={cardContent} size="sm" variant="outline"/>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          {/* Net Sentiment */}
          <div className="space-y-2">
            ...
            <Progress value={Math.max(0, Math.min(100, (deckMetrics.netSentiment + 100) / 2))} className="h-2"/>
          </div>
