...
export const TrustDeltaSubmissionCard = ({ target, userTier, userDID, onSubmissionComplete, className = '' }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                announceToScreenReader(`Trust feedback submitted successfully. ${feedbackType} recorded for ${target.displayName}.`);
            }
    ...
    return (<Card className={`max-w-2xl mx-auto ${className}`}>
      <CardHeader className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <TTSToggle deckId="trust_feedback" moduleId="submission" content={cardContent} size="sm" variant="outline"/>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <RadioGroup value={feedbackType} onValueChange={(value) => setFeedbackType(value)} className="flex gap-6">
        ...
        <div className="space-y-3">
          ...
          <RadioGroup value={intensity.toString()} onValueChange={(value) => setIntensity(parseInt(value))} className="flex gap-4">
            {[1, 2, 3, 4, 5].map((level) => (<div key={level} className="flex items-center space-x-2">
        ...
        <div className="space-y-3">
          ...
          <Textarea id="explanation" placeholder="Provide context for your feedback (no personal information will be stored)" value={explanation} onChange={(e) => setExplanation(e.target.value)} rows={3} maxLength={500} className="resize-none"/>
        ...
        {/* Submission Results */}
        {submissionResult && (<div className={`p-4 rounded-lg border ${submissionResult.success
                ? 'bg-green-50 border-green-200 dark:bg-green-950 dark:border-green-800'
                : 'bg-red-50 border-red-200 dark:bg-red-950 dark:border-red-800'}`}>
            <div className="flex items-start gap-3">
              {submissionResult.success ? (<CheckCircle className="h-5 w-5 text-green-600 mt-0.5"/>) : (<AlertCircle className="h-5 w-5 text-red-600 mt-0.5"/>)}
              <div className="space-y-1">
                <div className={`font-medium ${submissionResult.success ? 'text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200'}`}>
                  {submissionResult.success ? 'Feedback Submitted Successfully' : 'Submission Failed'}
                </div>
                {submissionResult.success && (<div className="text-sm text-green-700 dark:text-green-300 space-y-1">
