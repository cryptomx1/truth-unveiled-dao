...
const SocialMomentumCard = () => {
    ...
    const MetricCard = ({ icon: Icon, label, value, delta, color = "text-blue-600" }) => (<div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 space-y-2">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 text-sm">
          {delta > 0 ? (<TrendingUp className="h-3 w-3 text-green-500"/>) : (<TrendingDown className="h-3 w-3 text-red-500"/>)}
      ...
      <CardContent className="space-y-6">
        <Tabs value={selectedPlatform} onValueChange={setSelectedPlatform} className="w-full">
          <TabsList className="grid w-full grid-cols-3 mb-6">
            {socialData.map((platform) => (<TabsTrigger key={platform.platform} value={platform.platform} className="text-slate-700 dark:text-slate-300">
                {platform.platform}
              </TabsTrigger>))}
          </TabsList>

          {socialData.map((platform) => (<TabsContent key={platform.platform} value={platform.platform} className="space-y-6">
              {/* Metrics Grid */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <MetricCard icon={Users} label="Likes" value={platform.likes} delta={platform.delta24h.likes} color="text-red-500"/>
                <MetricCard icon={Share} label="Reposts" value={platform.reposts} delta={platform.delta24h.reposts} color="text-green-500"/>
                <MetricCard icon={MessageSquare} label="Quote Posts" value={platform.quotes} delta={platform.delta24h.quotes} color="text-blue-500"/>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3" role="region" aria-label="Hashtag performance metrics">
                  {platform.hashtags.map((hashtag, index) => (<div key={index} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Badge variant="outline" className="font-mono text-xs">
                            {hashtag.tag}
                          </Badge>
                          {hashtag.trend === 'up' ? (<TrendingUp className="h-3 w-3 text-green-500" aria-label="Trending up"/>) : (<TrendingDown className="h-3 w-3 text-red-500" aria-label="Trending down"/>)}
                        </div>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300" aria-label={`Performance score: ${Math.round(hashtag.performance)} percent`}>
                          {Math.round(hashtag.performance)}%
                        </span>
                      </div>
                      <Progress value={hashtag.performance} className="h-2" aria-label={`${hashtag.tag} performance: ${Math.round(hashtag.performance)}%`}/>
                    </div>))}
                </div>
              </div>
