...
const QRShareModule = () => {
    ...
    return (<Card className="w-full max-w-2xl bg-white dark:bg-slate-900">
      <CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <Select value={selectedPath} onValueChange={setSelectedPath}>
            ...
            <SelectContent>
              {predefinedPaths.map((option) => (<SelectItem key={option.path} value={option.path}>
          ...
          {selectedPath === 'custom' && (<div className="space-y-2">
              <Label htmlFor="custom-path" className="text-slate-700 dark:text-slate-300">
                Custom Path
              </Label>
              <Input id="custom-path" value={customPath} onChange={(e) => setCustomPath(e.target.value)} placeholder="/your/custom/path" className="font-mono text-sm"/>
            </div>)}
        </div>

        {/* Route Description */}
        {selectedOption && (<div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4">
        ...
        <div className="flex flex-col items-center space-y-4">
          <div id="qr-code-canvas" className="bg-white p-4 rounded-lg shadow-sm border" role="img" aria-label={`QR Code for Civic Route ${getCurrentPath()}`}>
            <QRCodeSVG value={getFullURL()} size={qrSize} level="M" includeMargin={true}/>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-2">
              {[128, 256, 384].map((size) => (<Button key={size} variant={qrSize === size ? "default" : "outline"} size="sm" onClick={() => setQrSize(size)} className="text-xs">
                  {size}px
                </Button>))}
            </div>
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            <Input value={getFullURL()} readOnly className="font-mono text-xs bg-slate-50 dark:bg-slate-800"/>
            <Button variant="outline" size="sm" onClick={copyToClipboard} className="flex items-center gap-2" title="Copy to clipboard">
              {copied ? (<>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Button onClick={downloadQR} className="flex items-center gap-2 flex-1" variant="default">
            <Download className="h-4 w-4"/>
            Download QR
          </Button>
          
          <Button onClick={shareQR} variant="outline" className="flex items-center gap-2 flex-1">
