...
const GlobalEngagementHeatmap = () => {
    ...
    return (<Card className="w-full max-w-6xl bg-white dark:bg-slate-900">
      <CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="relative bg-slate-100 dark:bg-slate-800 rounded-lg p-6 min-h-[400px]">
          <div className="relative w-full h-96 bg-gradient-to-br from-blue-50 to-green-50 dark:from-slate-700 dark:to-slate-600 rounded-lg border-2 border-slate-200 dark:border-slate-600 overflow-hidden" role="img" aria-label="World map showing global engagement intensity">
            ...
            <svg className="absolute inset-0 w-full h-full opacity-20" viewBox="0 0 100 60" preserveAspectRatio="xMidYMid meet">
              ...
              <path d="M10,20 L35,15 L40,25 L35,35 L15,40 Z" fill="currentColor" className="text-slate-400"/>
              <path d="M45,15 L65,10 L70,20 L65,30 L50,35 Z" fill="currentColor" className="text-slate-400"/>
              <path d="M70,20 L90,25 L85,45 L75,40 Z" fill="currentColor" className="text-slate-400"/>
            </svg>

            {/* Region Markers */}
            {regionData.map((region) => (<div key={region.id} className="absolute transform -translate-x-1/2 -translate-y-1/2 cursor-pointer" style={{
                left: `${region.coordinates.x}%`,
                top: `${region.coordinates.y}%`,
            }} onMouseEnter={(e) => handleRegionHover(region, e)} onMouseMove={(e) => handleRegionHover(region, e)} onMouseLeave={handleRegionLeave}>
                {/* Pulse Effect */}
                ...
                {/* Main Marker */}
                <div className="relative rounded-full border-2 border-white shadow-lg flex items-center justify-center" style={{
                backgroundColor: getIntensityColor(region.intensity),
            ...
            <div className="absolute inset-0 pointer-events-none">
              {regionData.filter(r => r.intensity > 70).map((region, index) => (<div key={`wave-${region.id}`} className="absolute transform -translate-x-1/2 -translate-y-1/2" style={{
                left: `${region.coordinates.x}%`,
                top: `${region.coordinates.y}%`,
          ...
          {/* Tooltip */}
          <div ref={tooltipRef} className="fixed z-50 bg-slate-900 text-white px-3 py-2 rounded-lg shadow-lg pointer-events-none hidden" style={{ display: 'none' }}>
            {hoveredRegion && (<div className="text-sm">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            .map((region) => (<div key={region.id} className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 border hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-full border border-white" style={{ backgroundColor: getIntensityColor(region.intensity) }}/>
