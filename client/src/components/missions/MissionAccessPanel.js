...
import { Unlock, Lock, Brain, Shield, TrendingUp, MessageSquare, Vote, ChevronRight, CheckCircle, AlertTriangle, User, Trophy, Eye, EyeOff } from 'lucide-react';
import { MissionUnlockEngine, createUserContext } from '../../missions/MissionUnlockEngine';
import { CivicMissionLedger } from '../../missions/CivicMissionLedger';
import { announceUnlockSuccess, announceUnlockBlocked, explainEligibility } from '../../missions/MissionNarrationNode';
import { logUnlockAttempt, logUnlockSuccess, logUnlockBlocked } from '../../services/UnlockTelemetryLogger';
// Main MissionAccessPanel component
export const MissionAccessPanel = ({ userId = 'did:civic:commander-mark', userTier = 'Administrator', trustScore = 98, showAdvancedInfo = false, onMissionClick }) => {
    // Component state
    ...
    const loadMissions = async () => {
        ...
        try {
            ...
            const userContext = createUserContext(userId, userTier, trustScore, {
                replayHistory: [
                    {
    ...
    const handleMissionClick = async (mission, eligibility) => {
        const startTime = performance.now();
        // Log unlock attempt
    ...
    // Get status icon
    const getStatusIcon = (eligibility) => {
        if (eligibility.isUnlocked) {
    ...
    // Get status text
    const getStatusText = (eligibility) => {
        if (eligibility.isUnlocked)
            return 'Unlocked';
        const primaryBlocker = eligibility.blockers[0];
        if (!primaryBlocker)
    ...
    const getStatusColorClass = (eligibility) => {
        if (eligibility.isUnlocked)
    ...
    return (<div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={loadMissions} className="flex items-center gap-2 px-3 py-2 bg-purple-900/30 border border-purple-600 text-purple-300 rounded-lg hover:bg-purple-800/30 transition-colors text-sm" aria-label="Refresh missions">
        ...
        <div className="bg-slate-700/30 rounded-lg p-4 mb-4">
          <div className="flex items-center justify-between">
            ...
            {showAdvancedInfo && (<button onClick={() => setExpandedDetails(prev => new Set())} className="flex items-center gap-1 px-2 py-1 text-xs text-slate-400 hover:text-slate-300 transition-colors">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              {['all', 'unlocked', 'locked'].map((status) => (<button key={status} onClick={() => setFilterStatus(status)} className={`px-3 py-1 text-sm rounded-lg transition-colors ${filterStatus === status
      ...
      <div className="space-y-4">
        {filteredMissions.length === 0 ? (<div className="bg-slate-800 border border-slate-600 rounded-lg p-8 text-center">
          ...
          </div>) : (filteredMissions.map(({ mission, eligibility }) => {
            const isExpanded = expandedDetails.has(mission.missionId);
            return (<div key={mission.missionId} className="bg-slate-800 border border-slate-600 rounded-lg overflow-hidden">
                {/* Mission Header */}
                <div className={`p-4 cursor-pointer transition-colors ${eligibility.isUnlocked
                    ? 'hover:bg-green-900/10'
                    : 'hover:bg-slate-700/30'}`} onClick={() => handleMissionClick(mission, eligibility)} role="button" tabIndex={0} aria-label={`${mission.title} - ${getStatusText(eligibility)}`} onKeyDown={(e) => {
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {showAdvancedInfo && (<button onClick={(e) => {
                        e.stopPropagation();
                        toggleMissionDetails(mission.missionId);
                    }} className="text-xs text-slate-400 hover:text-slate-300 transition-colors" aria-label={`${isExpanded ? 'Hide' : 'Show'} mission details`}>
                ...
                {isExpanded && (<div className="border-t border-slate-600 p-4 bg-slate-700/30">
                    {!eligibility.isUnlocked && (<div className="mb-4">
                        <h4 className="text-sm font-medium text-slate-300 mb-2">Requirements Not Met</h4>
                        <div className="space-y-2">
                          {eligibility.blockers.map((blocker, index) => (<div key={index} className="flex items-start gap-3 text-sm">
                    ...
                    {eligibility.nextSteps.length > 0 && (<div className="mb-4">
                        <h4 className="text-sm font-medium text-slate-300 mb-2">Next Steps</h4>
                        <ul className="space-y-1">
                          {eligibility.nextSteps.map((step, index) => (<li key={index} className="text-sm text-slate-400 flex items-start gap-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <div className="space-y-1">
                          ...
                          {mission.requirements.requireReplay && (<div className={`flex items-center justify-between ${eligibility.requirements.replayValidated ? 'text-green-400' : 'text-red-400'}`}>
