...
export const RoleBasedOverlay = ({ children, config, userRole, onAccessViolation, onRenderEvent }) => {
    const [visible, setVisible] = useState(false);
    const [fallbackActive, setFallbackActive] = useState(false);
    const [renderTime, setRenderTime] = useState(0);
    const [zkpSyncStatus, setZkpSyncStatus] = useState('synced');
    ...
    const checkRoleAccess = (role, overlayConfig) => {
    ...
    const getZkpStatusColor = (status) => {
        switch (status) {
    ...
    return (<div className="relative">
      ...
      {(config.testingMode || fallbackActive) && (<div className="absolute top-0 right-0 z-50 bg-slate-800 border border-slate-600 rounded-bl-lg p-2">
          <div className="flex items-center gap-2 text-xs">
            ...
            <div className="flex items-center gap-1">
              {visible ? (<Eye className="w-3 h-3 text-green-400"/>) : (<EyeOff className="w-3 h-3 text-red-400"/>)}
              <span className="text-slate-400">{renderTime.toFixed(1)}ms</span>
            </div>
          </div>
          
          {fallbackActive && (<div className="mt-1 text-xs text-amber-300">
      ...
      {fallbackActive && (<div className="absolute inset-0 flex items-center justify-center bg-slate-900/80 rounded-lg">
