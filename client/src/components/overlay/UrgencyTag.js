...
let urgencyRegistry = new Map();
export const UrgencyTag = ({ level, message, overlayId, onLevelChange, onPushbackTrigger, testingMode = false }) => {
    ...
    return (<div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border ${config.bgColor} ${config.borderColor} ${pulseActive ? 'animate-pulse' : ''}`}>
      ...
      {testingMode && (<div className="flex items-center gap-1 ml-2 border-l border-slate-600 pl-2">
          <button onClick={() => handleLevelChange('info')} className={`w-2 h-2 rounded-full ${currentLevel === 'info' ? 'bg-blue-400' : 'bg-slate-600'}`} title="Set to Info"/>
          <button onClick={() => handleLevelChange('warning')} className={`w-2 h-2 rounded-full ${currentLevel === 'warning' ? 'bg-amber-400' : 'bg-slate-600'}`} title="Set to Warning"/>
          <button onClick={() => handleLevelChange('urgent')} className={`w-2 h-2 rounded-full ${currentLevel === 'urgent' ? 'bg-orange-400' : 'bg-slate-600'}`} title="Set to Urgent"/>
          <button onClick={() => handleLevelChange('critical')} className={`w-2 h-2 rounded-full ${currentLevel === 'critical' ? 'bg-red-400' : 'bg-slate-600'}`} title="Set to Critical"/>
        </div>)}

      {/* Pushback indicator */}
      {globalUrgencyMetrics.pushbackTriggered && (<div className="flex items-center gap-1 ml-2 border-l border-red-600 pl-2">
      ...
      {simulationActive && (<div className="flex items-center gap-1 ml-2 border-l border-amber-600 pl-2">
...
export const getGlobalUrgencyMetrics = () => {
    return { ...globalUrgencyMetrics };
};
// Export registry for debugging
export const getUrgencyRegistry = () => {
