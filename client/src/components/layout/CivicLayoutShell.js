...
import { BookOpen, Shield, TrendingUp, Vote } from 'lucide-react';
...
export const useUserSession = () => {
    ...
    return {
        ...
        updateSession: (updates) => {
            if (updates.tier)
                sessionEngine.updateTier(updates.tier);
            if (updates.trustStreak)
                sessionEngine.updateStreak(updates.trustStreak);
        }
    };
};
// Internal CivicLayoutShell component (wrapped by UserSessionEngine)
const CivicLayoutShellInternal = () => {
    const sessionEngine = useUserSessionEngine();
    ...
    useEffect(() => {
        if (language !== sessionEngine.session.lang) {
            sessionEngine.updateLang(language);
        }
    ...
    return (<LanguageContext.Provider value={{ language, setLanguage }}>
      <LanguageContextAudit>
        ...
        <div className="max-w-md mx-auto min-h-screen flex flex-col">
          {/* View Container */}
          <div className="flex-1 relative" role="main" aria-label={t('view.current', { view: currentView })}>
            {renderCurrentView()}
          </div>

          {/* Unified Navigation Bar Extension */}
          <UnifiedNavBarExtension currentView={currentView} onViewChange={handleViewSwitch} userTier={sessionEngine.session.tier}/>
        </div>
...
export const CivicLayoutShell = () => {
    return (<UserSessionEngine initialSession={{ lang: 'es', tier: 'citizen', mission: 'voice' }}>
