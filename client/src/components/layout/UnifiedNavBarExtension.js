...
export const UnifiedNavBarExtension = ({ currentView, onViewChange, userTier }) => {
    const { t } = useTranslation();
    const zkpTier = useZKPTier();
    const effectiveTier = userTier || zkpTier;
    const navItems = getNavItems(t);
    // Filter nav items based on tier
    const visibleItems = navItems.filter(item => item.tierRequired.includes(effectiveTier));
    ...
    return (<nav className="fixed bottom-0 left-0 right-0 bg-slate-800 border-t border-slate-700 p-4 max-w-md mx-auto" role="tablist" aria-label="Main navigation">
      <div className="flex justify-around items-center">
        {visibleItems.map((item) => (<button key={item.id} onClick={() => handleNavClick(item.id)} onKeyDown={(e) => handleKeyDown(e, item.id)} className={`flex flex-col items-center gap-1 p-2 rounded-lg transition-all duration-200 min-w-[60px] ${currentView === item.id
                ? 'bg-blue-500/20 text-blue-400'
                : 'text-slate-400 hover:text-white hover:bg-slate-700/50'}`} role="tab" aria-current={currentView === item.id ? 'page' : undefined} aria-selected={currentView === item.id} aria-label={`Navigate to ${item.label}`} tabIndex={0}>
