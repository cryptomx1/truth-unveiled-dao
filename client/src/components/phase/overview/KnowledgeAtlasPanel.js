...
import { Brain, Filter, Globe, Users, Shield, Zap, Download, RefreshCw, AlertTriangle, MapPin, Database, Network, Eye, Volume2, VolumeX } from 'lucide-react';
export default function KnowledgeAtlasPanel() {
    ...
    const generateClusters = (nodes, pillarFilter) => {
    ...
    const checkFallbackConditions = (clusters) => {
    ...
    const generateOverlapMetrics = (nodes) => {
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      <div className="mb-6 space-y-4">
        ...
        <div className="p-3 bg-slate-700 border border-slate-600 rounded-md">
          ...
          <select value={selectedPillar} onChange={(e) => handlePillarChange(e.target.value)} className="w-full p-2 bg-slate-600 border border-slate-500 rounded text-sm text-white" style={{ minHeight: '48px' }} aria-label="Select pillar filter">
        ...
        <div className="p-3 bg-slate-700 border border-slate-600 rounded-md">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={loadAtlasData} className="py-1 px-3 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-medium transition-colors duration-200 flex items-center gap-1" style={{ minHeight: '32px' }} disabled={isLoading} aria-label="Refresh atlas data">
      ...
      <div className="mb-6 bg-slate-700 border border-slate-600 rounded-md">
        <div className="p-3 border-b border-slate-600 flex items-center justify-between">
          ...
          <button onClick={generateExportSnapshot} className="py-1 px-2 bg-purple-600 hover:bg-purple-700 text-white rounded text-xs font-medium transition-colors duration-200 flex items-center gap-1" style={{ minHeight: '32px' }} disabled={clusters.length === 0} aria-label="Export atlas snapshot">
        ...
        <div className="max-h-96 overflow-y-auto">
          {clusters.length > 0 ? (<div className="divide-y divide-slate-600">
              {clusters.map((cluster, index) => {
                ...
                return (<div key={cluster.clusterId} className={`p-3 hover:bg-slate-600 transition-colors duration-200 ${cluster.isEmpty ? 'bg-red-900/20' : ''}`}>
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center space-x-2">
                        ...
                        {cluster.isEmpty && (<AlertTriangle className="w-4 h-4 text-red-400"/>)}
                      </div>
                    ...
                    <div className="space-y-2 text-xs">
                      ...
                      {cluster.nodes.length > 0 && (<div className="mt-2">
                          <div className="text-slate-500 mb-1">Sample Nodes:</div>
                          {cluster.nodes.slice(0, 2).map(node => (<div key={node.id} className="text-slate-400 ml-2">
                              • {node.title} (Deck #{node.deckId})
                            </div>))}
                          {cluster.nodes.length > 2 && (<div className="text-slate-500 ml-2">
      ...
      {/* Overlap Metrics */}
      {knowledgeNodes.length > 0 && (<div className="mb-6 p-4 bg-slate-900 border border-slate-600 rounded-md">
      ...
      {/* Fallback Status */}
      {fallbackTriggered && (<div className="mb-6 p-4 bg-red-900/20 border border-red-600 rounded-md">
      ...
      {showExportModal && exportData && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-slate-800 border border-slate-700 rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button onClick={downloadExportData} className="py-2 px-4 bg-purple-600 hover:bg-purple-700 text-white rounded-md transition-colors duration-200" style={{ minHeight: '48px' }}>
                Download JSON
              </button>
            </div>
          </div>
