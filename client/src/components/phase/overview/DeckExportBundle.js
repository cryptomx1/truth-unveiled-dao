...
import { Download, Upload, Package, CheckCircle, X, AlertTriangle, Loader2, Shield, Copy, Archive } from 'lucide-react';
export default function DeckExportBundle({ decks = [], role = 'Citizen', userDid = 'did:civic:export_user_001', onExportComplete, onExportFailed, className = '' }) {
    ...
    const mockDecks = decks.length > 0 ? decks : [
        {
    ...
    const generateZKPProof = useCallback(async (deck) => {
    ...
    const uploadToIPFS = useCallback(async (bundle) => {
    ...
    const saveToLocalFallback = useCallback(async (bundle) => {
    ...
    const handleExport = async () => {
        if (!selectedDeckId || exportStatus !== 'idle')
            return;
    ...
    return (<div className={`w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6 ${className}`}>
      ...
      <div aria-live="off" aria-atomic="true" className="sr-only" aria-hidden="true">
        {ariaAnnouncement}
      </div>
      ...
      <div className="mb-6">
        ...
        {pathBTriggered && (<div className="mb-4 p-3 bg-red-900/20 border border-red-600 rounded-md">
            ...
            }} className="py-1 px-3 bg-red-700 hover:bg-red-600 text-white rounded-md text-xs transition-colors duration-200" style={{ minHeight: '32px' }}>
              Reset Fallback
            </button>
          </div>)}
      </div>
      ...
      <div className="mb-6">
        ...
        <select value={selectedDeckId} onChange={(e) => setSelectedDeckId(e.target.value)} className="w-full p-3 bg-slate-700 border border-slate-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label="Select deck to export" disabled={exportStatus !== 'idle'}>
          <option value="">Choose a deck...</option>
          {mockDecks.map(deck => (<option key={deck.id} value={deck.id}>
      ...
      {/* Selected Deck Info */}
      {selectedDeck && (<div className="mb-6 p-4 bg-slate-700 border border-slate-600 rounded-md">
      ...
      {/* Export Controls */}
      <div className="mb-6">
        <button onClick={handleExport} disabled={!selectedDeckId || exportStatus !== 'idle'} className="w-full py-3 px-4 bg-blue-700 hover:bg-blue-600 disabled:bg-slate-600 disabled:cursor-not-allowed text-white rounded-md font-medium transition-colors duration-200 flex items-center justify-center gap-2" style={{ minHeight: '48px' }} aria-label="Export selected deck bundle">
          {exportStatus === 'idle' ? (<>
        ...
        {exportStatus !== 'idle' && (<div className="mt-3">
            <div className="flex justify-between text-xs text-slate-400 mb-1">
              <span>Progress</span>
              <span>{progress}%</span>
            </div>
            <div className="w-full bg-slate-600 rounded-full h-2">
              <div className="bg-blue-400 h-2 rounded-full transition-all duration-300" style={{ width: `${progress}%` }}/>
            </div>
          </div>)}
      </div>
      ...
      <div className="mb-6 p-4 bg-slate-700 border border-slate-600 rounded-md">
        ...
        {error && (<div className="text-xs text-red-400 mb-2">
            Error: {error}
          </div>)}
        
        {exportCid && (<div className="text-xs">
            <span className="text-slate-400">IPFS CID: </span>
            <span className="text-green-400 font-mono break-all">{exportCid}</span>
            <button onClick={() => copyCID(exportCid)} className="ml-2 p-1 text-slate-400 hover:text-white transition-colors" aria-label="Copy CID to clipboard">
      ...
      {/* Export History */}
      {exportHistory.length > 0 && (<div className="mb-6 p-4 bg-slate-700 border border-slate-600 rounded-md">
          <h3 className="text-sm font-medium text-slate-300 mb-3">
            Export History ({exportHistory.length})
          </h3>
          <div className="space-y-2 max-h-40 overflow-y-auto">
            {exportHistory.map((bundle, index) => (<div key={bundle.exportId} className="p-2 bg-slate-800 rounded-md">
                ...
                <div className="text-xs text-slate-400">
                  {bundle.cid === 'local_fallback' ? (<span className="text-orange-400">Local Fallback</span>) : (<span className="text-green-400">IPFS: {bundle.cid?.substring(0, 16)}...</span>)}
                </div>
              </div>))}
          </div>
        </div>)}
      ...
      {/* Reset Controls */}
      {(exportStatus === 'success' || exportStatus === 'failed') && (<div className="mb-6">
          <button onClick={resetExport} className="w-full py-2 px-4 bg-slate-600 hover:bg-slate-500 text-white rounded-md text-sm transition-colors duration-200" style={{ minHeight: '40px' }}>
            Export Another Deck
          </button>
        </div>)}
