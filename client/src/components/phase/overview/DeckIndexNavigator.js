...
import { Search, ArrowLeft, Home, SkipForward, Brain, Users, Shield, Zap, Globe, Eye, Database, Volume2, VolumeX, AlertTriangle } from 'lucide-react';
export default function DeckIndexNavigator({ decks = [], onDeckSelect, onPillarJump, onLatestJump, onReturn }) {
    const [searchTerm, setSearchTerm] = useState('');
    const [debouncedSearchTerm, setDebouncedSearchTerm] = useState('');
    const [selectedPillar, setSelectedPillar] = useState('All');
    const [selectedCivicFunction, setSelectedCivicFunction] = useState('All');
    ...
    useEffect(() => {
        ...
        if (debouncedSearchTerm) {
            filtered = filtered.filter(deck => deck.name.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
    ...
    const handleKeyDown = (e) => {
        ...
        switch (e.key) {
            case 'ArrowDown':
                e.preventDefault();
                setFocusedIndex(prev => prev < filteredDecks.length - 1 ? prev + 1 : 0);
                break;
            case 'ArrowUp':
                e.preventDefault();
                setFocusedIndex(prev => prev > 0 ? prev - 1 : filteredDecks.length - 1);
                break;
            case 'Enter':
                e.preventDefault();
                if (focusedIndex >= 0) {
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      {/* ARIA Live Region */}
      ...
      <div className="mb-6 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          <button onClick={handleReturn} className="py-2 px-3 bg-slate-700 hover:bg-slate-600 text-white rounded-md transition-colors duration-200 flex items-center justify-center gap-1" style={{ minHeight: '48px' }} aria-label="Return to previous view">
            <ArrowLeft className="w-4 h-4"/>
            <span className="text-xs">Return</span>
          </button>
          
          <button onClick={() => handlePillarJump(selectedPillar)} className="py-2 px-3 bg-purple-700 hover:bg-purple-600 text-white rounded-md transition-colors duration-200 flex items-center justify-center gap-1" style={{ minHeight: '48px' }} aria-label="Jump to pillar root">
            <Home className="w-4 h-4"/>
            <span className="text-xs">Pillar</span>
          </button>
          
          <button onClick={handleLatestJump} className="py-2 px-3 bg-green-700 hover:bg-green-600 text-white rounded-md transition-colors duration-200 flex items-center justify-center gap-1" style={{ minHeight: '48px' }} aria-label="Jump to latest updates">
      ...
      <div className="mb-6 p-3 bg-slate-700 border border-slate-600 rounded-md">
        <div className="flex items-center justify-between">
          ...
          <button onClick={toggleTTS} className={`py-1 px-3 rounded text-xs font-medium transition-colors duration-200 ${ttsEnabled ? 'bg-blue-600 text-white' : 'bg-slate-600 text-slate-300'}`} style={{ minHeight: '32px' }} aria-label={`${ttsEnabled ? 'Disable' : 'Enable'} TTS`}>
      ...
      <div className="mb-6 space-y-3">
        ...
        <div className="relative">
          ...
          <input ref={searchInputRef} type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search decks..." className="w-full pl-10 pr-4 py-3 bg-slate-600 border border-slate-500 rounded-md text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500" style={{ minHeight: '48px' }} onKeyDown={handleKeyDown} aria-label="Search decks"/>
        </div>
      </div>
      ...
      <div className="mb-6 space-y-3">
        <div className="grid grid-cols-1 gap-3">
          <select value={selectedPillar} onChange={(e) => setSelectedPillar(e.target.value)} className="w-full p-3 bg-slate-600 border border-slate-500 rounded-md text-white" style={{ minHeight: '48px' }} aria-label="Filter by pillar">
            {pillars.map(pillar => (<option key={pillar} value={pillar}>Pillar: {pillar}</option>))}
          </select>
          
          <select value={selectedCivicFunction} onChange={(e) => setSelectedCivicFunction(e.target.value)} className="w-full p-3 bg-slate-600 border border-slate-500 rounded-md text-white" style={{ minHeight: '48px' }} aria-label="Filter by civic function">
            {civicFunctions.map(func => (<option key={func} value={func}>Function: {func}</option>))}
          </select>
          
          <select value={selectedUserType} onChange={(e) => setSelectedUserType(e.target.value)} className="w-full p-3 bg-slate-600 border border-slate-500 rounded-md text-white" style={{ minHeight: '48px' }} aria-label="Filter by user type">
            {userTypes.map(type => (<option key={type} value={type}>User: {type}</option>))}
          </select>
        </div>
      </div>

      {/* Federated Search Status */}
      {federatedSearchActive && (<div className="mb-6 p-4 bg-blue-900/20 border border-blue-600 rounded-md">
      ...
      {/* Path B Alert */}
      {pathBTriggered && (<div className="mb-6 p-4 bg-red-900/20 border border-red-600 rounded-md">
          <div className="flex items-center gap-2 mb-2">
      ...
      <div className="mb-6 bg-slate-700 border border-slate-600 rounded-md">
        <div className="p-3 border-b border-slate-600 flex items-center justify-between">
          ...
          {filteredDecks.length > 0 && (<div className="text-xs text-slate-400">
              Use ↑↓ keys to navigate
            </div>)}
        </div>
        
        <div className="max-h-96 overflow-y-auto">
          {filteredDecks.length > 0 ? (<ul ref={listRef} role="list" aria-live="polite" className="divide-y divide-slate-600">
              {filteredDecks.map((deck, index) => {
                const pillarIcon = getPillarIcon(deck.pillar);
                const statusColor = getStatusColor(deck.status);
                const isFocused = index === focusedIndex;
                return (<li key={deck.id} role="listitem" className={`p-3 cursor-pointer transition-colors duration-200 ${isFocused ? 'bg-blue-600/20 ring-2 ring-blue-500' : 'hover:bg-slate-600'}`} onClick={() => handleDeckSelect(deck.id)} tabIndex={0} onKeyDown={(e) => {
            ...
            </ul>) : (<div className="p-6 text-center">
