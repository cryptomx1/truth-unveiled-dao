...
import { Activity, AlertTriangle, CheckCircle, Settings, Volume2, VolumeX, Download, RefreshCw, Zap, Shield, Eye, Clock, Database, Cpu } from 'lucide-react';
export default function CivicSyncDiagnosticsPanel() {
    ...
    const [qaEnvelope, setQAEnvelope] = useState(null);
    const [lastDiagnosticRun, setLastDiagnosticRun] = useState(0);
    ...
    const generateDiagnosticResult = (test) => {
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      <div className="mb-6 space-y-4">
        ...
        <div className="flex items-center justify-between p-3 bg-slate-700 border border-slate-600 rounded-md">
          ...
          <button onClick={toggleFallbackSimulation} className={`py-1 px-3 rounded text-xs font-medium transition-colors duration-200 ${fallbackSimulationActive ? 'bg-orange-600 text-white' : 'bg-slate-600 text-slate-300'}`} style={{ minHeight: '32px' }} aria-label={`${fallbackSimulationActive ? 'Deactivate' : 'Activate'} fallback simulation`}>
        ...
        <div className="p-3 bg-slate-700 border border-slate-600 rounded-md">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={runDiagnostics} className="py-1 px-3 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-medium transition-colors duration-200 flex items-center gap-1" style={{ minHeight: '32px' }} disabled={isRunningDiagnostics} aria-label="Run diagnostic tests">
              <RefreshCw className={`w-3 h-3 ${isRunningDiagnostics ? 'animate-spin' : ''}`}/>
              {isRunningDiagnostics ? 'Running...' : 'Run Tests'}
            </button>
          </div>
          
          {lastDiagnosticRun > 0 && (<div className="text-xs text-slate-400">
      ...
      <div className="mb-6 bg-slate-700 border border-slate-600 rounded-md">
        <div className="p-3 border-b border-slate-600 flex items-center justify-between">
          ...
          <button onClick={generateExportData} className="py-1 px-2 bg-purple-600 hover:bg-purple-700 text-white rounded text-xs font-medium transition-colors duration-200 flex items-center gap-1" style={{ minHeight: '32px' }} disabled={diagnosticResults.length === 0} aria-label="Export diagnostic results">
        ...
        <div className="max-h-80 overflow-y-auto">
          {diagnosticResults.length > 0 ? (<div className="divide-y divide-slate-600">
              {diagnosticResults.map((result, index) => {
                ...
                return (<div key={result.testId} className={`p-3 hover:bg-slate-600 transition-colors duration-200 ${statusDisplay.bg}`}>
                    ...
                    <div className="space-y-1 text-xs">
                      ...
                      {result.details.length > 0 && (<div className="mt-2 space-y-1">
                          {result.details.map((detail, idx) => (<div key={idx} className="text-slate-400 ml-2">
                              • {detail}
                            </div>))}
                        </div>)}
      ...
      {/* QA Envelope Status */}
      {qaEnvelope && (<div className="mb-6 p-4 bg-slate-900 border border-slate-600 rounded-md">
          ...
          <div className="space-y-2 text-xs">
            ...
            <div className="flex justify-between">
              ...
              <span className={`font-medium ${qaEnvelope.lockInVerdict === 'sealed' ? 'text-green-400' :
