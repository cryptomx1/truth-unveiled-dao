...
import { Filter, Volume2, VolumeX, Download, RefreshCw, Shield, Lock, Unlock } from 'lucide-react';
export default function DeckControlSwitchboard({ currentUserDID = 'did:civic:commander_mark_0x7f3e2d1a8c9b5f2e' }) {
    ...
    const filterDecks = async (filter) => {
        setIsLoading(true);
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      {hasCommanderAccess() && (<div className="mb-6 p-4 bg-purple-900/20 border border-purple-700 rounded-md">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className={`text-xs px-2 py-1 rounded flex items-center gap-1 ${globalOverrideEnabled ? 'bg-green-900 text-green-200' : 'bg-slate-900 text-slate-300'}`}>
          ...
          <button onClick={toggleGlobalOverride} className={`w-full py-2 px-4 rounded-md font-medium transition-colors duration-200 ${globalOverrideEnabled
                ? 'bg-red-600 hover:bg-red-700 text-white'
                : 'bg-green-600 hover:bg-green-700 text-white'}`} style={{ minHeight: '48px' }} aria-label={`${globalOverrideEnabled ? 'Disable' : 'Enable'} global override`}>
      ...
      <div className="mb-6 space-y-4">
        ...
        <div className="p-3 bg-slate-700 border border-slate-600 rounded-md">
          ...
          <div className="grid grid-cols-2 gap-2">
            {['All', 'QA-Locked', 'Pending', 'Refactor'].map((filter) => (<button key={filter} onClick={() => filterDecks(filter)} className={`py-2 px-3 rounded text-xs font-medium transition-colors duration-200 ${currentFilter === filter
                ? 'bg-blue-600 text-white'
                : 'bg-slate-600 hover:bg-slate-500 text-slate-300'}`} style={{ minHeight: '36px' }} disabled={isLoading} aria-label={`Filter decks by ${filter}`}>
                {filter}
              </button>))}
          </div>
        ...
        <div className="flex items-center justify-between p-3 bg-slate-700 border border-slate-600 rounded-md">
          ...
          <button onClick={triggerPhaseSync} className="py-1 px-3 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-medium transition-colors duration-200" style={{ minHeight: '32px' }} disabled={isLoading} aria-label="Trigger phase sync">
      ...
      <div className="mb-6 bg-slate-700 border border-slate-600 rounded-md">
        <div className="p-3 border-b border-slate-600 flex items-center justify-between">
          ...
          <button onClick={exportActionLog} className="py-1 px-2 bg-purple-600 hover:bg-purple-700 text-white rounded text-xs font-medium transition-colors duration-200 flex items-center gap-1" style={{ minHeight: '32px' }} aria-label="Export action log">
        ...
        <div className="max-h-48 overflow-y-auto">
          {actionLog.length > 0 ? (<div className="divide-y divide-slate-600">
              {actionLog.slice(-maxLogEntries).reverse().map((entry, index) => (<div key={index} className="p-3 hover:bg-slate-600 transition-colors duration-200">
      ...
      {/* Export Modal */}
      {showExportModal && exportData && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-slate-800 border border-slate-700 rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setShowExportModal(false)} className="py-2 px-4 bg-slate-600 hover:bg-slate-700 text-white rounded-md transition-colors duration-200" style={{ minHeight: '48px' }}>
                Cancel
              </button>
              
              <button onClick={downloadExportData} className="py-2 px-4 bg-green-600 hover:bg-green-700 text-white rounded-md transition-colors duration-200" style={{ minHeight: '48px' }}>
                Download JSON
              </button>
            </div>
          </div>
