...
import { Target, CheckCircle, Clock, AlertTriangle, X, Trophy, Zap, Shield, Users, FileText, Loader2, Brain, MapPin } from 'lucide-react';
export default function DeckMissionCard({ userDid = 'did:civic:mission_user_001', onMissionSelect, onMissionComplete, className = '' }) {
    ...
    const calculateStats = useCallback((missionList) => {
    ...
    const handleMissionSubmit = async (mission) => {
        if (!mission || submissionActive)
            return;
        ...
        try {
            ...
            if (!submissionSuccess) {
                ...
                const updatedMissions = missions.map(m => m.id === mission.id
                    ? { ...m, status: 'failed', failureReason: 'ZKP attestation failed' }
                    : m);
    ...
    return (<div className={`w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6 ${className}`}>
      ...
      <div className="mb-6">
        ...
        {pathBTriggered && (<div className="mb-4 p-3 bg-red-900/20 border border-red-600 rounded-md">
            ...
            <button onClick={resetPathB} className="py-1 px-3 bg-red-700 hover:bg-red-600 text-white rounded-md text-xs transition-colors duration-200" style={{ minHeight: '32px' }}>
              Reset Fallback
            </button>
          </div>)}
      </div>
      ...
      <div className="mb-6 bg-slate-700 border border-slate-600 rounded-md">
        ...
        <div className="max-h-80 overflow-y-auto">
          {missions.length === 0 ? (<div className="p-6 text-center">
            ...
            </div>) : (<ul role="list" className="divide-y divide-slate-600">
              {missions.map((mission) => (<li key={mission.id} role="listitem" className="p-3">
                  ...
                  {mission.status === 'in_progress' && (<div className="mb-3">
                      <div className="flex justify-between text-xs text-slate-400 mb-1">
                        <span>Progress</span>
                        <span>{mission.progress}%</span>
                      </div>
                      <div className="w-full bg-slate-600 rounded-full h-2">
                        <div className="bg-blue-400 h-2 rounded-full transition-all duration-300" style={{ width: `${mission.progress}%` }}/>
                      </div>
                    </div>)}
                  
                  {mission.zkpHash && (<div className="mb-3 text-xs">
                  ...
                  <div className="flex gap-2">
                    {mission.status === 'available' && (<button onClick={() => handleMissionSelect(mission)} className="flex-1 py-2 px-3 bg-blue-700 hover:bg-blue-600 text-white rounded-md text-xs transition-colors duration-200" style={{ minHeight: '40px' }} aria-label={`Select mission: ${mission.title}`}>
                        Select Mission
                      </button>)}
                    
                    {mission.status === 'in_progress' && (<button onClick={() => handleMissionSubmit(mission)} disabled={submissionActive} className="flex-1 py-2 px-3 bg-green-700 hover:bg-green-600 disabled:bg-green-800 text-white rounded-md text-xs transition-colors duration-200 flex items-center justify-center gap-2" style={{ minHeight: '40px' }} aria-label={`Submit mission: ${mission.title}`}>
                        {submissionActive ? (<>
                    ...
                    {mission.status === 'completed' && (<div className="flex-1 py-2 px-3 bg-green-900 text-green-300 rounded-md text-xs flex items-center justify-center gap-2">
                        <Trophy className="w-3 h-3"/>
                        <span>Completed</span>
                      </div>)}
                    
                    {mission.status === 'failed' && (<div className="flex-1 py-2 px-3 bg-red-900 text-red-300 rounded-md text-xs flex items-center justify-center gap-2">
                        <X className="w-3 h-3"/>
                        <span>Failed</span>
                      </div>)}
                  </div>
      ...
      {selectedMission && (<div className="mb-6 p-4 bg-slate-900 border border-slate-600 rounded-md">
          ...
          <div className="space-y-2 text-xs">
            ...
            <div className="mt-3">
              ...
              <ul className="text-slate-300 space-y-1">
                {selectedMission.requirements.map((req, index) => (<li key={index} className="text-xs">
                    • {req}
                  </li>))}
              </ul>
            </div>
