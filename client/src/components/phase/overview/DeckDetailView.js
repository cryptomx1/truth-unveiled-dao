...
import { ArrowLeft, Check, X, Shield, Eye, Flag, CheckCircle, AlertTriangle, Clock, Users, Database, Globe, Zap, Brain, Loader2 } from 'lucide-react';
const DeckDetailView = React.memo(function DeckDetailView({ deckId = 1, onBack, onNavigate, userRole = { type: 'citizen', permissions: ['view'], did: 'did:civic:citizen_001' } }) {
    const [deckMetadata, setDeckMetadata] = useState(null);
    const [loading, setLoading] = useState(true);
    const [zkpValidating, setZkpValidating] = useState(false);
    const [zkpStatus, setZkpStatus] = useState('pending');
    ...
    const validationStartTime = useRef(0);
    // Generate mock deck metadata based on deckId
    const generateDeckMetadata = (id) => {
        const deckTemplates = [
            {
    ...
    const validateZKPMetadata = useCallback(async (metadata) => {
    ...
    const checkAccessControl = useCallback((metadata, role) => {
    ...
    const getRoleCapabilities = useCallback((role) => {
        switch (role.type) {
            case 'citizen':
                return {
                    canView: true,
    ...
    if (loading) {
        return (<div className="min-w-[300px] max-w-[460px] h-auto mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6 overflow-y-auto">
    ...
    if (!deckMetadata) {
        ...
        <div className="text-center">
          ...
          <button onClick={handleBack} className="py-2 px-4 bg-slate-700 hover:bg-slate-600 text-white rounded-md transition-colors duration-200" style={{ minHeight: '48px' }}>
            Back to Index
          </button>
        </div>
      </div>);
    ...
    return (<div className="min-w-[300px] max-w-[460px] h-auto mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6 overflow-y-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <button onClick={handleBack} className="flex items-center gap-2 py-2 px-3 bg-slate-700 hover:bg-slate-600 text-white rounded-md transition-colors duration-200" style={{ minHeight: '48px' }} aria-label="Back to deck index">
      ...
      {!accessGranted && (<div className="mb-6 p-4 bg-red-900/20 border border-red-600 rounded-md">
      ...
      <div className="mb-6 p-4 bg-slate-700 border border-slate-600 rounded-md">
        <div className="flex items-center justify-between mb-3">
          ...
          {zkpValidating ? (<Loader2 className="w-4 h-4 animate-spin text-blue-400"/>) : (<div className="flex items-center gap-1">
        ...
        <div className="space-y-2 text-xs">
          <div className="flex justify-between">
            ...
            <span className={zkpStatus === 'valid' ? 'text-green-400' :
        ...
        {fallbackMode && (<div className="mt-3 p-2 bg-yellow-900/20 border border-yellow-600 rounded">
      ...
      {/* Deck Summary */}
      {accessGranted && (<div className="mb-6 p-4 bg-slate-700 border border-slate-600 rounded-md">
      ...
      {/* Module List */}
      {accessGranted && deckMetadata.modules.length > 0 && (<div className="mb-6 bg-slate-700 border border-slate-600 rounded-md">
          ...
          <div className="max-h-64 overflow-y-auto">
            <ul role="list" className="divide-y divide-slate-600">
              {deckMetadata.modules.map((module) => (<li key={module.id} role="listitem" className="p-3">
      ...
      <div className="mb-6 p-4 bg-slate-700 border border-slate-600 rounded-md">
        ...
        <div className="space-y-3">
          ...
          {accessGranted && (<div className="grid grid-cols-2 gap-2 mt-4">
              {capabilities.canFlag && (<button onClick={() => handleContentAction('flag')} disabled={contentAction === 'flagging'} className="flex items-center justify-center gap-1 py-2 px-3 bg-yellow-700 hover:bg-yellow-600 disabled:bg-yellow-800 text-white rounded-md transition-colors duration-200" style={{ minHeight: '48px' }} aria-label="Flag content for review">
                  {contentAction === 'flagging' ? (<Loader2 className="w-4 h-4 animate-spin"/>) : (<Flag className="w-4 h-4"/>)}
                  <span className="text-xs">Flag</span>
                </button>)}
              
              {capabilities.canApprove && (<button onClick={() => handleContentAction('approve')} disabled={contentAction === 'approving'} className="flex items-center justify-center gap-1 py-2 px-3 bg-green-700 hover:bg-green-600 disabled:bg-green-800 text-white rounded-md transition-colors duration-200" style={{ minHeight: '48px' }} aria-label="Approve content for publication">
                  {contentAction === 'approving' ? (<Loader2 className="w-4 h-4 animate-spin"/>) : (<Check className="w-4 h-4"/>)}
