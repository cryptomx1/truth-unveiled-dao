...
import { Grid, AlertTriangle, CheckCircle, Clock, Download, RefreshCw, Thermometer, WifiOff } from 'lucide-react';
export default function CivicMirrorSyncGrid() {
    ...
    const generateSyncStatuses = () => {
        ...
        return deckMetadata.map(deck => {
            ...
            if (syncScore >= 80) {
                ...
                if (!zkpVerified)
                    faultDetails.push('ZKP verification pending');
            }
    ...
    const generateHeatmapData = (syncStatuses) => {
    ...
    const calculateOverallMetrics = (syncStatuses) => {
    ...
    const generateAriaAnnouncement = (syncStatuses, metrics) => {
    ...
    const getHeatmapStyling = (temperature, intensity) => {
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      <div className="mb-6 p-4 bg-slate-700 border border-slate-600 rounded-md">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={refreshSyncData} className="p-1 bg-blue-600 hover:bg-blue-700 text-white rounded transition-colors duration-200" disabled={isLoading} aria-label="Refresh sync data">
        ...
        <div className="space-y-2 text-xs">
          ...
          {pathBTriggered && (<div className="mt-2 p-2 bg-red-900/20 border border-red-700 rounded">
      ...
      <div className="mb-6 bg-slate-700 border border-slate-600 rounded-md">
        ...
        <div className="max-h-80 overflow-y-auto">
          {isLoading ? (<div className="p-6 text-center">
            ...
            </div>) : syncStatuses.length > 0 ? (<div className="divide-y divide-slate-600">
              {syncStatuses.map((sync, index) => {
                ...
                return (<div key={sync.deckId} className={`p-3 hover:bg-slate-600 transition-colors duration-200 ${statusDisplay.bg} relative`} style={heatmapStyle} role="gridcell" aria-label={`Deck ${sync.deckId}: ${sync.deckName}, sync score ${sync.syncScore}%, status ${sync.status}`}>
                    {/* Heatmap Overlay */}
                    {heatmap && heatmap.desyncLevel > 10 && (<div className={`absolute top-1 right-1 w-2 h-2 rounded-full ${heatmap.temperature === 'critical' ? 'bg-red-500' : heatmap.temperature === 'hot' ? 'bg-orange-500' : 'bg-yellow-500'} animate-pulse`}/>)}
                    ...
                    <div className="space-y-1 text-xs">
                      ...
                      {sync.faultDetails.length > 0 && (<div className="mt-2 space-y-1">
                          <span className="text-slate-400 text-xs">Faults:</span>
                          {sync.faultDetails.map((fault, idx) => (<div key={idx} className="text-xs text-red-400 ml-2">
                              • {fault}
                            </div>))}
                        </div>)}
                      
                      {heatmap && (<div className="mt-2 flex justify-between">
      ...
      {/* Export Control */}
      <div className="mb-4">
        <button onClick={handleExportSnapshot} className="w-full py-3 px-4 bg-green-600 hover:bg-green-700 text-white font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-slate-800 flex items-center justify-center gap-2" style={{ minHeight: '48px' }} aria-label="Export sync snapshot to JSON">
