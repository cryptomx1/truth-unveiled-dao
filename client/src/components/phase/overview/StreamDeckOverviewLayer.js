...
export default function StreamDeckOverviewLayer() {
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      {/* Controls */}
      <div className="mb-6">
        <button onClick={handleExportJSON} className="w-full py-3 px-4 bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-slate-800 flex items-center justify-center gap-2" style={{ minHeight: '48px' }} aria-label="Export deck overview to JSON">
      ...
      <div className="bg-slate-700 border border-slate-600 rounded-md">
        ...
        <div className="max-h-96 overflow-y-auto">
          {isLoading ? (<div className="p-6 text-center">
            ...
            </div>) : deckGrid.length > 0 ? (<div className="divide-y divide-slate-600">
              {deckGrid.map((deck) => {
                const phaseDisplay = getPhaseDisplay(deck.phase);
                return (<div key={deck.id} className={`p-3 hover:bg-slate-600 transition-colors duration-200 ${fallbackActive && deck.name === 'Fallback' ? 'bg-red-900/20' : ''}`} role="row" aria-label={`Deck ${deck.id}: ${formatDeckName(deck.name)} with ${deck.modules} modules, status ${deck.phase}`}>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {deck.swipeToggle && (<div className="flex items-center gap-1">
                    ...
                    {overrideMode && (<div className="mt-2 p-2 bg-purple-900/20 border border-purple-700 rounded flex items-center gap-2">
                    ...
                    {fallbackActive && deck.name === 'Fallback' && (<div className="mt-2 p-2 bg-red-900/20 border border-red-700 rounded">
      ...
      {showExportModal && exportData && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-slate-800 border border-slate-700 rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              ...
              <button onClick={downloadExportJSON} className="py-2 px-4 bg-green-600 hover:bg-green-700 text-white rounded-md transition-colors duration-200" style={{ minHeight: '48px' }}>
                Download JSON
              </button>
            </div>
          </div>
