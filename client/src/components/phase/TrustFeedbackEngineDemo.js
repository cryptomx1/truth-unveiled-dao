...
const TrustFeedbackEngineDemo = ({ className = '', userDid = 'did:civic:trust_engine_demo_001' }) => {
    ...
    return (<div className={`w-full max-w-4xl mx-auto space-y-6 ${className}`}>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={toggleEngine} className={`px-4 py-2 rounded-md font-medium transition-colors ${isEngineActive
            ? 'bg-red-600 hover:bg-red-700 text-white'
            : 'bg-green-600 hover:bg-green-700 text-white'}`} style={{ minHeight: '40px' }}>
            {isEngineActive ? (<>
        ...
        <div className="mt-4 flex justify-center">
          <button onClick={toggleHistory} className="px-6 py-2 bg-slate-700 hover:bg-slate-600 text-white font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800 flex items-center gap-2" style={{ minHeight: '48px' }} aria-label="Open trust history overlay">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className={`${!isEngineActive ? 'opacity-50 pointer-events-none' : ''}`}>
          <FeedbackStreakSyncCard userDid={userDid} onStreakUpdate={handleStreakUpdate} className="h-full"/>
        </div>

        {/* Trust Point Pulse Visualizer */}
        <div className={`${!isEngineActive ? 'opacity-50 pointer-events-none' : ''}`}>
          <TrustPointPulseVisualizer userDid={userDid} initialTrustPoints={totalTrustPoints} onTrustPointChange={handleTrustPointChange} className="h-full"/>
        </div>
      </div>
      ...
      <TrustHistoryOverlay userDid={userDid} isVisible={showHistory} onClose={() => setShowHistory(false)} onEventSelect={(event) => {
