...
import { MessageSquare, Send, AlertTriangle, CheckCircle, Loader2, Eye, EyeOff, User, Zap, RefreshCw, TrendingUp, TrendingDown, Minus } from 'lucide-react';
export default function SentimentCaptureForm({ userRole = 'Citizen', userDid = 'did:civic:sentiment_user_001', onSubmissionComplete, onSubmissionFailed, className = '' }) {
    ...
    const [streakCount, setStreakCount] = useState(2); // Current submission streak
    const [totalTruthPoints, setTotalTruthPoints] = useState(185);
    ...
    const generateZKPSignature = useCallback(async (submission) => {
    ...
    const calculateTruthPoints = useCallback((hasStreakBonus) => {
    ...
    const verifySubmission = useCallback(async (submission) => {
    ...
    const handleSubmission = async () => {
        if (!selectedSentiment || !comment.trim() || submitStatus !== 'idle')
            return;
    ...
    const getSubmitButtonState = () => {
        if (!selectedSentiment || !comment.trim())
            return { disabled: true, text: 'Select Sentiment & Add Comment' };
        if (submitStatus === 'submitting')
            return { disabled: true, text: 'Submitting...' };
        if (submitStatus === 'success')
            return { disabled: true, text: 'Sentiment Submitted!' };
        if (submitStatus === 'failed')
            return { disabled: false, text: 'Retry Submission' };
    ...
    return (<div className={`w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6 ${className}`}>
      {/* ARIA Live Region */}
      ...
      <div className="mb-6">
        ...
        {pathBTriggered && (<div className="mb-4 p-3 bg-red-900/20 border border-red-600 rounded-md">
            ...
            <button onClick={resetFallback} className="py-1 px-3 bg-red-700 hover:bg-red-600 text-white rounded-md text-xs transition-colors duration-200" style={{ minHeight: '32px' }}>
              Reset Fallback
            </button>
          </div>)}
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {['support', 'concern', 'neutral'].map((sentiment) => {
            const display = getSentimentDisplay(sentiment);
            const IconComponent = display.icon;
            return (<button key={sentiment} onClick={() => setSelectedSentiment(sentiment)} disabled={submitStatus === 'submitting'} className={`p-3 rounded-md border-2 transition-all duration-200 flex flex-col items-center gap-2 ${selectedSentiment === sentiment
                    ? `${display.borderColor} ${display.bgColor} ${display.color}`
                    : `border-slate-600 bg-slate-700 text-slate-300 hover:${display.borderColor} hover:${display.color}`} disabled:opacity-50 disabled:cursor-not-allowed`} style={{ minHeight: '48px' }} aria-label={`Select ${sentiment} sentiment`}>
      ...
      <div className="mb-6">
        ...
        <div className="relative">
          <textarea value={comment} onChange={(e) => setComment(e.target.value.slice(0, characterLimit))} placeholder="Share your thoughts and reasoning..." disabled={submitStatus === 'submitting'} className="w-full p-3 bg-slate-700 border border-slate-600 rounded-md text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" rows={4} maxLength={characterLimit} aria-label="Comment for sentiment submission"/>
      ...
      <div className="mb-6 p-4 bg-slate-700 border border-slate-600 rounded-md">
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={() => setIsAnonymous(!isAnonymous)} disabled={submitStatus === 'submitting'} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 ${isAnonymous ? 'bg-blue-600' : 'bg-slate-600'} disabled:opacity-50 disabled:cursor-not-allowed`} aria-label="Toggle anonymous submission">
            <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-200 ${isAnonymous ? 'translate-x-6' : 'translate-x-1'}`}/>
          </button>
        </div>
      ...
      {/* Submit Button */}
      <div className="mb-6">
        <button onClick={handleSubmission} disabled={submitButtonState.disabled} className={`w-full py-3 px-4 rounded-md font-medium transition-colors duration-200 flex items-center justify-center gap-2 ${submitStatus === 'success'
            ? 'bg-green-700 text-green-100'
            : submitStatus === 'failed'
                ? 'bg-red-700 hover:bg-red-600 text-white'
                : 'bg-blue-700 hover:bg-blue-600 disabled:bg-slate-600 disabled:cursor-not-allowed text-white'}`} style={{ minHeight: '48px' }} aria-label="Submit sentiment">
      ...
      {/* Status Display */}
      {error && (<div className="mb-4 p-3 bg-red-900/20 border border-red-600 rounded-md">
      ...
      {/* Submission History */}
      {submissionHistory.length > 0 && (<div className="mb-6 p-4 bg-slate-700 border border-slate-600 rounded-md">
          ...
          <div className="space-y-2 max-h-32 overflow-y-auto">
            {submissionHistory.slice(-5).map((submission) => {
                ...
                const IconComponent = display.icon;
                return (<div key={submission.id} className="p-2 bg-slate-800 rounded-md">
