...
import { ThumbsUp, ThumbsDown, Send, Shield, AlertTriangle, CheckCircle, Loader2, User, Zap, RefreshCw } from 'lucide-react';
export default function TrustVoteCard({ userRole = 'Citizen', userDid = 'did:civic:trust_voter_001', onVoteSubmitted, onVoteFailed, className = '' }) {
    ...
    const [streakCount, setStreakCount] = useState(2); // 2 days current streak
    const [totalTruthPoints, setTotalTruthPoints] = useState(145);
    const [error, setError] = useState('');
    const [renderTime, setRenderTime] = useState(0);
    const mountTimestamp = useRef(Date.now());
    const voteAttempts = useRef(0);
    ...
    const generateZKPProof = useCallback(async (vote) => {
        if (userRole === 'Citizen') {
    ...
    const calculateTruthPoints = useCallback((hasComment, hasStreakBonus) => {
    ...
    const verifyVote = useCallback(async (vote) => {
    ...
    const handleVoteSubmit = async () => {
        if (!selectedVote || submitStatus !== 'idle')
            return;
    ...
    const getSubmitButtonState = () => {
        if (!selectedVote)
            return { disabled: true, text: 'Select Vote Type' };
        if (submitStatus === 'submitting')
            return { disabled: true, text: 'Submitting...' };
        if (submitStatus === 'success')
            return { disabled: true, text: 'Vote Submitted!' };
        if (submitStatus === 'failed')
            return { disabled: false, text: 'Retry Vote' };
    ...
    return (<div className={`w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6 ${className}`}>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <button onClick={() => setSelectedVote('trust')} disabled={submitStatus === 'submitting'} className={`p-4 rounded-md border-2 transition-all duration-200 flex flex-col items-center gap-2 ${selectedVote === 'trust'
            ? 'border-green-500 bg-green-900/20 text-green-400'
            : 'border-slate-600 bg-slate-700 text-slate-300 hover:border-green-500 hover:text-green-400'} disabled:opacity-50 disabled:cursor-not-allowed`} style={{ minHeight: '48px' }} aria-label="Vote trust">
            <ThumbsUp className="w-5 h-5"/>
            <span className="text-sm font-medium">Trust</span>
          </button>
          
          <button onClick={() => setSelectedVote('distrust')} disabled={submitStatus === 'submitting'} className={`p-4 rounded-md border-2 transition-all duration-200 flex flex-col items-center gap-2 ${selectedVote === 'distrust'
            ? 'border-red-500 bg-red-900/20 text-red-400'
            : 'border-slate-600 bg-slate-700 text-slate-300 hover:border-red-500 hover:text-red-400'} disabled:opacity-50 disabled:cursor-not-allowed`} style={{ minHeight: '48px' }} aria-label="Vote distrust">
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            <textarea value={comment} onChange={(e) => setComment(e.target.value.slice(0, characterLimit))} placeholder="Add your reasoning (optional)..." disabled={submitStatus === 'submitting'} className="w-full p-3 bg-slate-700 border border-slate-600 rounded-md text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" rows={3} maxLength={characterLimit} aria-label="Optional comment for vote"/>
        ...
        {/* Submit Button */}
        <button onClick={handleVoteSubmit} disabled={submitButtonState.disabled} className={`w-full py-3 px-4 rounded-md font-medium transition-colors duration-200 flex items-center justify-center gap-2 ${submitStatus === 'success'
            ? 'bg-green-700 text-green-100'
            : submitStatus === 'failed'
                ? 'bg-red-700 hover:bg-red-600 text-white'
                : 'bg-blue-700 hover:bg-blue-600 disabled:bg-slate-600 disabled:cursor-not-allowed text-white'}`} style={{ minHeight: '48px' }} aria-label="Submit trust vote">
      ...
      {/* Vote History */}
      {voteHistory.length > 0 && (<div className="mb-6 p-4 bg-slate-700 border border-slate-600 rounded-md">
          <h3 className="text-sm font-medium text-slate-300 mb-3">
            Recent Votes ({voteHistory.length})
          </h3>
          <div className="space-y-2 max-h-32 overflow-y-auto">
            {voteHistory.slice(-3).map((vote) => (<div key={vote.id} className="p-2 bg-slate-800 rounded-md">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    {vote.type === 'trust' ? (<ThumbsUp className="w-3 h-3 text-green-400"/>) : (<ThumbsDown className="w-3 h-3 text-red-400"/>)}
