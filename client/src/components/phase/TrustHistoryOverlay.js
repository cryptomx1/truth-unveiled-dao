...
const TrustHistoryOverlay = ({ className = '', userDid = 'did:civic:trust_history_001', isVisible = false, onClose, onEventSelect }) => {
    ...
    const generateTrustEvents = useCallback(() => {
        const types = ['poll_response', 'confirmation', 'validation', 'feedback', 'referral', 'penalty'];
        const events = [];
        for (let i = 0; i < 15; i++) {
    ...
    const validateZKPHistory = useCallback(async () => {
    ...
    // Get event color
    const getEventColor = (type, trustChange) => {
        if (trustChange > 0)
    ...
    // Filter events
    const filteredEvents = trustEvents.filter(event => filterType === 'all' || event.type === filterType);
    ...
    return (<div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div ref={overlayRef} className={`w-full max-w-2xl max-h-[90vh] bg-slate-800 border border-slate-700 rounded-lg shadow-2xl overflow-hidden ${className}`}>
        ...
        <div className="p-6 border-b border-slate-700">
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleClose} className="p-2 text-slate-400 hover:text-white transition-colors" aria-label="Close trust history overlay">
              <X className="w-5 h-5"/>
            </button>
          </div>
          ...
          <div className="mt-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
            ...
            {pathBActive && (<div className="flex items-center gap-2 mt-2 text-xs text-amber-400">
        ...
        <div className="p-4 border-b border-slate-700">
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setFilterType('all')} className={`px-3 py-1 rounded text-xs font-medium transition-colors ${filterType === 'all'
            ? 'bg-blue-600 text-white'
            : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>
              All ({trustEvents.length})
            </button>
            {['poll_response', 'confirmation', 'validation', 'feedback', 'referral', 'penalty'].map(type => (<button key={type} onClick={() => setFilterType(type)} className={`px-3 py-1 rounded text-xs font-medium transition-colors ${filterType === type
                ? 'bg-blue-600 text-white'
                : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (<div className="p-6 text-center">
            ...
            </div>) : filteredEvents.length > 0 ? (<div className="divide-y divide-slate-700">
              {filteredEvents.map((event) => (<div key={event.id} onClick={() => handleEventSelect(event)} className={`p-4 hover:bg-slate-700 cursor-pointer transition-colors ${selectedEvent?.id === event.id ? 'bg-slate-700' : ''}`} role="button" tabIndex={0} aria-label={`Trust event: ${event.description}`}>
            ...
            </div>) : (<div className="p-6 text-center text-slate-400">
        ...
        <div className="p-4 border-t border-slate-700">
          <button onClick={toggleExpanded} className="w-full py-2 px-4 bg-slate-700 hover:bg-slate-600 text-white font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800 flex items-center justify-center gap-2" style={{ minHeight: '48px' }} aria-label={isExpanded ? 'Collapse overlay' : 'Expand overlay'}>
            {isExpanded ? (<>
