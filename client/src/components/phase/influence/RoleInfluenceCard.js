...
import { Users, Crown, Star, Award, Gem, TrendingUp, BarChart3, Target, Zap, Shield, AlertTriangle, CheckCircle, RefreshCw, Activity, Hash, ArrowRight, Loader2 } from 'lucide-react';
export default function RoleInfluenceCard({ userRole = 'Citizen', userDid = 'did:civic:role_influence_001', currentTrustScore = 87.3, onInfluenceUpdate, className = '' }) {
    ...
    const [crossRoleDistribution, setCrossRoleDistribution] = useState([
    ...
    const [zkpSignatureRequired, setZkpSignatureRequired] = useState(false);
    ...
    const determineTrustTier = useCallback((score) => {
    ...
    const syncTierStatus = useCallback(async () => {
    ...
    const calculateTpPreview = useCallback((tier, baseAmount = 15) => {
        return Math.round(baseAmount * tier.tpMultiplier);
    }, []);
    // Get role-based signature requirement
    const getSignatureRequirement = useCallback((role) => {
    ...
    return (<div className={`w-full max-w-4xl mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6 ${className}`}>
      {/* ARIA Live Region */}
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-4">
            ...
            {zkpSignatureRequired && (<div className="flex items-center gap-1">
      ...
      <div className="mb-8 p-4 bg-slate-700 border border-slate-600 rounded-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          {trustTiers.map((tier, index) => {
            ...
            return (<div key={tier.name} className="text-center">
                <div className={`p-3 rounded-md border transition-all duration-200 ${isCurrentTier
                  ...
                  <div className="flex flex-col items-center gap-2 mb-3">
                    ...
                    {isCurrentTier && (<span className="text-xs px-2 py-1 bg-blue-600 text-blue-100 rounded-full">
                        Current
                      </span>)}
                  </div>
                  ...
                  <div className="mb-2">
                    ...
                    <div className="w-full h-2 bg-slate-600 rounded-full overflow-hidden">
                      <div className={`h-full transition-all duration-300 ${isUnlocked ? tier.color.replace('text-', 'bg-') : 'bg-slate-600'}`} style={{ width: `${tier.influence}%` }}/>
                    </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="p-4 bg-slate-700 border border-slate-600 rounded-md">
          ...
          <div className="space-y-3">
            {crossRoleDistribution.map((distribution, index) => (<div key={index} className="p-3 bg-slate-800 rounded-md">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm ${syncStatus === 'success' ? 'text-green-400' :
            ...
            {/* Manual Sync Button */}
            <button onClick={syncTierStatus} disabled={syncStatus === 'syncing'} className="w-full py-2 px-3 bg-blue-700 hover:bg-blue-600 disabled:bg-slate-600 text-white rounded-md text-sm transition-colors duration-200 flex items-center justify-center gap-2" style={{ minHeight: '36px' }} aria-label="Sync tier status">
              {syncStatus === 'syncing' ? (<>
