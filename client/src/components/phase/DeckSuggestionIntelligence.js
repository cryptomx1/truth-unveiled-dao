...
const getTierAccessibleDecks = (tier) => {
    switch (tier) {
        case 'citizen':
...
const calculateDeckScore = (deck, input) => {
...
const generateReason = (deck, input, score) => {
    const reasons = [];
    // Mission match
    if (input.mission) {
...
export const getDeckSuggestions = (input) => {
    const accessibleDecks = getTierAccessibleDecks(input.tier);
    const suggestions = accessibleDecks.map(deck => {
...
const generateTopSuggestionsExplanation = (suggestions, input) => {
...
export const DeckSuggestionIntelligence = ({ input, onSuggestionsCalculated, autoInitialize = true }) => {
    const [initialized, setInitialized] = useState(false);
    const [currentSuggestions, setCurrentSuggestions] = useState([]);
    const [processingTime, setProcessingTime] = useState(0);
    // Initialize or update suggestions
    useEffect(() => {
        if (!autoInitialize || !input)
            return;
...
export const simulateDeckSuggestions = (mission = 'voice', lang = 'en', streakDays = 5, tier = 'citizen') => {
    const input = { mission, lang, streakDays, tier };
    return getDeckSuggestions(input);
};
export const getTopDeckSuggestion = (input) => {
    const suggestions = getDeckSuggestions(input);
    return suggestions.length > 0 ? suggestions[0] : null;
};
export const getDeckMetadata = (deckId) => {
    return DECK_METADATA.find(d => d.id === deckId) || null;
};
export const getAccessibleDecksForTier = (tier) => {
    return getTierAccessibleDecks(tier);
};
export const calculateDeckCompatibility = (deckId, input) => {
