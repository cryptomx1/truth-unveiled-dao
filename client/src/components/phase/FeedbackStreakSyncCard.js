...
const FeedbackStreakSyncCard = ({ className = '', userDid = 'did:civic:feedback_sync_001', onStreakUpdate }) => {
    ...
    const generateStreakData = useCallback(() => {
        const actions = ['vote', 'proposal', 'feedback', 'validation', 'referral'];
        const data = [];
        for (let i = 0; i < 14; i++) {
            const date = new Date();
    ...
    const validateZKPStreak = useCallback(async () => {
    ...
    return (<div className={`w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6 ${className}`}>
      ...
      <div className="mb-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
        <div className="flex items-center justify-between text-xs">
          ...
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${syncStatus === 'synced' ? 'bg-green-400' :
            syncStatus === 'syncing' ? 'bg-yellow-400 animate-pulse' :
                'bg-red-400'}`}></div>
            <span className={syncStatus === 'synced' ? 'text-green-400' :
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-7 gap-2" aria-hidden="true">
          {streakData.map((day, index) => (<div key={day.id} className={`relative aspect-square rounded-md border-2 flex items-center justify-center ${day.completed
                ? 'bg-green-900 border-green-500'
                : 'bg-slate-700 border-slate-600'}`} role="presentation">
              <div className={`${day.completed ? 'text-green-400' : 'text-slate-400'}`}>
                {getActionIcon(day.action)}
              </div>
              
              {/* Pulse animation for recent days */}
              {index >= 7 && day.completed && (<div className="absolute inset-0 rounded-md border-2 border-green-400 animate-pulse opacity-50"></div>)}
            </div>))}
        </div>
      </div>
      ...
      {/* Manual Sync Button */}
      <div className="mt-4">
        <button onClick={syncDailyCivicActions} disabled={syncStatus === 'syncing'} className="w-full py-2 px-4 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600 disabled:cursor-not-allowed text-white font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800 flex items-center justify-center gap-2" style={{ minHeight: '48px' }} aria-label="Sync daily civic actions">
          {syncStatus === 'syncing' ? (<>
