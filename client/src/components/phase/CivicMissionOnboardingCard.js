...
export const CivicMissionOnboardingCard = ({ className = '' }) => {
    ...
    return (<div className={`bg-slate-800 rounded-lg p-6 max-w-md mx-auto ${className}`}>
      ...
      <div className="space-y-4 mb-6">
        {missions.map((mission) => (<button key={mission.type} onClick={() => handleMissionSelect(mission.type)} className={`w-full p-4 rounded-lg border-2 transition-all duration-200 ${selectedMission === mission.type
                ? 'border-blue-500 bg-blue-500/20'
                : 'border-slate-600 bg-slate-700/50 hover:border-slate-500'}`} aria-pressed={selectedMission === mission.type} aria-label={`Select ${t(mission.titleKey)} mission`}>
      ...
      {/* Continue Button */}
      <button onClick={handleContinue} disabled={!selectedMission} className={`w-full py-3 px-4 rounded-lg font-medium transition-all duration-200 ${selectedMission
            ? 'bg-blue-600 hover:bg-blue-700 text-white'
            : 'bg-slate-600 text-slate-400 cursor-not-allowed'}`} aria-label={selectedMission ? `Continue with ${t(`mission.${selectedMission}`)}` : 'Select a mission to continue'}>
      ...
      {selectedMission && (<div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
