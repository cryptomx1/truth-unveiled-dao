...
const CivicValueBeacon = ({ className = '' }) => {
    ...
    const generateBeaconPayload = useCallback(() => {
    ...
    return (<div className={`max-w-md mx-auto bg-slate-800 rounded-lg p-6 border border-slate-700 ${className}`}>
      {/* Header */}
      ...
      {/* Status Display */}
      <div className="mb-6">
        <div className="flex items-center justify-center space-x-3 p-3 bg-slate-900 rounded-lg" aria-live="polite" aria-label="Beacon emission status">
      ...
      {/* Fallback Alert */}
      {fallbackTriggered && (<div className="mb-4 p-3 bg-red-900/30 border border-red-700 rounded-lg" role="alert" aria-live="assertive">
      ...
      {lastPayload && (<div className="space-y-3">
      ...
      <div className="mt-6 text-center">
        <button onClick={() => {
            setEmissionCount(prev => prev + 1);
            emitBeacon();
        }} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800" style={{ minHeight: '48px', minWidth: '48px' }} aria-label="Manually emit civic beacon signal">
          Emit Signal Now
        </button>
      </div>
