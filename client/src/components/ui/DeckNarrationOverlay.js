...
export const DeckNarrationOverlay = ({ deckId, deckName, isVisible = true, position = 'top-right', autoPlay = false, className = '' }) => {
    ...
    const [tutorialCid, setTutorialCid] = useState(null);
    const audioRef = useRef(null);
    const tutorialInjector = useRef(null);
    useEffect(() => {
    ...
    const loadAudioFromGateways = async (gatewayUrls) => {
    ...
    const getTutorialContent = (deckId, deckName, language) => {
        const scripts = {
            'en': {
    ...
    if (!isVisible)
        return null;
    return (<div className={`${getPositionClasses()} ${className}`}>
        ...
        <CardContent className="p-4">
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <div className="flex gap-1">
              {['en', 'es', 'fr', 'ja'].map((lang) => (<Button key={lang} variant={currentLanguage === lang ? 'default' : 'outline'} size="sm" className="h-6 px-2 text-xs" onClick={() => handleLanguageChange(lang)}>
                  {lang.toUpperCase()}
                </Button>))}
            </div>
          </div>
          ...
          <div className="space-y-3">
            {audioState.error ? (<div className="text-center py-2">
                <p className="text-xs text-red-500">{audioState.error}</p>
                <Button variant="outline" size="sm" className="mt-2" onClick={loadTutorialAudio}>
                  Retry
                </Button>
              </div>) : (<>
                {/* Progress Bar */}
                <div className="space-y-1">
                  ...
                  <Progress value={audioState.duration > 0 ? (audioState.currentTime / audioState.duration) * 100 : 0} className="h-1"/>
                </div>

                {/* Control Buttons */}
                <div className="flex items-center justify-center gap-2">
                  <Button variant="outline" size="sm" onClick={handleReplay} disabled={!audioState.isLoaded}>
                    <RotateCcw className="h-3 w-3"/>
                  </Button>
                  
                  <Button variant={audioState.isPlaying ? "secondary" : "default"} size="sm" onClick={audioState.isPlaying ? handlePause : handlePlay} disabled={audioState.isLoading || !audioState.isLoaded} className="px-4">
                    {audioState.isLoading ? (<div className="h-3 w-3 animate-spin rounded-full border-2 border-current border-t-transparent"/>) : audioState.isPlaying ? (<Pause className="h-3 w-3"/>) : (<Play className="h-3 w-3"/>)}
          ...
          <div className="mt-3 pt-3 border-t">
            ...
            {tutorialCid && (<p className="text-xs text-muted-foreground text-center mt-1">
