...
export const AudioOnboardingPlayer = ({ narrationCID, deckId, moduleId, autoPlay = true, onPlayStateChange, className = '' }) => {
    ...
    const getAudioUrl = (cid, retryIndex = 0) => {
    ...
    if (!narrationCID) {
        return (<div className={`flex items-center gap-2 p-3 bg-muted/30 rounded-lg ${className}`}>
    ...
    return (<div className={`flex items-center gap-3 p-3 bg-background border rounded-lg shadow-sm ${className}`} role="region" aria-label="Audio narration player">
      <audio ref={audioRef} onTimeUpdate={handleTimeUpdate} onEnded={handleEnded} preload="metadata" aria-hidden="true"/>

      {/* Play/Pause Button */}
      <Button variant="outline" size="sm" onClick={handlePlay} disabled={audioState.isLoading || audioState.hasError} aria-label={audioState.isPlaying ? 'Pause narration' : 'Play narration'} className="shrink-0">
      ...
      {/* Replay Button */}
      <Button variant="ghost" size="sm" onClick={handleReplay} disabled={audioState.isLoading || audioState.hasError} aria-label="Replay from beginning" className="shrink-0">
        <RotateCcw className="h-4 w-4"/>
      </Button>

      {/* Progress Section */}
      <div className="flex-1 min-w-0">
        {audioState.hasError ? (<div className="flex items-center gap-2">
            <span className="text-sm text-destructive">Audio unavailable</span>
            <Button variant="link" size="sm" onClick={handleRetry} className="p-0 h-auto text-xs">
              Retry
            </Button>
            ...
            <div className="w-full bg-muted/30 rounded-full h-1 cursor-pointer" onClick={(e) => {
            ...
            }} role="progressbar" aria-valuemin={0} aria-valuemax={audioState.duration} aria-valuenow={audioState.currentTime} aria-label="Audio progress">
              <div className="bg-primary h-1 rounded-full transition-all duration-150" style={{
      ...
      <div className="flex items-center gap-2 shrink-0">
        <Button variant="ghost" size="sm" onClick={handleMuteToggle} aria-label={audioState.isMuted ? 'Unmute' : 'Mute'} className="p-1">
          {audioState.isMuted ? (<VolumeX className="h-4 w-4"/>) : (<Volume2 className="h-4 w-4"/>)}
        </Button>
        
        <div className="w-16">
          <Slider value={[audioState.isMuted ? 0 : audioState.volume * 100]} onValueChange={handleVolumeChange} max={100} step={5} className="cursor-pointer" aria-label="Volume control"/>
        </div>
      </div>
