...
export const VaultOverlayPatch = ({ className = '', showDetailedBreakdown = true, enableRealTimeUpdates = true }) => {
    const [trustImpact, setTrustImpact] = useState({
        overall: 76,
        breakdown: {
    ...
    const [nudgeVisibility, setNudgeVisibility] = useState({
    ...
    useEffect(() => {
        ...
        window.addEventListener('trust-fusion-update', handleTrustUpdate);
        window.addEventListener('nudge-system-update', handleNudgeUpdate);
        // Simulate real-time updates
        const updateInterval = setInterval(() => {
            simulateRealTimeUpdates();
        }, 30000); // Every 30 seconds
        return () => {
            window.removeEventListener('trust-fusion-update', handleTrustUpdate);
            window.removeEventListener('nudge-system-update', handleNudgeUpdate);
    ...
    const getTrustLevelBadgeVariant = (score) => {
        if (score >= 80)
            return 'default';
        if (score >= 65)
            return 'secondary';
    ...
    return (<div className={`vault-overlay-patch space-y-4 ${className}`}>
      ...
      <Card className="border-l-4 border-l-blue-500">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Progress value={trustImpact.overall} className="h-2"/>
          </div>

          {/* Detailed Breakdown */}
          {showDetailedBreakdown && (<div className="grid grid-cols-2 gap-3 pt-2">
      ...
      {/* High Impact Alert */}
      {trustImpact.overall < 60 && (<Alert variant="destructive">
      ...
      {process.env.NODE_ENV === 'development' && (<details className="text-xs">
