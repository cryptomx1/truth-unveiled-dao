...
export const DeckSwipeEngine = ({ modules, currentModuleIndex, onModuleChange, deckId, className = '', swipeEnabled = true, preloadDistance = 1 }) => {
    ...
    useEffect(() => {
        const preloadModules = async () => {
            ...
            const newModulesToPreload = Array.from(indicesToPreload).filter(index => !preloadedArray.includes(index));
            if (newModulesToPreload.length > 0) {
                ...
                await Promise.all(newModulesToPreload.map(index => new Promise(resolve => setTimeout(resolve, 100))));
    ...
    const navigateToModule = useCallback((newIndex, direction) => {
    ...
    const handleTouchEnd = useCallback((e) => {
        if (!swipeEnabled || !swipeState.isDragging)
            return;
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            if (!swipeEnabled || swipeState.isAnimating)
                return;
            switch (e.key) {
                case 'ArrowLeft':
                    e.preventDefault();
    ...
    if (!currentModule) {
        return (<div className={`flex items-center justify-center min-h-[400px] ${className}`}>
    ...
    return (<div ref={containerRef} className={`relative overflow-hidden ${className}`} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} role="region" aria-label="Swipeable deck modules">
      {/* Navigation Buttons */}
      <div className="absolute inset-y-0 left-0 z-10 flex items-center">
        <Button variant="ghost" size="sm" onClick={handleSwipeRight} disabled={swipeState.currentIndex === 0 || swipeState.isAnimating} className="ml-2 bg-background/80 backdrop-blur-sm hover:bg-background/90" aria-label="Previous module">
          <ChevronLeft className="h-4 w-4"/>
        </Button>
      </div>

      <div className="absolute inset-y-0 right-0 z-10 flex items-center">
        <Button variant="ghost" size="sm" onClick={handleSwipeLeft} disabled={swipeState.currentIndex === modules.length - 1 || swipeState.isAnimating} className="mr-2 bg-background/80 backdrop-blur-sm hover:bg-background/90" aria-label="Next module">
      ...
      <div className="relative min-h-[400px]">
        <AnimatePresence mode="wait" custom={swipeState.direction}>
          <motion.div key={swipeState.currentIndex} custom={swipeState.direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{
            x: { type: "spring", stiffness: 300, damping: 30 },
            opacity: { duration: 0.2 }
        }} drag={swipeEnabled ? "x" : false} dragConstraints={{ left: 0, right: 0 }} dragElastic={0.1} onDragStart={handleDragStart} onDragEnd={handleDragEnd} className="absolute inset-0 cursor-grab active:cursor-grabbing" style={{
            touchAction: swipeEnabled ? 'pan-y' : 'auto'
        }}>
            {/* Loading State */}
            {!isPreloaded ? (<div className="flex items-center justify-center h-full">
                <div className="text-center space-y-3">
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-20">
        <div className="flex space-x-2 bg-background/80 backdrop-blur-sm rounded-full px-3 py-2">
          {modules.map((_, index) => (<div key={index} className={`w-2 h-2 rounded-full transition-all duration-200 ${index === swipeState.currentIndex
                ? 'bg-primary scale-125'
                : swipeState.preloadedModules.has(index)
                    ? 'bg-primary/50'
                    : 'bg-muted-foreground/30'}`} aria-label={`Module ${index + 1} ${index === swipeState.currentIndex ? '(current)' : ''}`}/>))}
        </div>
      </div>
      ...
      {/* Swipe Instructions */}
      {swipeEnabled && (<div className="absolute top-4 right-4 z-20">
