...
export const DeckNarrationTTSBridge = ({ deckId, enableRealTimeNarration = true, voicePriority = 'accessibility', className = '' }) => {
    ...
    useEffect(() => {
        if (!enableRealTimeNarration)
            return;
        // Listen for trust delta events
        const handleTrustDelta = (event) => {
        ...
        window.addEventListener('trust-delta-detected', handleTrustDelta);
        window.addEventListener('policy-vote-complete', handlePolicyVote);
        // Simulate trust delta events for testing
        if (deckId === 'governance_feedback') {
            const simulateDeltas = () => {
                const deltaTypes = ['spike', 'dip', 'stable'];
                const triggerSources = ['policy_vote', 'citizen_feedback', 'representative_action'];
            ...
            return () => {
                window.removeEventListener('trust-delta-detected', handleTrustDelta);
                window.removeEventListener('policy-vote-complete', handlePolicyVote);
    ...
    const generateDeltaNarration = (delta) => {
    ...
    const determinePriority = (delta) => {
        if (delta.magnitude >= CRITICAL_THRESHOLD)
            return 'critical';
        if (delta.magnitude >= SPIKE_THRESHOLD)
            return 'high';
        if (delta.magnitude >= 10)
            return 'medium';
        return 'low';
    };
    const addToNarrationQueue = (event) => {
        setNarrationQueue(prev => {
            // Sort by priority (critical first)
            const priorityOrder = { critical: 0, high: 1, medium: 2, low: 3 };
            const newQueue = [...prev, event].sort((a, b) => priorityOrder[a.priority] - priorityOrder[b.priority]);
    ...
    const processNarrationQueue = async () => {
        if (narrationQueue.length === 0 || isNarrating)
            return;
        setIsNarrating(true);
        try {
    ...
    const triggerTTSNarration = async (event) => {
        const startTime = Date.now();
        try {
            // Check if TTS agent is available
            const ttsAgent = window.ttsEngineAgent;
            if (ttsAgent && typeof ttsAgent.narrateContent === 'function') {
                // Use existing TTS agent with high priority for trust deltas
                await ttsAgent.narrateContent(event.deckId, 'trust_delta', event.content, event.priority === 'critical' ? 'high' : 'medium');
            }
            else {
    ...
    const getDeltaVariant = (magnitude) => {
        if (magnitude >= CRITICAL_THRESHOLD)
            return 'destructive';
    ...
    return (<div className={`trust-narration-bridge ${className}`}>
      {/* ARIA Live Region for Screen Readers */}
      <div ref={liveRegionRef} className="sr-only" aria-live="assertive" aria-atomic="true" role="status"/>

      {/* Visual Trust Delta Alert */}
      {activeDelta && activeDelta.magnitude >= SPIKE_THRESHOLD && (<Alert variant={getDeltaVariant(activeDelta.magnitude)} className="mb-4 border-l-4">
      ...
      {narrationQueue.length > 0 && (<div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
      ...
      {activeDelta && activeDelta.triggerSource === 'policy_vote' && (<div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg p-3 mt-2">
      ...
      {process.env.NODE_ENV === 'development' && activeDelta && (<details className="mt-4 text-xs">
