...
const FederationZKVotePanel = ({ proposalId = 'fed_prop_001' }) => {
    const [voteOptions] = useState([
        {
            id: 'approve',
            label: 'Approve',
    ...
    const generateEncryptedVote = async (vote) => {
    ...
    const generateZKPProof = async (encryptedVote) => {
            ...
            <CardContent className="space-y-6">
              {!voteSubmitted ? (<>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {voteOptions.map(option => (<Button key={option.id} variant={selectedVote === option.id ? 'default' : 'outline'} onClick={() => setSelectedVote(option.id)} className={`h-auto p-4 text-left ${selectedVote === option.id
                  ...
                  {encryptionProgress > 0 && (<div className="space-y-2">
                  ...
                  <Button onClick={submitVote} disabled={!selectedVote || isVoting} className="w-full bg-blue-600 hover:bg-blue-700">
                    {isVoting ? (<>
                  ...
                  {submittedBallot && (<div className="space-y-2 text-sm">
                      <div className="flex justify-between">
          ...
          <CardContent className="space-y-6">
            <div className="text-center">
              ...
              <div className="flex items-center justify-center gap-2 mt-2">
                {consensusData.quorumMet ? (<Badge variant="outline" className="text-green-400 border-green-400">
                    <CheckCircle className="h-3 w-3 mr-1"/>
                    Quorum Met
                  </Badge>) : (<Badge variant="outline" className="text-yellow-400 border-yellow-400">
            ...
            return (<div key={option.id} className="space-y-2">
                    ...
                    <Progress value={percentage} className={`h-2 ${option.color === 'green' ? 'bg-green-200' :
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <Button variant="outline" size="sm" onClick={() => setShowProofs(!showProofs)} className="border-slate-600 text-slate-200">
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {recentBallots.map((ballot, index) => (<div key={ballot.ballotId} className="bg-slate-800/50 rounded-lg p-3">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {ballot.verified && (<Shield className="h-3 w-3 text-green-400" title="ZKP Verified"/>)}
                      </div>
                    </div>
                    ...
                    <div className="space-y-1 text-xs">
                      ...
                      {showProofs && (<div className="flex justify-between">
                          <span className="text-slate-400">ZKP Proof:</span>
