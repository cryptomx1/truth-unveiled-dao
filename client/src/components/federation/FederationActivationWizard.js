...
const FederationActivationWizard = () => {
        ...
        switch (currentStep) {
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input id="nodeId" value={configuration.nodeId} onChange={(e) => setConfiguration(prev => ({ ...prev, nodeId: e.target.value }))} placeholder="fed_us_east_1" className="bg-slate-800 border-slate-600 text-slate-200"/>
              </div>
              
              <div>
                <Label htmlFor="country" className="text-slate-200">Country</Label>
                <Select value={configuration.jurisdiction.country} onValueChange={(value) => setConfiguration(prev => ({
                        ...prev,
                  ...
                  <SelectContent>
                    {availableCountries.map(country => (<SelectItem key={country.code} value={country.code}>
              ...
              {configuration.jurisdiction.country && (<div>
                  <Label htmlFor="region" className="text-slate-200">Region</Label>
                  <Select value={configuration.jurisdiction.region} onValueChange={(value) => setConfiguration(prev => ({
                            ...prev,
                    ...
                    <SelectContent>
                      {availableCountries
                            ...
                            .map(region => (<SelectItem key={region} value={region}>
                            {region}
                          </SelectItem>))}
                    </SelectContent>
                  </Select>
            ...
            <div>
              ...
              <Select value={configuration.governanceTier} onValueChange={(value) => setConfiguration(prev => ({ ...prev, governanceTier: value }))}>
                <SelectTrigger className="bg-slate-800 border-slate-600 text-slate-200">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="range" min="5" max="50" value={configuration.quorumSettings.minimumParticipation} onChange={(e) => setConfiguration(prev => ({
                        ...prev,
              ...
              <div>
                ...
                <Input id="decay" type="number" value={configuration.quorumSettings.decayThreshold} onChange={(e) => setConfiguration(prev => ({
                        ...prev,
              ...
              <div className="flex items-center space-x-2">
                <Checkbox id="emergency" checked={configuration.quorumSettings.emergencyOverride} onCheckedChange={(checked) => setConfiguration(prev => ({
                        ...prev,
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-2">
                {availableCountries.map(country => (<div key={country.code} className="flex items-center space-x-2">
                    <Checkbox id={country.code} checked={configuration.regionalScope.includes(country.code)} onCheckedChange={(checked) => {
                            if (checked) {
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-2">
                <Checkbox id="daoIntegration" checked={configuration.daoLinkage.existingDAOIntegration} onCheckedChange={(checked) => setConfiguration(prev => ({
                        ...prev,
              ...
              <div className="flex items-center space-x-2">
                <Checkbox id="deckSync" checked={configuration.daoLinkage.governanceDeckSync} onCheckedChange={(checked) => setConfiguration(prev => ({
                        ...prev,
              ...
              <div className="flex items-center space-x-2">
                <Checkbox id="crossBorder" checked={configuration.daoLinkage.crossBorderVoting} onCheckedChange={(checked) => setConfiguration(prev => ({
                        ...prev,
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input id="badgeholderDID" value={configuration.genesisAuthorization.badgeholderDID} onChange={(e) => setConfiguration(prev => ({
                        ...prev,
              ...
              <div>
                ...
                <Input id="signature" value={configuration.genesisAuthorization.signature} onChange={(e) => setConfiguration(prev => ({
                        ...prev,
                        genesisAuthorization: { ...prev.genesisAuthorization, signature: e.target.value }
                    }))} placeholder="0xfederation_signature_example" className="bg-slate-800 border-slate-600 text-slate-200"/>
              </div>
            </div>
            
            {activationResult && (<div className={`p-4 rounded-lg ${activationResult.success ? 'bg-green-900/50' : 'bg-red-900/50'}`}>
                <div className="flex items-center gap-2">
                  {activationResult.success ? (<CheckCircle className="h-5 w-5 text-green-400"/>) : (<AlertTriangle className="h-5 w-5 text-red-400"/>)}
      ...
      <Card className="bg-slate-900/50 dark:bg-slate-950/50 border-slate-700">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-5 gap-2">
            {wizardSteps.map((step) => (<div key={step.step} className={`text-center p-3 rounded-lg border ${step.step === currentStep
                ? 'bg-blue-900/50 border-blue-600'
                : step.completed
                    ? 'bg-green-900/50 border-green-600'
                    : 'bg-slate-800/50 border-slate-600'}`}>
                <div className={`text-sm font-medium ${step.step === currentStep
                ? 'text-blue-400'
                : step.completed
                    ? 'text-green-400'
                    : 'text-slate-400'}`}>
          ...
          <div className="flex justify-between">
            <Button onClick={handlePrevious} disabled={currentStep === 1} variant="outline" className="border-slate-600 text-slate-200">
              Previous
            </Button>

            {currentStep < 5 ? (<Button onClick={handleNext} disabled={!validateCurrentStep()} className="bg-blue-600 hover:bg-blue-700">
                Next
              </Button>) : (<Button onClick={activateFederation} disabled={!validateCurrentStep() || isActivating} className="bg-green-600 hover:bg-green-700">
                {isActivating ? 'Activating...' : 'Activate Federation'}
              </Button>)}
          </div>

          {/* Summary */}
          {currentStep === 5 && (<div className="bg-slate-800/50 rounded-lg p-4">
