...
import { Eye, ThumbsUp, ThumbsDown, Users, TrendingUp, MessageSquare, Shield } from 'lucide-react';
const FederationProposalReview = ({ proposalId = 'fed_prop_001' }) => {
    ...
    const submitReview = async () => {
        ...
        try {
            ...
            setSentimentData(prev => {
                ...
                if (currentReview.sentiment === 'positive')
                    newPositive++;
                else if (currentReview.sentiment === 'negative')
                    newNegative++;
                else
                    newNeutral++;
    ...
    return (<div className="w-full max-w-6xl mx-auto space-y-6">
            ...
            <div className="md:col-span-2 space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <Badge variant="outline" className={proposal.urgency === 'high' ? 'text-red-400 border-red-400' :
            proposal.urgency === 'medium' ? 'text-yellow-400 border-yellow-400' :
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  {proposal.targetJurisdictions.map(jurisdiction => (<Badge key={jurisdiction} variant="outline" className="text-cyan-400 border-cyan-400">
                      {jurisdiction}
                    </Badge>))}
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <CardContent className="space-y-6">
            <div className="text-center">
              ...
              <div className="flex items-center justify-center gap-1 mt-1">
                <TrendingUp className={`h-4 w-4 ${sentimentData.sentimentTrend === 'increasing' ? 'text-green-400' :
          ...
          <CardContent className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2 mt-2">
                {['positive', 'neutral', 'negative'].map(sentiment => (<Button key={sentiment} variant={currentReview.sentiment === sentiment ? 'default' : 'outline'} onClick={() => setCurrentReview(prev => ({ ...prev, sentiment: sentiment }))} className={`${currentReview.sentiment === sentiment
            ...
            <div>
              ...
              <Textarea id="comments" value={currentReview.comments} onChange={(e) => setCurrentReview(prev => ({ ...prev, comments: e.target.value }))} placeholder="Provide detailed feedback on the proposal..." rows={3} className="bg-slate-800 border-slate-600 text-slate-200"/>
            </div>
            ...
            <div>
              ...
              <input type="range" min="0" max="100" value={currentReview.confidenceLevel} onChange={(e) => setCurrentReview(prev => ({ ...prev, confidenceLevel: Number(e.target.value) }))} className="w-full mt-2"/>
            </div>
        ...
        ].map(action => (<Button key={action.value} variant={currentReview.recommendedAction === action.value ? 'default' : 'outline'} onClick={() => setCurrentReview(prev => ({ ...prev, recommendedAction: action.value }))} className={`text-xs ${currentReview.recommendedAction === action.value
            ...
            <Button onClick={submitReview} disabled={isSubmittingReview || !currentReview.comments.trim()} className="w-full bg-purple-600 hover:bg-purple-700">
              {isSubmittingReview ? (<>
      ...
      <Card className="bg-slate-900/50 dark:bg-slate-950/50 border-slate-700">
        ...
        <CardContent>
          <div className="space-y-4">
            {existingReviews.map((review, index) => (<div key={index} className="bg-slate-800/50 rounded-lg p-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-slate-400 text-sm flex items-center gap-2">
                        ...
                        {review.zkpVerified && (<Shield className="h-3 w-3 text-green-400"/>)}
                      </div>
                    </div>
                  </div>
