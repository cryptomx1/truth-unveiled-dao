...
import { Shield, FileText, Hash, CheckCircle, AlertTriangle, Globe, Vote } from 'lucide-react';
import { FederationNodeRegistry } from '@shared/federation/FederationNodeRegistry';
const FederationProposalSubmit = () => {
    ...
    const generateIntentHash = async () => {
        const intentData = {
    ...
    const verifyGenesisAuthorization = async () => {
        ...
        <CardContent className="space-y-8">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  ...
                  <Input id="title" value={proposal.metadata.title} onChange={(e) => setProposal(prev => ({
            ...prev,
            metadata: { ...prev.metadata, title: e.target.value }
        }))} placeholder="Federation Governance Enhancement..." className="bg-slate-800 border-slate-600 text-slate-200"/>
                </div>
                
                <div>
                  <Label htmlFor="category" className="text-slate-200">Category</Label>
                  <Select value={proposal.metadata.category} onValueChange={(value) => setProposal(prev => ({
            ...prev,
            metadata: { ...prev.metadata, category: value }
                ...
                <div>
                  ...
                  <Select value={proposal.metadata.urgency} onValueChange={(value) => setProposal(prev => ({
            ...prev,
            metadata: { ...prev.metadata, urgency: value }
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Select value={proposal.metadata.estimatedImpact} onValueChange={(value) => setProposal(prev => ({
            ...prev,
            metadata: { ...prev.metadata, estimatedImpact: value }
                ...
                <div>
                  ...
                  <Input id="votingPeriod" type="number" min="1" max="30" value={proposal.metadata.votingPeriod} onChange={(e) => setProposal(prev => ({
            ...prev,
            metadata: { ...prev.metadata, votingPeriod: Number(e.target.value) }
                ...
                <div>
                  ...
                  <input type="range" min="5" max="67" value={proposal.metadata.quorumRequired} onChange={(e) => setProposal(prev => ({
            ...prev,
            ...
            <div>
              ...
              <Textarea id="description" value={proposal.metadata.description} onChange={(e) => setProposal(prev => ({
            ...prev,
            metadata: { ...prev.metadata, description: e.target.value }
        }))} placeholder="Detailed description of the proposal, its objectives, implementation plan, and expected outcomes..." rows={4} className="bg-slate-800 border-slate-600 text-slate-200"/>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-2">
                {availableJurisdictions.map(jurisdiction => (<div key={jurisdiction.code} className="flex items-center space-x-2">
                    <Checkbox id={jurisdiction.code} checked={proposal.metadata.targetJurisdictions.includes(jurisdiction.code)} onCheckedChange={(checked) => {
                if (checked) {
                    setProposal(prev => ({
                        ...prev,
                        metadata: {
            ...
            <div className="flex items-center space-x-2">
              <Checkbox id="requiresZKP" checked={proposal.metadata.requiresZKP} onCheckedChange={(checked) => setProposal(prev => ({
            ...prev,
          ...
          <div className="space-y-6 border-t border-slate-700 pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  ...
                  <Input id="badgeholderDID" value={proposal.authorization.badgeholderDID} onChange={(e) => setProposal(prev => ({
            ...prev,
            authorization: { ...prev.authorization, badgeholderDID: e.target.value }
        }))} placeholder="did:genesis:commander_mark_authority" className="bg-slate-800 border-slate-600 text-slate-200"/>
                </div>
                
                <div>
                  <Label htmlFor="signature" className="text-slate-200">Authorization Signature</Label>
                  <Input id="signature" value={proposal.authorization.signature} onChange={(e) => setProposal(prev => ({
            ...prev,
            authorization: { ...prev.authorization, signature: e.target.value }
        }))} placeholder="0xfederation_proposal_signature" className="bg-slate-800 border-slate-600 text-slate-200"/>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Select value={proposal.authorization.guardianPillar} onValueChange={(value) => setProposal(prev => ({
            ...prev,
            authorization: { ...prev.authorization, guardianPillar: value }
                    ...
                    <SelectContent>
                      {guardianPillars.map(pillar => (<SelectItem key={pillar.code} value={pillar.code}>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    {proposal.authorization.verificationStatus === 'pending' && (<Badge variant="outline" className="text-yellow-400 border-yellow-400">
                        <AlertTriangle className="h-3 w-3 mr-1"/>
                        Pending Verification
                      </Badge>)}
                    {proposal.authorization.verificationStatus === 'verified' && (<Badge variant="outline" className="text-green-400 border-green-400">
                        <CheckCircle className="h-3 w-3 mr-1"/>
                        Verified
                      </Badge>)}
                    {proposal.authorization.verificationStatus === 'failed' && (<Badge variant="outline" className="text-red-400 border-red-400">
                        <AlertTriangle className="h-3 w-3 mr-1"/>
                        Verification Failed
                      </Badge>)}
                  </div>
                ...
                {zkpVerificationProgress > 0 && zkpVerificationProgress < 100 && (<div className="space-y-2">
          ...
          {(proposal.intentHash || proposal.cidBinding || proposal.authorization.zkpHash) && (<div className="space-y-4 border-t border-slate-700 pt-6">
              <div className="flex items-center gap-2 text-lg font-medium text-slate-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {proposal.intentHash && (<div className="bg-slate-800/50 rounded-lg p-3">
                ...
                {proposal.authorization.zkpHash && (<div className="bg-slate-800/50 rounded-lg p-3">
                ...
                {proposal.cidBinding && (<div className="bg-slate-800/50 rounded-lg p-3">
          ...
          {/* Validation Errors */}
          {validationErrors.length > 0 && (<div className="bg-red-900/50 rounded-lg p-4 border border-red-700">
              <div className="flex items-center gap-2 text-red-400 font-medium mb-2">
                <AlertTriangle className="h-4 w-4"/>
                Validation Errors
              </div>
              <ul className="list-disc list-inside space-y-1 text-red-300">
                {validationErrors.map((error, index) => (<li key={index} className="text-sm">{error}</li>))}
              </ul>
            </div>)}

          {/* Submission Status */}
          {proposal.submissionStatus === 'submitted' && (<div className="bg-green-900/50 rounded-lg p-4 border border-green-700">
          ...
          <div className="flex justify-between items-center pt-4 border-t border-slate-700">
            <Button onClick={resetForm} variant="outline" className="border-slate-600 text-slate-200" disabled={isSubmitting}>
              Reset Form
            </Button>

            <Button onClick={submitProposal} disabled={isSubmitting || proposal.submissionStatus === 'submitted'} className="bg-blue-600 hover:bg-blue-700">
              {isSubmitting ? (<>
