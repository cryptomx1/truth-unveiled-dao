...
export const LightOfTruthBadge = ({ isUnlocked = false, size = 'medium', showAnimation = true, badgeData }) => {
    ...
    const renderBadgeContent = () => (<div className={`${getSizeClasses()} relative mx-auto rounded-full border-4 border-white shadow-2xl flex items-center justify-center overflow-hidden`} style={badgeStyle} role="img" aria-label={isUnlocked ? "Light of Truth Guardian Badge - Unlocked" : "Light of Truth Guardian Badge - Locked"}>
      {/* Inner Badge Design */}
      <div className="absolute inset-2 bg-gradient-to-br from-blue-600 to-purple-800 rounded-full flex items-center justify-center">
        ...
        {/* Pillar Indicators */}
        {isUnlocked && badgeData && (<div className="absolute inset-0">
            {[0, 45, 90, 135, 180, 225, 270, 315].map((angle, index) => (<div key={index} className="absolute w-2 h-2 bg-white rounded-full" style={{
                    top: '10%',
                    left: '50%',
    ...
    return (<Card className="bg-transparent border-none shadow-none">
      <CardContent className="p-6 text-center space-y-4">
        ...
        <div className="space-y-2">
          ...
          {isUnlocked ? (<div className="text-sm text-green-600 space-y-1">
            ...
            </div>) : (<div className="text-sm text-amber-600">
