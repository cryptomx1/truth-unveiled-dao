...
export const GuardianPillarSync = ({ userDid = 'did:civic:guardian_candidate_789', onBadgeUnlock }) => {
    const [pillars, setPillars] = useState(GUARDIAN_PILLARS);
    ...
    const getPillarStatusColor = (completed) => {
    ...
    return (<Card className="w-full max-w-4xl mx-auto">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {pillars.map((pillar, index) => (<div key={index} className="border rounded-lg p-4 space-y-3">
              ...
              <Progress value={(pillar.tpEarned / pillar.requiredTp) * 100} className="h-2"/>
        ...
        <div className="flex justify-center gap-4">
          <Button onClick={performPillarSync} disabled={syncInProgress} className="min-w-48">
            {syncInProgress ? '🔄 Syncing...' : '🔄 Refresh Pillar Status'}
          </Button>
          
          {badgeStatus.badgeUnlocked && (<Button variant="outline" onClick={() => window.location.href = '/onboarding/verify'} className="min-w-48">
