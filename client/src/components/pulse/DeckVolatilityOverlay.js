...
export const DeckVolatilityOverlay = ({ deckId, deckName, className = '', position = 'bottom' }) => {
    ...
    const renderTooltip = () => {
        if (!showTooltip)
            return null;
        const tooltipContent = (<div className="absolute z-50 p-3 bg-slate-800 border border-slate-600 rounded-lg shadow-lg min-w-64">
        ...
        <div className="space-y-2 text-xs">
          ...
          <div className="flex justify-between">
            ...
            <span className={`font-medium ${volatilityIndex.riskLevel === 'critical' ? 'text-red-400' :
          ...
          <div className="flex justify-between">
            ...
            <span className={`font-medium ${sentimentVector.sentiment === 'positive' ? 'text-green-400' :
        ...
        return (<div className={tooltipPositionClasses[position]}>
        {tooltipContent}
      </div>);
    };
    return (<div className={`absolute ${getPositionClasses()} ${className} cursor-pointer`} onMouseEnter={() => setShowTooltip(true)} onMouseLeave={() => setShowTooltip(false)} style={{
      ...
      {volatilityIndex.riskLevel === 'critical' && (<div className="absolute inset-0 rounded animate-ping" style={{ backgroundColor: colors.glow }}/>)}
    </div>);
};
