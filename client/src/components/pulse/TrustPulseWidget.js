...
export const TrustPulseWidget = ({ deckId, deckName, className = '' }) => {
    ...
    useEffect(() => {
        ...
        const generatePulseData = () => {
            ...
            const generateStreak = (length, volatility) => {
                const streak = [];
        ...
        const interval = setInterval(() => {
            if (pulseData) {
                ...
                if (updated.trustLevel > 70)
                    updated.status = 'aligned';
                else if (updated.trustLevel >= 30)
                    updated.status = 'mixed';
                else
                    updated.status = 'unstable';
    ...
    const renderStreakGraph = () => {
        ...
        return (<div className="mt-3 p-3 bg-slate-700/30 rounded-lg">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-medium text-slate-300">Trust Streak</span>
          <div className="flex gap-1">
            {['24h', '7d', '30d'].map((period) => (<button key={period} onClick={() => setSelectedPeriod(period)} className={`px-2 py-1 text-xs rounded transition-colors ${selectedPeriod === period
        ...
        <div className="h-16 flex items-end gap-1 overflow-hidden">
          {currentStreak.map((value, index) => {
                ...
                return (<div key={index} className={`flex-1 ${color} rounded-sm transition-all duration-300`} style={{ height: `${Math.max(2, height)}%` }} title={`${value.toFixed(1)}% trust`}/>);
            })}
        </div>
    ...
    return (<div className={`relative ${className}`}>
      <div className="flex items-center gap-2 cursor-pointer hover:bg-slate-700/20 rounded-lg px-2 py-1 transition-colors" onClick={() => setShowStreakGraph(!showStreakGraph)} onMouseEnter={() => setShowStreakGraph(true)} onMouseLeave={() => setShowStreakGraph(false)}>
        ...
        {pulseData.trustLevel > 70 ? (<TrendingUp className="w-3 h-3 text-green-400"/>) : (<TrendingDown className="w-3 h-3 text-red-400"/>)}
      </div>

      {showStreakGraph && (<div className="absolute top-full left-0 right-0 z-50 mt-2">
          {renderStreakGraph()}
        </div>)}
    </div>);
};
