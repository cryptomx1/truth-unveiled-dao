class CivicPulseAPI {
    baseURL = '/api/pulse'; // Would be external in production
    mockData = new Map();
    constructor() {
    ...
    generateTrendData(deckId, period) {
    ...
    calculateTrendSummary(dataPoints) {
    ...
    generateRecommendation(deckId) {
        ...
        const deckRecommendations = recommendations[deckId] || recommendations.civic_memory;
        return deckRecommendations[Math.floor(Math.random() * deckRecommendations.length)];
    }
    // Public API methods
    async getPulse(deckId) {
    ...
    async getSystemHealth() {
        ...
        let status = 'healthy';
        if (overallTrust < 30 || systemVolatility > 0.7)
            status = 'critical';
        else if (overallTrust < 50 || systemVolatility > 0.5)
            status = 'warning';
        else if (overallTrust < 70 || systemVolatility > 0.3)
            status = 'monitoring';
        return {
            overallTrust,
            systemVolatility,
