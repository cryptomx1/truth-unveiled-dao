export class DeckWalkerAgent {
    ...
    async performDeepScan(options = {}) {
        ...
        const { includeSubpages = true, maxDepth = 3, failureThreshold = 0.15 // 15% failure threshold
         } = options;
        try {
     ...
     */
    async crawlSubpages(parentRoute, maxDepth, currentDepth = 1) {
     ...
     */
    getPriorityIssues() {
        return this.routeRegistry.filter(route => (route.status === 'broken' || route.status === 'unknown') &&
     ...
     */
    async mockRouteValidation(path) {
        ...
        if (pathSegments > 4)
            failureRate = 0.15; // 15% for module routes
        if (pathSegments > 6)
            failureRate = 0.25; // 25% for subpage routes
    ...
    calculatePriority(route) {
        ...
        if (coreModules.includes(route.moduleId || ''))
            return 'high';
        // Module routes are medium priority
        if (route.moduleId && !route.subpageId)
            return 'medium';
     ...
     */
    generateSubpageRoutes(parentRoute) {
        if (!parentRoute.moduleId)
            return []; // Only modules have subpages
     ...
     * Generate validation result
     */
    generateValidationResult(failureThreshold) {
     ...
     */
    generateSuggestions(failureRate, threshold, priorityIssues) {
        const suggestions = [];
