...
import { Vault, Clock, RefreshCw, Download, AlertTriangle, CheckCircle, Unlock, Activity, ChevronDown, ChevronRight } from 'lucide-react';
import { IdentityVaultCore, unlockVaultEntry } from '../../identity/IdentityVaultCore';
import { BiometricRefreshNode } from './BiometricRefreshNode';
import { exportReputationBundle } from '../../services/ZKReputationAssembler';
import { logVaultAccess, logVaultEntryUnlocked } from '../../services/VaultTelemetryLogger';
import { announceUnlockSuccess, announceUnlockFailure } from '../../services/VaultNarrationSystem';
// Main VaultManagerPanel component
export const VaultManagerPanel = ({ defaultDID = 'did:civic:commander-mark', autoLoadEntries = true, showAdvancedOptions = false }) => {
    // Component state
    ...
    const handleUnlockEntry = async (entryId) => {
        ...
        try {
            ...
            const mockUnlockData = unlockState.unlockMethod === 'biometric'
                ...
                : 'test_passphrase_123';
            const unlockResult = await unlockVaultEntry(entryId, unlockState.unlockMethod, mockUnlockData);
            const endTime = performance.now();
            const duration = Math.round(endTime - startTime);
            if (unlockResult.success && unlockResult.entry) {
                // Log successful unlock
                logVaultEntryUnlocked(unlockResult.entry.cid, entryId, unlockState.unlockMethod, unlockResult.metadata.accessCount, duration);
                // Announce success
                announceUnlockSuccess(unlockResult.entry.cid, unlockState.unlockMethod, unlockResult.metadata.accessCount);
                setUnlockState({
                    entryId: null,
                    isUnlocking: false,
                    unlockMethod: 'biometric',
                ...
                // Announce failure
                announceUnlockFailure('Unknown', unlockResult.error || 'Unknown error', unlockResult.metadata.remainingAttempts);
                setUnlockState(prev => ({
                    ...prev,
                    isUnlocking: false,
    ...
    const handleRefreshTrigger = (entry) => {
        setRefreshState({
            activeEntry: entry,
            isRefreshing: true
        });
    ...
    const handleRefreshComplete = async (refreshedEntry, refreshedBundle) => {
        setRefreshState({
            activeEntry: null,
            isRefreshing: false
        });
    ...
    const handleDownloadBundle = async (entry) => {
    ...
    const getTrustBadgeStyle = (trustIndex) => {
        ...
        if (trustIndex >= 75)
            return `${baseStyle} bg-blue-900/30 border border-blue-600 text-blue-300`;
        if (trustIndex >= 60)
            return `${baseStyle} bg-yellow-900/30 border border-yellow-600 text-yellow-300`;
        if (trustIndex >= 40)
            return `${baseStyle} bg-orange-900/30 border border-orange-600 text-orange-300`;
    ...
    if (refreshState.activeEntry && refreshState.isRefreshing) {
        return (<BiometricRefreshNode vaultEntry={refreshState.activeEntry} onRefreshComplete={handleRefreshComplete} onRefreshCancel={handleRefreshCancel} triggerReason="user_request"/>);
    }
    return (<div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={loadVaultEntries} disabled={vaultState.isLoading} className="flex items-center gap-2 px-3 py-2 bg-purple-900/30 border border-purple-600 text-purple-300 rounded-lg hover:bg-purple-800/30 transition-colors text-sm" aria-label="Refresh vault entries">
            {vaultState.isLoading ? (<div className="w-4 h-4 animate-spin border-2 border-purple-400 border-t-transparent rounded-full"/>) : (<RefreshCw className="w-4 h-4"/>)}
            <span>Refresh</span>
          </button>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex-1">
            ...
            <select value={filterDID} onChange={(e) => setFilterDID(e.target.value)} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 text-slate-200 rounded-lg text-sm">
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" id="show-expired" checked={showExpiredEntries} onChange={(e) => setShowExpiredEntries(e.target.checked)} className="w-4 h-4 text-purple-600 bg-slate-700 border-slate-600 rounded"/>
      ...
      <div className="space-y-4">
        {vaultState.entries.length === 0 ? (<div className="bg-slate-800 border border-slate-600 rounded-lg p-8 text-center">
          ...
          </div>) : (vaultState.entries.map((entry) => {
            const expiryInfo = getExpiryInfo(entry);
            const isExpanded = expandedEntries.has(entry.entryId);
            return (<div key={entry.entryId} className="bg-slate-800 border border-slate-600 rounded-lg overflow-hidden">
                {/* Entry Header */}
                <div className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <button onClick={() => toggleEntryExpansion(entry.entryId)} className="text-slate-400 hover:text-slate-200 transition-colors" aria-label={`${isExpanded ? 'Collapse' : 'Expand'} entry details`}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      {entry.status === 'active' && (<>
                          {expiryInfo.shouldRefresh && (<button onClick={() => handleRefreshTrigger(entry)} className="flex items-center gap-1 px-2 py-1 bg-yellow-900/30 border border-yellow-600 text-yellow-300 rounded text-xs hover:bg-yellow-800/30 transition-colors" aria-label="Refresh identity">
                              <RefreshCw className="w-3 h-3"/>
                              <span>Refresh</span>
                            </button>)}
                          
                          <button onClick={() => handleUnlockEntry(entry.entryId)} disabled={unlockState.isUnlocking && unlockState.entryId === entry.entryId} className="flex items-center gap-1 px-2 py-1 bg-blue-900/30 border border-blue-600 text-blue-300 rounded text-xs hover:bg-blue-800/30 transition-colors" aria-label="Unlock vault entry">
                            {unlockState.isUnlocking && unlockState.entryId === entry.entryId ? (<div className="w-3 h-3 animate-spin border border-blue-400 border-t-transparent rounded-full"/>) : (<Unlock className="w-3 h-3"/>)}
                            <span>Unlock</span>
                          </button>
                          
                          {entry.reputationBundle && (<button onClick={() => handleDownloadBundle(entry)} className="flex items-center gap-1 px-2 py-1 bg-green-900/30 border border-green-600 text-green-300 rounded text-xs hover:bg-green-800/30 transition-colors" aria-label="Download reputation bundle">
                ...
                {/* Expanded Details */}
                {isExpanded && (<div className="border-t border-slate-600 p-4 bg-slate-700/30">
                    ...
                    {/* Refresh History */}
                    {entry.metadata.refreshHistory.length > 0 && (<div className="mt-4">
                        <h4 className="text-sm font-medium text-slate-300 mb-2">Recent Refreshes</h4>
                        <div className="space-y-1">
                          {entry.metadata.refreshHistory.slice(-3).map((refresh) => (<div key={refresh.refreshId} className="flex items-center justify-between text-xs bg-slate-800/50 rounded p-2">
      ...
      {/* Error Display */}
      {(vaultState.error || unlockState.error) && (<div className="bg-red-900/10 border border-red-600/30 rounded-lg p-4">
