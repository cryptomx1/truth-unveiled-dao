...
export const IdentityVault = () => {
    ...
    const updateMetrics = (recordList) => {
    ...
    const triggerPathBFallback = (failureRate) => {
    ...
    const uploadNewRecord = async () => {
        ...
        try {
            ...
            // Attempt IPFS upload
            const uploadResult = await zkpTransportLayer.uploadToIPFS(newRecordData.content, {
                did,
            ...
            setRecords(prev => prev.map(r => r.id.includes(Date.now().toString())
    ...
    const retryFailedUploads = async () => {
        ...
        for (const record of failedRecords) {
            setRecords(prev => prev.map(r => r.id === record.id ? { ...r, uploadStatus: 'uploading' } : r));
        }
    ...
    return (<div className="max-w-4xl mx-auto p-6 bg-slate-900 min-h-screen">
      {/* Header */}
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          {pathBTriggered && (<div className="flex items-center gap-2">
      ...
      {/* Path B Alert */}
      {pathBTriggered && (<div className="bg-red-900 border border-red-600 rounded-lg p-4 mb-6">
          ...
          <button onClick={retryFailedUploads} className="mt-3 px-3 py-1 bg-red-800 hover:bg-red-700 text-red-200 text-sm rounded transition-colors">
      ...
      <div className="bg-slate-800 rounded-lg p-6 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div>
            ...
            <input type="text" value={newRecordData.name} onChange={(e) => setNewRecordData(prev => ({ ...prev, name: e.target.value }))} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded text-slate-200 focus:border-blue-500 focus:outline-none" placeholder="Enter record name"/>
          </div>
          <div>
            <label className="block text-sm text-slate-300 mb-2">Record Type</label>
            <select value={newRecordData.type} onChange={(e) => setNewRecordData(prev => ({ ...prev, type: e.target.value }))} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded text-slate-200 focus:border-blue-500 focus:outline-none">
          ...
          <div className="flex items-end">
            <button onClick={uploadNewRecord} disabled={isUploading || !newRecordData.name} className="w-full px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-600 disabled:cursor-not-allowed text-white rounded transition-colors flex items-center justify-center gap-2">
              {isUploading ? (<>
        ...
        <div>
          ...
          <textarea value={newRecordData.content} onChange={(e) => setNewRecordData(prev => ({ ...prev, content: e.target.value }))} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded text-slate-200 focus:border-blue-500 focus:outline-none h-24" placeholder='{"example": "content", "data": "here"}'/>
        </div>
      </div>
      ...
      <div className="bg-slate-800 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={retryFailedUploads} className="px-3 py-1 bg-slate-700 hover:bg-slate-600 text-slate-300 text-sm rounded transition-colors flex items-center gap-2">
        ...
        <div className="space-y-3">
          {records.map((record) => (<div key={record.id} className={`border rounded-lg p-4 cursor-pointer transition-all ${selectedRecord === record.id
                ? 'border-blue-500 bg-slate-700'
                : 'border-slate-700 hover:border-slate-600'}`} onClick={() => setSelectedRecord(selectedRecord === record.id ? null : record.id)}>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {record.ipfsHash && (<button onClick={(e) => {
                    e.stopPropagation();
                    downloadRecord(record);
                }} className="p-1 hover:bg-slate-600 rounded transition-colors">
                      <Download className="w-3 h-3 text-slate-400"/>
                    </button>)}
                  {record.verified && (<Shield className="w-3 h-3 text-green-400"/>)}
                </div>
              </div>

              {selectedRecord === record.id && (<div className="mt-4 pt-4 border-t border-slate-700">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                    ...
                    {record.ipfsHash && (<div className="md:col-span-2">
