      ...
      <Card className="bg-slate-900/50 dark:bg-slate-950/50 border-slate-700">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="h-64 overflow-y-auto space-y-3 p-3 bg-slate-800/30 rounded-lg">
            {messages.map((message, index) => (<div key={message.id} className={`flex ${message.sender === 'you_anon' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-xs px-3 py-2 rounded-lg ${message.sender === 'you_anon'
                ? 'bg-purple-600/80 text-white'
                : 'bg-slate-700/80 text-slate-100'}`}>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xs opacity-75">{message.sender}</span>
                    {message.zkpVerified && (<div className="w-3 h-3 bg-green-500 rounded-full flex items-center justify-center">
          ...
          <div className="flex gap-2">
            <Input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyPress={handleKeyPress} placeholder="Type encrypted message..." className="bg-slate-800/50 border-slate-600 text-slate-100 placeholder-slate-400" disabled={!isConnected}/>
            <Button onClick={handleSendMessage} disabled={!isConnected || !newMessage.trim()} size="sm" className="bg-purple-600 hover:bg-purple-700">
