...
import { Database, Shield, CheckCircle, Award, FileText, AlertCircle, RefreshCw, Loader2 } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
export default function VaultAnalyzer({ initialCid = '', autoVerify = true, maxRetries = 3 }) {
    ...
    const verifyZKProof = async (proof, entryId) => {
    ...
    const fetchVaultHistory = async (targetCid) => {
        if (!targetCid.trim()) {
    ...
    const verifyAllProofs = async (entries) => {
        setIsVerifying(true);
    ...
    // Get entry type icon
    const getEntryIcon = (type, verified) => {
    ...
    return (<div className="space-y-6 max-w-6xl mx-auto p-4">
        ...
        <CardContent>
          <div className="flex gap-4">
            <div className="flex-1">
              <Input placeholder="Enter IPFS CID for vault.history.json..." value={cid} onChange={(e) => setCid(e.target.value)} className="bg-slate-700 border-slate-600 text-white" disabled={isLoading}/>
            </div>
            <Button onClick={() => fetchVaultHistory(cid)} disabled={isLoading || !cid.trim()} className="bg-blue-600 hover:bg-blue-700 text-white">
              {isLoading ? (<Loader2 className="w-4 h-4 mr-2 animate-spin"/>) : (<Database className="w-4 h-4 mr-2"/>)}
              Analyze Vault
            </Button>
          </div>

          {error && (<div className="mt-4 bg-red-500/10 border border-red-500/30 rounded-lg p-3">
      ...
      {isVerifying && (<Card className="bg-slate-800 border-slate-700">
      ...
      {/* Analysis Summary */}
      {analysisResult && (<Card className="bg-slate-800 border-slate-700">
          <CardHeader>
          ...
          <CardContent>
            ...
            {Object.keys(analysisResult.categories).length > 0 && (<div className="mt-6">
                <h4 className="text-white font-medium mb-3">Categories</h4>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(analysisResult.categories).map(([category, count]) => (<Badge key={category} variant="outline" className="text-blue-400 border-blue-400">
                      {category}: {count}
                    </Badge>))}
                </div>
              </div>)}
      ...
      {/* Vault Entries */}
      {vaultHistory && (<Card className="bg-slate-800 border-slate-700">
          <CardHeader>
          ...
          <CardContent>
            <div className="space-y-4">
              {vaultHistory.entries.map((entry) => (<div key={entry.id} className="bg-slate-700/30 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    ...
                    {entry.verificationTimestamp && (<div>
            ...
            {vaultHistory.entries.length === 0 && (<div className="text-center py-8">
      ...
      {/* No data state */}
      {!vaultHistory && !isLoading && (<Card className="bg-slate-800 border-slate-700">
