...
const DynamicTierInfluenceMapExtension = () => {
    ...
    const [influenceSwings, setInfluenceSwings] = useState([]);
    ...
    const generateInitialDistricts = () => {
        const districtNames = [
        ...
        return districtNames.map((name, index) => {
            ...
            const totalInfluence = tierComposition.Commander * 1000 +
    ...
    const announceInfluenceSwing = useCallback((swing) => {
    ...
    const generateTierMigration = useCallback((fromDistrict, scenario) => {
    ...
    const applyMigration = useCallback((migration) => {
        setDistricts(prevDistricts => {
            const updated = prevDistricts.map(district => {
                if (district.id === migration.fromDistrict) {
                    ...
                    newComposition[migration.fromTier] = Math.max(0, newComposition[migration.fromTier] - migration.influence);
                    return {
                        ...district,
                    ...
                    const newComposition = { ...district.tierComposition };
                    newComposition[migration.toTier] = Math.min(1, newComposition[migration.toTier] + migration.influence);
                    return {
                        ...district,
                        tierComposition: newComposition,
                        isAnimating: true,
    ...
    const runSimulationStep = useCallback(() => {
        if (!simulation.isRunning || districts.length === 0)
            return;
    ...
    const getDistrictHeatColor = (district) => {
    ...
    const getDominantTier = (composition) => {
        const entries = Object.entries(composition);
        const dominant = entries.reduce((max, current) => current[1] > max[1] ? current : max);
        return dominant[0];
    };
    useEffect(() => {
        if (!isInitialized) {
    ...
    if (!isInitialized || districts.length === 0) {
        return (<Card className="w-full max-w-6xl mx-auto bg-slate-800 border-slate-700">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              ...
              <Select value={simulation.scenario} onValueChange={(value) => setSimulation(prev => ({ ...prev, scenario: value }))}>
                <SelectTrigger className="bg-slate-700 border-slate-600 text-slate-100">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-slate-700 border-slate-600">
                  {scenarios.map(scenario => (<SelectItem key={scenario.value} value={scenario.value} className="text-slate-100">
                      {scenario.label}
                    </SelectItem>))}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Switch checked={simulation.turnoutEnabled} onCheckedChange={(checked) => setSimulation(prev => ({ ...prev, turnoutEnabled: checked }))} className="data-[state=checked]:bg-cyan-600"/>
              </div>
              
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={simulation.isRunning ? pauseSimulation : startSimulation} className="border-slate-600 text-slate-300 hover:bg-slate-700 flex-1">
                ...
                <Button variant="outline" size="sm" onClick={resetSimulation} className="border-slate-600 text-slate-300 hover:bg-slate-700">
            ...
            <div className="space-y-2">
              ...
              <Slider value={[simulation.replayProgress]} onValueChange={handleReplaySlider} max={100} step={20} className="w-full" disabled={simulation.isRunning}/>
      ...
      <Card className="bg-slate-800 border-slate-700">
        <CardHeader>
          <CardTitle className="text-slate-100 flex items-center gap-2">
            ...
            Live Influence Map
            {simulation.isRunning && (<Badge variant="outline" className="border-cyan-600 text-cyan-400 animate-pulse">
                Simulating
              </Badge>)}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <TooltipProvider>
            <div className="relative w-full h-96 bg-slate-900 rounded-lg border border-slate-700 overflow-hidden" role="region" aria-label="Dynamic Influence Map" aria-live="polite">
              {districts.map((district) => {
            const heatColor = getDistrictHeatColor(district);
            const dominantTier = getDominantTier(district.tierComposition);
            return (<Tooltip key={district.id}>
                    <TooltipTrigger asChild>
                      <div className={`absolute w-16 h-16 rounded-lg border-2 cursor-pointer transition-all duration-200 hover:scale-110 hover:border-white ${heatColor}`} style={{
                    ...
                    boxShadow: district.pulseIntensity > 0 ? `0 0 ${district.pulseIntensity * 20}px #06b6d4` : 'none'
                        ...
                        {district.isAnimating && (<div className="absolute inset-0 border-2 border-cyan-400 rounded-lg animate-ping"></div>)}
                      </div>
                    </TooltipTrigger>
                    <TooltipContent className="bg-slate-700 border-slate-600 text-slate-100 max-w-xs">
                      <div className="space-y-1">
                        ...
                        {district.lastMigration && (<p className="text-xs text-cyan-400">
              ...
              {districts.some(d => d.isAnimating) && (<div className="absolute inset-0 pointer-events-none">
        ...
        <Card className="bg-slate-800 border-slate-700">
          ...
          <CardContent>
            <div className="space-y-3" aria-live="polite" aria-label="Influence swing updates">
              {influenceSwings.length === 0 ? (<p className="text-slate-400 text-sm">No influence changes detected</p>) : (influenceSwings.slice(-5).map((swing, index) => (<div key={`${swing.district}-${swing.timestamp}`} className="flex items-center justify-between p-3 bg-slate-900 rounded-lg animate-fade-in">
                    <div className="flex items-center gap-3">
                      <Badge variant="outline" className={`border-slate-600 ${swing.change > 0 ? 'text-green-400' : 'text-red-400'}`}>
        ...
        <Card className="bg-slate-800 border-slate-700">
          ...
          <CardContent>
            <div className="space-y-3">
              {migrations.length === 0 ? (<p className="text-slate-400 text-sm">No migrations recorded</p>) : (migrations.slice(-4).map((migration, index) => (<div key={migration.timestamp} className="p-3 bg-slate-900 rounded-lg">
