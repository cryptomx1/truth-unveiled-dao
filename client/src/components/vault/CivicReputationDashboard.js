...
import { validateCIDAccess, CIDTier } from '@/access/CIDTierMap';
const CivicReputationDashboard = () => {
    ...
    const getAccessTier = () => {
        ...
        if (metrics.truthPoints >= 1000)
            return CIDTier.TIER_X;
        if (metrics.truthPoints >= 500)
            return CIDTier.TIER_4;
        if (metrics.truthPoints >= 200)
            return CIDTier.TIER_3;
        if (metrics.truthPoints >= 100)
            return CIDTier.TIER_2;
        if (metrics.truthPoints >= 50)
    ...
    if (isLoading) {
        return (<Card className="w-full max-w-4xl mx-auto bg-slate-800 border-slate-700">
      ...
      <Card className="bg-slate-800 border-slate-700">
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" size="sm" onClick={refreshMetrics} disabled={isRefreshing} className="border-slate-600 text-slate-300 hover:bg-slate-700">
