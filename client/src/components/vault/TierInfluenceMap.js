...
const TierInfluenceMap = () => {
    ...
    const generateMockDistricts = () => {
        ...
        return districtNames.map((name, index) => {
            ...
            const aggregatedScore = tierComposition.Commander * 1000 +
    ...
    const getHeatIntensity = (influence, maxInfluence) => {
        return Math.min(100, (influence / maxInfluence) * 100);
    };
    const getHeatColor = (intensity) => {
        if (intensity >= 80)
            return 'bg-red-600'; // High influence
        if (intensity >= 60)
            return 'bg-orange-500'; // Medium-high
        if (intensity >= 40)
            return 'bg-yellow-500'; // Medium
        if (intensity >= 20)
            return 'bg-green-500'; // Low-medium
    ...
    const formatTooltipContent = (district) => {
...
Influence: ${Math.round(district.totalInfluence)}`;
    ...
    useEffect(() => {
        if (metrics && metrics.topDistricts.length > 0) {
            const topDistrictsText = metrics.topDistricts.map((d, i) => `${i + 1}. ${d.name} district ${d.id} with ${Math.round(d.totalInfluence)} influence`).join(', ');
            const announcement = new SpeechSynthesisUtterance(`Top 3 most influential districts: ${topDistrictsText}`);
    ...
    return (<div className="w-full max-w-6xl mx-auto space-y-6">
      ...
      <Card className="bg-slate-800 border-slate-700">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" size="sm" onClick={refreshInfluenceMap} disabled={isRefreshing} className="border-slate-600 text-slate-300 hover:bg-slate-700">
      ...
      <Card className="bg-slate-800 border-slate-700">
        ...
        <CardContent>
          <TooltipProvider>
            <div className="relative w-full h-96 bg-slate-900 rounded-lg border border-slate-700" role="region" aria-label="Influence Map">
            ...
            return (<Tooltip key={district.id}>
                    <TooltipTrigger asChild>
                      <div className={`absolute w-16 h-16 rounded-lg border-2 border-slate-600 cursor-pointer transition-all duration-200 hover:scale-110 hover:border-white ${heatColor}`} style={{
                    left: `${district.coordinates.x}%`,
                    top: `${district.coordinates.y}%`,
                    transform: 'translate(-50%, -50%)',
                    opacity: district.zkpValidated ? 0.9 : 0.5
                }} onMouseEnter={() => setHoveredDistrict(district)} onMouseLeave={() => setHoveredDistrict(null)} aria-label={`District ${district.id}: ${district.name}, ${Math.round(intensity)}% influence`}>
                        ...
                        {!district.zkpValidated && (<div className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full border border-white"></div>)}
                      </div>
                    </TooltipTrigger>
          ...
          <Card className="bg-slate-800 border-slate-700">
            ...
            <CardContent className="space-y-3">
              {metrics.topDistricts.map((district, index) => {
                const commanderRatio = Math.round(district.tierComposition.Commander * 100);
                const dominantTier = getDominantTier(district.tierComposition);
                return (<div key={district.id} className="flex items-center justify-between p-3 bg-slate-900 rounded-lg">
