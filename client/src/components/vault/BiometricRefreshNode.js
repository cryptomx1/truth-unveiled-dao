...
import { Fingerprint, RefreshCw, CheckCircle, AlertCircle, Download, Target } from 'lucide-react';
import { createBiometricSession, verifyBiometricSession, refreshVaultIdentity } from '../../identity/IdentityVaultCore';
import { getActivityProfile } from '../../identity/CivicIdentityMinter';
import { exportReputationBundle } from '../../services/ZKReputationAssembler';
import { logBiometricVerification, logIdentityRefresh } from '../../services/VaultTelemetryLogger';
import { announceBiometricSuccess, announceBiometricFailure, announceRefreshConfirmation } from '../../services/VaultNarrationSystem';
// Main BiometricRefreshNode component
export const BiometricRefreshNode = ({ vaultEntry, onRefreshComplete, onRefreshCancel, triggerReason = 'user_request' }) => {
    // Component state
    ...
    const [refreshedEntry, setRefreshedEntry] = useState(null);
    const [refreshedBundle, setRefreshedBundle] = useState(null);
    ...
    const performBiometricScan = async () => {
        ...
        try {
            ...
            // Verify biometric
            const verificationResult = verifyBiometricSession(biometricState.session.sessionId, mockBiometricData);
            const endTime = performance.now();
            const duration = Math.round(endTime - startTime);
            // Log biometric verification
            logBiometricVerification(vaultEntry.did, biometricState.session.sessionId, verificationResult.success, verificationResult.qualityScore, 'fingerprint', duration);
    ...
    const updateActivityProfile = async () => {
        if (!biometricState.session?.verified)
            return;
    ...
    const processIdentityRefresh = async (updatedProfile) => {
        if (!biometricState.session || !profileUpdateState.profile)
            return;
        setRefreshState(prev => ({ ...prev, isLoading: true }));
        const startTime = performance.now();
        try {
            const refreshResult = await refreshVaultIdentity(vaultEntry.entryId, biometricState.session, updatedProfile, triggerReason);
            const endTime = performance.now();
            const duration = Math.round(endTime - startTime);
            if (refreshResult.success && refreshResult.refreshedEntry) {
                ...
                logIdentityRefresh(vaultEntry.cid, vaultEntry.entryId, oldEpoch, newEpoch, profileUpdateState.trustChange, triggerReason, duration);
                // Announce refresh confirmation
                announceRefreshConfirmation(vaultEntry.cid, newEpoch, profileUpdateState.trustChange);
                setRefreshState(prev => ({
                    ...prev,
                    step: 'export_ready',
                    isLoading: false
    ...
    const getStepIndicatorStyle = (stepName) => {
    ...
    return (<div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={handleCancel} className="px-3 py-2 bg-slate-700 border border-slate-600 text-slate-300 rounded-lg hover:bg-slate-600 transition-colors text-sm" aria-label="Cancel refresh process">
            Cancel
          </button>
        </div>
        ...
        ].map((step, index) => (<div key={step.key} className="flex items-center gap-2">
      ...
      {refreshState.step === 'biometric_scan' && (<div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
          ...
          <div className="bg-blue-900/10 border border-blue-600/30 rounded-lg p-4 mb-4">
            <div className="flex items-center justify-center mb-4">
              <div className="relative">
                ...
                {biometricState.isScanning && (<div className="absolute inset-0 rounded-full border-4 border-blue-500 border-t-transparent animate-spin"/>)}
              </div>
            </div>
            
            {biometricState.isScanning && (<div className="mb-4">
                <div className="flex justify-between text-sm text-blue-300 mb-1">
                  <span>Scanning...</span>
                  <span>{biometricState.scanProgress}%</span>
                </div>
                <div className="w-full bg-blue-900/30 rounded-full h-2">
                  <div className="bg-blue-500 h-2 rounded-full transition-all duration-150" style={{ width: `${biometricState.scanProgress}%` }}/>
                </div>
              </div>)}
            
            <div className="text-center">
              {!biometricState.session ? (<div className="text-slate-400">Initializing biometric session...</div>) : !biometricState.isScanning && !biometricState.verified ? (<button onClick={performBiometricScan} className="flex items-center gap-2 px-4 py-2 bg-blue-900/30 border border-blue-600 text-blue-300 rounded-lg hover:bg-blue-800/30 transition-colors mx-auto" aria-label="Start biometric scan">
                  <Fingerprint className="w-4 h-4"/>
                  <span>Place Finger on Scanner</span>
                </button>) : biometricState.verified ? (<div className="flex items-center gap-2 text-green-400 justify-center">
                  <CheckCircle className="w-5 h-5"/>
                  <span>Biometric Verified ({biometricState.qualityScore}%)</span>
                </div>) : (<div className="text-blue-300">Scanning in progress...</div>)}
            </div>
          </div>
        </div>)}
      
      {/* Profile Update Section */}
      {(refreshState.step === 'profile_update' || refreshState.step === 'refresh_processing') && (<div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
          <h2 className="text-lg font-medium text-slate-200 mb-4 flex items-center gap-2">
            <RefreshCw className="w-5 h-5 text-purple-400"/>
            Profile Update & Refresh
          </h2>
          
          {profileUpdateState.profile && (<div className="bg-purple-900/10 border border-purple-600/30 rounded-lg p-4 mb-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {profileUpdateState.trustChange !== 0 && (<span className={`text-xs ${profileUpdateState.trustChange > 0 ? 'text-green-400' : 'text-red-400'}`}>
          ...
          {refreshState.step === 'refresh_processing' && (<div className="flex items-center gap-3 text-purple-300">
      ...
      {refreshState.step === 'export_ready' && refreshedBundle && (<div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
          <h2 className="text-lg font-medium text-slate-200 mb-4 flex items-center gap-2">
          ...
          <button onClick={handleExportBundle} disabled={refreshState.isLoading} className="flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-600 text-green-300 rounded-lg hover:bg-green-800/30 transition-colors" aria-label="Export refreshed identity bundle">
      ...
      {/* Complete Section */}
      {refreshState.step === 'complete' && (<div className="bg-green-900/10 border border-green-600/30 rounded-lg p-6">
          ...
          {refreshedEntry && (<div className="bg-green-800/20 rounded-lg p-3 text-sm">
      ...
      {/* Error Display */}
      {(refreshState.error || biometricState.error || profileUpdateState.error) && (<div className="bg-red-900/10 border border-red-600/30 rounded-lg p-4">
