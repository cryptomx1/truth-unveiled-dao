...
export const TruthCoinsGallery = ({ showGenesis = true, maxDisplay, size = 'medium', viewMode = 'grid', className = '' }) => {
    ...
    return (<Card className={`w-full bg-slate-800 border-slate-700 ${className}`}>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {loadedTokens > 0 && (<Badge className="bg-green-600/30 text-green-200">
                {loadedTokens} Loaded
              </Badge>)}
          </div>
          
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => setCurrentViewMode('grid')} className={`${currentViewMode === 'grid' ? 'bg-blue-600/20 border-blue-500' : 'border-slate-600'}`}>
              <Grid className="w-4 h-4"/>
            </Button>
            <Button variant="outline" size="sm" onClick={() => setCurrentViewMode('list')} className={`${currentViewMode === 'list' ? 'bg-blue-600/20 border-blue-500' : 'border-slate-600'}`}>
      ...
      <CardContent>
        {displayTokens.length === 0 ? (<div className="text-center py-12">
            ...
            {currentViewMode === 'grid' ? (<div className="grid gap-6 justify-items-center" style={{
                    gridTemplateColumns: `repeat(auto-fit, minmax(${tokenSize}px, 1fr))`
                }}>
                {displayTokens.map((token) => (<div key={token.id} className="text-center space-y-3">
                    <RotatingTokenDisplay frontCID={token.frontCID} backCID={token.backCID} guardian={token.guardian} isGenesis={token.isGenesis} size={tokenSize} onImageLoad={handleTokenLoad} className="mx-auto"/>
                ...
                {displayTokens.map((token) => (<div key={token.id} className="flex items-center gap-4 p-4 bg-slate-700/30 rounded-lg border border-slate-600/50">
                    <RotatingTokenDisplay frontCID={token.frontCID} backCID={token.backCID} guardian={token.guardian} isGenesis={token.isGenesis} size={80} onImageLoad={handleTokenLoad}/>
