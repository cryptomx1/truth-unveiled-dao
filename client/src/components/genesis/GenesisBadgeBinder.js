...
import { Zap, Star, Download, AlertCircle, CheckCircle, Coins, Shield, Eye, Sparkles } from 'lucide-react';
import { truthFusionEngine } from '@/fusion/TruthFusionEngine';
const GenesisBadgeBinder = ({ userDID = 'did:civic:alice123' }) => {
    const [tpAmount, setTpAmount] = useState('500');
    const [fusionValidation, setFusionValidation] = useState(null);
    ...
    const handleFusionExecute = async () => {
        if (!fusionValidation?.isEligible)
            return;
    ...
    return (<Card className="w-full max-w-4xl bg-white dark:bg-slate-900">
      <CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="tpAmount" type="number" value={tpAmount} onChange={(e) => setTpAmount(e.target.value)} min="500" max={tpBalance.availableTP} className="text-lg" placeholder="Enter TP amount"/>
            </div>

            <div className="flex items-end">
              <Button onClick={() => setShowPreview(!showPreview)} variant="outline" className="w-full flex items-center gap-2" disabled={!fusionValidation?.isEligible}>
          ...
          {/* Validation Status */}
          {fusionValidation && (<div className={`rounded-lg p-4 border-l-4 ${fusionValidation.isEligible
                ? 'bg-green-50 dark:bg-green-900/20 border-green-500'
                : 'bg-red-50 dark:bg-red-900/20 border-red-500'}`}>
              <div className="flex items-start gap-3">
                {fusionValidation.isEligible ? (<CheckCircle className="h-5 w-5 text-green-500 mt-0.5"/>) : (<AlertCircle className="h-5 w-5 text-red-500 mt-0.5"/>)}
                
                <div className="flex-1">
                  <p className={`font-medium ${fusionValidation.isEligible ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}`}>
        ...
        {/* Badge Preview */}
        {showPreview && badgePreview && (<div className="bg-gradient-to-br from-purple-100 to-blue-100 dark:from-slate-800 dark:to-slate-700 rounded-lg p-6 border-2 border-purple-200 dark:border-purple-700">
        ...
        {/* Fusion Action */}
        {!completedBadge ? (<div className="flex justify-center">
            <Button onClick={handleFusionExecute} disabled={!fusionValidation?.isEligible || fusionInProgress} className="px-8 py-3 text-lg bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700">
              {fusionInProgress ? (<>
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-lg p-6 border-2 border-green-200 dark:border-green-700">
            ...
            <div className="flex justify-center">
              <Button onClick={handleExportBadge} className="flex items-center gap-2 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700">
        ...
        {/* Progress Indicator */}
        {fusionInProgress && (<div className="space-y-2">
