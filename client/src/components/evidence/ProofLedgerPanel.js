...
import { CheckCircle, AlertTriangle, Lock, Clock, Download, Search, Filter, RefreshCw, FileText, Shield, Eye, EyeOff, ChevronDown, ChevronRight, Hash, User, Award } from 'lucide-react';
import { ProofVaultCore } from '../../evidence/ProofVaultCore';
import { ProofExporterNode } from '../../evidence/ProofExporterNode';
import { announceVaultAccessed, announceDownloadReady, announceVerificationSuccess } from '../../evidence/EvidenceNarrationNode';
// Main ProofLedgerPanel component
export const ProofLedgerPanel = ({ userHash, showAllEntries = true, enableExport = true, maxEntries = 100, onEntryClick }) => {
    // Component state
    ...
    const loadProofLedger = async () => {
        ...
        try {
            ...
            if (searchTerm) {
                ...
                ledgerEntries = ledgerEntries.filter(entry => entry.capsule.missionId.toLowerCase().includes(searchLower) ||
    ...
    const handleExport = async () => {
        if (!enableExport || exportInProgress)
            return;
    ...
    const getStatusColorClass = (entry) => {
        switch (entry.integrityStatus) {
    ...
    return (<div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            {enableExport && (<button onClick={handleExport} disabled={exportInProgress || entries.length === 0} className="flex items-center gap-2 px-3 py-2 bg-green-900/30 border border-green-600 text-green-300 rounded-lg hover:bg-green-800/30 transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed" aria-label="Export proof ledger">
                <Download className={`w-4 h-4 ${exportInProgress ? 'animate-spin' : ''}`}/>
                <span>{exportInProgress ? 'Exporting...' : 'Export'}</span>
              </button>)}
            
            <button onClick={loadProofLedger} className="flex items-center gap-2 px-3 py-2 bg-purple-900/30 border border-purple-600 text-purple-300 rounded-lg hover:bg-purple-800/30 transition-colors text-sm" aria-label="Refresh proof ledger">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            <div className="flex-1 relative">
              ...
              <input type="text" placeholder="Search missions, categories, or event types..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-slate-700 border border-slate-600 text-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"/>
            </div>
            
            <button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-colors ${showFilters
            ? 'bg-blue-900/30 border border-blue-600 text-blue-300'
            : 'bg-slate-700 border border-slate-600 text-slate-400 hover:text-slate-300'}`} aria-label="Toggle filters">
          ...
          {/* Filter Panel */}
          {showFilters && (<div className="bg-slate-700/30 rounded-lg p-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-2">Event Type</label>
                  <select value={filters.eventType || ''} onChange={(e) => setFilters({ ...filters, eventType: e.target.value || undefined })} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 text-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
                ...
                <div>
                  ...
                  <select value={filters.integrityStatus || ''} onChange={(e) => setFilters({ ...filters, integrityStatus: e.target.value || undefined })} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 text-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
                ...
                <div>
                  ...
                  <select value={filters.validationLevel || ''} onChange={(e) => setFilters({ ...filters, validationLevel: e.target.value || undefined })} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 text-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
                ...
                <div>
                  ...
                  <select value={filters.completionStatus || ''} onChange={(e) => setFilters({ ...filters, completionStatus: e.target.value || undefined })} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 text-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-400">
            <span>Show:</span>
            <button onClick={() => setExpandedEntries(entries.length > expandedEntries.size ? new Set(entries.map(e => e.entryId)) : new Set())} className="text-purple-400 hover:text-purple-300 transition-colors">
      ...
      {/* Evidence Ledger */}
      <div className="space-y-4">
        {entries.length === 0 ? (<div className="bg-slate-800 border border-slate-600 rounded-lg p-8 text-center">
            ...
            return (<div key={entry.entryId} className="bg-slate-800 border border-slate-600 rounded-lg overflow-hidden">
                {/* Entry Header */}
                <div className="p-4 cursor-pointer transition-colors hover:bg-slate-700/30" onClick={() => handleEntryClick(entry)} role="button" tabIndex={0} aria-label={`Evidence entry for ${entry.capsule.metadata.missionTitle} - ${getStatusText(entry)}`} onKeyDown={(e) => {
                ...
                }} className="text-slate-400 hover:text-slate-300 transition-colors" aria-label={`${isExpanded ? 'Hide' : 'Show'} entry details`}>
                ...
                {isExpanded && (<div className="border-t border-slate-600 p-4 bg-slate-700/30">
                    ...
                    {/* Cross-References */}
                    {entry.capsule.metadata.crossValidation && entry.capsule.metadata.crossValidation.length > 0 && (<div className="mt-4">
                        <h4 className="text-sm font-medium text-slate-300 mb-2">Cross-References</h4>
                        <div className="flex flex-wrap gap-2">
                          {entry.capsule.metadata.crossValidation.map((ref, index) => (<span key={index} className="px-2 py-1 text-xs bg-slate-600/50 border border-slate-500 text-slate-300 rounded font-mono">
                              {ref.substring(0, 20)}...
                            </span>))}
                        </div>
                      </div>)}
