...
const SentimentExplorerPanel = () => {
    ...
    const applyFilters = () => {
        ...
        if (filters.startDate && filters.endDate) {
            ...
            filtered = filtered.filter(entry => entry.timestamp >= startTime && entry.timestamp <= endTime);
        }
        // Filter by trust range
        filtered = filtered.filter(entry => entry.averageTrust >= filters.minTrust && entry.averageTrust <= filters.maxTrust);
        // Filter by volatility range
        filtered = filtered.filter(entry => entry.volatility >= filters.minVolatility && entry.volatility <= filters.maxVolatility);
    ...
    const getDeckDisplayName = (deckId) => {
        const deckNames = {
    ...
    const getTrustStatusColor = (trust) => {
        if (trust >= 70)
    ...
    const getVolatilityStatusColor = (volatility) => {
        if (volatility <= 10)
            return 'text-green-400';
        if (volatility <= 20)
            return 'text-yellow-400';
    ...
    return (<div className="bg-slate-800 rounded-lg border border-slate-600 p-6 max-w-4xl mx-auto">
      {/* Header */}
      ...
      <div className="bg-slate-700 rounded-lg p-4 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select value={filters.deckId} onChange={(e) => handleFilterChange('deckId', e.target.value)} className="w-full bg-slate-600 text-white rounded-md px-3 py-2 text-sm">
          ...
          <div>
            ...
            <input type="date" value={filters.startDate} onChange={(e) => handleFilterChange('startDate', e.target.value)} className="w-full bg-slate-600 text-white rounded-md px-3 py-2 text-sm"/>
          </div>
          
          <div>
            <label className="block text-slate-300 text-sm mb-1">End Date</label>
            <input type="date" value={filters.endDate} onChange={(e) => handleFilterChange('endDate', e.target.value)} className="w-full bg-slate-600 text-white rounded-md px-3 py-2 text-sm"/>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input type="range" min="0" max="100" value={filters.minTrust} onChange={(e) => handleFilterChange('minTrust', parseInt(e.target.value))} className="flex-1"/>
              <input type="range" min="0" max="100" value={filters.maxTrust} onChange={(e) => handleFilterChange('maxTrust', parseInt(e.target.value))} className="flex-1"/>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input type="range" min="0" max="100" value={filters.minVolatility} onChange={(e) => handleFilterChange('minVolatility', parseInt(e.target.value))} className="flex-1"/>
              <input type="range" min="0" max="100" value={filters.maxVolatility} onChange={(e) => handleFilterChange('maxVolatility', parseInt(e.target.value))} className="flex-1"/>
            </div>
          </div>

          {/* Reset Button */}
          <div className="flex items-end">
            <button onClick={resetFilters} className="bg-slate-600 hover:bg-slate-500 text-white px-4 py-2 rounded-md text-sm transition-colors">
              Reset Filters
            </button>
          </div>
        </div>
      ...
      <div className="space-y-3 max-h-96 overflow-y-auto">
        {filteredEntries.length === 0 ? (<div className="text-center text-slate-400 py-8">
            No entries match the current filter criteria
          </div>) : (filteredEntries.map((entry) => (<div key={entry.entryIndex} className="bg-slate-700 rounded-lg border border-slate-600 overflow-hidden">
              {/* Entry Header */}
              <div className="p-4 cursor-pointer hover:bg-slate-600 transition-colors" onClick={() => setExpandedEntry(expandedEntry === entry.entryIndex ? null : entry.entryIndex)}>
              ...
              {/* Expanded Details */}
              {expandedEntry === entry.entryIndex && (<div className="bg-slate-800 p-4 border-t border-slate-600">
