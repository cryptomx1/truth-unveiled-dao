...
class SentimentLedgerEngineClass {
    ...
    generateZKPHash(deckId, timestamp) {
    ...
    updateDailyDigest(date) {
        const dayEntries = this.ledger.filter(entry => new Date(entry.timestamp).toISOString().split('T')[0] === date);
    ...
    getLedgerByDeck(deckId) {
        return this.ledger.filter(entry => entry.deckId === deckId);
    }
    getLedgerByDateRange(startDate, endDate) {
        const start = startDate.getTime();
        const end = endDate.getTime();
        return this.ledger.filter(entry => entry.timestamp >= start && entry.timestamp <= end);
    }
    getLedgerByTrustThreshold(minTrust, maxTrust) {
        return this.ledger.filter(entry => entry.averageTrust >= minTrust && entry.averageTrust <= maxTrust);
    }
    getLedgerByVolatilityDelta(minVolatility, maxVolatility) {
        return this.ledger.filter(entry => entry.volatility >= minVolatility && entry.volatility <= maxVolatility);
    }
    getAllEntries() {
    ...
    getAllDailyDigests() {
        return Array.from(this.dailyDigests.values()).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    }
