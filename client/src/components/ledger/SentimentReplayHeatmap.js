...
const SentimentReplayHeatmap = () => {
    const [heatmapData, setHeatmapData] = useState([]);
    const [hoveredCell, setHoveredCell] = useState(null);
    const [selectedDeck, setSelectedDeck] = useState('all');
    const [timeRange, setTimeRange] = useState('30d');
    const [viewMode, setViewMode] = useState('trust');
    ...
    const getTrustColor = (trust, entryCount) => {
        if (entryCount === 0)
    ...
    const getVolatilityColor = (volatility, entryCount) => {
        if (entryCount === 0)
            return 'bg-slate-700'; // No data
        if (volatility <= 5)
            return 'bg-green-500';
    ...
    const getIntensityOpacity = (entryCount) => {
        if (entryCount === 0)
            return 'opacity-20';
        if (entryCount <= 2)
            return 'opacity-40';
    ...
    return (<div className="bg-slate-800 rounded-lg border border-slate-600 p-6 max-w-4xl mx-auto">
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={selectedDeck} onChange={(e) => setSelectedDeck(e.target.value)} className="bg-slate-700 text-white rounded-md px-3 py-1 text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={timeRange} onChange={(e) => setTimeRange(e.target.value)} className="bg-slate-700 text-white rounded-md px-3 py-1 text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={viewMode} onChange={(e) => setViewMode(e.target.value)} className="bg-slate-700 text-white rounded-md px-3 py-1 text-sm">
      ...
      {/* Heatmap Grid */}
      <div ref={heatmapRef} className="relative overflow-x-auto" style={{ minHeight: '200px' }}>
        <div className="grid gap-1" style={{
          ...
          {/* Day labels */}
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day, index) => (<div key={`day-${index}`} className="text-xs text-slate-400 flex items-center justify-center" style={{
                gridColumn: 1,
                gridRow: index + 1
            }}>
              {day}
          ...
          {/* Heatmap cells */}
          {heatmapData.map((cell, index) => (<div key={index} className={`
            ...
            }} onMouseEnter={(e) => handleCellHover(cell, e)} onMouseLeave={handleCellLeave} title={`${formatDate(cell.date)}: ${viewMode === 'trust' ? 'Trust' : 'Volatility'}: ${viewMode === 'trust' ? cell.trust.toFixed(1) : cell.volatility.toFixed(1)}%`}/>))}
        </div>

        {/* Hover Tooltip */}
        {hoveredCell && (<div className="absolute z-10 bg-slate-900 border border-slate-600 rounded-lg p-3 shadow-lg pointer-events-none" style={{
            ...
            <div className="text-slate-300 text-sm space-y-1">
              ...
              {hoveredCell.cell.entryCount > 0 && (<div className="text-xs text-slate-400 mt-2">
