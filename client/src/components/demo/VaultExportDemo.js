    ...
    const setupTestCredentials = async () => {
        try {
            ...
            const testCredentials = [
                { did: 'did:civic:0x1a2b3c4d5e', type: 'Identity', metadata: { role: 'Citizen' } },
                { did: 'did:civic:0x1a2b3c4d5e', type: 'Vault', metadata: { vaultAccess: ['personal'] } },
                { did: 'did:civic:0x2b3c4d5e6f', type: 'Role', metadata: { role: 'Moderator' } },
                { did: 'did:civic:0x3c4d5e6f7a', type: 'Governance', metadata: { role: 'Governor' } },
                { did: 'did:civic:0x4d5e6f7a8b', type: 'Identity', metadata: { role: 'Citizen' } },
                { did: 'did:civic:0x5e6f7a8b9c', type: 'Record', metadata: { recordType: 'voting' } }
            ];
    ...
    const runExportTestSuite = async () => {
        ...
        try {
            for (let i = 0; i < testScenarios.length; i++) {
                ...
                const result = await exportNode.exportVaultData(scenario.exporterDID, scenario.userRole, scenario.filters, scenario.overrideFlag);
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      <div className="text-center mb-6">
        ...
        <div className="text-sm text-slate-400 space-y-1">
          ...
          {!isSetupComplete && (<div className="text-amber-400">Setting up test credentials...</div>)}
        </div>
      </div>

      {/* Test Controls */}
      <div className="mb-4 space-y-2">
        <button onClick={runExportTestSuite} disabled={isRunning} className={`w-full py-3 px-4 rounded-md font-medium transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800 ${isRunning
            ...
            {testResults.map((result, index) => (<div key={result.scenario.id} className="text-xs">
                ...
                <div className="ml-8 mt-1 space-y-1">
                  {result.success && (<>
                      ...
                      {result.bundleCID && (<div className="text-slate-500">
