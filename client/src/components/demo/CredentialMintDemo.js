...
export default function CredentialMintDemo() {
    ...
    const testScenarios = [
        ...
        // Scenario 8: Invalid metadata (FAIL)
        {
            ...
            credentialType: 'Role',
            metadata: null, // Intentional invalid metadata
            expectedResult: false,
    ...
    const runMintTestSuite = async () => {
        ...
        try {
            for (let i = 0; i < testScenarios.length; i++) {
                ...
                const result = await mintLayer.mintCredential(scenario.did, scenario.credentialType, scenario.metadata);
    ...
    const testRevocation = async (successfulMints) => {
        ...
        for (let i = 0; i < Math.min(3, successfulMints.length); i++) {
            ...
            if (mint.zkHash) {
                const revokeResult = await mintLayer.revokeCredential(mint.zkHash, `Test revocation ${i + 1}`);
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      <div className="mb-4 space-y-2">
        <button onClick={runMintTestSuite} disabled={isRunning} className={`w-full py-3 px-4 rounded-md font-medium transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800 ${isRunning
      ...
      {testResults.length > 0 && (<div className="mb-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {testResults.map((result, index) => (<div key={result.scenario.id} className="text-xs">
                ...
                {result.error && (<div className="text-slate-400 ml-8 mt-1">
                    Error: {result.error}
                  </div>)}
                
                {result.pathBTriggered && (<div className="text-orange-400 ml-8 mt-1">
                    ⚠️ Path B fallback triggered
                  </div>)}

                {result.zkHash && (<div className="text-slate-500 ml-8 mt-1">
      ...
      {/* Revocation Results */}
      {revokeResults.length > 0 && (<div className="mb-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
          <div className="text-sm font-medium text-slate-300 mb-2">
            Revocation Tests ({revokeResults.length})
          </div>
          <div className="space-y-1 text-xs">
            {revokeResults.map((result, index) => (<div key={index} className="flex justify-between">
      ...
      {mintHistory.length > 0 && (<div className="mb-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
          <div className="text-sm font-medium text-slate-300 mb-2">
            Mint History ({mintHistory.length} entries)
          </div>
          <div className="space-y-1 max-h-32 overflow-y-auto text-xs">
            {mintHistory.slice(-5).map((entry) => (<div key={entry.id} className="flex justify-between">
      ...
      {testResults.length === testScenarios.length && (<div className="p-3 bg-slate-900 border border-slate-600 rounded-md">
          ...
          <div className="space-y-1 text-xs">
            ...
            {revokeResults.length > 0 && (<div className="flex justify-between">
