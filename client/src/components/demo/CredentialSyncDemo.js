...
export default function CredentialSyncDemo() {
    ...
    const setupTestEnvironment = async () => {
        try {
            ...
            const testCredentials = [
                { did: 'did:civic:0x1a2b3c4d5e', type: 'Identity', metadata: { role: 'Citizen', attestation: 'verified' } },
                { did: 'did:civic:0x2b3c4d5e6f', type: 'Identity', metadata: { role: 'Citizen', attestation: 'verified' } },
                { did: 'did:civic:0x3c4d5e6f7a', type: 'Governance', metadata: { role: 'Governor', permissions: ['consensus'] } },
                { did: 'did:civic:0x5e6f7a8b9c', type: 'Role', metadata: { role: 'Moderator', permissions: ['verify'] } },
                { did: 'did:civic:0x6f7a8b9c0d', type: 'Governance', metadata: { role: 'Council', permissions: ['vote'] } },
                { did: 'did:civic:0x7a8b9c0d1e', type: 'Identity', metadata: { role: 'Verifier', attestation: 'multi_factor' } },
                { did: 'did:civic:0x9c0d1e2f3a', type: 'Vault', metadata: { vaultAccess: ['secure'], exportRights: true } },
                { did: 'did:civic:0xa0d1e2f3b4', type: 'Record', metadata: { recordType: 'voting', attestation: 'verified' } }
            ];
    ...
    const runSyncTestSuite = async () => {
        ...
        try {
            for (let i = 0; i < testScenarios.length; i++) {
                ...
                const result = await syncLedger.syncCredential(scenario.credentialZkHash, scenario.sourceDID, scenario.targetDID, scenario.syncType);
    ...
    // Get result color
    const getResultColor = (success, expected) => {
        if (success === expected) {
    ...
    // Get result icon
    const getResultIcon = (success, expected) => {
        if (success === expected) {
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      <div className="text-center mb-6">
        ...
        <div className="text-sm text-slate-400 space-y-1">
          ...
          {isRunning && (<div className="flex items-center justify-center space-x-2">
              <div className="w-2 h-2 bg-blue-400 rounded-full animate-pulse"></div>
              <span className="text-blue-400">Running Test {currentTest}/10</span>
            </div>)}
          {!isSetupComplete && (<div className="text-amber-400">Setting up test environment...</div>)}
        </div>
      </div>

      {/* Test Controls */}
      <div className="mb-4 space-y-2">
        <button onClick={runSyncTestSuite} disabled={isRunning} className={`w-full py-3 px-4 rounded-md font-medium transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800 ${isRunning
            ? 'bg-slate-600 cursor-not-allowed text-slate-400'
            : 'bg-blue-600 hover:bg-blue-700 text-white'}`} style={{ minHeight: '48px' }} aria-describedby="test-info">
          {isRunning ? `Running Test ${currentTest}/10...` : 'Run Credential Sync Test Suite'}
        </button>

        <button onClick={clearTestData} disabled={isRunning} className="w-full py-2 px-4 bg-slate-600 hover:bg-slate-700 disabled:bg-slate-700 disabled:cursor-not-allowed text-white font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 focus:ring-offset-slate-800" style={{ minHeight: '48px' }}>
      ...
      {/* Sync Metrics */}
      {metrics && (<div className="mb-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
          ...
          <div className="space-y-2 text-sm">
            ...
            {/* Path B Status */}
            {metrics.pathBActivated && (<div className="mt-3 pt-3 border-t border-slate-600">
      ...
      {consensusNodes.length > 0 && (<div className="mb-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
          <div className="text-sm font-medium text-slate-300 mb-2">
            Consensus Nodes ({consensusNodes.filter(n => n.isOnline).length}/{consensusNodes.length} online)
          </div>
          <div className="space-y-1 max-h-32 overflow-y-auto text-xs">
            {consensusNodes.map((node, index) => (<div key={node.nodeId} className="flex justify-between">
      ...
      {/* Test Results */}
      {testResults.length > 0 && (<div className="mb-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
          <div className="text-sm font-medium text-slate-300 mb-2">Sync Test Results</div>
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {testResults.map((result, index) => (<div key={result.scenario.id} className="text-xs">
                ...
                <div className="ml-8 mt-1 space-y-1">
                  {result.success && (<>
                      ...
                      {result.syncEntry?.cidHash && (<div className="text-slate-500">
                          CID: {result.syncEntry.cidHash.slice(0, 12)}...
                        </div>)}
                    </>)}
                  
                  {result.error && (<div className="text-slate-400">
                      Error: {result.error}
                    </div>)}
                  
                  {result.pathBTriggered && (<div className="text-orange-400">
      ...
      {/* Ledger Log Preview */}
      {ledgerLog.length > 0 && (<div className="mb-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
          <div className="text-sm font-medium text-slate-300 mb-2">
            Ledger Log (Latest {ledgerLog.length})
          </div>
          <div className="space-y-1 max-h-32 overflow-y-auto text-xs">
            {ledgerLog.map((entry, index) => (<div key={entry.syncId || index} className="flex justify-between">
      ...
      {/* Performance Summary */}
      {testResults.length === testScenarios.length && (<div className="p-3 bg-slate-900 border border-slate-600 rounded-md">
