...
import { CredentialConflictResolver } from '../resolution/CredentialConflictResolver';
export default function CredentialConflictDemo() {
    ...
    const generateTestConflicts = async () => {
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      <div className="mb-6 space-y-3">
        <button onClick={runConflictTestSuite} disabled={isRunning} className={`w-full py-3 px-4 rounded-md font-medium transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800 ${isRunning
            ? 'bg-slate-600 text-slate-400 cursor-not-allowed'
            : 'bg-blue-600 hover:bg-blue-700 text-white'}`} style={{ minHeight: '48px' }}>
          {isRunning ? 'Running Conflict Tests...' : 'Run Conflict Test Suite'}
        </button>

        <div className="grid grid-cols-2 gap-2">
          <button onClick={runPerformanceTest} className="py-2 px-3 bg-green-600 hover:bg-green-700 text-white font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-slate-800" style={{ minHeight: '48px' }}>
            Performance Test
          </button>
          
          <button onClick={clearTestData} className="py-2 px-3 bg-red-600 hover:bg-red-700 text-white font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:ring-offset-slate-800" style={{ minHeight: '48px' }}>
            Clear Data
          </button>
        </div>
      </div>
      ...
      {/* Conflict Metrics */}
      {conflictMetrics && (<div className="mb-6 p-4 bg-slate-700 border border-slate-600 rounded-md">
          ...
          <div className="space-y-2 text-xs">
            ...
            {conflictMetrics.pathBActivated && (<div className="mt-2 p-2 bg-red-900/20 border border-red-700 rounded">
      ...
      {/* Performance Results */}
      {performanceResults && (<div className="mb-6 p-4 bg-slate-900 border border-slate-600 rounded-md">
          ...
          <div className="space-y-2 text-xs">
            ...
            <div className="mt-2 text-center">
              <span className={`px-2 py-1 rounded text-xs font-medium ${performanceResults.passed ? 'bg-green-900/20 text-green-400' : 'bg-red-900/20 text-red-400'}`}>
      ...
      {/* Test Results */}
      {conflictResults.length > 0 && (<div className="mb-6 p-4 bg-slate-700 border border-slate-600 rounded-md">
          <h3 className="text-sm font-medium text-slate-300 mb-3">
            Test Results ({conflictResults.length} conflicts)
          </h3>
          <div className="space-y-2 text-xs">
            {conflictResults.map((result, index) => (<div key={index} className="flex items-center justify-between">
      ...
      {/* Conflict Log */}
      {conflictLog.length > 0 && (<div className="bg-slate-700 border border-slate-600 rounded-md">
          ...
          <div className="max-h-64 overflow-y-auto">
            <div className="divide-y divide-slate-600">
              {conflictLog.slice(-10).reverse().map((conflict) => (<div key={conflict.conflictId} className="p-3 hover:bg-slate-600">
                  ...
                  <div className="space-y-1 text-xs">
                    ...
                    {conflict.nodeParticipation.length > 0 && (<div className="flex justify-between">
                        <span className="text-slate-400">Nodes:</span>
                        <span className="text-slate-300">{conflict.nodeParticipation.length}</span>
                      </div>)}
                    
                    {conflict.pathBTriggered && (<div className="mt-2 p-2 bg-red-900/20 border border-red-700 rounded">
                        <span className="text-red-400 text-xs font-medium">
      ...
      {/* No Data State */}
      {conflictLog.length === 0 && !isRunning && (<div className="text-center py-8 text-slate-400">
