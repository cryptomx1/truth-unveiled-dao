    ...
    return (<div className="max-w-md mx-auto bg-slate-900 border border-slate-700 rounded-lg p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {portfolio.zkpVerified && (<Shield className="w-4 h-4 text-green-400" title="ZKP Verified"/>)}
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="bg-slate-800 rounded-lg p-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => handleCopyToClipboard(portfolio.didId, 'did')} className="p-1 hover:bg-slate-700 rounded">
              {copiedField === 'did' ? (<CheckCircle className="w-3 h-3 text-green-400"/>) : (<Copy className="w-3 h-3 text-slate-400"/>)}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="space-y-3">
          {portfolio.holdings.map((holding) => (<div key={holding.id} className="bg-slate-800 rounded-lg p-3">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-700">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => handleCopyToClipboard(holding.zkpHash, holding.id)} className="p-1 hover:bg-slate-700 rounded">
                    {copiedField === holding.id ? (<CheckCircle className="w-3 h-3 text-green-400"/>) : (<Copy className="w-3 h-3 text-slate-400"/>)}
                  </button>
                  {holding.verified && (<Shield className="w-3 h-3 text-green-400"/>)}
                </div>
              </div>
            </div>))}
        </div>
      ...
      <div className="mb-4">
        ...
        <div className="bg-slate-800 rounded-lg p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={triggerSync} disabled={syncStatus === 'syncing'} className="text-xs text-blue-400 hover:text-blue-300 disabled:text-slate-500">
              Refresh
            </button>
          </div>
