...
export const TransactionStabilityCard = () => {
    ...
    const generateInitialTransactions = () => {
        ...
        for (let i = 0; i < 15; i++) {
            transactions.push({
                ...
                type: ['send', 'receive', 'stake', 'swap'][Math.floor(Math.random() * 4)],
    ...
    const getHealthColor = (value, threshold = 95) => {
        if (value >= threshold)
    ...
    return (<div className="max-w-md mx-auto bg-slate-900 border border-slate-700 rounded-lg p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-lg ${stabilityMetrics.zebekSyncStatus === 'connected' ? 'bg-green-900' : 'bg-amber-900'}`}>
            <Activity className={`w-6 h-6 ${stabilityMetrics.zebekSyncStatus === 'connected' ? 'text-green-400' : 'text-amber-400'}`}/>
          </div>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-slate-800 rounded-lg p-3">
            ...
            <div className={`text-lg font-semibold ${stabilityMetrics.avgLatency < 250 ? 'text-green-400' :
          ...
          <div className="bg-slate-800 rounded-lg p-3">
            ...
            <div className={`text-lg font-semibold ${stabilityMetrics.bufferUtilization < 70 ? 'text-green-400' :
        ...
        <div className="space-y-2">
          ...
          <div className="bg-slate-800 rounded-lg p-3">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-xs font-medium ${stabilityMetrics.solanaRPC === 'healthy' ? 'text-green-400' :
      ...
      {/* Pushback Alert */}
      {pushbackTriggered && (<div className="mb-6 p-3 bg-red-900 border border-red-700 rounded-lg">
          <div className="flex items-center gap-2">
      ...
      <div className="mb-6">
        ...
        <div className="max-h-32 overflow-y-auto space-y-1">
          {activeUsers.slice(0, 5).map((user) => (<div key={user.id} className="bg-slate-800 rounded p-2 text-xs">
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`${user.successRate > 98 ? 'text-green-400' :
                user.successRate > 95 ? 'text-amber-400' : 'text-red-400'}`}>
                  {user.successRate.toFixed(1)}%
                </span>
              </div>
            </div>))}
          {activeUsers.length > 5 && (<div className="text-xs text-slate-400 text-center py-1">
      ...
      <div>
        ...
        <div className="max-h-48 overflow-y-auto space-y-2">
          {transactionBuffer.slice(0, 8).map((tx) => (<div key={tx.id} className="bg-slate-800 rounded-lg p-2">
          ...
          {transactionBuffer.length === 0 && (<div className="text-xs text-slate-500 italic text-center py-4">
