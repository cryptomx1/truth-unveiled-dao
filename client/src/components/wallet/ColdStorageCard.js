...
export const ColdStorageCard = () => {
    ...
    useEffect(() => {
        ...
        const proofUpdateInterval = setInterval(() => {
            setVaultAssets(prev => prev.map(asset => ({
                ...asset,
                preservationStatus: Math.random() > 0.1 ? 'active' : 'pending'
            })));
    ...
    const initiateTransfer = async (assetId) => {
    ...
    const formatTimeUntilUnlock = (unlockDate) => {
        const now = new Date();
    ...
    return (<div className="max-w-md mx-auto bg-slate-900 border border-slate-700 rounded-lg p-6">
      ...
      <div className="mb-6">
        ...
        <div className="space-y-3">
          {vaultAssets.map((asset) => (<div key={asset.id} className={`bg-slate-800 rounded-lg p-3 border ${selectedAsset?.id === asset.id ? 'border-blue-500' : 'border-slate-700'} cursor-pointer transition-colors`} onClick={() => setSelectedAsset(selectedAsset?.id === asset.id ? null : asset)}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
              ...
              {selectedAsset?.id === asset.id && (<div className="mt-3 pt-3 border-t border-slate-700">
                ...
                }} disabled={transferInProgress === asset.id || asset.preservationStatus !== 'active'} className="w-full flex items-center justify-center gap-2 bg-amber-600 hover:bg-amber-700 disabled:bg-slate-600 text-white py-2 px-3 rounded text-xs font-medium transition-colors">
                  ...
                  {asset.preservationStatus !== 'active' && (<div className="mt-2 p-2 bg-amber-900/20 border border-amber-600 rounded text-xs text-amber-200">
