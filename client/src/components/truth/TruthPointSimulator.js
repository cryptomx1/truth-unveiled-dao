...
export const TruthPointSimulator = () => {
    ...
    const generateSimulationBatch = (batchNumber, batchSize) => {
        ...
        const contributionPoints = Array.from({ length: batchSize }, () => {
            ...
            if (tierRoll < 0.4)
                return 10; // Basic (40%)
            if (tierRoll < 0.7)
                return 50; // Verified (30%)
            if (tierRoll < 0.9)
                return 100; // Expert (20%)
        ...
        const variance = contributionPoints.reduce((sum, points) => sum + Math.pow(points - averagePoints, 2), 0) / batchSize;
    ...
    return (<div className="max-w-md mx-auto bg-slate-900 border border-slate-700 rounded-lg p-6">
      ...
      <div className="flex gap-3 mb-6">
        <button onClick={runSimulation} disabled={isRunning} className="flex-1 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 disabled:bg-slate-600 text-white py-2 px-4 rounded-lg text-sm font-medium transition-colors">
          <Play className="w-4 h-4"/>
          {isRunning ? 'Running...' : 'Start'}
        </button>
        <button onClick={pauseSimulation} disabled={!isRunning} className="flex-1 flex items-center justify-center gap-2 bg-amber-600 hover:bg-amber-700 disabled:bg-slate-600 text-white py-2 px-4 rounded-lg text-sm font-medium transition-colors">
          <Pause className="w-4 h-4"/>
          Pause
        </button>
        <button onClick={resetSimulation} className="flex-1 flex items-center justify-center gap-2 bg-slate-600 hover:bg-slate-700 text-white py-2 px-4 rounded-lg text-sm font-medium transition-colors">
      ...
      {/* Pushback Alert */}
      {metrics.pushbackTriggered && (<div className="mb-6 p-3 bg-red-900 border border-red-700 rounded-lg">
      ...
      <div className="mb-6">
        ...
        <div className="bg-slate-800 rounded-lg p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={`text-sm font-medium ${metrics.averageLatency > 100 ? 'text-red-400' :
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1 bg-slate-700 rounded-full h-1">
              <div className={`h-1 rounded-full ${metrics.averageLatency > 100 ? 'bg-red-500' :
            metrics.averageLatency > 50 ? 'bg-amber-500' : 'bg-green-500'}`} style={{ width: `${Math.min(100, (metrics.averageLatency / 200) * 100)}%` }}/>
            </div>
      ...
      <div>
        ...
        <div className="bg-slate-800 rounded-lg p-3 max-h-48 overflow-y-auto">
          {simulationLog.length > 0 ? (<div className="space-y-1">
              {simulationLog.map((logEntry, index) => (<div key={index} className="text-xs text-slate-400 font-mono">
                  {logEntry}
                </div>))}
            </div>) : (<div className="text-xs text-slate-500 italic">
