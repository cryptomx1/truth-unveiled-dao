...
export const TruthPointCalculator = () => {
    ...
    const [selectedAction, setSelectedAction] = useState('vote');
    ...
    const updateMetrics = (actions) => {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const dailyActions = actions.filter(action => action.timestamp >= today);
    ...
    return (<div className="max-w-md mx-auto bg-slate-900 border border-slate-700 rounded-lg p-6">
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          {Object.entries(tiers).map(([tier, points]) => (<div key={tier} className="bg-slate-800 rounded-lg p-3">
              <div className={`text-xs font-medium ${getTierColor(tier)} mb-1`}>
      ...
      <div className="mb-6">
        ...
        <div className="mb-4">
          ...
          <select value={selectedAction} onChange={(e) => setSelectedAction(e.target.value)} className="w-full bg-slate-800 border border-slate-600 rounded px-3 py-2 text-slate-200 text-sm">
        ...
        <div className="mb-4">
          ...
          <select value={selectedTier} onChange={(e) => setSelectedTier(e.target.value)} className="w-full bg-slate-800 border border-slate-600 rounded px-3 py-2 text-slate-200 text-sm">
        ...
        {/* Simulate Button */}
        <button onClick={handleActionSimulation} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-lg text-sm font-medium transition-colors">
      ...
      <div>
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto">
          {recentActions.map((action) => (<div key={action.id} className="bg-slate-800 rounded-lg p-3">
      ...
      {calculatorMetrics.dailyPoints >= 800 && (<div className="mt-4 p-3 bg-amber-900 border border-amber-700 rounded-lg">
          <div className="flex items-center gap-2">
