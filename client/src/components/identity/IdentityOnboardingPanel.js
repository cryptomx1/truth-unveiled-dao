...
import { User, Shield, Award, Download, RefreshCw, CheckCircle, AlertCircle, Eye, EyeOff, TrendingUp, Calendar, Star, Zap } from 'lucide-react';
import { mintCivicIdentity, getActivityProfile } from '../../identity/CivicIdentityMinter';
import { createReputationBundle, exportReputationBundle } from '../../services/ZKReputationAssembler';
// Main IdentityOnboardingPanel component
export const IdentityOnboardingPanel = ({ did = 'did:civic:commander-mark', autoLoadProfile = true, showAdvancedOptions = false }) => {
    // Component state
    ...
    const handleAssembleReputation = async () => {
        if (!identityState.identity || !profileState.profile)
            return;
    ...
    return (<div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <button onClick={handleReset} className="flex items-center gap-2 px-3 py-2 bg-slate-700 border border-slate-600 text-slate-300 rounded-lg hover:bg-slate-600 transition-colors text-sm" aria-label="Reset onboarding process">
        ...
        <div className="flex items-center gap-4 mb-4">
          {['profile', 'minting', 'reputation', 'export', 'complete'].map((step, index) => (<div key={step} className="flex items-center gap-2">
        ...
        <div className="bg-slate-700/30 rounded-lg p-3">
          ...
          <div className="flex items-center gap-3">
            <input type="text" value={currentDID} onChange={(e) => setCurrentDID(e.target.value)} className="flex-1 px-3 py-2 bg-slate-800 border border-slate-600 text-slate-200 rounded-lg text-sm" placeholder="did:civic:username" aria-label="Enter your DID"/>
            <button onClick={loadActivityProfile} disabled={profileState.isLoading} className="flex items-center gap-2 px-3 py-2 bg-indigo-900/30 border border-indigo-600 text-indigo-300 rounded-lg hover:bg-indigo-800/30 transition-colors text-sm" aria-label="Load activity profile">
              {profileState.isLoading ? (<div className="w-4 h-4 animate-spin border-2 border-indigo-400 border-t-transparent rounded-full"/>) : (<RefreshCw className="w-4 h-4"/>)}
              <span>Load</span>
            </button>
          </div>
        </div>
      ...
      {profileState.profile && (<div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
          ...
          {onboardingState.step === 'profile' && (<button onClick={handleMintIdentity} disabled={identityState.isLoading} className="flex items-center gap-2 px-4 py-2 bg-indigo-900/30 border border-indigo-600 text-indigo-300 rounded-lg hover:bg-indigo-800/30 transition-colors" aria-label="Mint civic identity">
      ...
      {/* Identity Section */}
      {identityState.identity && (<div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
          <h2 className="text-lg font-medium text-slate-200 mb-4 flex items-center gap-2">
          ...
          {onboardingState.step === 'reputation' && (<button onClick={handleAssembleReputation} disabled={reputationState.isLoading} className="flex items-center gap-2 px-4 py-2 bg-purple-900/30 border border-purple-600 text-purple-300 rounded-lg hover:bg-purple-800/30 transition-colors" aria-label="Assemble reputation proof">
      ...
      {reputationState.bundle && (<div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setReputationState(prev => ({ ...prev, showDetails: !prev.showDetails }))} className="flex items-center gap-2 text-sm text-purple-400 hover:text-purple-300 transition-colors" aria-label="Toggle reputation details" aria-expanded={reputationState.showDetails}>
          ...
          <div className="bg-purple-900/10 border border-purple-600/30 rounded-lg p-4 mb-4">
            ...
            {reputationState.showDetails && (<div className="mt-4 pt-4 border-t border-purple-600/30">
          ...
          {onboardingState.step === 'export' && (<button onClick={handleExportBundle} disabled={onboardingState.isLoading} className="flex items-center gap-2 px-4 py-2 bg-emerald-900/30 border border-emerald-600 text-emerald-300 rounded-lg hover:bg-emerald-800/30 transition-colors" aria-label="Export reputation bundle">
      ...
      {/* Completion Section */}
      {onboardingState.step === 'complete' && (<div className="bg-green-900/10 border border-green-600/30 rounded-lg p-6">
          ...
          <div className="flex items-center gap-3">
            <button onClick={handleReset} className="flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-600 text-green-300 rounded-lg hover:bg-green-800/30 transition-colors" aria-label="Start new onboarding">
              <RefreshCw className="w-4 h-4"/>
      ...
      {/* Error Display */}
      {(onboardingState.error || profileState.error || identityState.error || reputationState.error) && (<div className="bg-red-900/10 border border-red-600/30 rounded-lg p-4">
