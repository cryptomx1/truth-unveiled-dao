...
import { llmSentimentRefiner } from '@/utils/LLMSentimentRefiner';
import { nudgeSignalEmitter } from '@/utils/NudgeSignalEmitter';
const PressRepIntegration = ({ representativeId, representativeName, dissonanceLevel }) => {
    ...
    const performCrossIntegration = async () => {
        ...
        try {
            ...
            const nudgeSignal = nudgeSignalEmitter.generateNudge(representativeId, representativeName, dissonanceLevel, Math.abs(sentimentAnalysis.overallSentiment), sentimentAnalysis.keyTopics.join(', '));
    ...
    return (<Card className="border-l-4 border-l-orange-500">
      ...
      <CardContent className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <Button onClick={performCrossIntegration} disabled={isLoading} size="sm">
            {isLoading ? '🔄 Syncing...' : '🚀 Run Integration'}
          </Button>
        </div>

        {analysis && (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1">
                {snippets.slice(0, 2).map((snippet, index) => (<div key={index} className="text-xs p-2 bg-gray-50 dark:bg-gray-800 rounded">
        ...
        {nudges.length > 0 && (<div className="border-t pt-3">
            <h4 className="font-semibold text-sm mb-2">Recent Nudge Signals</h4>
            <div className="space-y-2">
              {nudges.slice(0, 2).map(nudge => (<div key={nudge.id} className="text-xs p-2 border rounded flex justify-between items-center">
                  <div>
                    <div className="font-medium">{nudge.nudgeMessage}</div>
                    <div className="text-gray-500">{nudge.timestamp.toLocaleTimeString()}</div>
                  </div>
                  <Badge className={nudge.dissonanceLevel === 'red' ? 'bg-red-500' :
