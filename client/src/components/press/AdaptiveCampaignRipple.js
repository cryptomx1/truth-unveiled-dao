...
export function AdaptiveCampaignRipple() {
    const [momentumVectors, setMomentumVectors] = useState([]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                {momentumVectors.map((vector, index) => (<div key={index} className="p-3 border rounded-lg">
            ...
            <div>
              ...
              {activeSequence.length === 0 ? (<div className="text-center py-8">
                  <Button onClick={generateAdaptiveSequence} disabled={isGenerating} className="w-full">
                    {isGenerating ? "Generating Sequence..." : "Generate Adaptive Sequence"}
                  </Button>
                </div>) : (<div className="space-y-3">
                  {activeSequence.map((step, index) => (<div key={step.id} className={`p-3 border rounded-lg ${step.status === 'active' ? 'border-blue-500 bg-blue-50' : ''}`}>
                      <div className="flex justify-between items-center mb-2">
                      ...
                      {step.messages.map((message) => (<div key={message.id} className="text-xs text-gray-600 mb-2 p-2 bg-gray-50 rounded">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        {step.actualReach > 0 && (<span className="text-green-600">
                            Actual: {step.actualReach.toLocaleString()}
                          </span>)}
                      </div>
                      
                      {step.status !== 'pending' && (<Progress value={step.actualReach > 0 ? (step.actualReach / step.targetReach) * 100 : 0} className="mt-2"/>)}
                    </div>))}
                  
                  {totalReach > 0 && (<div className="mt-4 p-3 bg-purple-50 border border-purple-200 rounded-lg">
