...
export class Deck10FeedbackSync {
    ...
    findRepresentativeByZip(zipCode) {
        if (!zipCode)
            return null;
    ...
    getRepresentativeDissonance() {
        ...
        for (const rep of this.representatives.values()) {
            ...
            let dissonanceLevel = 'low';
            if (alignmentGap > 0.5)
                dissonanceLevel = 'high';
            else if (alignmentGap > 0.3)
    ...
    triggerManualSync(deckId = 'deck_10_governance') {
