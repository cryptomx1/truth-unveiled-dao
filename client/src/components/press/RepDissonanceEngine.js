...
const RepDissonanceEngine = () => {
    const [dissonanceData, setDissonanceData] = useState([]);
    ...
    useEffect(() => {
        ...
        const initializeDissonanceTracking = async () => {
            ...
            const mockRepData = alignmentData.map(rep => {
    ...
    const getDissonanceColor = (level) => {
        switch (level) {
    ...
    const getDissonanceEmoji = (level) => {
        switch (level) {
            case 'green': return '✅';
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {dissonanceData.map(rep => (<Card key={rep.representativeId} className={`cursor-pointer transition-all duration-200 hover:shadow-lg ${selectedRep?.representativeId === rep.representativeId ? 'ring-2 ring-blue-500' : ''}`} onClick={() => setSelectedRep(rep)}>
      ...
      {/* Detailed View */}
      {selectedRep && (<Card className="border-l-4 border-l-blue-500">
          <CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-3">
                  {selectedRep.issueBreakdown.map((issue, index) => (<div key={index} className="border rounded-lg p-3">
              ...
              <div>
                ...
                <div className="space-y-2">
                  <Button onClick={() => handleTriggerNudge(selectedRep)} className="w-full">
                    📣 Send Nudge Signal
                  </Button>
                  <Button variant="outline" className="w-full" onClick={() => window.open('/press/wave', '_blank')}>
                    📊 View Press Wave Data
                  </Button>
                  <Button variant="outline" className="w-full" onClick={() => window.open('/deck/10', '_blank')}>
