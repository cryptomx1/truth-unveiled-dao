...
import { Activity, Target, Zap, MapPin, Users, TrendingUp, AlertTriangle } from 'lucide-react';
...
export const PressReplayDashboard = () => {
    ...
    const handleZIPSelection = (zipCode) => {
    ...
    const generateAIMessage = async () => {
        if (formData.targetZIPs.length === 0)
            return;
    ...
    const createCampaign = async () => {
        if (!formData.name || formData.targetZIPs.length === 0)
            return;
    ...
    const pauseResumeCampaign = (campaignId, currentStatus) => {
    ...
    return (<div className="container mx-auto px-4 py-6 max-w-7xl" role="main">
      {/* ARIA Live Region */}
      ...
      <Tabs defaultValue="campaigns" className="space-y-6">
        ...
        <TabsContent value="campaigns" className="space-y-4">
          <Card>
            ...
            <CardContent>
              {campaigns.length === 0 ? (<p className="text-gray-500 text-center py-8">No campaigns created yet</p>) : (<div className="space-y-4">
                  {campaigns.map((campaign) => (<div key={campaign.id} className="border rounded-lg p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {(campaign.status === 'active' || campaign.status === 'paused') && (<Button variant="outline" size="sm" onClick={() => pauseResumeCampaign(campaign.id, campaign.status)}>
                    ...
                    return (<Badge key={zip} variant="secondary">
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="campaign-name" value={formData.name} onChange={(e) => setFormData(prev => ({ ...prev, name: e.target.value }))} placeholder="Healthcare Access Initiative"/>
                </div>
                ...
                <div>
                  ...
                  <Select value={selectedZIP} onValueChange={setSelectedZIP}>
                    ...
                    <SelectContent>
                      {zipZones.filter(zone => !formData.targetZIPs.includes(zone.zipCode)).map(zone => (<SelectItem key={zone.zipCode} value={zone.zipCode}>
              ...
              {selectedZIP && (<Button onClick={() => handleZIPSelection(selectedZIP)} className="w-full">
                  Add {selectedZIP} to Campaign
                </Button>)}

              {formData.targetZIPs.length > 0 && (<div>
                  <Label>Selected Target ZIPs</Label>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {formData.targetZIPs.map(zip => {
                const zone = zipZones.find(z => z.zipCode === zip);
                return (<Badge key={zip} variant="secondary" className="cursor-pointer" onClick={() => removeZIP(zip)}>
                          {zip} {zone && `(${zone.city})`} ×
                        </Badge>);
            })}
                  </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Button variant="outline" onClick={generateAIMessage} disabled={isGenerating || formData.targetZIPs.length === 0} className="flex items-center gap-2">
                    <Zap className="h-4 w-4"/>
                    {isGenerating ? 'Generating...' : '✨ AI Generate'}
                  </Button>
                </div>

                {generatedMessage && (<div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded p-4">
                    <p className="text-sm font-medium mb-2">AI-Generated Message:</p>
                    <p className="text-sm">{generatedMessage}</p>
                    <div className="mt-2">
                      <input type="checkbox" id="use-ai" checked={formData.useAIGeneration} onChange={(e) => setFormData(prev => ({ ...prev, useAIGeneration: e.target.checked }))} className="mr-2"/>
                      <Label htmlFor="use-ai" className="text-sm">Use this AI-generated message</Label>
                    </div>
                  </div>)}
              </div>

              <Button onClick={createCampaign} className="w-full" disabled={!formData.name || formData.targetZIPs.length === 0}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {zipZones.map((zone) => (<div key={zone.zipCode} className="border rounded-lg p-4 space-y-3">
                    <div className="flex items-center justify-between">
                    ...
                    {zone.representativeAlignment < 70 && (<div className="flex items-center gap-2 text-amber-600">
