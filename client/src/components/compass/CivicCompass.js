...
import { MessageSquare, Compass, ArrowRight, Users, Scale, Hammer } from 'lucide-react';
export function CivicCompass({ currentTP = 87, onDirectionSelect, missionId }) {
    ...
    const handleDirectionClick = (direction) => {
        ...
        setCompassRotation(rotationMap[direction.id] || 0);
        ...
        const recommendedPillar = missionId ? pillarMapping[missionId] : null;
    ...
    const isDirectionUnlocked = (direction) => {
    ...
    return (<Card className="w-full max-w-3xl mx-auto bg-slate-800 border-slate-700 text-white">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="relative w-48 h-48 mx-auto">
          <div className="absolute inset-0 rounded-full border-4 border-slate-600 bg-slate-700/50">
            {/* Compass Needle */}
            <div className="absolute top-1/2 left-1/2 w-1 h-16 bg-red-500 origin-bottom transform -translate-x-1/2 -translate-y-full transition-transform duration-500" style={{ transform: `translate(-50%, -100%) rotate(${compassRotation}deg)` }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {directions.map((direction) => {
            ...
            return (<Card key={direction.id} className={`cursor-pointer transition-all duration-200 ${isSelected
                    ? 'border-blue-500 bg-blue-900/20'
                    : isUnlocked
                        ? 'border-slate-600 bg-slate-700/50 hover:border-slate-500'
                        : 'border-slate-700 bg-slate-800/50 opacity-50'}`} onClick={() => isUnlocked && handleDirectionClick(direction)}>
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex space-x-2">
                      ...
                      {!isUnlocked && (<Badge variant="destructive" className="text-xs">
                  ...
                  {isSelected && isUnlocked && (<Button className="w-full mt-3 bg-blue-600 hover:bg-blue-700" onClick={(e) => {
                        e.stopPropagation();
