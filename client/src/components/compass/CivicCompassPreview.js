...
import { Eye, Users, MessageSquare, Scale, Hammer, ArrowRight } from 'lucide-react';
export function CivicCompassPreview({ currentTP = 87, onDeckSelect }) {
    ...
    const handleDeckPreview = (deck) => {
        ...
        if ('speechSynthesis' in window) {
            const utterance = new SpeechSynthesisUtterance(`Previewing ${deck.title}. ${deck.modules} modules available.`);
            utterance.rate = 0.9;
            utterance.volume = 0.7;
            speechSynthesis.speak(utterance);
        }
    };
    const handleDeckAccess = (deck) => {
        if (onDeckSelect) {
    ...
    const isDeckUnlocked = (deck) => {
        return currentTP >= deck.tpRequirement;
    };
    const getTPBadgeColor = (tpRequirement) => {
        if (currentTP >= tpRequirement)
            return 'bg-green-600';
        if (currentTP >= tpRequirement * 0.7)
            return 'bg-amber-600';
        return 'bg-red-600';
    };
    return (<Card className="w-full max-w-4xl mx-auto bg-slate-800 border-slate-700 text-white">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {compassDecks.map((deck) => {
            ...
            return (<Card key={deck.id} className={`relative cursor-pointer transition-all duration-200 ${isSelected
                    ? 'border-blue-500 bg-blue-900/20'
                    : isUnlocked
                        ? 'border-slate-600 bg-slate-700/50 hover:border-slate-500'
                        : 'border-slate-700 bg-slate-800/50 opacity-75'}`} onClick={() => handleDeckPreview(deck)}>
                {/* TP Badge Overlay */}
                <div className="absolute top-2 right-2 z-10">
                  <Badge className={`text-xs ${getTPBadgeColor(deck.tpRequirement)} text-white`}>
                    {deck.tpRequirement} TP
                  </Badge>
                </div>

                {/* Unlock Status Badge */}
                {!isUnlocked && (<div className="absolute top-2 left-2 z-10">
                ...
                <CardContent className="p-4 space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {isSelected && (<Button size="sm" className={`${isUnlocked
                        ? 'bg-blue-600 hover:bg-blue-700'
                        : 'bg-slate-600 cursor-not-allowed'}`} onClick={(e) => {
                        e.stopPropagation();
                        if (isUnlocked) {
