...
export const ModuleOrbit = ({ deckId, deckTitle, modules, radius, centerPosition, isActive = false, rotationSpeed = 0, onModuleClick, onModuleHover, className = '' }) => {
    ...
    const renderModuleNode = (module) => {
        ...
        return (<div key={module.id} className={`absolute rounded-full border transition-all duration-300 cursor-pointer ${statusColors}`} style={{
                left: `${position.x}px`,
                top: `${position.y}px`,
                width: `${nodeSize}px`,
                height: `${nodeSize}px`,
            ...
            }} onClick={() => handleModuleClick(module.id)} onMouseEnter={() => handleModuleHover(module.id)} onMouseLeave={() => handleModuleHover(null)} role="listitem" aria-label={`Module: ${module.name} — ${module.status} — ${module.completionRate}% complete — ${module.engagementScore}% engagement`} tabIndex={0}>
        ...
        {/* Completion ring */}
        {module.completionRate > 0 && (<div className="absolute inset-0 rounded-full border-2 border-blue-400" style={{
    ...
    return (<div className={`relative ${className}`} role="list" aria-label={`Module orbit for ${deckTitle} — ${modules.length} modules`}>
      {/* Orbit ring */}
      <div className={`absolute rounded-full border border-dashed transition-all duration-500 ${isActive ? 'border-blue-400/50' : 'border-slate-600/30'}`} style={{
      ...
      <div className={`absolute w-2 h-2 rounded-full transition-all duration-300 ${isActive ? 'bg-blue-400' : 'bg-slate-600'}`} style={{
            left: `${centerPosition.x}px`,
            top: `${centerPosition.y}px`,
            transform: 'translate(-50%, -50%)'
        }}/>
      ...
      {hoveredModule && ((() => {
            ...
            if (!module)
                return null;
            const position = getModulePosition(module);
            return (<div className="absolute bg-slate-800/90 text-white text-xs p-2 rounded-lg border border-slate-600 pointer-events-none z-20" style={{
                    left: `${position.x}px`,
      ...
      {/* Orbit statistics */}
      {isActive && (<div className="absolute bg-slate-800/80 text-white text-xs p-2 rounded-lg border border-slate-600 pointer-events-none" style={{
                left: `${centerPosition.x}px`,
      ...
      {/* Development info */}
      {process.env.NODE_ENV === 'development' && renderTime > 0 && (<div className="absolute text-xs text-slate-500 pointer-events-none" style={{
                left: `${centerPosition.x}px`,
