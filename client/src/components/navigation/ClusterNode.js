...
import { Star, Lock, Eye, Shield, MessageCircle, BookOpen, Scale, TrendingUp } from 'lucide-react';
// Mock ZKP tier hook (citizen default as per directive)
const useZKPTier = () => {
    return 'citizen';
};
// Mock engagement data generator
const generateEngagementData = (clusterId) => {
    ...
    return baseEngagement[clusterId] || 25000;
};
// Cluster theme icon mapping
const getClusterIcon = (theme) => {
    const iconMap = {
...
export const ClusterNode = ({ id, title, theme, decks, position, radius, glowIntensity, isSelected = false, userTier, onClusterClick, onDeckHover, className = '' }) => {
    ...
    const accessibleDecks = decks.filter(deck => {
        return deck.accessible && (deck.tier === 'citizen' ||
    ...
    const getClusterDescription = () => {
        ...
        return descriptions[id] || 'Civic engagement cluster';
    };
    // Render individual deck node with enhanced tier access
    const renderDeckNode = (deck) => {
        ...
        return (<div key={deck.id} className={`absolute rounded-full transition-all duration-300 cursor-pointer ${isAccessible ? 'bg-blue-400 hover:bg-blue-300' : 'bg-red-400/60 hover:bg-red-400/80'}`} style={{
                ...
                setHovered(false);
                onDeckHover?.(null);
            }} role="listitem" aria-label={`Deck: ${deck.title} — ${isAccessible ? `Accessible to ${deck.tier} tier` : `Locked for ${deck.tier} tier`} — ${deck.moduleCount} modules`} tabIndex={0} title={!isAccessible ? "Upgrade tier to access this deck" : `${deck.title} - ${deck.moduleCount} modules`}>
        {/* Tier lock indicator */}
        {!isAccessible && (<div className="absolute -top-1 -right-1">
            <Lock className="w-3 h-3 text-slate-400"/>
          </div>)}
        
        {/* Module count indicator */}
        {isAccessible && deck.moduleCount > 0 && (<div className="absolute -bottom-1 -right-1 bg-slate-800 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">
            {deck.moduleCount}
          </div>)}
      </div>);
    };
    ...
    return (<div className={`absolute group ${className}`} style={{
            left: `${position.x}px`,
            top: `${position.y}px`,
            transform: `translate(-50%, -50%) scale(${clusterScale})`,
            transition: 'transform 0.3s ease-out'
        }} role="listitem" aria-label={`Cluster: ${title} - ${theme} - ${accessibleDecks.length}/${decks.length} decks accessible`}>
      {/* Cluster background glow with expansion states */}
      <div className={`absolute rounded-full border-2 transition-all duration-500 cursor-pointer ${expanded
            ? 'border-green-400 bg-green-500/20 ring-4 ring-green-400/30'
            : isSelected
                ? 'border-blue-400 bg-blue-500/20'
                : hovered
                    ? 'border-blue-500 bg-blue-500/10'
                    : 'border-slate-600 bg-slate-700/10 hover:border-slate-500'}`} style={{
            width: `${radius * 2}px`,
            height: `${radius * 2}px`,
            boxShadow: `0 0 ${radius / 2}px rgba(59, 130, 246, ${clusterOpacity * borderIntensity})`,
            transform: 'translate(-50%, -50%)'
        }} onClick={handleClusterClick} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} onFocus={handleFocus} onBlur={handleBlur} onKeyDown={handleKeyDown} aria-expanded={expanded} aria-describedby={showTooltip ? `${id}-tooltip` : undefined} tabIndex={0} role="button"/>
      
      {/* Cluster title */}
      <div className="absolute text-white font-semibold text-sm pointer-events-none select-none" style={{
            transform: 'translate(-50%, -50%)',
            top: `-${radius + 20}px`,
            left: '0'
        }}>
      ...
      <div className={`absolute transition-all duration-300 ${expanded ? 'text-green-400 scale-125' : 'text-blue-400'}`} style={{
            transform: 'translate(-50%, -50%)'
        }}>
      ...
      {expanded && (<div className="absolute -top-2 -right-2 bg-green-500 text-white text-xs rounded-full w-6 h-6 flex items-center justify-center animate-pulse" style={{
                transform: 'translate(-50%, -50%)'
            }}>
      ...
      <div className="absolute text-xs text-slate-400 pointer-events-none select-none" style={{
            transform: 'translate(-50%, -50%)',
            top: `${radius + 15}px`,
            left: '0'
        }}>
      ...
      {showTooltip && (<div id={`${id}-tooltip`} className="absolute bg-slate-800/95 text-white text-sm p-3 rounded-lg border border-slate-600 pointer-events-none z-20 shadow-xl" style={{
                transform: 'translate(-50%, -50%)',
                top: `${radius + 55}px`,
                left: '0',
                minWidth: '200px',
          ...
          <div className="text-center">
            ...
            {expanded && (<div className="mt-2 pt-2 border-t border-slate-600 text-xs">
...
{ /* Development info */ }
{
    process.env.NODE_ENV === 'development' && renderTime > 0 && (<div className="absolute text-xs text-slate-500 pointer-events-none select-none" style={{
            transform: 'translate(-50%, -50%)',
            top: `${radius + 60}px`,
            left: '0'
        }}>
