...
import { ZoomIn, ZoomOut, Lock, Star } from 'lucide-react';
// Mock cluster data
const MOCK_CLUSTERS = [
    {
        id: 'identity',
...
export const CivicConstellationExplorer = ({ userTier = 'citizen', initialZoom = 1, onClusterFocus, onDeckHover, className = '' }) => {
    ...
    // Render deck node
    const renderDeckNode = (deck, clusterPosition) => {
        const nodeX = clusterPosition.x + deck.position.x;
        const nodeY = clusterPosition.y + deck.position.y;
        const isAccessible = deck.accessible && (deck.tier === 'citizen' ||
        ...
        return (<div key={deck.id} className={`absolute rounded-full transition-all duration-300 cursor-pointer ${isAccessible ? 'bg-blue-400 hover:bg-blue-300' : 'bg-slate-600'}`} style={{
                left: `${nodeX}px`,
                top: `${nodeY}px`,
                width: `${nodeSize}px`,
                height: `${nodeSize}px`,
            ...
            }} onMouseEnter={() => onDeckHover?.(deck.id)} onMouseLeave={() => onDeckHover?.(null)} role="listitem" aria-label={`Deck: ${deck.title} — ${isAccessible ? `Accessible to ${deck.tier} tier` : `Locked for ${deck.tier} tier`}`} tabIndex={0}>
        {/* Tier lock indicator */}
    ...
    const renderCluster = (cluster) => {
        ...
        return (<div key={cluster.id} className="absolute group" style={{
                left: `${cluster.position.x}px`,
                top: `${cluster.position.y}px`,
                transform: `translate(-50%, -50%) scale(${clusterScale})`,
                transition: 'transform 0.3s ease-out'
            }} role="listitem" aria-label={`Cluster: ${cluster.title} - ${cluster.theme}`}>
        {/* Cluster background glow */}
        <div className={`absolute rounded-full border-2 transition-all duration-500 cursor-pointer ${isSelected ? 'border-blue-400 bg-blue-500/20' : 'border-slate-600 bg-slate-700/10 hover:border-slate-500'}`} style={{
        ...
        {/* Cluster center star */}
        <div className="absolute text-blue-400" style={{
                transform: 'translate(-50%, -50%)'
            }}>
    ...
    return (<div ref={canvasRef} className={`relative overflow-hidden bg-slate-900 rounded-lg border border-slate-700 ${className}`} style={{ maxWidth: '460px', height: '400px' }} role="application" aria-label="Civic Constellation Explorer - Navigate deck clusters" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      {/* Zoom Controls */}
      <div className="absolute top-4 right-4 z-10 flex flex-col gap-2">
        <button onClick={() => handleZoom('in')} className="p-2 bg-slate-800/80 hover:bg-slate-700/80 rounded-lg border border-slate-600 text-white transition-colors" aria-label="Zoom in">
          <ZoomIn className="w-4 h-4"/>
        </button>
        <button onClick={() => handleZoom('out')} className="p-2 bg-slate-800/80 hover:bg-slate-700/80 rounded-lg border border-slate-600 text-white transition-colors" aria-label="Zoom out">
      ...
      <div ref={constellationRef} className="w-full h-full relative" style={{
            transform: `scale(${constellationState.zoomLevel})`,
            transformOrigin: 'center center',
            transition: 'transform 0.3s ease-out'
        }} role="list" aria-label="Deck clusters">
        ...
        <svg className="absolute inset-0 pointer-events-none opacity-20">
          <line x1={MOCK_CLUSTERS[0].position.x} y1={MOCK_CLUSTERS[0].position.y} x2={MOCK_CLUSTERS[2].position.x} y2={MOCK_CLUSTERS[2].position.y} stroke="#64748b" strokeWidth="1" strokeDasharray="5,5"/>
          <line x1={MOCK_CLUSTERS[1].position.x} y1={MOCK_CLUSTERS[1].position.y} x2={MOCK_CLUSTERS[3].position.x} y2={MOCK_CLUSTERS[3].position.y} stroke="#64748b" strokeWidth="1" strokeDasharray="5,5"/>
        </svg>
      </div>

      {/* Cluster Info Panel */}
      {constellationState.selectedCluster && (<div className="absolute bottom-4 left-4 right-4 z-10">
      ...
      {/* Development Info */}
      {process.env.NODE_ENV === 'development' && (<div className="absolute bottom-2 right-2 text-xs text-slate-500 bg-slate-900/80 px-2 py-1 rounded">
