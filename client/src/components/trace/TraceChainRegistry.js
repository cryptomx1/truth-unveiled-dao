...
class TraceChainRegistryClass {
    ...
    generateTraceFingerprint(action) {
    ...
    getTracesByDeck(deckName) {
        return this.traceChain.filter(event => event.sourceDeck === deckName);
    }
    // Get trace events by timeframe
    getTracesByTimeframe(startTime, endTime) {
        return this.traceChain.filter(event => event.timestamp >= startTime && event.timestamp <= endTime);
    }
