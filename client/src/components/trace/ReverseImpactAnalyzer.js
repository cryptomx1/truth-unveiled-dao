...
class ReverseImpactAnalyzerClass {
    ...
    detectInfluence(priorEvent, targetEvent) {
    ...
    generateChainNarrative(targetEvent, influences, rootCause) {
        ...
        if (influences.length > 1) {
            ...
            const secondaryDescriptions = secondaryInfluences.map(inf => `${inf.influencerEvent.action} "${inf.influencerEvent.value}"`);
    ...
    getInfluenceSummary(events) {
        let totalInfluences = 0;
        let strongInfluences = 0;
        const influencerCounts = new Map();
    ...
    generateAccessibleNarrative(causalChain) {
...
export const useReverseImpactAnalyzer = () => {
    ...
    const getAccessibleNarrative = (causalChain) => {
