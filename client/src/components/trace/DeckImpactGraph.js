...
export const DeckImpactGraph = ({ className = '', selectedEvent, onEventSelect }) => {
    const { getChain, getStats } = useTraceChain();
    const [impactRadius, setImpactRadius] = useState(1);
    ...
    const filteredConnections = useMemo(() => {
        ...
        return connections.filter(conn => relevantDecks.has(conn.from) && relevantDecks.has(conn.to));
    ...
    const getNodeColor = (node) => {
        ...
        if (hoursSinceActivity < 24)
            return '#f59e0b'; // Amber - recent
        return '#6b7280'; // Gray - older
    };
    return (<div className={`w-full h-80 bg-slate-700 rounded-lg border border-slate-600 p-4 ${className}`}>
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={() => setImpactRadius(1)} className={`px-2 py-1 text-xs rounded ${impactRadius === 1
            ? 'bg-blue-600 text-white'
            : 'bg-slate-600 text-slate-300 hover:bg-slate-500'}`}>
              1
            </button>
            <button onClick={() => setImpactRadius(2)} className={`px-2 py-1 text-xs rounded ${impactRadius === 2
            ? 'bg-blue-600 text-white'
            : 'bg-slate-600 text-slate-300 hover:bg-slate-500'}`}>
              2
            </button>
            <button onClick={() => setImpactRadius('full')} className={`px-2 py-1 text-xs rounded ${impactRadius === 'full'
            ? 'bg-blue-600 text-white'
            : 'bg-slate-600 text-slate-300 hover:bg-slate-500'}`}>
              All
            </button>
          ...
          {/* Timeline Toggle */}
          <button onClick={() => setShowTimeline(!showTimeline)} className="p-1 text-slate-400 hover:text-slate-200" title={showTimeline ? 'Hide Timeline' : 'Show Timeline'}>
      ...
      <div className="relative w-full h-64 bg-slate-800 rounded border border-slate-600 overflow-hidden">
        <svg width="100%" height="100%" className="absolute inset-0">
          ...
          {filteredConnections.map((conn, index) => {
            ...
            if (!fromNode || !toNode)
                return null;
            return (<g key={`${conn.from}-${conn.to}-${index}`}>
                <line x1={fromNode.x} y1={fromNode.y} x2={toNode.x} y2={toNode.y} stroke="#475569" strokeWidth={Math.min(conn.strength, 4)} strokeOpacity={0.6}/>
                
                {/* Arrow head */}
                <polygon points={`${toNode.x - 5},${toNode.y - 3} ${toNode.x},${toNode.y} ${toNode.x - 5},${toNode.y + 3}`} fill="#475569" opacity={0.6}/>
              </g>);
        })}

          {/* Deck Nodes */}
          {deckNodes.map((node) => (<g key={node.id}>
              <circle cx={node.x} cy={node.y} r={Math.max(8, Math.min(20, node.eventCount * 2))} fill={getNodeColor(node)} stroke="#1e293b" strokeWidth={2} className="cursor-pointer transition-all duration-200" onMouseEnter={() => setHoveredNode(node.id)} onMouseLeave={() => setHoveredNode(null)} onClick={() => {
              ...
              {/* Node Label */}
              <text x={node.x} y={node.y + 35} textAnchor="middle" className="text-xs fill-slate-300 pointer-events-none" fontSize="10">
                {node.name.length > 12 ? `${node.name.substring(0, 12)}...` : node.name}
              </text>
              
              {/* Event Count */}
              <text x={node.x} y={node.y + 3} textAnchor="middle" className="text-xs fill-white font-bold pointer-events-none" fontSize="10">
                {node.eventCount}
              </text>
            </g>))}
        </svg>
        ...
        {/* Hovered Node Info */}
        {hoveredNode && (<div className="absolute top-2 left-2 bg-slate-900 p-2 rounded text-xs text-slate-200 border border-slate-600">
      ...
      {/* Timeline Bar */}
      {showTimeline && traceChain.length > 0 && (<div className="mt-2 h-8 bg-slate-800 rounded border border-slate-600 relative overflow-hidden">
          <div className="flex items-center h-full px-2">
            <Clock className="w-3 h-3 text-slate-400 mr-2"/>
            <div className="flex-1 relative">
              {traceChain.slice(-20).map((event, index) => {
                const position = (index / 19) * 100;
                return (<div key={event.actionId} className="absolute w-1 h-4 bg-blue-400 opacity-60 hover:opacity-100 cursor-pointer" style={{ left: `${position}%` }} title={`${event.action}: ${event.value}`} onClick={() => onEventSelect?.(event)}/>);
            })}
            </div>
          </div>
        </div>)}
      ...
      {/* Empty State */}
      {traceChain.length === 0 && (<div className="flex items-center justify-center h-64 text-slate-400">
