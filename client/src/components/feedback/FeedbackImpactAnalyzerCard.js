...
export const FeedbackImpactAnalyzerCard = ({ userId = 'current_user' }) => {
    ...
    return (<div className="max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-4">
      ...
      <div className="space-y-3 mb-4 max-h-64 overflow-y-auto">
        {impactEvents.length === 0 ? (<div className="text-center p-4 text-slate-400">
          ...
          </div>) : (impactEvents.map((event) => (<div key={event.id} className={`p-3 rounded-lg border cursor-pointer transition-all duration-200 hover:border-opacity-50 ${getEventColor(event.type)} ${selectedEvent?.id === event.id ? 'ring-2 ring-blue-500/50' : ''}`} onClick={() => analyzeImpact(event)}>
      ...
      {selectedEvent && (<div className="p-3 bg-blue-600/10 border border-blue-500/30 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-sm font-medium text-blue-300">Impact Analysis</span>
            {isAnalyzing && (<div className="animate-spin w-3 h-3 border border-blue-400 border-t-transparent rounded-full"></div>)}
          </div>
          
          {!isAnalyzing && (<div className="text-sm text-slate-300">
              {selectedEvent.benefitDescription}
            </div>)}
          
          {isAnalyzing && (<div className="text-sm text-slate-400">
              Analyzing causal relationship between feedback and outcome...
            </div>)}
        </div>)}

      {/* ARIA Live Region */}
      <div aria-live="polite" aria-label="Impact analysis announcements" className="sr-only">
