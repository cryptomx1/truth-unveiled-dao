...
import { Download, Copy, Eye, Shield, CheckCircle, AlertCircle, FileText, Hash, Clock, User, MapPin, X, Key, Zap, ShieldCheck, Upload, Send, ExternalLink } from 'lucide-react';
import { exportFeedbackProof, copyProofHash, FeedbackProofSigner } from '../../services/FeedbackProofSigner';
import { bindDIDToProof } from '../../services/DIDSessionBinder';
import { assembleZKPProof, exportZKPProof } from '../../services/ZKPProofAssembler';
import { verifyZKPProof } from '../../agents/ZKPVerifierAgent';
import { exportToChainFormat, downloadChainPackage } from '../../services/ZKPChainExporter';
import { submitProofToDAO } from '../../services/ZKPProofRegistrar';
// Main FeedbackExportPanel component
export const FeedbackExportPanel = ({ proof, onClose, onExportComplete, className = "" }) => {
    ...
    const [signedBundle, setSignedBundle] = useState(null);
    ...
    const [chainExportStatus, setChainExportStatus] = useState('idle');
    const [verificationResult, setVerificationResult] = useState(null);
    const [chainExportResult, setChainExportResult] = useState(null);
    const [showVerificationDetails, setShowVerificationDetails] = useState(false);
    // Phase XXII: DAO Submission state
    const [daoSubmissionStatus, setDAOSubmissionStatus] = useState('idle');
    ...
    const handleZKPAssembly = async () => {
        if (!signedBundle || zkpStatus === 'assembling')
            return;
    ...
    const handleZKPVerification = async () => {
        if (!zkpResult || verificationStatus === 'verifying')
            return;
    ...
    const handleChainExport = async () => {
        if (!zkpResult || !verificationResult || chainExportStatus === 'exporting')
            return;
    ...
    const handleDAOSubmission = async () => {
        if (!chainExportResult || daoSubmissionStatus === 'submitting')
            return;
    ...
    return (<div className={`bg-slate-800 border border-slate-700 rounded-lg p-6 space-y-6 ${className}`} role="region" aria-label={`Feedback proof export panel for ${proof.metadata.billId}`}>
      {/* Header */}
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          {isVerified ? (<div className="flex items-center gap-1 text-green-400">
              <CheckCircle className="w-4 h-4"/>
              <span className="text-xs">Verified</span>
            </div>) : (<div className="flex items-center gap-1 text-red-400">
              <AlertCircle className="w-4 h-4"/>
              <span className="text-xs">Unverified</span>
            </div>)}
          {onClose && (<button onClick={onClose} className="text-slate-400 hover:text-white transition-colors" aria-label="Close export panel">
              <X className="w-5 h-5"/>
            </button>)}
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-3">
          <button onClick={handleDownload} disabled={exportStatus === 'downloading'} className={getDownloadButtonStyle()} aria-label={`Download feedback proof for ${proof.metadata.billId}`}>
          ...
          <button onClick={handleCopyHash} disabled={copyStatus === 'copying'} className={getCopyButtonStyle()} aria-label="Copy proof hash to clipboard">
            {copyStatus === 'copying' ? (<>
                <div className="w-4 h-4 animate-spin border-2 border-blue-400 border-t-transparent rounded-full"/>
                <span>Copying...</span>
              </>) : copyStatus === 'copied' ? (<>
          ...
          <button onClick={() => setShowMetadata(!showMetadata)} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-700 border border-slate-600 text-slate-300 hover:bg-slate-600 hover:border-slate-500 transition-all duration-200 text-sm font-medium" aria-label="Toggle metadata panel" aria-expanded={showMetadata}>
        ...
        <div className="space-y-3 pt-4 border-t border-slate-600">
          ...
          <div className="flex gap-3">
            <button onClick={handleDIDBinding} disabled={didStatus === 'binding'} className={getDIDButtonStyle()} aria-label="Sign proof with decentralized identity">
              {didStatus === 'binding' ? (<>
                  <div className="w-4 h-4 animate-spin border-2 border-blue-400 border-t-transparent rounded-full"/>
            ...
            {signedBundle && (<button onClick={handleZKPAssembly} disabled={zkpStatus === 'assembling'} className={getZKPButtonStyle()} aria-label="Assemble zero-knowledge proof">
            ...
            {zkpResult && (<button onClick={handleZKPExport} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-purple-900/30 border border-purple-600 text-purple-300 hover:bg-purple-800/30 hover:border-purple-500 transition-all duration-200 text-sm font-medium" aria-label="Export ZKP proof bundle">
                <Download className="w-4 h-4"/>
          ...
          {/* ZKP Preview Panel */}
          {zkpResult && (<div className="mt-4">
              <button onClick={() => setShowZKPPreview(!showZKPPreview)} className="flex items-center gap-2 text-sm text-purple-400 hover:text-purple-300 transition-colors" aria-label="Toggle ZKP preview" aria-expanded={showZKPPreview}>
                <Eye className="w-3 h-3"/>
                <span>{showZKPPreview ? 'Hide' : 'View'} ZKP Preview</span>
              </button>

              {showZKPPreview && (<div className="mt-3 bg-purple-900/10 border border-purple-600/30 rounded-lg p-3 space-y-2">
          ...
          {zkpResult && (<div className="space-y-3 pt-4 border-t border-slate-600">
              <h4 className="text-sm font-medium text-slate-300">Verification & Chain Export</h4>
              
              <div className="flex gap-3">
                <button onClick={handleZKPVerification} disabled={verificationStatus === 'verifying'} className={getVerificationButtonStyle()} aria-label="Verify ZKP proof for blockchain compatibility">
                ...
                {verificationResult && verificationResult.isValid && (<button onClick={handleChainExport} disabled={chainExportStatus === 'exporting'} className={getChainExportButtonStyle()} aria-label="Export proof to blockchain format">
                ...
                {chainExportResult && (<>
                    <button onClick={handleChainDownload} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-900/30 border border-emerald-600 text-emerald-300 hover:bg-emerald-800/30 hover:border-emerald-500 transition-all duration-200 text-sm font-medium" aria-label="Download blockchain package">
                      <Download className="w-4 h-4"/>
                      <span>Download Chain</span>
                    </button>

                    <button onClick={handleDAOSubmission} disabled={daoSubmissionStatus === 'submitting'} className={getDAOSubmissionButtonStyle()} aria-label="Submit proof to DAO for voting">
              ...
              {submissionProgress && daoSubmissionStatus === 'submitting' && (<div className="mt-4 bg-indigo-900/10 border border-indigo-600/30 rounded-lg p-3">
                  ...
                  <div className="w-full bg-indigo-900/30 rounded-full h-2 mb-2">
                    <div className="bg-indigo-500 h-2 rounded-full transition-all duration-300" style={{ width: `${submissionProgress.progress}%` }}/>
                  </div>
              ...
              {submissionResult && daoSubmissionStatus === 'submitted' && (<div className="mt-4 bg-green-900/10 border border-green-600/30 rounded-lg p-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-green-300">Vote Successfully Submitted</span>
                    <button onClick={handleViewResults} className="flex items-center gap-1 px-2 py-1 bg-green-900/30 border border-green-600 text-green-300 rounded text-xs hover:bg-green-800/30 transition-colors" aria-label="View vote results">
              ...
              {verificationResult && (<div className="mt-4">
                  <button onClick={() => setShowVerificationDetails(!showVerificationDetails)} className="flex items-center gap-2 text-sm text-emerald-400 hover:text-emerald-300 transition-colors" aria-label="Toggle verification details" aria-expanded={showVerificationDetails}>
                    <Eye className="w-3 h-3"/>
                    <span>{showVerificationDetails ? 'Hide' : 'View'} Verification Details</span>
                  </button>

                  {showVerificationDetails && (<div className="mt-3 bg-emerald-900/10 border border-emerald-600/30 rounded-lg p-3 space-y-2">
                      ...
                      {verificationResult.errors.length > 0 && (<div className="text-xs">
      ...
      {/* Metadata Panel */}
      {showMetadata && (<div className="bg-slate-700/20 border border-slate-600 rounded-lg p-4 space-y-4">
