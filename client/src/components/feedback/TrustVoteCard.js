...
export const TrustVoteCard = ({ memoryAction, onVoteCast }) => {
    ...
    return (<div className="max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-4">
      ...
      {memoryAction && (<div className="mb-4 p-3 bg-slate-700/30 rounded-lg">
      ...
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          {/* Trust Vote */}
          <button onClick={() => handleVote('trust')} disabled={isProcessing || zkpCommitted} className={`flex flex-col items-center gap-2 p-3 rounded-lg border transition-all duration-200 ${selectedVote === 'trust'
            ? 'bg-green-600/30 border-green-500/50 text-green-300'
            : 'bg-slate-700/20 hover:bg-slate-700/40 border-slate-600 text-slate-300 hover:text-white'} ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''}`} aria-label="Cast trust vote">
          ...
          {/* Concern Vote */}
          <button onClick={() => handleVote('concern')} disabled={isProcessing || zkpCommitted} className={`flex flex-col items-center gap-2 p-3 rounded-lg border transition-all duration-200 ${selectedVote === 'concern'
            ? 'bg-amber-600/30 border-amber-500/50 text-amber-300'
            : 'bg-slate-700/20 hover:bg-slate-700/40 border-slate-600 text-slate-300 hover:text-white'} ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''}`} aria-label="Cast concern vote">
        ...
        {/* ZKP Status */}
        {zkpCommitted && (<div className="flex items-center gap-2 p-2 bg-purple-600/20 border border-purple-500/30 rounded-lg">
        ...
        {/* Reset Button */}
        {zkpCommitted && (<button onClick={resetVote} className="w-full py-2 px-3 bg-slate-600/20 hover:bg-slate-600/30 border border-slate-500/30 text-slate-300 text-sm rounded transition-colors">
            Cast New Vote
          </button>)}
      </div>
