...
export const DeckModulePreview = ({ modules, showTokenRewards = true, onModuleSelect, className = '' }) => {
      ...
      <Card className="bg-slate-800 border-slate-700">
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2">
            {displayModules.map((module) => (<Card key={module.id} className={`relative overflow-hidden border transition-all duration-300 cursor-pointer hover:scale-[1.02] ${getStatusColor(module.status)} ${module.status === 'locked' ? 'cursor-not-allowed opacity-60' : ''}`} onClick={() => handleModuleClick(module)}>
                {/* Status Icon */}
                <div className="absolute top-3 right-3">
                  {module.status === 'completed' && (<CheckCircle className="w-5 h-5 text-green-400"/>)}
                  {module.status === 'available' && (<Play className="w-5 h-5 text-blue-400"/>)}
                  {module.status === 'locked' && (<Lock className="w-5 h-5 text-slate-400"/>)}
                </div>
                ...
                <CardContent className="p-4 space-y-3">
                  ...
                  {showTokenRewards && module.rewardTokens && (<div className="flex items-center gap-3 pt-2 border-t border-slate-600">
                      <div className="flex-shrink-0">
                        <RotatingTokenDisplay frontCID={module.rewardTokens.frontCID} backCID={module.rewardTokens.backCID} guardian={module.rewardTokens.guardian} size={60}/>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {module.completedAt && (<p className="text-green-400 text-xs mt-1">
                  ...
                  {/* Action Button */}
                  <div className="pt-2">
                    <Button variant="outline" size="sm" className={`w-full ${module.status === 'completed'
                ? 'border-green-500 text-green-400 hover:bg-green-500/10'
                : module.status === 'available'
                    ? 'border-blue-500 text-blue-400 hover:bg-blue-500/10'
                    : 'border-slate-500 text-slate-400 cursor-not-allowed'}`} disabled={module.status === 'locked'}>
          ...
          {selectedModule && (<Card className="mt-6 bg-slate-700/50 border-slate-600">
              <CardHeader>
                <CardTitle className="text-white flex items-center gap-3">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <div className="text-sm text-slate-400">
                      ...
                      {selectedModule.completedAt && (<p>Completed: {new Date(selectedModule.completedAt).toLocaleString()}</p>)}
                    </div>
                  </div>

                  {/* Enhanced Token Reward Display */}
                  {selectedModule.rewardTokens && (<div className="flex flex-col items-center space-y-3">
                      <RotatingTokenDisplay frontCID={selectedModule.rewardTokens.frontCID} backCID={selectedModule.rewardTokens.backCID} guardian={selectedModule.rewardTokens.guardian} size={120}/>
