...
export default function StreamRewardCard() {
    ...
    const [wallet, setWallet] = useState({ connected: false, provider: null });
    const [newStream, setNewStream] = useState({
        rewardType: 'municipal_participation',
     ...
     */
    const calculateTierMultiplier = useCallback((userTier = 'Citizen') => {
        ...
        return tierMultipliers[userTier] || 1.0;
    }, []);
    /**
    ...
    const cancelStream = useCallback(async (streamId) => {
        ...
        setStreams(prev => prev.map(stream => stream.streamId === streamId
     ...
     */
    const formatRewardType = useCallback((type) => {
        const typeMap = {
    ...
    return (<Card className="w-full max-w-4xl mx-auto">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center justify-between p-4 border rounded-lg">
          ...
          {!wallet.connected && (<Button onClick={connectWallet} variant="outline">
              Connect Wallet
            </Button>)}
        </div>
        ...
        {wallet.connected && (<>
            ...
            <div className="space-y-4 p-4 border rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={newStream.rewardType} onValueChange={(value) => setNewStream(prev => ({ ...prev, rewardType: value }))}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" value={newStream.truthPointAmount} onChange={(e) => setNewStream(prev => ({ ...prev, truthPointAmount: parseInt(e.target.value) || 0 }))} min="50" max="5000" step="25"/>
                </div>

                <div className="space-y-2">
                  <label className="text-sm font-medium">Stream Duration (min)</label>
                  <Input type="number" value={newStream.streamDuration} onChange={(e) => setNewStream(prev => ({ ...prev, streamDuration: parseInt(e.target.value) || 30 }))} min="15" max="1440" step="15"/>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              {streams.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
                  ...
                  {streams.map((stream) => {
                    ...
                    return (<div key={stream.streamId} className={`p-4 border rounded-lg ${statusBg}`}>
                        <div className="flex items-start justify-between">
                          <div className="space-y-2">
                            ...
                            {stream.tierMultiplier > 1 && (<div className="text-xs text-blue-600 dark:text-blue-400">
                                Tier bonus: {stream.tierMultiplier}x multiplier applied
                              </div>)}
                          </div>

                          {stream.status === 'active' && (<Button variant="outline" size="sm" onClick={() => cancelStream(stream.streamId)}>
                              Cancel
                            </Button>)}
                        </div>
                      </div>);
