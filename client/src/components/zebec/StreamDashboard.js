...
import { Activity, DollarSign, Clock, Zap, CheckCircle, XCircle, Pause, Play, Search, Download, TrendingUp } from 'lucide-react';
export default function StreamDashboard() {
    ...
    const filteredTransactions = useMemo(() => {
        ...
        // Search filter
        if (searchFilter) {
            filtered = filtered.filter(tx => tx.id.toLowerCase().includes(searchFilter.toLowerCase()) ||
        ...
        const filterDuration = timeFilterMs[timeFilter] || Infinity;
    ...
    const toggleStreamStatus = (streamId) => {
        setActiveStreams(prev => prev.map(stream => stream.id === streamId
            ? {
                ...stream,
      ...
      <Tabs defaultValue="streams" className="w-full">
        ...
        <TabsContent value="streams" className="space-y-4">
          <Card>
            ...
            <CardContent>
              {activeStreams.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
                  No active streams
                </div>) : (<div className="space-y-4">
                  {activeStreams.map((stream) => (<div key={stream.id} className="border rounded-lg p-4 space-y-3">
                      ...
                      {stream.status === 'active' && (<div className="flex justify-end">
                          <Button variant="outline" size="sm" onClick={() => toggleStreamStatus(stream.id)}>
                            <Pause className="h-4 w-4 mr-1"/>
                            Pause
                          </Button>
                        </div>)}

                      {stream.status === 'paused' && (<div className="flex justify-end">
                          <Button variant="outline" size="sm" onClick={() => toggleStreamStatus(stream.id)}>
            ...
            <CardContent className="p-4">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="relative">
                  ...
                  <Input placeholder="Search transactions..." value={searchFilter} onChange={(e) => setSearchFilter(e.target.value)} className="pl-10"/>
                </div>
          ...
          <Card>
            ...
            <CardContent>
              {filteredTransactions.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
                  ...
                  {filteredTransactions.map((tx) => (<div key={tx.id} className="border rounded-lg p-4">
                      <div className="flex items-start justify-between">
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                          ...
                          <div className="text-sm text-muted-foreground space-y-1">
                            ...
                            {tx.metadata.tierMultiplier > 1 && (<div className="text-blue-600 dark:text-blue-400">
                        ...
                        <div className="text-right space-y-1">
                          ...
                          {tx.metadata.gasFeePaid && (<div className="text-xs text-muted-foreground">
