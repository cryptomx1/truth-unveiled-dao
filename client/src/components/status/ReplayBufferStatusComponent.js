...
export const ReplayBufferStatusComponent = ({ className = '' }) => {
    const { isOnline, queueLength, isProcessing, getQueueStatus, flushQueue, clearQueue } = useOfflineReplayBuffer();
    ...
    const statusIndicator = (<div className={`
        ...
        ${className}
      `} onClick={() => setShowDetails(!showDetails)} role="button" tabIndex={0} aria-label={`Replay buffer status: ${getStatusMessage()}`} aria-live="polite" aria-expanded={showDetails}>
    ...
    const detailsOverlay = showDetails && (<div className="fixed bottom-16 left-4 z-50 bg-slate-800 border border-slate-700 rounded-lg p-4 text-white text-xs min-w-72 max-w-sm">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-semibold text-sm">Replay Buffer Status</h3>
        <button onClick={() => setShowDetails(false)} className="text-slate-400 hover:text-white" aria-label="Close details">
          <X className="w-4 h-4"/>
        </button>
      </div>
      ...
      <div className="space-y-2">
        ...
        {/* Processing Status */}
        {isProcessing && (<div className="border-t border-slate-700 pt-2">
            <div className="flex items-center space-x-2">
        ...
        {/* Action Buttons */}
        {queueLength > 0 && (<div className="border-t border-slate-700 pt-2 flex space-x-2">
            {isOnline && (<button onClick={handleFlush} disabled={isProcessing} className="flex-1 px-2 py-1 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600 rounded text-xs transition-colors">
                {isProcessing ? 'Syncing...' : 'Flush Now'}
              </button>)}
            <button onClick={clearQueue} className="flex-1 px-2 py-1 bg-red-600 hover:bg-red-700 rounded text-xs transition-colors">
              Clear Queue
            </button>
          </div>)}
      </div>
