...
import { Users, TrendingUp, Activity, CheckCircle, XCircle, Minus, Eye, BarChart3, Clock, Zap } from 'lucide-react';
import { getVoteTally, getRecentVotes } from '../../dao/DAOVoteEngine';
// Main VoteResultsPanel component
export const VoteResultsPanel = ({ billId, district, autoRefresh = true, refreshInterval = 10000 // 10 seconds
 }) => {
    // Component state
    ...
    const previousTallyRef = useRef(null);
    // Load vote tally
    ...
    if (tallyState.isLoading && recentActivity.isLoading) {
        return (<div className="max-w-4xl mx-auto p-6 bg-slate-800 border border-slate-600 rounded-lg">
    ...
    if (tallyState.error && recentActivity.error) {
        return (<div className="max-w-4xl mx-auto p-6 bg-slate-800 border border-red-600 rounded-lg">
        ...
        <button onClick={refreshData} className="mt-4 px-4 py-2 bg-red-900/30 border border-red-600 text-red-300 rounded-lg hover:bg-red-800/30 transition-colors">
          Retry
        </button>
      </div>);
    }
    return (<div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <button onClick={refreshData} className="flex items-center gap-2 px-3 py-2 bg-slate-700 border border-slate-600 text-slate-300 rounded-lg hover:bg-slate-600 transition-colors" aria-label="Refresh vote data">
              <Activity className="w-4 h-4"/>
              <span>Refresh</span>
            </button>
            
            <button onClick={() => setShowDetails(!showDetails)} className="flex items-center gap-2 px-3 py-2 bg-slate-700 border border-slate-600 text-slate-300 rounded-lg hover:bg-slate-600 transition-colors" aria-label="Toggle detailed view" aria-expanded={showDetails}>
      ...
      {/* Vote Tally Summary */}
      {tallyState.tally && (<div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
          ...
          {/* Tier Distribution */}
          {showDetails && (<div className="pt-4 border-t border-slate-600">
      ...
      <div className="bg-slate-800 border border-slate-600 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <select value={selectedTier} onChange={(e) => setSelectedTier(e.target.value)} className="px-3 py-2 bg-slate-700 border border-slate-600 text-slate-300 rounded-lg text-sm" aria-label="Filter by voter tier">
        ...
        {recentActivity.isLoading ? (<div className="flex items-center gap-3 py-8">
          ...
          </div>) : filteredRecentVotes.length === 0 ? (<div className="text-center py-8 text-slate-400">
            No recent votes found for the selected criteria.
          </div>) : (<div className="space-y-3 max-h-96 overflow-y-auto">
            {filteredRecentVotes.map((vote) => (<div key={vote.voteId} className="flex items-center justify-between p-3 bg-slate-700/30 border border-slate-600/50 rounded-lg">
                <div className="flex items-center gap-3">
