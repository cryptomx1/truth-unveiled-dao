...
export default function ConsensusTracker() {
    ...
    const [quorumView, setQuorumView] = useState({
        requiredVotes: 100,
        currentVotes: 0,
        citizenVotes: 0,
        moderatorVotes: 0,
    ...
    const desyncSimulationRef = useRef(0);
    // Mock vote data for simulation
    const mockVotes = useRef([
        {
            id: 'vote_001',
    ...
    const updateQuorumView = (proposal) => {
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      <div className="mb-4">
        ...
        <select id="proposal-select" value={selectedProposal} onChange={(e) => handleProposalSelect(e.target.value)} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-md text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent" style={{ minHeight: '48px' }} aria-describedby="proposal-info">
          {consensusData.map(proposal => (<option key={proposal.proposalId} value={proposal.proposalId}>
              {proposal.title}
            </option>))}
        </select>
      ...
      {/* Consensus Status */}
      {selectedProposalData && (<div className="mb-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
      ...
      <div className="mb-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
        ...
        <div className="space-y-2 text-sm">
          ...
          {/* Path B Status */}
          {volatilityMetrics.pathBTriggered && (<div className="mt-3 pt-3 border-t border-slate-600">
      ...
      <div className="flex space-x-3">
        <button onClick={toggleTracking} className={`flex-1 py-2 px-4 rounded-md font-medium transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800 ${isTracking
            ? 'bg-red-600 hover:bg-red-700 text-white'
            : 'bg-green-600 hover:bg-green-700 text-white'}`} style={{ minHeight: '48px' }} aria-describedby="tracking-status">
          {isTracking ? 'Pause Tracking' : 'Resume Tracking'}
        </button>
        
        <button onClick={() => updateConsensusData()} disabled={!isTracking} className="flex-1 py-2 px-4 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600 disabled:cursor-not-allowed text-white font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800" style={{ minHeight: '48px' }} aria-describedby="refresh-info">
          Refresh Now
        </button>
      </div>
