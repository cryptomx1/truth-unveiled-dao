...
import { ZKVoteVerifier } from '../../layers/ZKVoteVerifier';
export default function ZKVoteVerifierDemo() {
    ...
    // Get result color
    const getResultColor = (valid, expected) => {
        if (valid === expected) {
    ...
    // Get result icon
    const getResultIcon = (valid, expected) => {
        if (valid === expected) {
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      {/* Test Controls */}
      <div className="mb-4">
        <button onClick={runTestSuite} disabled={isRunning} className={`w-full py-3 px-4 rounded-md font-medium transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800 ${isRunning
      ...
      {metrics && (<div className="mb-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
          ...
          <div className="space-y-2 text-sm">
            ...
            {/* Fallback Status */}
            {metrics.fallbackActivated && (<div className="mt-3 pt-3 border-t border-slate-600">
      ...
      {testResults.length > 0 && (<div className="mb-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {testResults.map((result, index) => (<div key={result.scenario.id} className="text-xs">
                ...
                {result.desyncReason && (<div className="text-slate-400 ml-8 mt-1">
                    Reason: {result.desyncReason}
                  </div>)}
                
                {result.replayDetected && (<div className="text-amber-400 ml-8 mt-1">
      ...
      {vaultHistory.length > 0 && (<div className="mb-4 p-3 bg-slate-700 border border-slate-600 rounded-md">
          <div className="text-sm font-medium text-slate-300 mb-2">
            Vault History ({vaultHistory.length} entries)
          </div>
          <div className="space-y-1 max-h-32 overflow-y-auto text-xs">
            {vaultHistory.slice(-5).map((entry) => (<div key={entry.id} className="flex justify-between">
