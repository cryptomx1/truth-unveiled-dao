...
import { GovernanceSyncLayer } from '../../layers/GovernanceSyncLayer';
import { LocalSaveLayer } from '../../layers/LocalSaveLayer';
export default function ProposalSubmission({ userDID = 'did:civic:0x7f3e2d1a8c9b5f2e', userTier = 'Citizen', onSubmissionComplete }) {
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input id="proposal-title" type="text" value={formData.title} onChange={(e) => handleInputChange('title', e.target.value)} placeholder="Enter proposal title..." maxLength={100} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-md text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent" style={{ minHeight: '48px' }} aria-describedby="title-counter title-requirements"/>
        ...
        <div>
          ...
          <textarea id="proposal-description" value={formData.description} onChange={(e) => handleInputChange('description', e.target.value)} placeholder="Describe your proposal in detail..." maxLength={300} rows={4} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-md text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none" aria-describedby="description-counter description-requirements"/>
        ...
        <div>
          ...
          <select id="proposal-tag" value={formData.tag} onChange={(e) => handleInputChange('tag', e.target.value)} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-md text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent" style={{ minHeight: '48px' }} aria-describedby="tag-requirements">
          ...
          {/* Tag Preview */}
          {formData.tag && (<div className="mt-2">
        ...
        <div>
          ...
          <input id="proposal-attachment" type="file" onChange={handleFileAttachment} accept=".pdf,.doc,.docx,.txt,.png,.jpg,.jpeg" className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-md text-white file:bg-slate-600 file:border-0 file:text-white file:px-2 file:py-1 file:rounded file:mr-2" style={{ minHeight: '48px' }} aria-describedby="attachment-info"/>
          <div id="attachment-info" className="text-xs text-slate-400 mt-1">
            Max 5MB • PDF, DOC, TXT, PNG, JPG
          </div>
          
          {/* Attachment Preview */}
          {formData.attachment && (<div className="mt-2 p-2 bg-slate-700 rounded border border-slate-600">
        ...
        {/* Validation Errors */}
        {validationErrors.length > 0 && (<div className="p-3 bg-red-900/50 border border-red-600 rounded-md" role="alert" aria-live="polite">
        ...
        {/* Submission Status */}
        {submissionState.status !== 'idle' && (<div className="p-3 bg-slate-700 border border-slate-600 rounded-md" aria-live="polite">
            ...
            {/* ZKP Hash Display */}
            {submissionState.proposal && (<div className="mt-2 text-xs text-slate-400">
                ZKP Hash: <span className="font-mono text-blue-400">{submissionState.proposal.zkpHash}</span>
              </div>)}
          </div>)}

        {/* Submit Button */}
        <button type="submit" disabled={submissionState.status === 'validating' || submissionState.status === 'submitting'} className="w-full py-3 px-4 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600 disabled:cursor-not-allowed text-white font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800" style={{ minHeight: '48px' }} aria-describedby="submit-requirements">
          {submissionState.status === 'validating' && 'Validating...'}
          {submissionState.status === 'submitting' && 'Submitting...'}
