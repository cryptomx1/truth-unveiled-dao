...
export const CivicFusionRouter = () => {
    const [location] = useLocation();
    // Telemetry logging for route access
    React.useEffect(() => {
        console.log(`🧬 Civic Fusion Route accessed: ${location}`);
    }, [location]);
    return (<div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 dark:from-slate-900 dark:to-slate-800">
      <div className="container mx-auto px-4 py-8">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2 text-sm">
                <Button variant="ghost" size="sm" onClick={() => window.location.href = '/'} className="text-blue-600 hover:text-blue-800">
                  Home
                </Button>
        ...
        {/* Route Content */}
        <div role="main" aria-label="Civic Fusion Cycle Content" className="w-full max-w-6xl mx-auto">
          <Switch>
            {/* Fusion Dashboard */}
            ...
            <Route path="/fusion/onboarding">
              <CivicMissionOnboardingCard onMissionSelect={(route) => {
            ...
            <Route path="/fusion/onboarding/:missionId">
              {(params) => (<CivicCompass onDirectionSelect={(direction) => {
                console.log(`🧭 Pillar direction selected: ${direction.deckRoute}`);
                // Announce pillar selection
                if (window.speechSynthesis) {
                    const utterance = new SpeechSynthesisUtterance(`You've selected ${params.missionId}. Recommended Pillar: ${direction.name}. Guardian: ${direction.guardian || 'Athena'}.`);
                    utterance.rate = 0.8;
            ...
            <Route path="/fusion">
              <div className="text-center py-16">
                ...
                <div className="flex justify-center gap-4">
                  <Button onClick={() => window.location.href = '/fusion/onboarding'} size="lg" className="min-w-48">
                    🚀 Start Civic Mission
                  </Button>
                  <Button variant="outline" onClick={() => window.location.href = '/fusion/dashboard'} size="lg" className="min-w-48">
                    📊 View Progress
                  </Button>
                </div>
              </div>
        ...
        <div aria-live="polite" aria-atomic="true" className="sr-only" id="fusion-announcements">
