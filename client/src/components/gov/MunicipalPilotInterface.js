...
import { Building2, MapPin, Users, FileText, CheckCircle, Clock, Settings } from "lucide-react";
export function MunicipalPilotInterface() {
    const [cityTemplates] = useState([
        {
            id: "austin_tx",
            name: "Austin",
            state: "Texas",
    ...
    const [didRegistry, setDidRegistry] = useState({
        registered: 0,
        pending: 0,
        verified: 0,
        total: 0
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                {cityTemplates.map((city) => (<div key={city.id} className={`p-4 border rounded-lg cursor-pointer transition-colors ${selectedCity === city.id
                ? 'border-blue-500 bg-blue-50'
                : 'border-gray-200 hover:border-gray-300'}`} onClick={() => handleCitySelection(city.id)}>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {city.features.map((feature, index) => (<Badge key={index} variant="outline" className="text-xs">
                          {feature}
                        </Badge>))}
                    </div>
                  </div>))}
            ...
            <div>
              ...
              {selectedCityData ? (<div className="space-y-4">
                  {/* Target Decks */}
                  <div>
                    <Label className="text-sm font-medium">Target Civic Decks</Label>
                    <div className="mt-2 space-y-2">
                      {deckOptions.map((deck) => (<label key={deck.id} className="flex items-center space-x-2">
                          <input type="checkbox" checked={pilotConfig.targetDecks.includes(deck.id)} onChange={(e) => {
                    if (e.target.checked) {
                        setPilotConfig(prev => ({
                            ...prev,
                  ...
                  <div>
                    ...
                    <Input id="citizenGoal" type="number" value={pilotConfig.citizenGoal} onChange={(e) => setPilotConfig(prev => ({
                ...prev,
                citizenGoal: parseInt(e.target.value) || 0
                  ...
                  <div>
                    ...
                    <Select value={pilotConfig.timeframe} onValueChange={(value) => setPilotConfig(prev => ({ ...prev, timeframe: value }))}>
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg">
                    ...
                    <Progress value={(didRegistry.verified / didRegistry.total) * 100} className="mt-3"/>
                  </div>

                  {/* Deployment Button */}
                  <Button onClick={deployPilot} disabled={isDeploying || pilotConfig.targetDecks.length === 0} className="w-full">
                    {isDeploying ? (<div className="flex items-center gap-2">
