...
const GlobalDeploymentDashboard = () => {
    ...
    const availableJurisdictions = supportedJurisdictions.filter(j => !deployments.some(d => d.jurisdiction === j));
      ...
      <Card className="bg-slate-900/50 dark:bg-slate-950/50 border-slate-700">
        ...
        <CardContent className="space-y-6">
            ...
            <div className="flex gap-4">
              <Select value={selectedJurisdiction} onValueChange={setSelectedJurisdiction}>
                ...
                <SelectContent className="bg-slate-800 border-slate-600">
                  {availableJurisdictions.map(jurisdiction => (<SelectItem key={jurisdiction} value={jurisdiction} className="text-slate-200">
                      {jurisdiction} ({getTierForJurisdiction(jurisdiction)})
                    </SelectItem>))}
                </SelectContent>
              </Select>
              
              <Button onClick={handleDeployment} disabled={!selectedJurisdiction || isDeploying} className="bg-orange-600 hover:bg-orange-700">
          ...
          <div className="space-y-3">
            ...
            {deployments.map(deployment => (<Card key={deployment.id} className="bg-slate-800/50 border-slate-600">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      {deployment.complianceStatus && (<Badge variant="outline" className="text-green-400 border-green-400">
                          Compliant
                        </Badge>)}
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      {deployment.culturalAccuracy > 0 && (<div className="text-xs text-slate-400">
                  ...
                  <div className="flex justify-between items-center text-xs text-slate-400">
                    ...
                    {deployment.deployedAt && (<span>Deployed: {deployment.deployedAt.toLocaleDateString()}</span>)}
                  </div>
                </CardContent>
              </Card>))}
          </div>
