...
const CivicDeploymentWizard = () => {
    ...
    return (<Card className="max-w-3xl mx-auto my-10">
      <CardHeader>
      ...
      <CardContent>
        ...
        {currentStep === 2 && (<div className="space-y-4">
            ...
            {Object.keys(FederationNodeRegistry).map((nodeKey) => (<div key={nodeKey} className="flex items-center space-x-2">
                <Checkbox id={nodeKey} checked={regions.includes(nodeKey)} onCheckedChange={(checked) => {
                    setRegions((prev) => checked ? [...prev, nodeKey] : prev.filter((r) => r !== nodeKey));
                }}/>
