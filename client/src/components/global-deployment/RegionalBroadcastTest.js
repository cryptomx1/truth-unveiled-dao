...
import { Radio, CheckCircle, AlertTriangle, Activity, Database } from 'lucide-react';
import { DeploymentBroadcastAgent } from './DeploymentBroadcastAgent';
const RegionalBroadcastTest = () => {
    ...
    const runSinglePropagation = async (country, tier) => {
        ...
        const deckGroup = tier === 'tier1' ? ['governance', 'privacy', 'finance'] :
            tier === 'tier2' ? ['governance', 'privacy'] :
                ['governance'];
        try {
            const result = await broadcastAgent.propagateRegionalCID(deploymentId, country, `${country.toLowerCase()}_locale`, tier, deckGroup, 'manual-test');
    ...
    const getNetworkHealthColor = (health) => {
        switch (health) {
      ...
      <Card className="bg-slate-900/50 dark:bg-slate-950/50 border-slate-700">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Button onClick={runQASimulations} disabled={isRunning} className="bg-orange-600 hover:bg-orange-700">
              {isRunning ? 'Running QA Suite...' : 'Run Full QA Suite (10 Simulations)'}
            </Button>

            <div className="grid grid-cols-3 gap-2">
              <Button onClick={() => runSinglePropagation('US', 'tier1')} disabled={isRunning} variant="outline" className="border-slate-600 text-slate-200 text-xs">
                US T1
              </Button>
              <Button onClick={() => runSinglePropagation('DE', 'tier2')} disabled={isRunning} variant="outline" className="border-slate-600 text-slate-200 text-xs">
                DE T2
              </Button>
              <Button onClick={() => runSinglePropagation('BR', 'tier3')} disabled={isRunning} variant="outline" className="border-slate-600 text-slate-200 text-xs">
                BR T3
              </Button>
            </div>
          </div>
          ...
          {/* Status */}
          {currentStep && (<div className="bg-slate-800/50 rounded-lg p-4">
          ...
          {/* QA Results Summary */}
          {qaResults && (<div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          {/* Simulation History */}
          {simulationHistory.length > 0 && (<div className="space-y-4">
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {simulationHistory.map((result, index) => (<div key={result.broadcastId} className="bg-slate-800/50 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <Badge variant="outline" className={`text-xs ${getStatusColor(result.propagationPayload.propagationStatus)}`}>
                          {result.propagationPayload.propagationStatus}
                        </Badge>
                      </div>
                      
                      <div className="flex items-center gap-2">
                        {result.fallbackTriggered && (<Badge variant="outline" className="text-amber-400 border-amber-400 text-xs">
                            Fallback
                          </Badge>)}
                        <Badge variant="outline" className={`text-xs ${getNetworkHealthColor(result.networkHealth)}`}>
