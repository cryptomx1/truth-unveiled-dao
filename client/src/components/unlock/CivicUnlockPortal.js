...
const CivicUnlockPortal = () => {
    ...
    const [guardianClaims, setGuardianClaims] = useState([]);
    ...
    const handleClaimGuardianship = async (pillarId) => {
        const pillar = guardianClaims.find(p => p.pillarId === pillarId);
        if (!pillar || !pillar.unlocked)
            return;
        ...
        try {
            ...
            if (verificationResult.success) {
                ...
                setGuardianClaims(prev => prev.map(p => p.pillarId === pillarId
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <TTSToggle deckId="unlock-portal" moduleId="guardian-onboarding" content="Welcome to the Civic Unlock Portal. Here you can claim Guardian access to the eight civic pillars by completing truth rituals and staking requirements. Each pillar requires specific TruthPoint amounts, Genesis Badges, and ZKP proofs."/>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {guardianClaims.map((pillar) => (<Card key={pillar.pillarId} className={`transition-all hover:shadow-md ${getStatusColor(pillar)}`}>
            ...
            <CardContent className="space-y-4">
              ...
              <Button onClick={() => handleClaimGuardianship(pillar.pillarId)} disabled={!pillar.unlocked || isProcessing || unlockHistory.some(h => h.pillarId === pillar.pillarId)} className="w-full" size="sm">
      ...
      {unlockHistory.length > 0 && (<Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {unlockHistory.map((record) => (<div key={record.id} className="flex items-center justify-between p-3 bg-green-50 rounded-lg border">
