...
export const TTSToggle = ({ deckId, moduleId, content, className = '', size = 'sm', variant = 'outline', mode = 'standard', tutorialPath, children }) => {
    ...
    const getTooltipContent = () => {
        if (!isEnabled)
            return 'TTS disabled for this deck';
        if (isPlaying)
            return 'Stop narration';
    ...
    return (<div className={`flex items-center gap-1 ${className}`} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      <Button onClick={handleToggle} variant={variant} size={size} title={getTooltipContent()} className={`
          transition-all duration-200
          ${!isEnabled ? 'opacity-50' : ''}
          ${isPlaying ? 'bg-blue-100 dark:bg-blue-900' : ''}
        `} aria-label={getTooltipContent()} aria-pressed={isPlaying}>
        {getButtonContent()}
        {children && (<span className="ml-1">{getButtonText()}</span>)}
      </Button>

      {/* Enhanced metadata tooltip with voice/tone info */}
      {isHovered && isEnabled && voiceMetadata && (<div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900 text-xs rounded-lg whitespace-nowrap z-20 shadow-lg">
      ...
      <div id={`tts-live-${deckId}-${moduleId}`} aria-live="polite" aria-atomic="true" className="sr-only"/>
    </div>);
};
