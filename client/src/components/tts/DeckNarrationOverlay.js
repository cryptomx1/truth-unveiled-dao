...
import { Volume2, Pause, Play, Timer, X } from 'lucide-react';
import TTSEngineAgent from '@/agents/TTSEngineAgent';
const DeckNarrationOverlay = ({ isVisible = false, onClose }) => {
    ...
    const getToneFromDeckConfig = (deckId) => {
    ...
    return (<div className="fixed top-4 left-1/2 transform -translate-x-1/2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-50 max-w-md w-full mx-4" role="region" aria-label="Active narration overlay" aria-live="polite" aria-atomic="true">
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" onClick={handleClose} className="h-6 w-6 p-0" aria-label="Close narration overlay">
            <X className="h-3 w-3"/>
          </Button>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Progress value={session.progress} className="h-1" aria-label={`Narration progress: ${Math.round(session.progress)}%`}/>
        </div>

        {/* Controls */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={handlePlayPause} className="h-8 px-3" aria-label={isPlaying ? 'Pause narration' : 'Resume narration'}>
              {isPlaying ? (<>
