...
export const LangToggle = ({ deckId, moduleId, variant = 'button', showTutorialLink = false, className = '' }) => {
    ...
    if (variant === 'flags') {
        ...
        {SUPPORTED_LANGUAGES.map(lang => (<button key={lang.code} onClick={() => handleLanguageChange(lang.code)} className={`
              text-lg transition-all duration-200 hover:scale-110
              ${currentLanguage === lang.code ? 'opacity-100 scale-110' : 'opacity-60'}
            `} title={lang.name} aria-label={`Switch to ${lang.name}`}>
            {lang.flag}
          </button>))}
      </div>);
    }
    if (variant === 'compact') {
        ...
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="sm" className={`text-xs ${className}`} disabled={isChanging}>
            {getCurrentLanguageDisplay().flag}
            <ChevronDown className="h-3 w-3 ml-1"/>
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="w-40">
          {SUPPORTED_LANGUAGES.map(lang => (<DropdownMenuItem key={lang.code} onClick={() => handleLanguageChange(lang.code)} className={currentLanguage === lang.code ? 'bg-blue-50 dark:bg-blue-950' : ''}>
    ...
    return (<DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="sm" className={`flex items-center gap-2 ${className}`} disabled={isChanging}>
      ...
      <DropdownMenuContent align="end" className="w-48">
        {SUPPORTED_LANGUAGES.map(lang => (<DropdownMenuItem key={lang.code} onClick={() => handleLanguageChange(lang.code)} className={`
              flex items-center gap-3 px-3 py-2 cursor-pointer
              ${currentLanguage === lang.code ? 'bg-blue-50 dark:bg-blue-950 font-medium' : ''}
            `}>
            <span className="text-lg">{lang.flag}</span>
            <span>{lang.name}</span>
            {currentLanguage === lang.code && (<div className="ml-auto w-2 h-2 bg-blue-500 rounded-full"/>)}
          </DropdownMenuItem>))}
        
        {showTutorialLink && (deckId || moduleId) && (<>
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={handleTutorialAccess} className="flex items-center gap-3 px-3 py-2 cursor-pointer text-blue-600 dark:text-blue-400">
