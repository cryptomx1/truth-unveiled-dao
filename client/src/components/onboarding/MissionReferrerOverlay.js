...
export function MissionReferrerOverlay({ onAccept, onDismiss, onIndependentProceed }) {
    ...
    const logReferralTrace = (referrer) => {
        const traceEntry = {
    ...
    return (<div className={`fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-opacity duration-300 ${fadeOut ? 'opacity-0' : 'opacity-100'}`} role="dialog" aria-label="Referral Invite Detected — Begin Mission">
      <Card className={`w-full max-w-md bg-slate-800 border-slate-700 text-white shadow-2xl transform transition-all duration-300 ${fadeOut ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}>
        <CardContent className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button variant="ghost" size="sm" onClick={handleDismiss} className="text-slate-400 hover:text-white" aria-label="Dismiss referral invite">
              <X className="h-4 w-4"/>
            </Button>
          </div>
          ...
          {referrerProfile.isValid ? (<>
              ...
              <div className="space-y-3">
                <Button onClick={handleAcceptReferral} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-3" aria-label="Accept mission invite and begin onboarding">
                  Accept Invite & Begin Mission
                </Button>
                <Button onClick={handleIndependentProceed} variant="outline" className="w-full border-slate-600 text-slate-300 hover:bg-slate-700 hover:text-white" aria-label="Proceed as independent citizen">
              ...
              <Button onClick={handleIndependentProceed} className="w-full bg-slate-600 hover:bg-slate-500 text-white font-medium py-3" aria-label="Continue to standard onboarding">
