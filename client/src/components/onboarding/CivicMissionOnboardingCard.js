...
export function CivicMissionOnboardingCard({ citizenCID, onMissionSelect, fusionMode = false }) {
    ...
    const handleStepSelect = (step, index) => {
        ...
        if (fusionMode && mockCidTier === 'Citizen' && !isCommanderMode) {
            ...
            if ('speechSynthesis' in window) {
                const utterance = new SpeechSynthesisUtterance('Mission requires higher civic tier. Complete more activities to unlock.');
                utterance.rate = 0.8;
    ...
    const handleReferralAccepted = (referrerData) => {
        ...
        TPRewardEngine.injectTestReward(citizenCID || 'new-citizen', 75, `Referral bonus from ${referrerData.alias}`);
    ...
    return (<>
      ...
      {showReferrerOverlay && (<MissionReferrerOverlay onAccept={handleReferralAccepted} onIndependentProceed={handleIndependentProceed} onDismiss={() => setShowReferrerOverlay(false)}/>)}
      ...
      <Card className="w-full max-w-2xl mx-auto bg-slate-800 border-slate-700 text-white">
        ...
        {isCommanderMode && (<Badge className="bg-amber-600 text-white">
        ...
        <div className="text-sm text-slate-300 space-y-2">
          ...
          <div className="flex gap-2">
            <UniversalCopyButton content="https://gateway.pinata.cloud/ipfs/QmXj5llhfmbendtruthunveiled" label="Copy IPFS Link" size="sm" className="bg-blue-600 hover:bg-blue-700 text-white border-blue-500"/>
            <UniversalCopyButton content={`Truth Unveiled Civic Genome - Mission Onboarding
...
For JASMY/Claude collaboration`} label="Copy Status" size="sm" className="bg-green-600 hover:bg-green-700 text-white border-green-500"/>
          </div>
        </div>
        </CardHeader>

        <CardContent className="space-y-6">
        {isCommanderMode ? (<div className="text-center space-y-4">
            ...
            <Button onClick={handleCommanderAccess} className="w-full bg-amber-600 hover:bg-amber-700">
            ...
            <div className="space-y-3">
              ...
              {missionSteps.map((step, index) => (<div key={step.id} className={`p-4 rounded-lg border cursor-pointer transition-all duration-200 ${index === currentStep
                    ? 'border-blue-500 bg-blue-900/20'
                    : 'border-slate-600 bg-slate-700/50 hover:border-slate-500'}`} onClick={() => handleStepSelect(step, index)}>
                  <div className="flex items-start space-x-3">
                    <div className={`p-2 rounded-full ${step.completed ? 'bg-green-600' :
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        ...
                        {step.completed && (<Badge className="bg-green-600 text-white text-xs">
                            Complete
                          </Badge>)}
                      </div>
            ...
            <div className="pt-4 border-t border-slate-600">
              <Button onClick={() => handleStepSelect(missionSteps[currentStep], currentStep)} className="w-full bg-blue-600 hover:bg-blue-700" disabled={!missionSteps[currentStep]}>
