...
export default function LedgerStreamVisualizer() {
    ...
    const applyFilters = useCallback(() => {
        ...
        if (filters.searchQuery.trim()) {
            ...
            filtered = filtered.filter(entry => entry.sourceDID.toLowerCase().includes(query) ||
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      <div className="mb-4 space-y-3">
        ...
        <div className="space-y-2">
          ...
          <select id="credential-state" value={filters.credentialState} onChange={(e) => setFilters(prev => ({ ...prev, credentialState: e.target.value }))} className="w-full py-2 px-3 bg-slate-700 border border-slate-600 rounded-md text-white focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800" style={{ minHeight: '48px' }}>
        ...
        <div className="space-y-2">
          ...
          <input id="search-query" type="text" placeholder="Search by DID, credential ID, or sync ID..." onChange={(e) => handleSearchChange(e.target.value)} className="w-full py-2 px-3 bg-slate-700 border border-slate-600 rounded-md text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800" style={{ minHeight: '48px' }}/>
        </div>

        {/* Refresh Controls */}
        <div className="flex space-x-2">
          <button onClick={toggleAutoRefresh} className={`flex-1 py-2 px-3 rounded-md font-medium transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800 ${isAutoRefresh
            ? 'bg-green-600 hover:bg-green-700 text-white'
            : 'bg-slate-600 hover:bg-slate-700 text-slate-300'}`} style={{ minHeight: '48px' }}>
            Auto Refresh {isAutoRefresh ? 'ON' : 'OFF'}
          </button>
          
          <button onClick={handleSyncNow} className="flex-1 py-2 px-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800" style={{ minHeight: '48px' }}>
            Sync Now
          </button>
        </div>
      </div>
      ...
      <div className="bg-slate-700 border border-slate-600 rounded-md">
        ...
        <div className="max-h-96 overflow-y-auto">
          {filteredEntries.length === 0 ? (<div className="p-4 text-center text-slate-400">
              {ledgerEntries.length === 0 ? ('No ledger entries found. Run credential sync tests to populate data.') : ('No entries match current filters.')}
            </div>) : (<div className="divide-y divide-slate-600">
              {filteredEntries.map((entry) => {
                const statusDisplay = getStatusDisplay(entry);
                const isNewEntry = newEntryIds.has(entry.syncId);
                return (<div key={entry.syncId} className={`p-3 transition-all duration-300 ${isNewEntry ? 'animate-pulse bg-slate-600' : 'hover:bg-slate-600'}`} role="row" aria-label={`Sync entry ${entry.credentialZkHash.slice(0, 8)} at ${formatTimestamp(entry.timestamp)}`}>
                    ...
                    <div className="space-y-1 text-xs">
                      ...
                      {/* Consensus Info */}
                      {entry.consensusReached && (<div className="flex justify-between">
                      ...
                      {/* CID Info */}
                      {entry.cidHash && (<div className="flex justify-between">
                      ...
                      {/* Path B Indicator */}
                      {entry.syncStatus === 'failed' && (<div className="mt-2 p-2 bg-red-900/20 border border-red-700 rounded">
