...
import { AlertTriangle, CheckCircle, XCircle, Clock, Shield, Hash, Users, FileText, Flag, RefreshCw } from 'lucide-react';
...
const validateProofSync = (proofHash) => {
    return DECK_6_PROOF_TRACES.includes(proofHash);
};
export const CivicVoteDisputeCard = ({ className }) => {
    const [disputeSession, setDisputeSession] = useState(MOCK_DISPUTE_SESSION);
    const [userRole, setUserRole] = useState('Registered Voter');
    const [newDispute, setNewDispute] = useState('');
    const [selectedProposal, setSelectedProposal] = useState('');
    const [selectedDisputeType, setSelectedDisputeType] = useState('vote_inconsistency');
    ...
    const handleSubmitDispute = async () => {
        if (!newDispute.trim() || !selectedProposal || !isRoleAuthorized(userRole))
            return;
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', getSessionStatusRing(), className)} role="region" aria-label="Civic Vote Dispute Interface">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="mt-3">
            ...
            <div className="w-full bg-slate-700/50 rounded-full h-2">
              <div className="bg-green-500 h-2 rounded-full transition-all duration-300" style={{ width: `${disputeSession.resolutionRate}%` }}/>
            </div>
          </div>
          ...
          {disputeSession.isPushbackActive ? (<div className="mt-3 p-2 bg-red-500/20 border border-red-500/30 rounded text-xs text-red-400">
              ⚠️ Pushback: &gt;5% unresolved disputes
            </div>) : (<div className="mt-3 p-2 bg-green-500/20 border border-green-500/30 rounded text-xs text-green-400">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Select value={selectedProposal} onValueChange={setSelectedProposal} disabled={!isRoleAuthorized(userRole)}>
              <SelectTrigger className="bg-slate-800/50 border-slate-700/50 text-slate-200">
                <SelectValue placeholder="Proposal ID"/>
              </SelectTrigger>
              <SelectContent>
                {proposalOptions.map((proposalId) => (<SelectItem key={proposalId} value={proposalId}>
                    {proposalId}
                  </SelectItem>))}
              </SelectContent>
            </Select>
            
            <Select value={selectedDisputeType} onValueChange={(value) => setSelectedDisputeType(value)} disabled={!isRoleAuthorized(userRole)}>
              <SelectTrigger className="bg-slate-800/50 border-slate-700/50 text-slate-200">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <Textarea value={newDispute} onChange={(e) => setNewDispute(e.target.value)} placeholder={isRoleAuthorized(userRole) ? "Describe the dispute or conflict..." : "Access restricted to registered users"} className={cn('min-h-[80px] bg-slate-800/50 border-slate-700/50', 'text-slate-200 placeholder-slate-400', 'focus:border-orange-500/50 focus:ring-orange-500/20')} maxLength={500} disabled={!isRoleAuthorized(userRole)} aria-label="Dispute description"/>
          ...
          <Button onClick={handleSubmitDispute} disabled={isProcessing || !newDispute.trim() || !selectedProposal || !isRoleAuthorized(userRole)} className={cn('w-full min-h-[48px] font-medium', isRoleAuthorized(userRole)
            ? 'bg-orange-600 hover:bg-orange-700 text-white'
            : 'bg-slate-700/50 text-slate-500 cursor-not-allowed', 'disabled:bg-slate-700/50 disabled:text-slate-500')} aria-label="Submit dispute">
            {isProcessing ? (<>
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-64 rounded-lg border border-slate-700/50 bg-slate-800/30">
            <div className="p-3 space-y-3" aria-live="polite">
              {disputeSession.disputes.map((dispute) => (<div key={dispute.id} className={cn('bg-slate-800/50 rounded-lg border border-slate-700/50 p-3', getStatusRing(dispute.status))}>
                  ...
                  <div className="space-y-1">
                    ...
                    {dispute.verdict && (<div className="mt-2 p-2 bg-slate-700/50 rounded text-xs">
                        <div className="font-medium text-slate-200">Verdict:</div>
                        <div className="text-slate-300">{dispute.verdict}</div>
                      </div>)}
                    
                    {dispute.conflictSummary && (<div className="text-xs text-slate-400">
                    ...
                    <div className="flex items-center justify-between text-xs text-slate-400">
                      <div className="flex items-center gap-1">
                        ...
                        {validateProofSync(dispute.proofHash) && (<CheckCircle className="w-3 h-3 text-green-400"/>)}
                      </div>
