...
import { FileText, CheckCircle, XCircle, Shield, Clock, Hash, Eye, AlertTriangle, RefreshCw, Lock } from 'lucide-react';
...
const validateZKPSync = (hash) => {
    // Path B validation with expanded hash pool
    const isInDeck6 = DECK_6_EXPANDED_HASHES.some(deck6Hash => hash.includes(deck6Hash.substring(8, 24)) || deck6Hash.includes(hash.substring(8, 24)));
    const isInDeck8 = DECK_8_AUDIT_HASHES.some(auditHash => auditHash.includes(hash.substring(8, 24)) || hash.includes(auditHash.substring(8, 24)));
    const isInDeck9 = DECK_9_VOTE_HASHES.some(voteHash => voteHash.includes(hash.substring(8, 24)) || hash.includes(voteHash.substring(8, 24)));
    // Enhanced validation with expanded pool - higher success rate for Path B
    if (isInDeck6 || isInDeck8 || isInDeck9)
        return 'verified';
    // Reduced failure rate for Path B (2% instead of 10%)
    return Math.random() > 0.02 ? 'verified' : 'sync_failed';
};
export const ZKProposalLogCard = ({ className }) => {
    const [logSession, setLogSession] = useState(MOCK_PROPOSAL_LOG);
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', getLogStatusRing(), className)} role="region" aria-label="ZK Proposal Log">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <button onClick={playLogUpdateTTS} disabled={ttsStatus.isPlaying} className="ml-auto text-slate-400 hover:text-slate-200 transition-colors" aria-label="Play log statistics">
          ...
          <div className="mt-3">
            ...
            <div className="w-full bg-slate-700/50 rounded-full h-2">
              <div className="bg-green-500 h-2 rounded-full transition-all duration-300" style={{ width: `${(logSession.verifiedEntries / logSession.totalEntries) * 100}%` }}/>
            </div>
          </div>
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="space-y-2">
            ...
            {logSession.isPushbackActive && (<div className="mt-2 p-2 bg-red-500/20 border border-red-500/30 rounded text-xs text-red-400">
        ...
        <div className="space-y-3">
          <Button onClick={handleValidateSync} disabled={isValidating} className={cn('w-full min-h-[48px] font-medium', 'bg-blue-600 hover:bg-blue-700 text-white', 'disabled:bg-slate-700/50 disabled:text-slate-500')} aria-label="Validate ZKP sync">
            {isValidating ? (<>
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-64 rounded-lg border border-slate-700/50 bg-slate-800/30">
            <div className="p-3 space-y-3" aria-live="polite">
              {logSession.entries.map((entry) => (<div key={entry.id} className={cn('bg-slate-800/50 rounded-lg border border-slate-700/50 p-3', getVerificationRing(entry.verificationStatus))}>
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {entry.verificationStatus === 'verified' && (<Badge variant="outline" className="bg-green-500/20 text-green-400 border-green-500/30 text-xs h-5 px-1">
                  ...
                  <div className="space-y-1">
                    ...
                    {entry.voteCount !== undefined && entry.voteCount > 0 && (<div className="flex items-center justify-between text-xs">
        ...
        {/* Pushback Status */}
        {logSession.isPushbackActive && (<div className="bg-red-500/20 rounded-lg border border-red-500/30 p-4">
