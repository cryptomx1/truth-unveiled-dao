...
import { Vote, CheckCircle, XCircle, Users, TrendingUp, AlertTriangle, Clock, Shield, Activity } from 'lucide-react';
...
const getStatusRing = (status, hashMismatchRate) => {
...
const getConsensusProgressColor = (consensus, threshold) => {
...
const validateZKPSync = (voteHashes) => {
    const matchedHashes = voteHashes.filter(hash => DECK_6_VOTE_HASHES.some(deckHash => deckHash.includes(hash.substring(0, 16))));
    return voteHashes.length > 0 ? ((voteHashes.length - matchedHashes.length) / voteHashes.length) * 100 : 0;
};
export const VoteConsensusCard = ({ className }) => {
    const [consensusData, setConsensusData] = useState(MOCK_VOTE_CONSENSUS);
    ...
    const playConsensusUpdateTTS = (outcome) => {
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', getStatusRing(consensusData.status, consensusData.hashMismatchRate), className)} role="region" aria-label="Vote Consensus Monitor">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <button onClick={() => playConsensusUpdateTTS(consensusData.outcome)} disabled={ttsStatus.isPlaying} className="ml-auto text-slate-400 hover:text-slate-200 transition-colors" aria-label="Play consensus status">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <Progress value={consensusData.currentConsensus} className="h-3" aria-label={`Consensus progress: ${consensusData.currentConsensus.toFixed(1)}%`}/>
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            {isPushbackActive && (<AlertTriangle className="w-4 h-4 text-orange-400"/>)}
          </div>
          ...
          <div className="space-y-2">
            ...
            {isPushbackActive && (<div className="mt-2 p-2 bg-orange-500/20 border border-orange-500/30 rounded text-xs text-orange-400">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-32 rounded-lg border border-slate-700/50 bg-slate-800/30">
            <div className="p-3 space-y-2">
              {consensusData.zkpHashes.map((hash, index) => (<div key={index} className="flex items-center justify-between p-2 rounded bg-slate-800/50 border border-slate-700/50">
        ...
        {/* Aggregate Control */}
        <div className="space-y-3">
          <Button onClick={handleAggregateVotes} disabled={isAggregating || consensusData.status !== 'active'} className={cn('w-full min-h-[48px] font-medium', 'bg-blue-600 hover:bg-blue-700 text-white', 'disabled:bg-slate-700/50 disabled:text-slate-500')} aria-label="Aggregate votes">
            {isAggregating ? (<>
        ...
        {/* Consensus Outcome */}
        {consensusData.outcome !== 'pending' && (<div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4" aria-live="polite" aria-label="Consensus outcome">
