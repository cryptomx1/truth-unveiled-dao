...
export const Deck12Module1Subpage2 = ({ subpageId = 'analytics-reports', reportType = 'detailed' }) => {
    const { isAuthenticated, cidStatus, validateCID } = useCIDAuthentication();
    const [selectedReport, setSelectedReport] = useState('');
    if (!isAuthenticated) {
        return (<CIDAuthenticationGate onValidate={validateCID} title="Analytics Reports Access" description="Access to detailed analytics reports requires CID authentication for data privacy compliance."/>);
    }
    const analyticsData = {
        totalReports: 47,
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {keyMetrics.map((metric, index) => (<div key={index} className="p-4 bg-gray-50 dark:bg-gray-800 rounded">
                <div className="text-sm text-gray-600 dark:text-gray-400">
                  {metric.label}
                </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {availableReports.map((report) => (<Card key={report.id} className="border border-gray-200 dark:border-gray-700">
                <CardContent className="pt-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" onClick={() => handleGenerateReport(report.id)} disabled={report.status === 'Generating'}>
                        <TrendingUp className="h-4 w-4 mr-1"/>
                        Generate
                      </Button>
                      {report.status === 'Ready' && (<Button size="sm" onClick={() => handleDownloadReport(report)}>
                  ...
                  {selectedReport === report.id && (<div className="mt-3 pt-3 border-t">
