...
import { MessageCircle, Send, Lock, Shield, UserCheck } from 'lucide-react';
...
export const CommunityForumCard = ({ className }) => {
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-800 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Community Forum Discussion">
      ...
      <CardContent className="space-y-3">
        ...
        <div className="bg-slate-700/30 rounded-lg border border-slate-600/50 p-3 max-h-64 overflow-y-auto">
          <div className="space-y-3">
            {posts.map((post) => (<div key={post.id} className={cn('flex', post.side === 'left' ? 'justify-start' : 'justify-end')}>
                <div className={cn('max-w-[80%] rounded-lg p-2', post.side === 'left'
                ...
                : 'bg-blue-600/30 rounded-tr-none')}>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <Badge variant="outline" className={cn('text-xs px-1 py-0', getCategoryColor(post.category))}>
                      {post.category}
                    </Badge>
                  </div>
        ...
        <div className="bg-slate-700/30 rounded-lg border border-slate-600/50 p-3">
          <div className="space-y-2">
            <Textarea value={message} onChange={(e) => handleMessageChange(e.target.value)} onKeyDown={handleKeyDown} placeholder="Ask a question or share your thoughts..." className={cn('min-h-[60px] resize-none border-slate-600 bg-slate-800/50', 'text-slate-100 placeholder:text-slate-400', 'focus:border-blue-500 focus:ring-blue-500/20')} maxLength={150} aria-label="Forum message input" aria-describedby="char-count" aria-live="polite"/>
            <div className="flex items-center justify-between">
              <span id="char-count" className={cn('text-xs', message.length > 140 ? 'text-amber-400' : 'text-slate-400')}>
                {message.length}/150
              </span>
              <Button onClick={handleSubmitMessage} disabled={!message.trim() || isPosting} size="sm" className={cn('bg-blue-600 hover:bg-blue-700 text-white', 'min-h-[48px] px-4', 'disabled:bg-slate-600 disabled:text-slate-400')} aria-label="Send message to forum">
                {isPosting ? (<>Posting...</>) : (<>
