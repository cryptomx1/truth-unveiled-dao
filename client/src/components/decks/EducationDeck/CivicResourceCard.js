...
export const CivicResourceCard = ({ className }) => {
    ...
    const handleResourceHover = (resourceId) => {
    ...
    const handleResourceClick = (url, title) => {
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-800 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Civic Resources Library">
      ...
      <CardContent className="space-y-3">
        ...
        <div className="space-y-2">
          {MOCK_CIVIC_RESOURCES.slice(0, 3).map((resource) => (<div key={resource.id} className={cn('bg-slate-700/30 rounded-lg border border-slate-600/50 p-3', 'transition-all duration-200 hover:bg-slate-600/40 hover:border-slate-500', hoveredResource === resource.id && 'bg-slate-600/40 border-slate-500')} onMouseEnter={() => handleResourceHover(resource.id)} onMouseLeave={() => handleResourceHover(null)}>
              <div className="flex items-start gap-3">
                <div className="flex-shrink-0 mt-1">
                  <Badge variant="outline" className={cn('text-xs', getSourceColor(resource.source))}>
                    {getSourceIcon(resource.source)}
                    <span className="ml-1">{getSourceLabel(resource.source)}</span>
                  </Badge>
                </div>
                <div className="flex-1 min-w-0">
                  <button onClick={() => handleResourceClick(resource.url, resource.title)} className={cn('w-full text-left group min-h-[48px] p-2 -m-2 rounded', 'focus:outline-none focus:ring-2 focus:ring-blue-500/50', 'hover:bg-slate-600/20 transition-colors duration-200')} aria-label={`Open ${resource.title} - ${resource.description}`} role="link">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <ExternalLink className={cn('w-4 h-4 text-slate-500 flex-shrink-0 mt-0.5', 'group-hover:text-blue-400 transition-colors')}/>
                    </div>
                  </button>
                </div>
              </div>
        ...
        <div className="bg-slate-700/30 rounded-lg border border-slate-600/50 p-3">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" size="sm" onClick={handlePDFDownload} className={cn('bg-slate-600/50 border-slate-500 text-slate-200', 'hover:bg-slate-500/50 hover:border-slate-400', 'min-h-[48px] px-3')} aria-label="Download Civic Engagement Handbook PDF">
        ...
        <div className="space-y-2">
          {MOCK_CIVIC_RESOURCES.slice(3).map((resource) => (<div key={resource.id} className={cn('bg-slate-700/30 rounded-lg border border-slate-600/50 p-3', 'transition-all duration-200 hover:bg-slate-600/40 hover:border-slate-500', hoveredResource === resource.id && 'bg-slate-600/40 border-slate-500')} onMouseEnter={() => handleResourceHover(resource.id)} onMouseLeave={() => handleResourceHover(null)}>
              <div className="flex items-start gap-3">
                <div className="flex-shrink-0 mt-1">
                  <Badge variant="outline" className={cn('text-xs', getSourceColor(resource.source))}>
                    {getSourceIcon(resource.source)}
