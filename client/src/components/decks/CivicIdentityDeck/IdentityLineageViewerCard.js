...
import { History, CheckCircle, XCircle, Shield, User, Clock, Zap, Star, IdCard, Vote, FileText, Link2 } from 'lucide-react';
// Mock DID lineage data with expanded historical entries (10-15 entries)
const MOCK_LINEAGE_ENTRIES = [
    {
        id: 'lineage_001',
...
export const IdentityLineageViewerCard = ({ className }) => {
    const [lineageEntries] = useState(MOCK_LINEAGE_ENTRIES);
    const [selectedEntry, setSelectedEntry] = useState(null);
    const [ttsStatus, setTtsStatus] = useState({ isReady: false, isPlaying: false, lastTrigger: 0 });
    const [renderStartTime] = useState(performance.now());
    const [ttsTriggerCount, setTtsTriggerCount] = useState(0);
    const [hasMismatchDistortion, setHasMismatchDistortion] = useState(false);
    ...
    const narrateLineageEntry = (entry) => {
    ...
    const mostRecentEntry = lineageEntries.length > 0
        ? lineageEntries.reduce((latest, current) => current.timestamp > latest.timestamp ? current : latest)
        : null;
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Identity Lineage Viewer Interface">
      {/* Status Indicator Dot */}
      <div className="absolute top-3 right-3 w-3 h-3 rounded-full bg-blue-500" aria-label="Status: Active" title="Lineage viewer active"/>
      ...
      <CardContent className="space-y-4">
        ...
        {mostRecentEntry && (<div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
            <div className="flex items-center gap-2 mb-3">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className={cn('text-xs', getValidationStatusInfo(mostRecentEntry.validationStatus).color)}>
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <ScrollArea className="h-64 pr-4">
            ...
            return (<button key={entry.id} onClick={() => {
                ...
                }} className={cn('w-full p-3 rounded-lg border transition-all duration-200', 'min-h-[48px] text-left relative', isSelected
                ...
                hasMismatchDistortion && entry.validationStatus === 'mismatch' &&
                    'relative overflow-hidden before:absolute before:inset-0 before:bg-gradient-to-r before:from-transparent before:via-red-400/20 before:to-transparent before:animate-shimmer')} style={{
                    ...
                    transform: hasMismatchDistortion && entry.validationStatus === 'mismatch'
                      ...
                      {/* Timeline connector */}
                      {index < lineageEntries.length - 1 && (<div className="absolute left-6 top-12 w-px h-6 bg-slate-600/50"/>)}
                      
                      <div className="flex items-start justify-between">
                        <div className="flex items-start gap-3">
                          {/* Validation status ring */}
                          <div className={cn('relative p-1.5 rounded-full mt-0.5', validationInfo.bgColor, entry.validationStatus === 'verified' ? 'ring-2 ' + validationInfo.ringColor : '')}>
                            <div className={credentialInfo.color}>
                              {credentialInfo.icon}
                            </div>
                            
                            {/* Pulsing animation for verified entries */}
                            {entry.validationStatus === 'verified' && (<div className="absolute inset-0 rounded-full ring-2 ring-green-400/30 animate-ping"/>)}
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <Badge variant="outline" className={cn('text-xs border-opacity-50', credentialInfo.color.replace('text-', 'border-').replace('-400', '-500/30'), credentialInfo.color.replace('text-', 'bg-').replace('-400', '-500/10'), credentialInfo.color)}>
                                {credentialInfo.narrativeTitle}
                              </Badge>
                              {isRecent && (<Badge variant="outline" className="text-xs bg-cyan-500/20 text-cyan-400 border-cyan-400/50">
                                  Latest
                                </Badge>)}
                            </div>
                      ...
                      {/* Expanded details */}
                      {isSelected && (<div className="mt-3 pt-3 border-t border-slate-600/50 space-y-2">
          ...
          <div className="space-y-2 text-xs">
            ...
            <div className="pt-2 border-t border-slate-600/50">
              ...
              <div className="flex flex-wrap gap-1">
                {Array.from(new Set(lineageEntries.map(entry => getCredentialTypeInfo(entry.credentialType).narrativeTitle))).map(title => (<Badge key={title} variant="outline" className="text-xs bg-slate-700/30 text-slate-300 border-slate-600">
                    {title}
                  </Badge>))}
              </div>
            </div>
