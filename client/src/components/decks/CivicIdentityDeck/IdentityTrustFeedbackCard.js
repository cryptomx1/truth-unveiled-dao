...
export const IdentityTrustFeedbackCard = ({ userDID = "did:civic:user123", credentialCount = 3, verificationLevel = 'civic-grade' }) => {
    ...
    const handleVoteCast = (voteType, zkpHash) => {
    ...
    return (<div className="max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-4">
      ...
      <div className="mb-4">
        <SentimentAggregationCard deckId="civic_identity" deckName="Identity Verification Trust"/>
      </div>

      {/* Action Buttons */}
      <div className="space-y-2">
        <button onClick={() => setShowVoteCard(!showVoteCard)} className={`w-full flex items-center justify-center gap-2 py-2 px-3 rounded-lg border transition-colors ${showVoteCard
        ...
        <button onClick={() => setShowHistoryPanel(true)} className="w-full flex items-center justify-center gap-2 py-2 px-3 bg-slate-700/20 hover:bg-slate-700/40 border border-slate-600 text-slate-300 hover:text-white rounded-lg transition-colors">
      ...
      {/* Trust Vote Card */}
      {showVoteCard && (<div className="mt-4 p-3 bg-slate-700/10 border border-slate-600 rounded-lg">
      ...
      <FeedbackHistoryPanel isOpen={showHistoryPanel} onClose={() => setShowHistoryPanel(false)} onReplayFeedback={(entry) => {
