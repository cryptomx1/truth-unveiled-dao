...
export const ParticipationStreakVisual = ({ streak = MOCK_STREAK, className }) => {
    ...
    return (<div className={cn('bg-slate-800 border border-slate-700 rounded-xl p-6 shadow-xl max-h-[600px] w-full', className)} role="region" aria-label="Participation Streak Visualization" data-testid="participation-streak-visual">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        ...
        {/* TTS Control */}
        <Button onClick={handleTTSToggle} variant="ghost" size="sm" className="text-slate-400 hover:text-white hover:bg-slate-700" aria-label={ttsStatus.isPlaying ? "Stop motivation message" : "Play motivation message"} disabled={!ttsStatus.isReady} style={{ minHeight: '32px', minWidth: '32px' }}>
      ...
      <div className="bg-slate-900 rounded-lg p-4 border border-slate-700 mb-6">
        <div className="flex items-end justify-between h-32 px-2">
          {streakData.map((item, index) => (<div key={item.day} className="flex flex-col items-center flex-1">
              <div className={cn('w-6 rounded-t transition-all duration-300 mb-2', item.active
                ? 'bg-gradient-to-t from-green-600 to-green-400'
                : 'bg-slate-600')} style={{ height: `${item.height}px` }} aria-label={`${item.day}: ${item.active ? 'Active' : 'Inactive'}`}/>
              <span className={cn('text-xs font-medium', item.active ? 'text-green-400' : 'text-slate-500')}>
                {item.day}
              </span>
            </div>))}
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-slate-900 rounded-lg p-4 border border-slate-700">
          ...
          <div className="w-full bg-slate-700 rounded-full h-2">
            <div className="bg-gradient-to-r from-green-500 to-emerald-600 h-2 rounded-full transition-all duration-300" style={{ width: `${Math.min((streak.currentStreak / streak.longestStreak) * 100, 100)}%` }}/>
          </div>
        </div>
