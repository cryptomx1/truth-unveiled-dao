...
export default function OutcomeVerificationCard() {
    ...
    const [chainProof, setChainProof] = useState(null);
    ...
    useEffect(() => {
        const failureTimer = setInterval(() => {
            ...
            const failedOutcomes = outcomes.filter(o => o.status === "unverified" || o.status === "disputed" || o.confidenceScore < 50).length;
    ...
    const generateChainProof = (outcome) => {
    ...
    const handleVerifyOutcome = async () => {
        ...
        const newStatus = newConfidenceScore >= 80 ? "verified" :
        ...
        setTasks(prev => prev.map(task => task.outcomeId === selectedOutcome
    ...
    const getVariancePercentage = (expected, verified) => {
        if (expected === 0)
            return 0;
    ...
    return (<Card className="w-full max-w-md mx-auto h-[600px] bg-slate-800 border-slate-700 text-white">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={selectedOutcome} onValueChange={setSelectedOutcome}>
            ...
            <SelectContent className="bg-slate-700 border-slate-600">
              {outcomes.filter(o => o.status === "pending" || o.status === "partial").map(outcome => (<SelectItem key={outcome.id} value={outcome.id} className="text-white hover:bg-slate-600">
        ...
        {/* Chain Proof Display */}
        {selectedOutcomeData && (<div className="space-y-2">
            ...
            <div className="p-2 bg-slate-700/50 rounded-md border border-slate-600">
              <div className="space-y-1 text-xs">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Badge className={`text-xs ${selectedOutcomeData.confidenceScore >= 80 ? "bg-green-600" :
                selectedOutcomeData.confidenceScore >= 50 ? "bg-yellow-600" : "bg-red-600"} text-white`}>
        ...
        {/* Verification Action */}
        <div className="space-y-2">
          <Button onClick={handleVerifyOutcome} disabled={!canVerify} className="w-full h-10 bg-purple-600 hover:bg-purple-700 disabled:bg-slate-600" aria-label="Verify outcome">
            {isVerifying ? (<>
          ...
          {!canVerify && !isVerifying && (<div className="text-xs text-amber-400 text-center" role="alert" aria-live="polite">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-40">
            ...
            return (<div key={outcome.id} className="p-2 rounded-md bg-slate-700/50 border border-slate-600">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                    ...
                    <div className="mt-1 text-xs text-slate-300">
                      ...
                      {outcome.verifiedOutcome > 0 && (<div className="flex items-center justify-between text-slate-500">
                          <span>Variance: {variance.toFixed(1)}%</span>
                          <span>{outcome.status.charAt(0).toUpperCase() + outcome.status.slice(1)}</span>
                        </div>)}
                    </div>

                    {relatedTask && (<div className="mt-1 text-xs text-slate-500">
                        <div className="flex items-center gap-1">
                          <span>Task: {relatedTask.taskType.replace('_', ' ')}</span>
                          <Badge variant="secondary" className={`text-xs ${relatedTask.status === "completed" ? "bg-green-700" :
                        relatedTask.status === "failed" ? "bg-red-700" :
                            relatedTask.status === "in_progress" ? "bg-blue-700" : "bg-slate-700"} text-white`}>
                            {relatedTask.status}
                          </Badge>
                        </div>
                      </div>)}
