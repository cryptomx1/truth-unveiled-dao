...
export default function SustainabilityAllocationCard() {
    ...
    const [totalBudget] = useState(100);
    const [isAllocating, setIsAllocating] = useState(false);
    const [allocationHistory, setAllocationHistory] = useState(mockHistory);
    const [equityMetrics, setEquityMetrics] = useState({
        overall: 85.7,
        energy: 88.2,
        housing: 76.4,
        food: 89.1,
    ...
    const [equityMismatch, setEquityMismatch] = useState(0);
    ...
    const calculateEquityScore = (cats) => {
    ...
    const updateAllocation = (categoryId, newValue) => {
    ...
    const getCategoryStatus = (category) => {
        ...
        if (deviation > 0.3)
            return "bg-red-600"; // High deviation
        if (deviation > 0.15)
            return "bg-amber-600"; // Moderate deviation
      ...
      <CardContent className="space-y-4">
            ...
            <div className="space-y-3 pr-2">
              {categories.map((category) => (<div key={category.id} className="space-y-2">
                  ...
                  <div className="flex items-center gap-3">
                    <Slider value={[category.allocation]} onValueChange={(value) => updateAllocation(category.id, value)} max={category.maxAllocation} min={0} step={1} className="flex-1" aria-label={`${category.name} allocation slider`}/>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary" className={`text-xs ${currentEquityScore >= 90 ? "bg-green-700" :
            currentEquityScore >= 80 ? "bg-yellow-700" :
                currentEquityScore >= 70 ? "bg-orange-700" : "bg-red-700"} text-white`}>
              {currentEquityScore.toFixed(1)}%
            </Badge>
          </div>

          <div className="grid grid-cols-2 gap-2 text-xs">
            {Object.entries(equityMetrics).filter(([key]) => key !== 'overall').map(([key, value]) => (<div key={key} className="flex items-center justify-between p-2 rounded bg-slate-700/50">
                <span className="text-slate-300 capitalize">{key}</span>
                <div className="flex items-center gap-2">
                  <Progress value={value} className="w-12 h-2" aria-label={`${key} equity progress`}/>
        ...
        {/* Allocation Action */}
        <div className="space-y-2">
          <Button onClick={handleAllocateResources} disabled={!canAllocate || isAllocating} className="w-full h-10 bg-green-600 hover:bg-green-700 disabled:bg-slate-600" aria-label="Allocate resources">
            {isAllocating ? (<>
          ...
          {!canAllocate && (<div className="text-xs text-amber-400 text-center" role="alert" aria-live="polite">
              {remainingBudget > 0
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-24">
            <div className="space-y-1">
              {allocationHistory.slice(0, 3).map((allocation) => (<div key={allocation.id} className="p-2 rounded-md bg-slate-700/50 border border-slate-600">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${allocation.equityScore >= 90 ? "bg-green-500" :
                allocation.equityScore >= 80 ? "bg-yellow-500" :
                    allocation.equityScore >= 70 ? "bg-orange-500" : "bg-red-500"}`}/>
                      <Badge className={`text-xs ${allocation.equityScore >= 90 ? "bg-green-600" :
                allocation.equityScore >= 80 ? "bg-yellow-600" :
                    allocation.equityScore >= 70 ? "bg-orange-600" : "bg-red-600"} text-white`}>
        ...
        {/* Pushback Alert */}
        {showPathB && (<div className="p-2 bg-amber-900/50 border border-amber-600 rounded-md animate-pulse" role="alert" aria-live="polite">
            <div className="flex items-center gap-2">
