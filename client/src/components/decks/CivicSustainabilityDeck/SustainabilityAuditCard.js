...
export default function SustainabilityAuditCard() {
    ...
    const [unverifiedRate, setUnverifiedRate] = useState(0);
    ...
    const handleExecuteAudit = async () => {
        ...
        const newResult = newConfidence >= 85 ? "pass" :
            newConfidence >= 60 ? "partial" :
                newConfidence >= 30 ? "fail" : "fail";
        const newStatus = auditSuccess ? "audited" : "failed";
        const updatedAudit = {
            ...audit,
            status: newStatus,
        ...
        setTasks(prev => prev.map(task => task.auditId === selectedAudit
    ...
    return (<Card className="w-full max-w-md mx-auto h-[600px] bg-slate-800 border-slate-700 text-white">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={selectedAudit} onValueChange={setSelectedAudit}>
            ...
            <SelectContent className="bg-slate-700 border-slate-600">
              {auditablAudits.map(audit => (<SelectItem key={audit.id} value={audit.id} className="text-white hover:bg-slate-600">
        ...
        {/* ZKP Chain Display */}
        {selectedAuditData && (<div className="space-y-2">
            ...
            <div className="p-2 bg-slate-700/50 rounded-md border border-slate-600">
              <div className="space-y-1 text-xs">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Badge className={`text-xs ${selectedAuditData.confidence >= 80 ? "bg-green-600" :
                selectedAuditData.confidence >= 60 ? "bg-yellow-600" : "bg-red-600"} text-white`}>
        ...
        {/* Audit Execution */}
        <div className="space-y-2">
          <Button onClick={handleExecuteAudit} disabled={!canAudit} className="w-full h-10 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-600" aria-label="Execute audit">
            {isAuditing ? (<>
          ...
          {!canAudit && !isAuditing && (<div className="text-xs text-amber-400 text-center" role="alert" aria-live="polite">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-40">
            ...
            return (<div key={audit.id} className="p-2 rounded-md bg-slate-700/50 border border-slate-600">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
