...
import { Crown, Shield, Heart, Palette, Bird, Atom, FileText, Scale, Eye, Sparkles, ChevronDown, ChevronUp } from 'lucide-react';
import { cn } from '@/lib/utils';
import RotatingTokenDisplay from '../../tokens/RotatingTokenDisplay';
// Guardian icons mapping
const GUARDIAN_ICONS = {
...
export default function VaultGuardianDisplay({ ownedCoins = [], compact = false, maxDisplay = 4, onGuardianClick, className }) {
    const [isExpanded, setIsExpanded] = useState(false);
    const [selectedCoin, setSelectedCoin] = useState(null);
    // TTS for mini narration
    const speakGuardianName = useCallback((guardianName) => {
    ...
    const handleGuardianSelect = useCallback((coin) => {
    ...
    if (ownedCoins.length === 0) {
        return (<Card className={cn("bg-slate-800 border-slate-700", className)}>
      ...
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {genesisCount > 0 && (<Badge variant="outline" className="text-xs text-gold-400 border-gold-400">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2">
            {displayCoins.map((coin) => {
                ...
                return (<div key={coin.id} className={cn("flex-shrink-0 cursor-pointer transition-transform hover:scale-110", coin.isJasmyGenesis && "relative")} onClick={() => handleGuardianSelect(coin)} role="button" tabIndex={0} aria-label={`Guardian ${coin.guardian}`}>
                  <div className="relative">
                    <RotatingTokenDisplay frontCID={coin.frontImageCID} backCID={coin.backImageCID} size={48} guardian={coin.guardian} isGenesis={coin.isJasmyGenesis}/>
                    
                    {/* Guardian badge overlay */}
                    <div className={cn("absolute -top-1 -right-1 w-4 h-4 rounded-full flex items-center justify-center text-xs", coin.isJasmyGenesis ? "bg-gold-400 text-black" : "bg-slate-700 text-white")}>
                      {guardian?.avatar || '🛡️'}
                    </div>
                    
                    {coin.isJasmyGenesis && (<Sparkles className="absolute -top-2 -left-2 w-3 h-3 text-gold-400 animate-pulse"/>)}
                  </div>
                </div>);
            })}
            
            {hasMore && !isExpanded && (<Button variant="ghost" size="sm" onClick={() => setIsExpanded(true)} className="flex-shrink-0 text-slate-400 hover:text-white">
        ...
        <div className="grid grid-cols-2 gap-3">
            {displayCoins.map((coin) => {
                const guardian = GUARDIAN_ICONS[coin.guardian];
                return (<div key={coin.id} className={cn("group cursor-pointer rounded-lg p-3 transition-all duration-200 hover:bg-slate-700/50", selectedCoin?.id === coin.id && "bg-slate-700 ring-1 ring-blue-400", coin.isJasmyGenesis && "bg-gradient-to-br from-slate-800 to-amber-900/20")} onClick={() => handleGuardianSelect(coin)} role="button" tabIndex={0} aria-label={`Guardian ${coin.guardian} - ${coin.name}`}>
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <RotatingTokenDisplay frontCID={coin.frontImageCID} backCID={coin.backImageCID} size={56} guardian={coin.guardian} isGenesis={coin.isJasmyGenesis}/>
                      
                      {coin.isJasmyGenesis && (<div className="absolute -top-1 -right-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {coin.isJasmyGenesis && (<div className="flex items-center gap-1 mt-1">
        ...
        {hasMore && !compact && (<div className="mt-3 text-center">
            <Button variant="ghost" size="sm" onClick={() => setIsExpanded(!isExpanded)} className="text-slate-400 hover:text-white">
              {isExpanded ? (<>
        ...
        {selectedCoin && !compact && (<div className="mt-4 pt-4 border-t border-slate-600">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-white">
                {selectedCoin.guardian} Details
              </span>
              <Button variant="ghost" size="sm" onClick={() => setSelectedCoin(null)} className="text-slate-400 hover:text-white p-1">
