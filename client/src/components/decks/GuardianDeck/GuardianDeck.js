...
import { Crown, Shield, Heart, Palette, Bird, Atom, FileText, Scale, Sparkles, Eye, Star } from 'lucide-react';
...
export default function GuardianDeck({ userAddress = '0x1234567890123456789012345678901234567890', testMode = false }) {
    ...
    const speakGuardianAwakening = useCallback((guardian) => {
        if ('speechSynthesis' in window) {
            const utterance = new SpeechSynthesisUtterance(`${guardian.avatar} ${guardian.name} — ${guardian.mythicTitle}, ${guardian.awakening}.`);
            utterance.rate = 0.9;
            utterance.pitch = 1.1;
    ...
    const getGuardianArchetype = (guardianName) => {
    ...
    const renderGuardianGrid = () => {
        ...
        {ownedCoins.map((coin) => {
                ...
                if (!guardian)
                    return null;
                return (<Card key={coin.id} className={cn("bg-slate-800 border-slate-700 cursor-pointer transition-all duration-300 hover:scale-105", selectedCoin?.id === coin.id && "ring-2 ring-blue-400 bg-slate-750", coin.isJasmyGenesis && "border-gold-400 bg-gradient-to-br from-slate-800 to-amber-900/20")} onClick={() => handleGuardianSelect(coin)}>
              ...
              <CardContent className="text-center">
                <div className="mb-4">
                  <RotatingTokenDisplay frontCID={coin.frontImageCID} backCID={coin.backImageCID} size={120} guardian={coin.guardian} isGenesis={coin.isJasmyGenesis}/>
                </div>
                ...
                {coin.isJasmyGenesis && (<div className="mt-3 flex items-center justify-center gap-2">
    ...
    const renderDetailedView = () => {
        ...
        if (!guardian)
            return null;
        return (<Card className="bg-slate-800 border-slate-700 mt-6">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button variant="ghost" size="sm" onClick={() => setShowingBack(!showingBack)} className="text-slate-400">
        ...
        <CardContent>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="text-center">
              <RotatingTokenDisplay frontCID={selectedCoin.frontImageCID} backCID={selectedCoin.backImageCID} size={200} guardian={selectedCoin.guardian} isGenesis={selectedCoin.isJasmyGenesis} key={`${selectedCoin.id}-${showingBack}`}/>
            </div>
    ...
    return (<div className="min-h-screen bg-slate-900 text-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {celebrationActive && (<div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center">
            <div className="text-center animate-pulse">
              ...
              <div className="flex items-center justify-center gap-2 mt-4">
                {[...Array(8)].map((_, i) => (<Sparkles key={i} className="w-6 h-6 text-gold-400 animate-ping" style={{ animationDelay: `${i * 100}ms` }}/>))}
              </div>
            </div>
          </div>)}
