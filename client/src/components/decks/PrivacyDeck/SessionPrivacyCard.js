...
export const SessionPrivacyCard = ({ className }) => {
    ...
    useEffect(() => {
        if (!privacySettings.keyRotation)
            return;
    ...
    const playToggleConfirmation = (feature, enabled) => {
    ...
    const handleIPMaskingToggle = (enabled) => {
    ...
    const handleSessionObfuscationToggle = (enabled) => {
        const toggleStart = performance.now();
    ...
    const handleKeyRotationToggle = (enabled) => {
        const toggleStart = performance.now();
    ...
    const getOverallStatus = () => {
        ...
        if (activeCount === 3)
            return { status: 'Fully Protected', color: 'text-blue-400' };
        if (activeCount === 2)
            return { status: 'Partially Protected', color: 'text-amber-400' };
        if (activeCount === 1)
            return { status: 'Minimal Protection', color: 'text-amber-400' };
        return { status: 'Unprotected', color: 'text-red-400' };
    };
    const overallStatus = getOverallStatus();
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Session Privacy Controls">
      ...
      <CardContent className="space-y-5">
        {/* Overall Status */}
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4 text-center" aria-live="polite" aria-label="Overall privacy status">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <Switch checked={privacySettings.ipMasking} onCheckedChange={handleIPMaskingToggle} className="data-[state=checked]:bg-blue-500" aria-label="Toggle IP masking"/>
            </div>
          ...
          <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                {privacySettings.sessionObfuscation ? (<EyeOff className="w-4 h-4 text-blue-400"/>) : (<Eye className="w-4 h-4 text-slate-400"/>)}
                <span className="font-medium text-slate-100">Session Obfuscation</span>
              </div>
              <Switch checked={privacySettings.sessionObfuscation} onCheckedChange={handleSessionObfuscationToggle} className="data-[state=checked]:bg-blue-500" aria-label="Toggle session obfuscation"/>
            </div>
          ...
          <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <Switch checked={privacySettings.keyRotation} onCheckedChange={handleKeyRotationToggle} className="data-[state=checked]:bg-blue-500" aria-label="Toggle automatic key rotation"/>
            </div>
            ...
            <div className="flex items-center gap-2 mt-3">
              ...
              <Button onClick={handleManualKeyRotation} variant="outline" size="sm" disabled={!privacySettings.keyRotation} className={cn('min-h-[48px] px-3 flex-1', 'bg-slate-700/50 border-slate-600 text-slate-200', 'hover:bg-slate-600/70 hover:text-slate-50', 'disabled:bg-slate-800/50 disabled:text-slate-500')} aria-label="Manually rotate encryption keys">
