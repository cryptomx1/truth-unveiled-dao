...
export const VaultAccessCard = ({ className }) => {
    const [vaultFiles] = useState(MOCK_VAULT_FILES);
    ...
    const renderBlurredPreview = (file) => {
        if (file.type === 'image') {
            return (<div className="relative bg-slate-700/50 rounded-lg h-40 flex items-center justify-center">
        ...
        if (file.type === 'audio') {
            return (<div className="bg-slate-700/50 rounded-lg p-4">
        ...
        if (file.type === 'pdf') {
          ...
          <div className="space-y-2">
            {[1, 2, 3].map((page) => (<div key={page} className="relative bg-slate-800/50 rounded h-16 flex items-center justify-center">
                <div className="absolute inset-0 bg-slate-900/60 rounded flex items-center justify-center">
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Secure File Vault Access">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50">
          <ScrollArea className="h-64 p-4">
            <div className="space-y-3" aria-live="polite" aria-label="Vault file list">
              {vaultFiles.map((file) => (<div key={file.id} className="bg-slate-700/30 rounded-lg border border-slate-600/50 p-3 cursor-pointer hover:bg-slate-600/30 transition-colors" onClick={() => handleFilePreview(file)} role="button" tabIndex={0} aria-label={`Preview ${file.name}`} onKeyPress={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-shrink-0 flex items-center gap-2">
                      ...
                      {file.isLocked && (<Lock className="w-4 h-4 text-amber-400"/>)}
                    </div>
                  </div>
                </div>))}
            </div>
        ...
        {/* File Preview Modal */}
        {selectedFile && (<div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Eye className="w-4 h-4 text-blue-400"/>
                <span className="text-sm font-medium text-slate-100">Preview</span>
              </div>
              <Button onClick={handleClosePreview} variant="ghost" size="sm" className="min-h-[48px] px-3 text-slate-400 hover:text-slate-200" aria-label="Close preview">
            ...
            <div className="mt-3 pt-3 border-t border-slate-700/50">
              ...
              <div className="flex gap-2 mt-3">
                <Button variant="outline" size="sm" disabled className={cn('flex-1 min-h-[48px]', 'bg-slate-700/50 border-slate-600 text-slate-500', 'cursor-not-allowed')} aria-label="Download file (requires ZKP authentication)">
                  <Download className="w-3 h-3 mr-2"/>
                  Download
                </Button>
                <Button variant="outline" size="sm" disabled className={cn('flex-1 min-h-[48px]', 'bg-slate-700/50 border-slate-600 text-slate-500', 'cursor-not-allowed')} aria-label="Decrypt file (requires ZKP authentication)">
        ...
        {!selectedFile && (<div className="space-y-3">
            <Button variant="outline" className={cn('w-full min-h-[48px] font-medium', 'bg-slate-700/50 border-slate-600 text-slate-200', 'hover:bg-slate-600/70 hover:text-slate-50')} disabled aria-label="Upload new file (requires ZKP authentication)">
              <Vault className="w-4 h-4 mr-2"/>
              Upload to Vault
            </Button>
            
            <Button variant="outline" className={cn('w-full min-h-[48px] font-medium', 'bg-slate-700/50 border-slate-600 text-slate-200', 'hover:bg-slate-600/70 hover:text-slate-50')} disabled aria-label="Manage vault access (requires ZKP authentication)">
