...
export const ZKPStatusCard = ({ className }) => {
    ...
    const getAnonymityLevelColor = (level) => {
        if (level >= 95)
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Zero-Knowledge Proof Privacy Status">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4" aria-live="polite" aria-label="Current privacy protection status">
          <div className="flex items-center justify-between mb-3">
            ...
            <Switch checked={privacyStatus === 'active'} onCheckedChange={handleStatusToggle} className="data-[state=checked]:bg-blue-500" aria-label="Toggle privacy protection"/>
          </div>
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              {showProofTrace ? (<Eye className="w-4 h-4 text-blue-400"/>) : (<EyeOff className="w-4 h-4 text-slate-400"/>)}
              <span className="text-sm font-medium text-slate-100">
                Proof Trace
              </span>
            </div>
            <Button onClick={handleToggleProofTrace} variant="outline" size="sm" className={cn('min-h-[48px] px-4', 'bg-slate-700/50 border-slate-600 text-slate-200', 'hover:bg-slate-600/70 hover:text-slate-50')} aria-label={showProofTrace ? 'Hide proof trace' : 'Show proof trace'}>
              {showProofTrace ? 'Hide' : 'Show'}
            </Button>
          </div>
          
          {showProofTrace && (<div className="space-y-2 mt-3 pt-3 border-t border-slate-700/50">
        ...
        {/* Privacy Actions */}
        <div className="space-y-3">
          <Button variant="outline" className={cn('w-full min-h-[48px] font-medium', 'bg-slate-700/50 border-slate-600 text-slate-200', 'hover:bg-slate-600/70 hover:text-slate-50')} disabled={privacyStatus === 'disabled'} aria-label="Generate new zero-knowledge proof">
            <Shield className="w-4 h-4 mr-2"/>
            Generate New Proof
          </Button>
          
          <Button variant="outline" className={cn('w-full min-h-[48px] font-medium', 'bg-slate-700/50 border-slate-600 text-slate-200', 'hover:bg-slate-600/70 hover:text-slate-50')} disabled={privacyStatus === 'disabled'} aria-label="Verify current privacy status">
        ...
        {/* Status Message */}
        {privacyStatus === 'active' && (<div className="text-center pt-2 border-t border-slate-700/50">
            <p className="text-xs text-green-400">
              Your identity and transactions are fully protected
            </p>
          </div>)}
        
        {privacyStatus === 'warning' && (<div className="text-center pt-2 border-t border-slate-700/50">
            <p className="text-xs text-amber-400">
              Privacy protection requires attention
            </p>
          </div>)}
        
        {privacyStatus === 'disabled' && (<div className="text-center pt-2 border-t border-slate-700/50">
