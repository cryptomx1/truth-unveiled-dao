...
export const Deck15Module4Subpage2 = ({ subpageId = 'system-performance', monitoringLevel = 'advanced' }) => {
    const { isAuthenticated, cidStatus, validateCID } = useCIDAuthentication();
    const [selectedMetric, setSelectedMetric] = useState('');
    if (!isAuthenticated) {
        return (<CIDAuthenticationGate onValidate={validateCID} title="System Performance Monitoring" description="Advanced system performance metrics require CID authentication for administrative access."/>);
    }
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {systemComponents.map((component, index) => (<Card key={index} className="border border-gray-200 dark:border-gray-700">
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {performanceAlerts.map((alert) => (<div key={alert.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded">
