...
import { RippleCampaignEngine } from '@/agents/RippleCampaignEngine';
import { LLMPromptEmitter } from '@/agents/LLMPromptEmitter';
const PressReplayDashboard = () => {
    ...
    const [selectedTopic, setSelectedTopic] = useState('healthcare');
    const [selectedZips, setSelectedZips] = useState([]);
    const [customMessage, setCustomMessage] = useState('');
    // LLM generation form
    const [llmTopic, setLlmTopic] = useState('healthcare');
    const [llmTone, setLlmTone] = useState('informative');
    const [llmZip, setLlmZip] = useState('78701');
    ...
    const handleCreateCampaign = () => {
        if (!newCampaignName || selectedZips.length === 0)
            return;
        const messageTemplate = customMessage || `${selectedTopic} policy update for your community.`;
        campaignEngine.createCampaign(newCampaignName, selectedZips, selectedTopic, messageTemplate);
    ...
    const getReachProgress = (campaign) => {
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {campaigns.map((campaign) => (<div key={campaign.id} className="border rounded-lg p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        {campaign.status === 'active' && (<Button size="sm" variant="outline" onClick={() => handleCampaignAction(campaign.id, 'pause')}>
                            <Pause className="h-4 w-4"/>
                          </Button>)}
                        {campaign.status === 'paused' && (<Button size="sm" variant="outline" onClick={() => handleCampaignAction(campaign.id, 'resume')}>
                ...
                {campaigns.length === 0 && (<div className="text-center py-8 text-gray-500">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Input value={newCampaignName} onChange={(e) => setNewCampaignName(e.target.value)} placeholder="Healthcare Access Initiative" className="mt-1"/>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    {availableZips.map((zip) => (<label key={zip.code} className="flex items-center space-x-2">
                        <input type="checkbox" checked={selectedZips.includes(zip.code)} onChange={(e) => {
                if (e.target.checked) {
                ...
                <div>
                  ...
                  <Textarea value={customMessage} onChange={(e) => setCustomMessage(e.target.value)} placeholder="Override the default message template..." className="mt-1" rows={3}/>
                </div>
                
                <Button onClick={handleCreateCampaign} disabled={!newCampaignName || selectedZips.length === 0} className="w-full">
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <Select value={llmZip} onValueChange={setLlmZip}>
                    ...
                    <SelectContent>
                      {availableZips.map((zip) => (<SelectItem key={zip.code} value={zip.code}>
                          {zip.code} - {zip.city}
                        </SelectItem>))}
                    </SelectContent>
                  </Select>
                ...
                <Button onClick={handleGenerateLLMMessage} disabled={isGenerating} className="w-full">
                  {isGenerating ? (<>
            ...
            'Using local templates (no API key)'}
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {recentMessages.map((msg) => (<div key={msg.id} className="text-xs p-2 bg-gray-50 rounded">
