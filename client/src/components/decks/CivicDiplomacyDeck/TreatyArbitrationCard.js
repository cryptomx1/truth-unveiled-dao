...
import { CheckCircle2, Clock, AlertTriangle, FileText, Hash, Sparkles, Eye, Scale, ArrowUp, Archive } from "lucide-react";
const mockDisputes = [
    {
        id: "dispute_001",
        treatyId: "treaty_001",
...
export default function TreatyArbitrationCard() {
    ...
    const [arbitrators, setArbitrators] = useState(mockArbitrators);
    ...
    const [unresolvedRate, setUnresolvedRate] = useState(0);
    ...
    const handleDisputeAction = async (actionType) => {
    ...
    const getArbitratorStatusColor = (status) => {
        switch (status) {
            case "Available":
                return "bg-green-500";
            case "Busy":
    ...
    return (<Card className="w-full max-w-md mx-auto h-[600px] bg-slate-800 border-slate-700 text-white">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={selectedDispute} onValueChange={setSelectedDispute}>
            ...
            <SelectContent className="bg-slate-700 border-slate-600">
              {disputes.filter(d => d.status === "open" || d.status === "reviewing").map(dispute => (<SelectItem key={dispute.id} value={dispute.id} className="text-white hover:bg-slate-600">
        ...
        {/* Dispute Details */}
        {selectedDisputeData && (<div className="p-3 bg-slate-700 rounded-md border border-slate-600">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-1">
                ...
                {selectedDisputeData.escalated && (<ArrowUp className="h-3 w-3 text-amber-400"/>)}
              </div>
            </div>
            ...
            <div className="text-xs text-slate-300 space-y-1">
              ...
              {selectedDisputeData.arbitratorDID && (<div><strong>Arbitrator:</strong> {selectedDisputeData.arbitratorDID}</div>)}
            </div>
          </div>)}

        {/* Resolution Interface */}
        {selectedDisputeData && (selectedDisputeData.status === "open" || selectedDisputeData.status === "reviewing") && (<div className="space-y-3">
            <div className="space-y-1">
              <label className="text-xs font-medium text-slate-300">Resolution Details</label>
              <Textarea placeholder="Enter arbitration decision and reasoning..." value={resolutionText} onChange={(e) => setResolutionText(e.target.value)} className="bg-slate-700 border-slate-600 text-white placeholder:text-slate-400 h-20 text-xs resize-none" maxLength={500} aria-label="Resolution details textarea"/>
            ...
            <div className="flex gap-2">
              <Button onClick={() => handleDisputeAction("resolve")} disabled={!resolutionText.trim() || arbitrationStatus === "processing"} className="flex-1 h-9 bg-green-600 hover:bg-green-700 disabled:bg-slate-600" aria-label="Resolve dispute">
              ...
              <Button onClick={() => handleDisputeAction("escalate")} disabled={!resolutionText.trim() || arbitrationStatus === "processing"} className="flex-1 h-9 bg-amber-600 hover:bg-amber-700 disabled:bg-slate-600" aria-label="Escalate dispute">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-32">
            <div className="space-y-1">
              {actions.slice(0, 5).map((action) => (<div key={action.id} className="p-2 rounded-md bg-slate-700/50 border border-slate-600">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${action.action === "Resolved" ? "bg-green-500" :
                action.action === "Escalated" ? "bg-amber-500" :
                    action.action === "Reviewed" ? "bg-blue-500" : "bg-slate-500"}`}/>
                      <Badge className={`text-xs ${action.action === "Resolved" ? "bg-green-600" :
                action.action === "Escalated" ? "bg-amber-600" :
                    action.action === "Reviewed" ? "bg-blue-600" : "bg-slate-600"} text-white`}>
                        {action.action}
                      </Badge>
                    </div>
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-24">
            <div className="space-y-1">
              {arbitrators.map((arbitrator) => (<div key={arbitrator.did} className="p-1 rounded-md bg-slate-700/30 border border-slate-600">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                    ...
                    <Badge variant="secondary" className={`text-xs ${arbitrator.status === "Available" ? "bg-green-700" :
                arbitrator.status === "Busy" ? "bg-amber-700" : "bg-red-700"} text-white`}>
                      {arbitrator.status}
                    </Badge>
                  </div>
                </div>))}
