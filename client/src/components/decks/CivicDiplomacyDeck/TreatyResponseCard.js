...
import { CheckCircle2, Clock, AlertTriangle, FileText, Users, Hash, Sparkles, Edit3, MessageSquare } from "lucide-react";
const mockResponses = [
    {
        id: "response_001",
        treatyId: "treaty_001",
...
export default function TreatyResponseCard() {
    ...
    const [zkpSync, setZkpSync] = useState(false);
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          <div className="space-y-1">
            ...
            <Select value={selectedTreaty} onValueChange={setSelectedTreaty}>
              ...
              <SelectContent className="bg-slate-700 border-slate-600">
                {availableTreaties.map(treaty => (<SelectItem key={treaty.id} value={treaty.id} className="text-white hover:bg-slate-600">
          ...
          <div className="space-y-1">
            ...
            <Select value={responseType} onValueChange={(value) => setResponseType(value)}>
          ...
          {responseType === "modify" && (<div className="space-y-1">
              <label className="text-xs font-medium text-slate-300">Counter-Terms</label>
              <Textarea placeholder="Describe your proposed modifications..." value={counterTerms} onChange={(e) => setCounterTerms(e.target.value)} className="bg-slate-700 border-slate-600 text-white placeholder:text-slate-400 h-20 text-xs resize-none" maxLength={400} aria-label="Counter-terms textarea"/>
              <div className="text-xs text-slate-400 text-right">
                {counterTerms.length}/400
              </div>
            </div>)}

          <Button onClick={handleSubmitResponse} disabled={!selectedTreaty || !responseType || (responseType === "modify" && !counterTerms.trim())} className="w-full h-9 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600" aria-label="Submit treaty response">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-40">
            <div className="space-y-2">
              {responses.map((response) => (<div key={response.id} className={`p-2 rounded-md border cursor-pointer transition-all ${selectedResponse === response.id
                ? "bg-slate-700 border-slate-500"
                : "bg-slate-800 border-slate-600 hover:bg-slate-700"}`} onClick={() => setSelectedResponse(selectedResponse === response.id ? null : response.id)} role="button" tabIndex={0} aria-label={`Response to: ${response.treatyTitle}`} onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  ...
                  {selectedResponse === response.id && (<div className="mt-2 pt-2 border-t border-slate-600">
                      <div className="text-xs text-slate-300 space-y-1">
                        ...
                        {response.counterTerms && (<div className="mt-2">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-20">
            <div className="space-y-1">
              {decisions.slice(0, 3).map((decision) => (<div key={decision.id} className="text-xs text-slate-400 p-1 bg-slate-700/50 rounded">
