...
import { CheckCircle2, Clock, AlertTriangle, X, Users, Hash, Sparkles, Shield, Vote } from "lucide-react";
const mockTreaties = [
    {
        id: "treaty_001",
...
export default function TreatyRatificationCard() {
    ...
    const [ratificationStatus, setRatificationStatus] = useState("idle");
    const [showPathB, setShowPathB] = useState(false);
    const [quorumFailRate, setQuorumFailRate] = useState(0);
    const [didMismatchRate, setDidMismatchRate] = useState(0);
    ...
    const calculateProgress = (treatyId) => {
    ...
    const handleTreatySelection = (treatyId) => {
    ...
    const handleVoteSubmission = async () => {
        ...
        if (updatedProgress.outcome) {
            setTreaties(prev => prev.map(treaty => treaty.id === selectedTreaty
                ? {
                    ...treaty,
    ...
    return (<Card className="w-full max-w-md mx-auto h-[600px] bg-slate-800 border-slate-700 text-white">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={selectedTreaty} onValueChange={handleTreatySelection}>
            ...
            <SelectContent className="bg-slate-700 border-slate-600">
              {treaties.map(treaty => (<SelectItem key={treaty.id} value={treaty.id} className="text-white hover:bg-slate-600">
        ...
        {/* Treaty Preview */}
        {selectedTreatyData && (<div className="p-3 bg-slate-700 rounded-md border border-slate-600">
            ...
            <div className="text-xs text-slate-300 space-y-1">
              ...
              {selectedTreatyData.responderDID && (<div><strong>Responder:</strong> {selectedTreatyData.responderDID}</div>)}
              <div><strong>Original ZKP:</strong> {selectedTreatyData.originalZkpHash}</div>
              {selectedTreatyData.responseZkpHash && (<div><strong>Response ZKP:</strong> {selectedTreatyData.responseZkpHash}</div>)}
              <div><strong>Updated:</strong> {formatDate(selectedTreatyData.lastUpdated)}</div>
            </div>
          </div>)}

        {/* Ratification Progress */}
        {currentProgress && (<div className="space-y-2">
            <div className="flex items-center gap-2">
            ...
            <div className="space-y-2">
              ...
              <Progress value={(currentProgress.ratifyVotes + currentProgress.vetoVotes) / currentProgress.totalRequired * 100} className="h-2"/>
              ...
              {currentProgress.quorumMet && (<div className="flex items-center gap-1 text-xs">
                  <CheckCircle2 className="h-3 w-3 text-green-400"/>
                  <span className="text-green-400">⅔ Quorum Achieved</span>
                </div>)}
              
              {currentProgress.outcome && (<Badge className={`text-xs w-full justify-center ${currentProgress.outcome === "ratified" ? "bg-green-600" : "bg-red-600"} text-white`}>
        ...
        {/* Voting Interface */}
        {selectedTreatyData && !currentProgress?.outcome && (<div className="space-y-3">
            <div className="space-y-2">
              <label className="text-xs font-medium text-slate-300">Cast Your Vote</label>
              <div className="flex gap-2">
                <Button onClick={() => setUserVote("ratify")} variant={userVote === "ratify" ? "default" : "outline"} className={`flex-1 h-9 ${userVote === "ratify"
                ? "bg-green-600 hover:bg-green-700"
                : "border-slate-600 text-slate-300 hover:bg-slate-700"}`} aria-label="Vote to ratify treaty">
                  <CheckCircle2 className="h-4 w-4 mr-1"/>
                  Ratify
                </Button>
                <Button onClick={() => setUserVote("veto")} variant={userVote === "veto" ? "default" : "outline"} className={`flex-1 h-9 ${userVote === "veto"
                ? "bg-red-600 hover:bg-red-700"
                : "border-slate-600 text-slate-300 hover:bg-slate-700"}`} aria-label="Vote to veto treaty">
            ...
            <Button onClick={handleVoteSubmission} disabled={!userVote || ratificationStatus === "processing"} className="w-full h-9 bg-purple-600 hover:bg-purple-700 disabled:bg-slate-600" aria-label="Submit ratification vote">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-32">
            <div className="space-y-1">
              {signers.map((signer) => (<div key={signer.id} className="p-2 rounded-md bg-slate-700/50 border border-slate-600">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${signer.hasVoted
                ...
                : "bg-slate-500"}`}/>
                    ...
                    <div className="text-xs text-slate-400">
                      {signer.hasVoted ? (<div className="flex flex-col items-end">
                          <Badge className={`text-xs ${signer.vote === "ratify" ? "bg-green-600" : "bg-red-600"} text-white`}>
                            {signer.vote}
                          </Badge>
                          {signer.timestamp && (<span className="mt-1">{formatDate(signer.timestamp)}</span>)}
                        </div>) : (<span>Pending</span>)}
                    </div>
                  </div>
                  
                  {signer.zkpSignature && (<div className="mt-1 text-xs text-slate-500">
