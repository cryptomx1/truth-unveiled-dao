...
export default function TreatyProposalCard() {
    const [treaties, setTreaties] = useState(mockTreaties);
    ...
    const [currentStatus, setCurrentStatus] = useState("drafting");
    ...
    return (<Card className="w-full max-w-md mx-auto h-[600px] bg-slate-800 border-slate-700 text-white">
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-lg">
          <Globe className="h-5 w-5 text-blue-400"/>
          Treaty Proposal
          {showPathB && (<div className="ml-auto animate-pulse">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          <div className="space-y-1">
            ...
            <Input placeholder="Enter treaty title..." value={formData.title} onChange={(e) => setFormData(prev => ({ ...prev, title: e.target.value }))} className="bg-slate-700 border-slate-600 text-white placeholder:text-slate-400 h-8" maxLength={100} aria-label="Treaty title input"/>
          </div>
          ...
          <div className="space-y-1">
            ...
            <Select value={formData.scope} onValueChange={(value) => setFormData(prev => ({ ...prev, scope: value }))}>
              ...
              <SelectContent className="bg-slate-700 border-slate-600">
                {scopeOptions.map(option => (<SelectItem key={option.value} value={option.value} className="text-white hover:bg-slate-600">
          ...
          <div className="space-y-1">
            ...
            {formData.parties.map((party, index) => (<div key={index} className="flex gap-1">
                <Input placeholder={`Party ${index + 1}...`} value={party} onChange={(e) => updateParty(index, e.target.value)} className="bg-slate-700 border-slate-600 text-white placeholder:text-slate-400 h-8" maxLength={50} aria-label={`Party ${index + 1} input`}/>
                {formData.parties.length > 2 && (<Button variant="outline" size="sm" onClick={() => removeParty(index)} className="h-8 px-2 bg-slate-700 border-slate-600 hover:bg-slate-600" aria-label={`Remove party ${index + 1}`}>
                    ×
                  </Button>)}
              </div>))}
            {formData.parties.length < 5 && (<Button variant="outline" size="sm" onClick={addParty} className="h-7 text-xs bg-slate-700 border-slate-600 hover:bg-slate-600" aria-label="Add party">
                + Add Party
              </Button>)}
          </div>
          ...
          <div className="space-y-1">
            ...
            <Textarea placeholder="Describe treaty terms and conditions..." value={formData.terms} onChange={(e) => setFormData(prev => ({ ...prev, terms: e.target.value }))} className="bg-slate-700 border-slate-600 text-white placeholder:text-slate-400 h-16 text-xs resize-none" maxLength={500} aria-label="Treaty terms textarea"/>
          ...
          <div className="space-y-1">
            ...
            <Input placeholder="e.g., 5 years, indefinite..." value={formData.duration} onChange={(e) => setFormData(prev => ({ ...prev, duration: e.target.value }))} className="bg-slate-700 border-slate-600 text-white placeholder:text-slate-400 h-8" maxLength={30} aria-label="Treaty duration input"/>
          </div>

          <Button onClick={handleSubmit} disabled={!formData.title || !formData.scope || !formData.terms || !formData.duration || formData.parties.filter(p => p.trim()).length < 2} className="w-full h-9 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600" aria-label="Submit treaty proposal">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-32">
            <div className="space-y-2">
              {treaties.map((treaty) => (<div key={treaty.id} className={`p-2 rounded-md border cursor-pointer transition-all ${selectedTreaty === treaty.id
                ? "bg-slate-700 border-slate-500"
                : "bg-slate-800 border-slate-600 hover:bg-slate-700"}`} onClick={() => setSelectedTreaty(selectedTreaty === treaty.id ? null : treaty.id)} role="button" tabIndex={0} aria-label={`Treaty: ${treaty.title}`} onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  ...
                  {selectedTreaty === treaty.id && (<div className="mt-2 pt-2 border-t border-slate-600">
                      <div className="text-xs text-slate-300 space-y-1">
                        ...
                        {treaty.ratificationRequired && (<div className="flex items-center gap-1 text-amber-400">
        ...
        {/* Pushback Alert */}
        {showPathB && (<div className="p-2 bg-amber-900/50 border border-amber-600 rounded-md" role="alert" aria-live="polite">
            <div className="flex items-center gap-2">
