...
import { Gavel, CheckCircle, XCircle, AlertTriangle, Clock, User, FileText, ArrowUp, TrendingUp, Activity, Zap } from 'lucide-react';
// Mock anomaly cases from LedgerAnomalyScannerCard data
const MOCK_ANOMALY_CASES = [
    {
        id: 'anom_001',
        type: 'gas_spike',
        severity: 'high',
...
export const AuditResolutionPanelCard = ({ className }) => {
    const [selectedCase, setSelectedCase] = useState(null);
    const [resolutionNotes, setResolutionNotes] = useState('');
    const [cases, setCases] = useState(MOCK_ANOMALY_CASES);
    ...
    const playResolutionTTS = (status) => {
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', 'ring-2 ring-amber-500/50 animate-pulse', className)} role="region" aria-label="Audit Resolution Panel">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-3">
          ...
          <ScrollArea className="h-32 rounded-lg border border-slate-700/50 bg-slate-800/30">
            <div className="p-3 space-y-2">
              {pendingCases.map((caseItem) => (<button key={caseItem.id} onClick={() => {
                setSelectedCase(caseItem);
                setResolutionNotes('');
            }} className={cn('w-full p-3 rounded-lg border text-left transition-colors', 'hover:bg-slate-700/30 focus:outline-none focus:ring-2 focus:ring-amber-500/50', 'min-h-[48px]', selectedCase?.id === caseItem.id
                ? 'bg-amber-600/20 border-amber-500/50'
                : 'bg-slate-800/50 border-slate-700/50')} aria-label={`Select case ${caseItem.id}`}>
                  <div className="flex items-center gap-2 mb-1">
                    {getAnomalyIcon(caseItem.type)}
                    <span className="text-xs font-medium text-slate-200">
                      {formatAnomalyType(caseItem.type)}
                    </span>
                    <Badge variant="outline" className={cn('text-xs ml-auto', getSeverityColor(caseItem.severity))}>
              ...
              {pendingCases.length === 0 && (<div className="text-center py-6 text-slate-400">
        ...
        {selectedCase && (<div className="space-y-4">
            ...
            {/* Resolution Notes */}
            {selectedCase.status === 'pending' && (<div className="space-y-2">
                <label className="text-sm font-medium text-slate-200">
                  Resolution Notes
                </label>
                <Textarea value={resolutionNotes} onChange={(e) => setResolutionNotes(e.target.value)} placeholder="Enter resolution details and analysis..." className={cn('bg-slate-700/50 border-slate-600 text-slate-100', 'placeholder:text-slate-400 resize-none', 'focus:border-amber-500 focus:ring-amber-500/20', 'min-h-[80px]')} aria-label="Enter resolution notes"/>
              </div>)}

            {/* Resolution Actions */}
            {selectedCase.status === 'pending' && (<div className="space-y-3">
                <div className="grid grid-cols-3 gap-2">
                  <Button onClick={() => handleResolution('resolved')} className={cn('min-h-[48px] text-xs font-medium', 'bg-green-600 hover:bg-green-700 text-white')} aria-label="Resolve case">
                    <CheckCircle className="w-3 h-3 mr-1"/>
                    Resolve
                  </Button>
                  
                  <Button onClick={() => handleResolution('escalated')} className={cn('min-h-[48px] text-xs font-medium', 'bg-amber-600 hover:bg-amber-700 text-white')} aria-label="Escalate case">
                    <ArrowUp className="w-3 h-3 mr-1"/>
                    Escalate
                  </Button>
                  
                  <Button onClick={() => handleResolution('rejected')} className={cn('min-h-[48px] text-xs font-medium', 'bg-red-600 hover:bg-red-700 text-white')} aria-label="Reject case">
            ...
            {/* Resolution Result */}
            {selectedCase.status !== 'pending' && (<div className="bg-slate-800/50 rounded-lg border border-slate-700/50 p-4" aria-live="polite" aria-label="Resolution result">
                <div className="flex items-center gap-2 mb-3">
                  {getStatusIcon(selectedCase.status)}
                ...
                {selectedCase.resolutionNotes && (<div className="mt-3 pt-3 border-t border-slate-700/50">
                ...
                <div className="mt-3 pt-3 border-t border-slate-700/50">
                  <Button onClick={handleReset} variant="outline" size="sm" className="w-full bg-slate-700/50 border-slate-600 text-slate-200 hover:bg-slate-600/70" aria-label="Reset case to pending status">
                    Reset to Pending
                  </Button>
                </div>
              </div>)}
