...
import { AlertTriangle, Shield, Search, Eye, Clock, TrendingUp, Activity, Zap, CheckCircle, XCircle } from 'lucide-react';
...
export const LedgerAnomalyScannerCard = ({ className }) => {
    ...
    const playAnomalyAlertTTS = (anomalyCount, criticalCount) => {
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', getPulseRingClass(), className)} role="region" aria-label="Ledger Anomaly Scanner">
      ...
      <CardContent className="space-y-5">
        {/* Scanner Status */}
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4" aria-live="polite" aria-label="Scanner status">
          ...
          {scanStatus === 'scanning' && (<div className="space-y-2">
          ...
          {lastScanTime && scanStatus === 'completed' && (<div className="text-xs text-slate-400">
              Last scan: {getRelativeTime(lastScanTime)}
            </div>)}
        </div>

        {/* Scan Metrics */}
        {scanStatus === 'completed' && (<div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
            <div className="flex items-center gap-2 mb-3">
        ...
        {/* Detected Anomalies */}
        {detectedAnomalies.length > 0 && (<div className="space-y-2">
            ...
            <ScrollArea className="h-48 rounded-lg border border-slate-700/50 bg-slate-800/30">
              <div className="p-3 space-y-3">
                {detectedAnomalies.map((anomaly) => (<div key={anomaly.id} className={cn('p-3 rounded-lg border transition-colors', 'hover:bg-slate-700/30', anomaly.severity === 'critical'
                    ...
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        <Badge variant="outline" className={cn('text-xs', getSeverityBadgeColor(anomaly.severity))}>
                          {anomaly.severity.toUpperCase()}
                        </Badge>
                        {anomaly.autoResolved ? (<CheckCircle className="w-3 h-3 text-green-400"/>) : (<XCircle className="w-3 h-3 text-red-400"/>)}
                      </div>
                    </div>
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setIsAutoScanEnabled(!isAutoScanEnabled)} className={cn('relative inline-flex h-6 w-11 items-center rounded-full transition-colors', 'focus:outline-none focus:ring-2 focus:ring-blue-500/50', isAutoScanEnabled ? 'bg-blue-600' : 'bg-slate-600')} aria-label="Toggle auto-scan">
              <span className={cn('inline-block h-4 w-4 transform rounded-full bg-white transition-transform', isAutoScanEnabled ? 'translate-x-6' : 'translate-x-1')}/>
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <Button onClick={runAnomalyScan} disabled={scanStatus === 'scanning'} className={cn('flex-1 min-h-[48px] font-medium', 'bg-blue-600 hover:bg-blue-700 text-white', 'disabled:bg-slate-700/50 disabled:text-slate-500')} aria-label="Start anomaly scan">
            {scanStatus === 'scanning' ? (<>
          ...
          {scanStatus === 'completed' && (<Button onClick={handleResetScan} variant="outline" className={cn('min-h-[48px] px-4', 'bg-slate-700/50 border-slate-600 text-slate-200', 'hover:bg-slate-600/70 hover:text-slate-50')} aria-label="Reset scanner">
              Reset
            </Button>)}
        </div>
