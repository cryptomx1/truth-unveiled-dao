...
const searchAuditChain = (query) => {
    if (!query.trim())
        return null;
    return MOCK_AUDIT_CHAIN.find(entry => entry.proofHash.includes(query.trim()) ||
        entry.circuitId.includes(query.trim()) ||
        entry.sourceDeck.toLowerCase().includes(query.toLowerCase())) || null;
};
...
export const AuditChainOverviewCard = ({ className }) => {
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', 'ring-2 ring-blue-500/50 animate-pulse', className)} role="region" aria-label="Civic Audit Chain Overview">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Enter proof hash, circuit ID, or deck..." onKeyDown={(e) => e.key === 'Enter' && handleSearch()} className={cn('flex-1 bg-slate-700/50 border-slate-600 text-slate-100', 'placeholder:text-slate-400', 'focus:border-blue-500 focus:ring-blue-500/20', 'min-h-[48px]')} aria-label="Search audit chain entries"/>
            <Button onClick={handleSearch} disabled={!searchQuery.trim()} className={cn('min-h-[48px] px-4', 'bg-blue-600 hover:bg-blue-700 text-white', 'disabled:bg-slate-700/50 disabled:text-slate-500')} aria-label="Search audit chain">
              <Search className="w-4 h-4"/>
            </Button>
          </div>
          
          {matchedEntry && (<Button onClick={handleClearSearch} variant="outline" size="sm" className="w-full bg-slate-700/50 border-slate-600 text-slate-200 hover:bg-slate-600/70" aria-label="Clear search results">
              Clear Search
            </Button>)}
        </div>

        {/* Search Result */}
        {matchedEntry && (<div className="bg-slate-800/50 rounded-lg border border-blue-500/50 p-4" aria-live="polite" aria-label="Search result">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-64 rounded-lg border border-slate-700/50 bg-slate-800/30">
            <div className="p-3 space-y-3">
              {MOCK_AUDIT_CHAIN.map((entry) => (<div key={entry.id} className={cn('p-3 rounded-lg border transition-colors', 'hover:bg-slate-700/30', entry === matchedEntry
                ...
                : 'bg-slate-800/50 border-slate-700/50')}>
                  ...
                  <div className="space-y-1 text-xs text-slate-400">
                    ...
                    {entry.gasUsed > 0 && (<div className="flex justify-between">
                        <span>Gas:</span>
