...
import { BarChart3, TrendingUp, Shield, CheckCircle, Eye, Activity, Award, Target } from 'lucide-react';
// Mock transparency metrics aggregated from Modules 1-3
const MOCK_TRANSPARENCY_METRICS = {
    totalAudits: 10,
    verifiedAudits: 7,
    resolvedAnomalies: 2,
    totalAnomalies: 5,
...
const getTrustIndexBadge = (index) => {
    if (index >= 90)
...
export const TransparencyMetricsCard = ({ className }) => {
    const [metrics, setMetrics] = useState(MOCK_TRANSPARENCY_METRICS);
    ...
    useEffect(() => {
        ...
        const updateInterval = setInterval(() => {
            ...
            setMetrics(prev => {
                ...
                newMetrics.trustIndex = (auditSuccessRate * 0.3 +
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', getTrustIndexRing(metrics.trustIndex), className)} role="region" aria-label="Transparency Metrics Dashboard">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={playTrustIndexTTS} disabled={ttsStatus.isPlaying} className="text-slate-400 hover:text-slate-200 transition-colors" aria-label="Play trust index announcement">
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1">
              ...
              <Badge variant="outline" className={cn('text-xs', getTrustIndexBadge(metrics.trustIndex))}>
          ...
          <div className="mt-3">
            <Progress value={metrics.trustIndex} className="h-2" aria-label={`Trust index: ${metrics.trustIndex.toFixed(1)}%`}/>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            ...
            <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-3">
              ...
              <Progress value={auditSuccessRate} className="h-1.5" aria-label={`Audit success rate: ${formatPercentage(auditSuccessRate)}`}/>
            </div>
            ...
            <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-3">
              ...
              <Progress value={anomalyResolutionRate} className="h-1.5" aria-label={`Anomaly resolution rate: ${formatPercentage(anomalyResolutionRate)}`}/>
            </div>
            ...
            <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-3">
              ...
              <Progress value={metrics.zkpValidationRate} className="h-1.5" aria-label={`ZKP validation rate: ${formatPercentage(metrics.zkpValidationRate)}`}/>
            </div>
            ...
            <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-3">
              ...
              <Progress value={metrics.ledgerCoverage} className="h-1.5" aria-label={`Ledger coverage: ${formatPercentage(metrics.ledgerCoverage)}`}/>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setIsLiveMode(!isLiveMode)} className={cn('relative inline-flex h-6 w-11 items-center rounded-full transition-colors', 'focus:outline-none focus:ring-2 focus:ring-blue-500/50', isLiveMode ? 'bg-blue-600' : 'bg-slate-600')} aria-label="Toggle live updates">
              <span className={cn('inline-block h-4 w-4 transform rounded-full bg-white transition-transform', isLiveMode ? 'translate-x-6' : 'translate-x-1')}/>
            </button>
          </div>
        </div>
