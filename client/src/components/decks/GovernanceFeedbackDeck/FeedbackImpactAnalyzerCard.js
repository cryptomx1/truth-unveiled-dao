...
import { Target, TrendingUp, TrendingDown, AlertTriangle, CheckCircle, Activity, BarChart3, Zap, Shield, Users, FileText, Clock } from 'lucide-react';
// Mock cross-deck inputs simulating real data from other components
const MOCK_CROSS_DECK_INPUTS = {
...
const calculateImpactScore = (inputs) => {
    ...
    let policyStatus;
    if (impactScore >= 75 && dissentWeight > 25)
        policyStatus = 'emergency';
    else if (impactScore >= 60)
        policyStatus = 'escalating';
    else if (impactScore >= 40 || amendRatio > 0.15)
        policyStatus = 'revision_likely';
    else
        policyStatus = 'stable';
    ...
    else if (dissentRatio > 0.3)
        sentimentTrend = 'negative';
    else if (amendRatio > 0.2)
...
const getStatusRing = (policyStatus) => {
    switch (policyStatus) {
        case 'stable':
...
const formatSentimentTrend = (trend) => {
    return trend.charAt(0).toUpperCase() + trend.slice(1);
};
const getImpactMessage = (data) => {
...
export const FeedbackImpactAnalyzerCard = ({ className }) => {
    const [crossDeckInputs, setCrossDeckInputs] = useState(MOCK_CROSS_DECK_INPUTS);
    const [impactData, setImpactData] = useState(() => calculateImpactScore(MOCK_CROSS_DECK_INPUTS));
    ...
    const handleRefreshAnalysis = async () => {
        ...
        setTimeout(() => {
            ...
            if (ttsStatus.isReady) {
                const utterance = new SpeechSynthesisUtterance(`Impact analysis shows ${newImpactData.impactScore.toFixed(1)} percent policy effect.`);
                utterance.rate = 0.9;
                speechSynthesis.speak(utterance);
            }
        }, 1500);
    };
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', getStatusRing(impactData.policyStatus), className)} role="region" aria-label="Feedback Impact Analyzer Interface">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="text-center">
            ...
            <Badge variant="outline" className={cn('mb-3', getImpactBadgeColor(impactData.impactLevel))}>
              {impactData.impactLevel.toUpperCase()} IMPACT
            </Badge>
            
            <div className="w-full bg-slate-700/50 rounded-full h-3 mb-3">
              <div className={cn('h-3 rounded-full transition-all duration-500', impactData.impactLevel === 'critical' ? 'bg-red-500' :
            impactData.impactLevel === 'high' ? 'bg-orange-500' :
                impactData.impactLevel === 'medium' ? 'bg-yellow-500' : 'bg-green-500')} style={{ width: `${impactData.impactScore}%` }}/>
            </div>
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="space-y-3">
            <div>
              ...
              <Progress value={impactData.votePassRate} className="h-2 bg-slate-700/50"/>
            </div>
            ...
            <div>
              ...
              <Progress value={impactData.consensusAlignment} className="h-2 bg-slate-700/50"/>
            </div>
            ...
            <div>
              ...
              <Progress value={impactData.dissentLevel} className="h-2 bg-slate-700/50"/>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <Button onClick={handleRefreshAnalysis} disabled={isUpdating} size="sm" className={cn('min-h-[32px] text-xs', 'bg-blue-600/80 hover:bg-blue-700 text-white', 'disabled:bg-slate-700/50 disabled:text-slate-500')}>
            {isUpdating ? (<>
