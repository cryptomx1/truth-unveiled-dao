...
import { BarChart3, TrendingUp, Activity, AlertTriangle, Shield, Hash, CheckCircle, Clock, Smile, Frown, Meh, Users, Zap } from 'lucide-react';
...
const calculateConfidenceLevel = (confidence) => {
    if (confidence >= 0.9)
        return 'Very High';
    if (confidence >= 0.8)
        return 'High';
...
export const SentimentAggregationCard = ({ className }) => {
    const [sentimentMetrics, setSentimentMetrics] = useState(MOCK_SENTIMENT_METRICS);
    ...
    const getOverallStatusRing = () => {
        if (sentimentMetrics.isPushbackActive) {
            return 'ring-2 ring-red-500/50 animate-pulse';
        }
        return getIndexRing(sentimentMetrics.overallIndex);
    };
    const getDissentSurgeColor = () => {
        if (sentimentMetrics.dissentSurgeRate > 20)
            return 'text-red-400';
        if (sentimentMetrics.dissentSurgeRate > 15)
            return 'text-orange-400';
        return 'text-green-400';
    };
    const getVolatilityColor = () => {
        if (sentimentMetrics.volatilityIndex > 20)
            return 'text-red-400';
        if (sentimentMetrics.volatilityIndex > 15)
            return 'text-orange-400';
        return 'text-green-400';
    };
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', getOverallStatusRing(), className)} role="region" aria-label="Sentiment Aggregation Interface">
      ...
      <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                {sentimentMetrics.dissentSurgeRate > 20 && (<AlertTriangle className="w-3 h-3 text-red-400"/>)}
              </div>
            </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                {sentimentMetrics.volatilityIndex > 20 && (<AlertTriangle className="w-3 h-3 text-red-400"/>)}
              </div>
            </div>
            
            <div className="w-full bg-slate-700/50 rounded-full h-2">
              <div className={cn('h-2 rounded-full transition-all duration-300', sentimentMetrics.isPushbackActive ? 'bg-red-500' : 'bg-green-500')} style={{ width: `${100 - Math.max(sentimentMetrics.dissentSurgeRate, sentimentMetrics.volatilityIndex)}%` }}/>
            </div>
            
            {sentimentMetrics.isPushbackActive ? (<div className="p-2 bg-red-500/20 border border-red-500/30 rounded text-xs text-red-400">
                ⚠️ Pushback: &gt;20% sentiment volatility detected
              </div>) : (<div className="p-2 bg-green-500/20 border border-green-500/30 rounded text-xs text-green-400">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="space-y-2">
            {WEIGHTING_SOURCES.map((source, index) => (<div key={index} className="flex justify-between items-center text-xs">
                <div className="flex items-center gap-2">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Button onClick={handleRefreshSentiment} disabled={isUpdating} size="sm" className={cn('min-h-[32px] text-xs', 'bg-blue-600/80 hover:bg-blue-700 text-white', 'disabled:bg-slate-700/50 disabled:text-slate-500')}>
              {isUpdating ? (<>
          ...
          <ScrollArea className="h-48 rounded-lg border border-slate-700/50 bg-slate-800/30" aria-live="polite">
            <div className="p-3 space-y-2">
              {sentimentMetrics.proposals.map((proposal) => (<div key={proposal.proposalId} className="bg-slate-800/50 rounded-lg border border-slate-700/50 p-3">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <Badge variant="outline" className={cn('text-xs', getSentimentBadgeColor(proposal.sentiment))}>
                  ...
                  <div className="flex items-center justify-between text-xs text-slate-400 mt-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {validateZKPHash(proposal.zkpHash) && (<CheckCircle className="w-3 h-3 text-green-400"/>)}
                    </div>
                  </div>
