...
const validateProofHash = (hash, requiredHash) => {
...
export const CivicAssetTransferCard = ({ className }) => {
    const [selectedAsset, setSelectedAsset] = useState(TRANSFERABLE_ASSETS[0]);
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', getPulseRingClass(), className)} role="region" aria-label="Civic Asset Transfer System">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <Select value={selectedAsset.id} onValueChange={(value) => {
            const asset = TRANSFERABLE_ASSETS.find(a => a.id === value);
            if (asset)
                setSelectedAsset(asset);
        }} disabled={transferStatus === 'validating' || transferStatus === 'transferring'}>
            <SelectTrigger className={cn('bg-slate-700/50 border-slate-600 text-slate-100', 'focus:border-blue-500 focus:ring-blue-500/20', 'min-h-[48px]')} aria-label="Select asset to transfer">
              <SelectValue placeholder="Choose asset"/>
            </SelectTrigger>
            <SelectContent className="bg-slate-800 border-slate-700">
              {TRANSFERABLE_ASSETS.map((asset) => (<SelectItem key={asset.id} value={asset.id} className="text-slate-100 focus:bg-slate-700">
        ...
        <div className="space-y-2">
          ...
          <Input value={recipientDID} onChange={(e) => setRecipientDID(e.target.value)} placeholder="did:truth:ABC123..." disabled={transferStatus === 'validating' || transferStatus === 'transferring'} className={cn('bg-slate-700/50 border-slate-600 text-slate-100', 'placeholder:text-slate-400', 'focus:border-blue-500 focus:ring-blue-500/20', 'min-h-[48px]', recipientDID && !validateDID(recipientDID) && 'border-red-500/50')} aria-label="Enter recipient decentralized identifier"/>
          {recipientDID && !validateDID(recipientDID) && (<p className="text-xs text-red-400">
        ...
        <div className="space-y-2">
          ...
          <Input value={proofHash} onChange={(e) => setProofHash(e.target.value)} placeholder="Enter proof hash from Deck #6..." disabled={transferStatus === 'validating' || transferStatus === 'transferring'} className={cn('bg-slate-700/50 border-slate-600 text-slate-100', 'placeholder:text-slate-400', 'focus:border-blue-500 focus:ring-blue-500/20', 'min-h-[48px]', proofHash && !validateProofHash(proofHash, selectedAsset.requiredProofHash) && 'border-red-500/50')} aria-label="Enter zero-knowledge proof hash for authorization"/>
          <div className="text-xs text-slate-500">
            Required: {selectedAsset.requiredProofHash}
          </div>
          {proofHash && !validateProofHash(proofHash, selectedAsset.requiredProofHash) && (<p className="text-xs text-red-400">
              Hash must match the required proof for this asset
            </p>)}
        </div>

        {/* Transfer Status */}
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4" aria-live="polite" aria-label="Transfer status">
          ...
          {(transferStatus === 'validating' || transferStatus === 'transferring') && (<div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Button onClick={handleTransfer} disabled={!canTransfer() || transferStatus === 'validating' || transferStatus === 'transferring'} className={cn('flex-1 min-h-[48px] font-medium', 'bg-blue-600 hover:bg-blue-700 text-white', 'disabled:bg-slate-700/50 disabled:text-slate-500')} aria-label="Initiate secure asset transfer">
          ...
          {(transferStatus === 'completed' || transferStatus === 'failed') && (<Button onClick={handleReset} variant="outline" className={cn('min-h-[48px] px-4', 'bg-slate-700/50 border-slate-600 text-slate-200', 'hover:bg-slate-600/70 hover:text-slate-50')} aria-label="Reset transfer interface">
              Reset
            </Button>)}
        </div>

        {/* Transfer Result */}
        {transferResult && (transferStatus === 'completed' || transferStatus === 'failed') && (<div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
            <div className="flex items-center gap-2 mb-3">
              {transferResult.status === 'success' ? (<CheckCircle className="w-4 h-4 text-green-400"/>) : (<AlertCircle className="w-4 h-4 text-red-400"/>)}
              <span className={cn('text-sm font-medium', transferResult.status === 'success' ? 'text-green-400' : 'text-red-400')}>
                Transfer {transferResult.status === 'success' ? 'Successful' : 'Failed'}
              </span>
            </div>
            
            {transferResult.status === 'success' && (<div className="space-y-3">
