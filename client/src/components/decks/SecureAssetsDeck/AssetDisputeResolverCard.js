...
import { Scale, Gavel, CheckCircle, XCircle, AlertTriangle, Clock } from 'lucide-react';
...
const getLinkedAsset = (hash) => {
    return HASH_TO_ASSET[hash] || 'unknown_asset.dat';
};
const generateClaimantId = () => {
...
const getPulseRingClass = (status) => {
    switch (status) {
        case 'valid':
...
export const AssetDisputeResolverCard = ({ className }) => {
    const [challengedHash, setChallengedHash] = useState('');
    const [disputeType, setDisputeType] = useState('ownership');
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', currentDispute ? getPulseRingClass(currentDispute.status) : 'ring-2 ring-slate-500/50', className)} role="region" aria-label="Asset Dispute Resolution System">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <Input value={challengedHash} onChange={(e) => setChallengedHash(e.target.value)} placeholder="Enter hash from Deck #6 or #7..." disabled={currentDispute?.status === 'pending'} className={cn('bg-slate-700/50 border-slate-600 text-slate-100', 'placeholder:text-slate-400', 'focus:border-blue-500 focus:ring-blue-500/20', 'min-h-[48px]', challengedHash && !validateZKPHash(challengedHash) && 'border-red-500/50')} aria-label="Enter zero-knowledge proof hash to challenge"/>
          {challengedHash && !validateZKPHash(challengedHash) && (<p className="text-xs text-red-400">
              Hash not found in audit trail or signature registry
            </p>)}
          {challengedHash && validateZKPHash(challengedHash) && (<p className="text-xs text-green-400">
        ...
        <div className="space-y-2">
          ...
          <Select value={disputeType} onValueChange={(value) => setDisputeType(value)} disabled={currentDispute?.status === 'pending'}>
            <SelectTrigger className={cn('bg-slate-700/50 border-slate-600 text-slate-100', 'focus:border-blue-500 focus:ring-blue-500/20', 'min-h-[48px]')} aria-label="Select dispute type">
        ...
        {currentDispute && (<div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4" aria-live="polite" aria-label="Current dispute status">
            ...
            {currentDispute.status === 'pending' && (<div className="mt-3 space-y-2">
            ...
            {currentDispute.verdict && (<div className="mt-3 pt-3 border-t border-slate-700/50">
        ...
        <div className="flex gap-2">
          <Button onClick={handleSubmitDispute} disabled={!canSubmit()} className={cn('flex-1 min-h-[48px] font-medium', 'bg-blue-600 hover:bg-blue-700 text-white', 'disabled:bg-slate-700/50 disabled:text-slate-500')} aria-label="Submit asset dispute for resolution">
          ...
          {currentDispute && currentDispute.status !== 'pending' && (<Button onClick={handleReset} variant="outline" className={cn('min-h-[48px] px-4', 'bg-slate-700/50 border-slate-600 text-slate-200', 'hover:bg-slate-600/70 hover:text-slate-50')} aria-label="Reset dispute interface">
              Reset
            </Button>)}
        </div>
