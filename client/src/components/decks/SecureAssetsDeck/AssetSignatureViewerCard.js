...
const verifySignatureHash = (hash) => {
...
export const AssetSignatureViewerCard = ({ className }) => {
    const [selectedSignature, setSelectedSignature] = useState(MOCK_SIGNATURES[0]);
    ...
    useEffect(() => {
        ...
        if (isVerified) {
            ...
            if (ttsStatus.isReady) {
                const utterance = new SpeechSynthesisUtterance(`Asset signature verified. Hash: ${selectedSignature.signatureHash}`);
                utterance.rate = 0.9;
                utterance.pitch = 1.0;
                utterance.volume = 0.8;
                setTtsStatus(prev => ({ ...prev, isPlaying: true }));
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Asset Signature Viewer">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {MOCK_SIGNATURES.map((signature) => (<button key={signature.signatureHash} onClick={() => handleSignatureChange(signature)} className={cn('w-full p-3 rounded-lg border text-left transition-colors', 'hover:bg-slate-700/30 focus:outline-none focus:ring-2 focus:ring-blue-500/50', 'min-h-[48px]', selectedSignature.signatureHash === signature.signatureHash
                ? 'bg-slate-700/50 border-blue-500/50'
                : 'bg-slate-800/30 border-slate-700/50')} aria-label={`View signature for ${signature.assetId}`}>
                <div className="flex items-center justify-between">
        ...
        {/* Verification Status */}
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4" aria-live="polite" aria-label="Signature verification status">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              ...
              <div className={cn('text-sm font-mono bg-slate-700/30 rounded px-2 py-1', getStatusColor(selectedSignature.verificationStatus))}>
        ...
        <div className="relative bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="relative">
            ...
            {/* Verified Watermark */}
            {selectedSignature.verificationStatus === 'verified' && (<div className="absolute inset-0 flex items-center justify-center pointer-events-none">
