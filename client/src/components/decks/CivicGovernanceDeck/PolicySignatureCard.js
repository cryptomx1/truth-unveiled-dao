...
import { FileSignature, CheckCircle, XCircle, AlertTriangle, Crown, User, Scale, Fingerprint, Eye, Zap, Stamp, Shield, Clock, Settings } from 'lucide-react';
...
const getPolicySourceInfo = (source) => {
    switch (source) {
        case 'deck-11':
...
const getSignatureStatusInfo = (status) => {
    switch (status) {
        case 'pending':
            return {
...
export const PolicySignatureCard = ({ className }) => {
    ...
    const [signatureMismatchRate, setSignatureMismatchRate] = useState(0);
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Policy Signature Interface">
      {/* Status Indicator Dot */}
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', pushbackTriggered ? 'bg-red-500 animate-pulse' : 'bg-green-500')} aria-label={pushbackTriggered ? "Status: Alert" : "Status: Active"} title={pushbackTriggered ? `${signatureMismatchRate.toFixed(1)}% signature mismatch detected` : "Signature system active"}/>
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className={cn('border-opacity-50', roleInfo.color.replace('text-', 'border-').replace('-400', '-500/30'), roleInfo.color.replace('text-', 'bg-').replace('-400', '-500/10'), roleInfo.color)}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            {Object.entries(USER_ROLES).map(([role, info]) => (<Button key={role} onClick={() => handleRoleChange(role)} variant={currentUserRole === role ? "default" : "outline"} size="sm" className={cn('flex-1 min-h-[40px]', currentUserRole === role
        ...
        <div className="space-y-3">
          ...
          <Select value={selectedPolicyId} onValueChange={handlePolicyChange}>
            ...
            <SelectContent className="bg-slate-800 border-slate-600">
              {MOCK_POLICIES.map(policy => (<SelectItem key={policy.id} value={policy.id}>
                  <div className="flex items-center gap-2">
                    <span>{policy.title}</span>
                    {policy.requiresBiometric && (<Fingerprint className="w-3 h-3 text-amber-400"/>)}
                  </div>
                </SelectItem>))}
            </SelectContent>
          </Select>
        ...
        {selectedPolicy && (<div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              {selectedPolicy.requiresBiometric && (<div className="flex items-center gap-1 text-amber-400">
        ...
        {/* Signature Status */}
        {signatureData && statusInfo && (<div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className={cn('relative p-2 rounded-full', statusInfo.bgColor, signatureData.status === 'signed' ? 'ring-2 ' + statusInfo.ringColor : '')}>
                {statusInfo.icon}
                
                {/* Animated signature ring for signing process */}
                {signatureData.status === 'signing' && (<div className="absolute inset-0 rounded-full ring-2 ring-blue-400/30 animate-ping"/>)}
                
                {/* Stamp effect for signed documents */}
                {signatureData.status === 'signed' && (<div className="absolute -top-1 -right-1">
              ...
              <div className="flex-1 ml-3">
                ...
                {signatureData.signedAt && (<div className="text-xs text-slate-400">
                    Signed: {formatTimestamp(signatureData.signedAt)}
                  </div>)}
                {signatureData.notarizedBy && (<div className="text-xs text-green-400">
            ...
            {(signatureData.tamperDetected || signatureData.signatureMismatch) && (<div className="mt-3 p-2 bg-red-500/10 border border-red-500/30 rounded text-xs">
                <div className="flex items-center gap-1 text-red-400">
                  <AlertTriangle className="w-3 h-3"/>
                  <span>Security Issues Detected</span>
                </div>
                {signatureData.tamperDetected && (<div className="text-red-300 mt-1">• Tamper detection triggered</div>)}
                {signatureData.signatureMismatch && (<div className="text-red-300 mt-1">• Signature hash mismatch</div>)}
              </div>)}
          </div>)}

        {/* Cross-Deck ZKP Validation */}
        {signatureData && (<div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
            <div className="flex items-center gap-2 mb-3">
        ...
        {/* Pushback Alert */}
        {pushbackTriggered && (<div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
            <div className="flex items-center gap-2 mb-2">
        ...
        {/* Action Buttons */}
        <div className="flex gap-2">
          {!signatureData && (<Button onClick={handleSignPolicy} disabled={!selectedPolicyId || isProcessing} className={cn('flex-1 min-h-[48px]', 'bg-green-600 hover:bg-green-700 text-white', 'disabled:opacity-50 disabled:cursor-not-allowed')}>
          ...
          {signatureData && (<Button onClick={handleResetSignature} variant="outline" className={cn('flex-1 min-h-[48px]', 'border-slate-600 text-slate-300 hover:bg-slate-700')}>
              <div className="flex items-center gap-2">
