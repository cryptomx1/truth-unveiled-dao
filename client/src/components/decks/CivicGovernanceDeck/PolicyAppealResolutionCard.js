...
import { Scale, Gavel, CheckCircle, XCircle, AlertTriangle, Clock, Eye, Crown, FileText, ArrowUp, Settings } from 'lucide-react';
...
const getResolutionStatusInfo = (status) => {
    switch (status) {
        case 'pending':
            return {
...
const getResolutionOutcomeInfo = (outcome) => {
    switch (outcome) {
        case 'approved':
            return {
...
export const PolicyAppealResolutionCard = ({ className }) => {
    const [currentUserRole, setCurrentUserRole] = useState('arbitrator');
    const [selectedAppeal, setSelectedAppeal] = useState(null);
    const [appeals, setAppeals] = useState(MOCK_APPEAL_RECORDS);
    ...
    const [autoDenialRate, setAutoDenialRate] = useState(0);
    ...
    const handleResolveAppeal = async (outcome, escalationReason) => {
        ...
        setAppeals(prev => prev.map(appeal => appeal.id === selectedAppeal.id ? updatedAppeal : appeal));
    ...
    const handleEscalateAll = async () => {
        if (currentUserRole !== 'arbitrator')
            return;
        setIsProcessing(true);
        const pendingAppeals = appeals.filter(appeal => appeal.status === 'pending');
        const updatedAppeals = appeals.map(appeal => appeal.status === 'pending'
            ? {
                ...appeal,
                status: 'resolved',
                outcome: 'escalated',
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Policy Appeal Resolution Interface">
      {/* Status Indicator Dot */}
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', pushbackTriggered ? 'bg-red-500 animate-pulse' : 'bg-green-500')} aria-label={pushbackTriggered ? "Status: Alert" : "Status: Active"} title={pushbackTriggered ? `${autoDenialRate.toFixed(1)}% auto-denial rate detected` : "Resolution system active"}/>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <Button onClick={() => handleRoleChange('arbitrator')} variant={currentUserRole === 'arbitrator' ? "default" : "outline"} size="sm" className={cn('flex-1 min-h-[40px]', currentUserRole === 'arbitrator'
            ...
            <Button onClick={() => handleRoleChange('observer')} variant={currentUserRole === 'observer' ? "default" : "outline"} size="sm" className={cn('flex-1 min-h-[40px]', currentUserRole === 'observer'
        ...
        {pushbackTriggered && (<div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
            ...
            {roleInfo.canResolve && (<Button onClick={handleEscalateAll} disabled={isProcessing} size="sm" className="w-full bg-amber-600 hover:bg-amber-700 text-white">
                <div className="flex items-center gap-2">
        ...
        <div className="space-y-3">
          ...
          <ScrollArea className="h-[200px] w-full">
            ...
            return (<div key={appeal.id} onClick={() => handleAppealSelect(appeal)} className={cn('p-3 rounded-lg border cursor-pointer transition-all', selectedAppeal?.id === appeal.id
                    ...
                    {outcomeInfo && (<div className="flex items-center gap-1 mt-2">
                        {outcomeInfo.icon}
        ...
        {selectedAppeal && (<div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
            <div className="flex items-center gap-2 mb-3">
        ...
        {/* Resolution Actions */}
        {selectedAppeal && selectedAppeal.status === 'pending' && roleInfo.canResolve && (<div className="grid grid-cols-3 gap-2">
            <Button onClick={() => handleResolveAppeal('approved')} disabled={isProcessing} size="sm" className="bg-green-600 hover:bg-green-700 text-white min-h-[48px]">
            ...
            <Button onClick={() => handleResolveAppeal('denied')} disabled={isProcessing} size="sm" className="bg-red-600 hover:bg-red-700 text-white min-h-[48px]">
              <div className="flex flex-col items-center gap-1">
            ...
            <Button onClick={() => handleResolveAppeal('escalated', 'Requires DAO review')} disabled={isProcessing} size="sm" className="bg-amber-600 hover:bg-amber-700 text-white min-h-[48px]">
              <div className="flex flex-col items-center gap-1">
        ...
        {isProcessing && (<div className="flex items-center justify-center p-4">
            <div className="flex items-center gap-2">
