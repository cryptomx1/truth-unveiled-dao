...
import { AlertCircle, Plus, Trash2, Vote, Zap, Clock, Users, CheckCircle } from 'lucide-react';
...
export default function CivicPollForge({ testMode = false }) {
    ...
    const [currentUserTier, setCurrentUserTier] = useState(TruthTokenRole.CITIZEN);
    const [currentUserDID, setCurrentUserDID] = useState('');
    const [isCreating, setIsCreating] = useState(false);
    const [creationStatus, setCreationStatus] = useState('idle');
    const [lastCreatedPollId, setLastCreatedPollId] = useState(null);
    ...
    const createPoll = async (isLive = true) => {
        if (!isFormValid()) {
            toast({
    ...
    return (<div className="space-y-6 max-w-4xl mx-auto p-4">
      {/* Header */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="bg-slate-800 border-slate-700">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input id="title" value={formData.title} onChange={(e) => setFormData(prev => ({ ...prev, title: e.target.value }))} placeholder="Enter poll title..." className="bg-slate-700 border-slate-600 text-white" maxLength={100}/>
                </div>
                ...
                <div>
                  ...
                  <Textarea id="description" value={formData.description} onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))} placeholder="Optional poll description..." className="bg-slate-700 border-slate-600 text-white" rows={3} maxLength={500}/>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select value={formData.pollType} onValueChange={(value) => setFormData(prev => ({ ...prev, pollType: value }))}>
                      <SelectTrigger className="bg-slate-700 border-slate-600 text-white">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={addOption} className="border-slate-600 text-slate-300 hover:bg-slate-700" disabled={formData.options.length >= 6}>
                ...
                <div className="space-y-3">
                  {formData.options.map((option, index) => (<div key={option.id} className="flex gap-2">
                      <div className="flex-1">
                        <Input value={option.text} onChange={(e) => updateOptionText(option.id, e.target.value)} placeholder={`Option ${index + 1}...`} className="bg-slate-700 border-slate-600 text-white" maxLength={100}/>
                      </div>
                      {formData.options.length > 2 && (<Button type="button" variant="outline" size="sm" onClick={() => removeOption(option.id)} className="border-slate-600 text-red-400 hover:bg-red-500/10">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input id="truthCoinStaked" type="number" min="0" max="50" value={formData.truthCoinStaked} onChange={(e) => setFormData(prev => ({
            ...prev,
            truthCoinStaked: parseInt(e.target.value) || 0
                  ...
                  <div>
                    ...
                    <Select value={formData.expirationDays.toString()} onValueChange={(value) => setFormData(prev => ({
            ...prev,
            expirationDays: parseInt(value)
              ...
              <div className="flex gap-3 pt-4">
                {testMode ? (<>
                    <Button onClick={() => createPoll(false)} disabled={!isFormValid() || isCreating} className="bg-amber-600 hover:bg-amber-700 text-white">
                      <Zap className="w-4 h-4 mr-2"/>
                      Simulate Poll (Mock)
                    </Button>
                    <Button onClick={() => createPoll(true)} disabled={!isFormValid() || isCreating} className="bg-blue-600 hover:bg-blue-700 text-white">
                ...
                <Button onClick={resetForm} variant="outline" className="border-slate-600 text-slate-300 hover:bg-slate-700">
                  Reset Form
                </Button>
              </div>

              {/* Path B Retry */}
              {creationStatus === 'failed' && pathBRetryCount > 0 && (<div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
                  ...
                  <Button onClick={() => createPoll(true)} disabled={isCreating} className="bg-red-600 hover:bg-red-700 text-white mt-3" size="sm">
                    Retry Creation
                  </Button>
                </div>)}

              {/* Success Status */}
              {creationStatus === 'success' && lastCreatedPollId && (<div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4">
        ...
        <div className="space-y-6">
          ...
          {testMode && (<Card className="bg-slate-800 border-slate-700">
              <CardHeader>
              ...
              <CardContent className="space-y-2">
                {TEST_PRESETS.map((preset, index) => (<Button key={index} onClick={() => loadPreset(preset)} variant="outline" size="sm" className="w-full justify-start border-slate-600 text-slate-300 hover:bg-slate-700 text-xs">
          ...
          {!testMode && (<Card className="bg-slate-800 border-slate-700">
              ...
              <CardContent>
                {isLoadingPolls ? (<div className="text-slate-400 text-sm">Loading polls...</div>) : polls.length === 0 ? (<div className="text-slate-400 text-sm">No polls created yet</div>) : (<div className="space-y-3">
                    {polls.slice(0, 3).map((poll) => (<div key={poll.id} className="bg-slate-700/50 rounded-lg p-3">
