...
const getTierWeight = (tier) => {
    const tierRole = typeof tier === 'string' ? tier : tier;
    switch (tierRole) {
...
export class PollResponseAggregator {
    ...
    calculatePollImpact() {
        ...
        const engagementScore = Math.min((this.responses.length / expectedResponseThreshold) * 100, 100);
        // Calculate weighted influence (0-100)
        const highTierParticipation = Object.entries(tierParticipation)
            .filter(([tier, _]) => [TruthTokenRole.GOVERNOR, TruthTokenRole.COMMANDER].includes(tier))
        ...
        const dominantTier = Object.entries(tierParticipation)
            .reduce((max, [tier, data]) => data.responseCount > (tierParticipation[max]?.responseCount || 0) ? tier : max, Object.keys(tierParticipation)[0] || 'CITIZEN');
        ...
        Object.entries(tierParticipation).forEach(([tier, tierData]) => {
            ...
            this.responses.filter(r => r.responderTier === tier).forEach(response => {
                response.response.forEach(selection => {
                    if (oppositionKeywords.some(keyword => selection.toLowerCase().includes(keyword))) {
                        tierOppositionCount++;
                    }
                });
            });
    ...
    getPollReplayData() {
        const replayData = [];
        const sortedResponses = [...this.responses].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
     ...
     */
    static validateAccess(userTier) {
 ...
 */
export async function createPollAggregator(pollId) {
    try {
 ...
 */
export async function quickPollAggregation(pollId) {
 ...
 */
export async function batchPollAnalysis(pollIds) {
    const results = {};
