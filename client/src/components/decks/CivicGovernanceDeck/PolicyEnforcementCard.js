...
import { Shield, AlertTriangle, CheckCircle, XCircle, Clock, User, FileText, Zap, Volume2, Settings, Crown, Users, Scale } from 'lucide-react';
...
const getEnforcementStatusInfo = (status) => {
    switch (status) {
        case 'enforced':
            return {
...
export const PolicyEnforcementCard = ({ className }) => {
    const [selectedPolicy, setSelectedPolicy] = useState('curfew');
    const [currentPolicy, setCurrentPolicy] = useState(MOCK_POLICIES.curfew);
    ...
    const [zkpSyncStartTime, setZkpSyncStartTime] = useState(0);
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Policy Enforcement Interface">
      {/* Status Indicator Dot */}
      <div className="absolute top-3 right-3 w-3 h-3 rounded-full bg-green-500" aria-label="Status: Active" title="Enforcement system active"/>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-1">
                {currentPolicy.credentialVerified ? (<CheckCircle className="w-3 h-3 text-green-400"/>) : (<XCircle className="w-3 h-3 text-red-400"/>)}
                <span className={cn('text-xs', currentPolicy.credentialVerified ? 'text-green-400' : 'text-red-400')}>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-1">
                {currentPolicy.didLineageValid ? (<CheckCircle className="w-3 h-3 text-green-400"/>) : (<XCircle className="w-3 h-3 text-red-400"/>)}
                <span className={cn('text-xs', currentPolicy.didLineageValid ? 'text-green-400' : 'text-red-400')}>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-1">
                {currentPolicy.zkpSignatureValid ? (<CheckCircle className="w-3 h-3 text-green-400"/>) : (<XCircle className="w-3 h-3 text-red-400"/>)}
                <span className={cn('text-xs', currentPolicy.zkpSignatureValid ? 'text-green-400' : 'text-red-400')}>
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          <div className="flex items-center gap-2 mb-3">
            <div className={cn('relative p-2 rounded-full', statusInfo.bgColor, currentPolicy.enforcementStatus === 'enforced' ? 'ring-2 ' + statusInfo.ringColor : '')}>
              {statusInfo.icon}
              
              {/* Animated enforcement seal for enforced policies */}
              {currentPolicy.enforcementStatus === 'enforced' && (<div className="absolute inset-0 rounded-full ring-2 ring-green-400/30 animate-ping"/>)}
            </div>
          ...
          <Button onClick={handleEnforcement} disabled={isEnforcing} className={cn('w-full min-h-[48px]', 'bg-blue-600 hover:bg-blue-700 text-white', 'disabled:opacity-50 disabled:cursor-not-allowed')}>
            {isEnforcing ? (<div className="flex items-center gap-2">
