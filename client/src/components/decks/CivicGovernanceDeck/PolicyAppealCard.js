...
import { Scale, AlertTriangle, CheckCircle, XCircle, Clock, Send, FileText, Zap, Eye, MessageSquare } from 'lucide-react';
// Mock appeal data
const INITIAL_APPEAL_DATA = {
    id: '',
...
const getAppealReasonInfo = (reason) => {
    switch (reason) {
...
const getAppealStatusInfo = (status) => {
    switch (status) {
        case 'draft':
            return {
...
export const PolicyAppealCard = ({ className }) => {
    const [appealData, setAppealData] = useState(INITIAL_APPEAL_DATA);
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Policy Appeal Interface">
      {/* Status Indicator Dot */}
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', pushbackTriggered ? 'bg-red-500 animate-pulse' : 'bg-blue-500')} aria-label={pushbackTriggered ? "Status: Alert" : "Status: Active"} title={pushbackTriggered ? "Processing errors detected" : "Appeal system active"}/>
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className={cn('border-opacity-50', statusInfo.color.replace('text-', 'border-').replace('-400', '-500/30'), statusInfo.color.replace('text-', 'bg-').replace('-400', '-500/10'), statusInfo.color)}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="flex items-center justify-between mb-2">
            <div className={cn('relative p-2 rounded-full', statusInfo.bgColor, ['submitted', 'under-review', 'approved', 'rejected'].includes(appealData.status)
            ? 'ring-2 ' + statusInfo.ringColor : '')}>
              {statusInfo.icon}
              
              {/* Animated status ring for active processing */}
              {isProcessing && appealData.status === 'under-review' && (<div className="absolute inset-0 rounded-full ring-2 ring-amber-400/30 animate-ping"/>)}
            </div>
            ...
            <div className="flex-1 ml-3">
              ...
              {appealData.submittedAt && (<div className="text-xs text-slate-400">
                  Submitted: {getRelativeTime(appealData.submittedAt)}
                </div>)}
              {appealData.reviewedAt && (<div className="text-xs text-slate-400">
          ...
          {pushbackTriggered && currentError && (<div className="mt-3 p-2 bg-red-500/10 border border-red-500/30 rounded text-xs">
        ...
        {appealData.status === 'draft' && (<>
            ...
            <div className="space-y-3">
              ...
              <Textarea value={appealData.justification} onChange={(e) => handleJustificationChange(e.target.value)} placeholder="Provide detailed justification for your appeal..." className="bg-slate-800/50 border-slate-600 text-slate-200 min-h-[100px]" maxLength={500}/>
        ...
        {appealData.status !== 'draft' && (<div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
            <div className="flex items-center gap-2 mb-3">
        ...
        <div className="flex gap-2">
          {canSubmit && (<Button onClick={handleSubmitAppeal} disabled={isSubmitting} className={cn('flex-1 min-h-[48px]', 'bg-purple-600 hover:bg-purple-700 text-white', 'disabled:opacity-50 disabled:cursor-not-allowed')}>
          ...
          {canReset && (<Button onClick={handleResetAppeal} variant="outline" className={cn('min-h-[48px]', canSubmit ? 'flex-none px-4' : 'flex-1', 'border-slate-600 text-slate-300 hover:bg-slate-700')}>
