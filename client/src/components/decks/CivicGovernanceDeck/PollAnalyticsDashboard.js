...
import { BarChart3, TrendingUp, AlertTriangle, Download, Users, Target, Scale, CheckCircle, XCircle, AlertCircle } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useToast } from '@/hooks/use-toast';
// Import aggregation functionality
import PollResponseAggregator, { createPollAggregator } from './PollResponseAggregator';
// Import access control
import { TruthTokenRole } from '../../../tokenomics/TruthTokenomicsSpec';
export default function PollAnalyticsDashboard({ userTier = TruthTokenRole.MODERATOR, restrictedMode = false }) {
    ...
    const analyzePoll = async (pollId) => {
        if (!hasAccess) {
            toast({
                title: "Access Denied",
        ...
        try {
            ...
            const [aggregatedResults, tierParticipation, impactAnalysis, alignmentReport, pushbackAnalysis] = await Promise.all([
    ...
    const exportPollReport = async () => {
        if (!analyticsState.selectedPollId || !hasAccess)
            return;
    ...
    return (<div className="space-y-6 max-w-6xl mx-auto p-4">
      {/* Header */}
      <Card className="bg-slate-800 border-slate-700">
        <CardHeader>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <Card className="bg-slate-800 border-slate-700">
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Select value={analyticsState.selectedPollId?.toString() || ''} onValueChange={(value) => {
            ...
            if (!isNaN(pollId)) {
                analyzePoll(pollId);
                  ...
                  <SelectContent>
                    {polls.map(poll => (<SelectItem key={poll.id} value={poll.id.toString()}>
              ...
              {analyticsState.isAnalyzing && (<div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3">
              ...
              {analyticsState.lastAnalyzedAt && (<div className="text-xs text-slate-500">
          ...
          {/* Export Controls */}
          {analyticsState.aggregatedResults && (<Card className="bg-slate-800 border-slate-700">
              <CardHeader>
              ...
              <CardContent className="space-y-3">
                ...
                <Button onClick={exportPollReport} disabled={exportStatus === 'exporting'} className="w-full bg-green-600 hover:bg-green-700 text-white" size="sm">
                  <Download className="w-4 h-4 mr-2"/>
                  {exportStatus === 'exporting' ? 'Exporting...' : 'Export Report'}
                </Button>

                {exportStatus === 'success' && (<div className="text-xs text-green-400">Export completed successfully</div>)}
                {exportStatus === 'failed' && (<div className="text-xs text-red-400">Export failed - check console</div>)}
              </CardContent>
            </Card>)}
        ...
        <div className="lg:col-span-3 space-y-6">
          {!analyticsState.aggregatedResults ? (<Card className="bg-slate-800 border-slate-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Impact Analysis */}
                {analyticsState.impactAnalysis && (<Card className="bg-slate-800 border-slate-700">
                    <CardHeader>
                    ...
                    <CardContent className="space-y-4">
                      <div>
                        ...
                        <Progress value={analyticsState.impactAnalysis.publicOpinionStrength} className="h-2 bg-slate-700"/>
                      </div>
                      ...
                      <div>
                        ...
                        <Progress value={analyticsState.impactAnalysis.engagementScore} className="h-2 bg-slate-700"/>
                      </div>
                ...
                {/* Pushback Analysis */}
                {analyticsState.pushbackAnalysis && (<Card className="bg-slate-800 border-slate-700">
                    <CardHeader>
                      <CardTitle className="text-white flex items-center gap-2">
                    ...
                    <CardContent className="space-y-4">
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className={cn("font-medium", analyticsState.pushbackAnalysis.severityScore >= 50 ? "text-red-400" : "text-green-400")}>
                            {analyticsState.pushbackAnalysis.severityScore.toFixed(1)}%
                          </span>
                        </div>
                        <Progress value={analyticsState.pushbackAnalysis.severityScore} className="h-2 bg-slate-700"/>
                      </div>

                      <div className="flex items-center gap-2">
                        {analyticsState.pushbackAnalysis.isActionRequired ? (<XCircle className="w-4 h-4 text-red-400"/>) : (<CheckCircle className="w-4 h-4 text-green-400"/>)}
                        <span className={cn("text-sm", analyticsState.pushbackAnalysis.isActionRequired ? "text-red-400" : "text-green-400")}>
                          {analyticsState.pushbackAnalysis.isActionRequired ? "Action Required" : "No Action Needed"}
                        </span>
                      </div>

                      {analyticsState.pushbackAnalysis.oppositionTiers.length > 0 && (<div>
                          <div className="text-slate-400 text-sm mb-2">Opposition Tiers:</div>
                          <div className="flex flex-wrap gap-1">
                            {analyticsState.pushbackAnalysis.oppositionTiers.map(tier => (<Badge key={tier} variant="outline" className="text-red-400 border-red-400">
                                {tier}
                              </Badge>))}
                          </div>
                        </div>)}
              ...
              {/* Tier Participation */}
              {analyticsState.tierParticipation && (<Card className="bg-slate-800 border-slate-700">
                  <CardHeader>
                    <CardTitle className="text-white flex items-center gap-2">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {Object.entries(analyticsState.tierParticipation).map(([tier, data]) => (<div key={tier} className="bg-slate-700/30 rounded-lg p-4">
              ...
              {/* Alignment Report */}
              {analyticsState.alignmentReport && (<Card className="bg-slate-800 border-slate-700">
                  <CardHeader>
                    <CardTitle className="text-white flex items-center gap-2">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <Progress value={analyticsState.alignmentReport.overallConsensus} className="h-2 bg-slate-700"/>
                          </div>
                          <div>
                            <div className="flex items-center justify-between mb-2">
                            ...
                            <Progress value={analyticsState.alignmentReport.polarizationIndex} className="h-2 bg-slate-700"/>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        {analyticsState.alignmentReport.minorityTierConcerns.length > 0 ? (<div className="space-y-2">
                            {analyticsState.alignmentReport.minorityTierConcerns.map((concern, index) => (<div key={index} className="bg-amber-500/10 border border-amber-500/30 rounded p-2">
                                <div className="text-amber-400 text-sm">{concern}</div>
                              </div>))}
                          </div>) : (<div className="text-slate-400 text-sm">No minority tier concerns detected</div>)}
                      </div>
                    </div>
                  </CardContent>
                </Card>)}
