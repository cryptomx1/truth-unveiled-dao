...
import { Crown, Users, ShieldCheck, Star, CheckCircle, Lock, Sparkles, Trophy, Target, Clock, ArrowUp, Award } from 'lucide-react';
...
const getTierProgressColor = (status) => {
    switch (status) {
        case 'unlocked':
            return 'bg-green-500';
        case 'current':
...
export const ReputationLadderCard = ({ className }) => {
    const [reputationTiers, setReputationTiers] = useState(MOCK_REPUTATION_TIERS);
    ...
    const [announcedTiers, setAnnouncedTiers] = useState(new Set());
        ...
        const interval = setInterval(() => {
            ...
            setReputationTiers(prev => {
                const newTiers = prev.map(tier => {
                    ...
                    const meetsRequirements = userMetrics.trustScore >= requirements.trustScore &&
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Reputation Ladder Interface">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="space-y-3">
            {reputationTiers.map((tier, index) => (<div key={tier.id} className={cn('relative p-3 rounded-lg border transition-all duration-300', tier.status === 'current'
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className={cn('font-medium text-sm', tier.status === 'current' ? 'text-blue-300' :
                tier.status === 'unlocked' ? 'text-green-300' : tier.color)}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {tier.status === 'unlocked' && tier.unlockedAt && (<span className="text-xs text-green-400">
                ...
                <div className="space-y-1 mb-2">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className={cn(tier.status === 'current' ? 'text-blue-400' :
                tier.status === 'unlocked' ? 'text-green-400' : 'text-slate-400')}>
                      {tier.progress.toFixed(0)}%
                    </span>
                  </div>
                  <Progress value={tier.progress} className={cn('h-2', tier.status === 'current' ? 'bg-blue-900/50' :
                ...
                {tier.status === 'current' && (<div className="absolute inset-0 pointer-events-none">
        ...
        }} disabled={isUpdating} className={cn('px-3 py-1.5 text-xs rounded', 'bg-purple-600/80 hover:bg-purple-700 text-white', 'disabled:bg-slate-700/50 disabled:text-slate-500', 'min-h-[32px] transition-colors')}>
            {isUpdating ? (<>
