...
import { Award, Star, Shield, Trophy, TrendingUp, CheckCircle, Lock, Zap, Target, Clock } from 'lucide-react';
...
const validateZKPTrustHash = (hash) => {
...
export const TrustStreakRewardCard = ({ className }) => {
    const [rewardTiers, setRewardTiers] = useState(MOCK_REWARD_TIERS);
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Trust Streak Reward Interface">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="space-y-3">
            {rewardTiers.map((tier, index) => (<div key={tier.id} className={cn('relative p-3 rounded-lg border transition-all duration-300', tier.status === 'unlocked'
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className={cn('font-medium text-sm', tier.status === 'unlocked' ? 'text-green-300' : tier.color)}>
                        {tier.name}
                      </div>
                      <div className="text-xs text-slate-400">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className={cn(tier.status === 'unlocked' ? 'text-green-400' : 'text-slate-400')}>
                      {tier.progress.toFixed(0)}%
                    </span>
                  </div>
                  <Progress value={tier.progress} className={cn('h-2', tier.status === 'unlocked' ? 'bg-green-900/50' : 'bg-slate-700/50')}/>
                </div>

                {/* Celebratory animation for newly unlocked tiers */}
                {tier.status === 'unlocked' && (<div className="absolute inset-0 pointer-events-none">
