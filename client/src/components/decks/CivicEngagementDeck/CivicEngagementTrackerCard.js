...
import { Users, Trophy, Target, Calendar, Zap, Shield, Clock, Star, Activity } from 'lucide-react';
...
const getEngagementLevelBadge = (level) => {
    switch (level) {
        case 'novice':
...
const getStatusDotColor = (streak, zkpMismatchRate) => {
    if (zkpMismatchRate > 10)
        return 'bg-red-500';
    if (streak >= 7)
        return 'bg-green-500';
    if (streak >= 3)
...
const getStatusDotLabel = (streak, zkpMismatchRate) => {
    if (zkpMismatchRate > 10)
...
export const CivicEngagementTrackerCard = ({ className }) => {
    const [engagementMetrics, setEngagementMetrics] = useState(MOCK_ENGAGEMENT_METRICS);
    ...
    const [announcedBadges, setAnnouncedBadges] = useState(new Set());
    ...
    const handleBadgeUnlock = (badgeId) => {
        setEngagementMetrics(prev => ({
            ...prev,
            badges: prev.badges.map(badge => badge.id === badgeId
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Civic Engagement Tracker Interface">
      {/* Status Indicator Dot */}
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', getStatusDotColor(engagementMetrics.currentStreak, zkpMismatchRate), 'animate-ping')} aria-label={getStatusDotLabel(engagementMetrics.currentStreak, zkpMismatchRate)} title={getStatusDotLabel(engagementMetrics.currentStreak, zkpMismatchRate)}/>
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', getStatusDotColor(engagementMetrics.currentStreak, zkpMismatchRate))}/>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="mt-3">
            ...
            <Progress value={engagementMetrics.trustScore} className="h-2 bg-slate-700/50"/>
          </div>
        </div>
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="mt-3">
            ...
            <Progress value={(engagementMetrics.referralsVerified / engagementMetrics.referralsSent) * 100} className="h-2 bg-slate-700/50"/>
          </div>
        </div>
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="grid grid-cols-7 gap-1">
            {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, index) => (<div key={index} className="text-center">
                <div className="text-xs text-slate-400 mb-1">{day}</div>
                <div className={cn('w-8 h-8 rounded flex items-center justify-center text-xs font-medium', getActivityLevel(engagementMetrics.weeklyHeatmap[index]))}>
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {engagementMetrics.badges.slice(0, 4).map((badge) => (<div key={badge.id} className={cn('p-2 rounded-lg border text-center', badge.isUnlocked
                ? 'bg-green-500/20 border-green-500/30'
                : 'bg-slate-700/30 border-slate-600/30')}>
                <div className={cn('flex justify-center mb-1', badge.isUnlocked ? 'text-green-400' : 'text-slate-500')}>
                  {badge.icon}
                </div>
                <div className={cn('text-xs font-medium', badge.isUnlocked ? 'text-green-300' : 'text-slate-400')}>
                  {badge.name}
                </div>
                {badge.isUnlocked && badge.unlockedAt && (<div className="text-xs text-slate-400 mt-1">
        ...
        }} disabled={isUpdating} size="sm" className={cn('min-h-[32px] text-xs', 'bg-blue-600/80 hover:bg-blue-700 text-white', 'disabled:bg-slate-700/50 disabled:text-slate-500')}>
            {isUpdating ? (<>
