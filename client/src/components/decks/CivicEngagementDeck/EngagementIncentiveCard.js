...
import { Gift, Star, Zap, Trophy, TrendingUp, CheckCircle, Lock, Sparkles, Target, Clock, Users, ShieldCheck, Award, Flame } from 'lucide-react';
...
const MOCK_INCENTIVE_MILESTONES = [
    {
        id: 'streak_milestone',
        name: '7-Day Streak',
        requirement: 7,
...
const getIncentiveStatusIcon = (status) => {
    switch (status) {
        case 'unlocked':
...
const getIncentiveProgressColor = (status) => {
    switch (status) {
        case 'unlocked':
            return 'bg-green-500';
        case 'active':
...
export const EngagementIncentiveCard = ({ className }) => {
    const [incentiveMilestones, setIncentiveMilestones] = useState(MOCK_INCENTIVE_MILESTONES);
    ...
    const [announcedIncentives, setAnnouncedIncentives] = useState(new Set());
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Engagement Incentive Interface">
      {/* Status Indicator Dot */}
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', isPushbackActive ? 'bg-red-500' : 'bg-green-500', 'animate-ping')} aria-label={isPushbackActive ? 'Status: Validation issue' : 'Status: Live'} title={isPushbackActive ? 'Status: Validation issue' : 'Status: Live'}/>
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', isPushbackActive ? 'bg-red-500' : 'bg-green-500')}/>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-center text-sm">
            ...
            <div className="bg-yellow-500/20 rounded p-2">
              <div className="flex items-center justify-center gap-1">
                {totalMultiplier > 1.0 && (<Sparkles className="w-4 h-4 text-yellow-400 animate-pulse"/>)}
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="space-y-3">
            {incentiveMilestones.map((incentive, index) => (<div key={incentive.id} className={cn('relative p-3 rounded-lg border transition-all duration-300', incentive.status === 'active'
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className={cn('font-medium text-sm', incentive.status === 'active' ? 'text-yellow-300' :
                incentive.status === 'unlocked' ? 'text-green-300' : incentive.color)}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {incentive.status === 'unlocked' && incentive.unlockedAt && (<span className="text-xs text-green-400">
                ...
                <div className="space-y-1 mb-2">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className={cn(incentive.status === 'active' ? 'text-yellow-400' :
                incentive.status === 'unlocked' ? 'text-green-400' : 'text-slate-400')}>
                      {incentive.progress.toFixed(0)}%
                    </span>
                  </div>
                  <Progress value={incentive.progress} className={cn('h-2', incentive.status === 'active' ? 'bg-yellow-900/50' :
                ...
                {incentive.status === 'active' && (<div className="absolute inset-0 pointer-events-none">
        ...
        }} disabled={isUpdating} className={cn('px-3 py-1.5 text-xs rounded', 'bg-orange-600/80 hover:bg-orange-700 text-white', 'disabled:bg-slate-700/50 disabled:text-slate-500', 'min-h-[48px] transition-colors')}>
            {isUpdating ? (<>
