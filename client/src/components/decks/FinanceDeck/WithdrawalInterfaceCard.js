...
export const WithdrawalInterfaceCard = ({ className }) => {
    ...
    const playConfirmationTTS = (amount, currency) => {
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-800 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Withdrawal Interface">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Button onClick={() => handleCurrencyChange('TP')} variant={withdrawalRequest.currency === 'TP' ? 'default' : 'outline'} className={cn('flex-1 min-h-[48px]', withdrawalRequest.currency === 'TP'
            ? 'bg-blue-500 hover:bg-blue-600 text-white'
            : 'bg-slate-700/50 border-slate-600 text-slate-200 hover:bg-slate-600/70')} aria-pressed={withdrawalRequest.currency === 'TP'}>
              TP
            </Button>
            <Button onClick={() => handleCurrencyChange('CC')} variant={withdrawalRequest.currency === 'CC' ? 'default' : 'outline'} className={cn('flex-1 min-h-[48px]', withdrawalRequest.currency === 'CC'
            ? 'bg-green-500 hover:bg-green-600 text-white'
            : 'bg-slate-700/50 border-slate-600 text-slate-200 hover:bg-slate-600/70')} aria-pressed={withdrawalRequest.currency === 'CC'}>
              CC
            </Button>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label htmlFor="withdrawal-amount" className="text-sm font-medium text-slate-200">
              Amount
            </label>
          ...
          <Input id="withdrawal-amount" type="number" value={withdrawalRequest.amount} onChange={(e) => handleAmountChange(e.target.value)} placeholder={`Enter ${withdrawalRequest.currency} amount`} className={cn('min-h-[48px] bg-slate-700/50 border-slate-600 text-slate-100 placeholder-slate-400', validationErrors.amount && 'border-red-500 focus:border-red-500')} step="0.01" min="0" max={availableBalance} aria-invalid={!!validationErrors.amount} aria-describedby={validationErrors.amount ? 'amount-error' : undefined}/>
          {validationErrors.amount && (<div id="amount-error" className="flex items-center gap-1 text-xs text-red-400" role="alert">
        ...
        {/* Withdrawal Method */}
        <div className="space-y-2">
          <label htmlFor="withdrawal-method" className="text-sm font-medium text-slate-200 block">
            Withdrawal Method
          </label>
          <Select value={withdrawalRequest.method} onValueChange={handleMethodChange}>
            <SelectTrigger id="withdrawal-method" className={cn('min-h-[48px] bg-slate-700/50 border-slate-600 text-slate-100', validationErrors.method && 'border-red-500 focus:border-red-500')} aria-invalid={!!validationErrors.method} aria-describedby={validationErrors.method ? 'method-error' : undefined}>
              <SelectValue placeholder="Select withdrawal method"/>
            </SelectTrigger>
            <SelectContent className="bg-slate-800 border-slate-700">
              {WITHDRAWAL_METHODS.map((method) => (<SelectItem key={method.value} value={method.value} className="text-slate-100 focus:bg-slate-700">
          ...
          {validationErrors.method && (<div id="method-error" className="flex items-center gap-1 text-xs text-red-400" role="alert">
              <AlertCircle className="w-3 h-3"/>
        ...
        {/* Method Details */}
        {selectedMethod && (<div className="bg-slate-700/30 rounded-lg border border-slate-600/50 p-3">
        ...
        {/* Submit Button */}
        <Button onClick={handleSubmit} disabled={isSubmitting || hasErrors || !withdrawalRequest.amount || !withdrawalRequest.method} className={cn('w-full min-h-[48px] font-medium', submissionStatus === 'success'
            ? 'bg-green-600 hover:bg-green-700 text-white'
            : submissionStatus === 'error'
                ? 'bg-red-600 hover:bg-red-700 text-white'
                : 'bg-green-500 hover:bg-green-600 text-white disabled:bg-slate-600 disabled:text-slate-400')} aria-label={isSubmitting ? 'Processing withdrawal' : 'Submit withdrawal request'}>
          {isSubmitting ? ('Processing...') : submissionStatus === 'success' ? (<>
        ...
        <div className="text-center" aria-live="polite" aria-atomic="true">
          {submissionStatus === 'success' && (<p className="text-sm text-green-400">
              Withdrawal request submitted successfully
            </p>)}
          {submissionStatus === 'error' && hasErrors && (<p className="text-sm text-red-400">
