...
export const TransactionHistoryCard = ({ className }) => {
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-800 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Transaction History">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-slate-700/30 rounded-lg border border-slate-600/50">
          <ScrollArea className="max-h-64">
            <div className="p-3 space-y-3">
              {displayedTransactions.map((transaction) => (<div key={transaction.id} className="flex items-center justify-between p-2 rounded border border-slate-600/30 bg-slate-800/50" role="row" aria-label={`Transaction: ${transaction.description}, ${transaction.amount} ${transaction.currency}, ${formatTimeAgo(transaction.timestamp)}`}>
                  <div className="flex items-center gap-3 flex-1">
                    <div className={cn('p-1.5 rounded-full', getTransactionColor(transaction.type))}>
                  ...
                  <div className="text-right flex-shrink-0">
                    <div className={cn('text-sm font-semibold', transaction.currency === 'TP' ? 'text-blue-400' : 'text-green-400')}>
        ...
        {transactions.length > 5 && (<Button onClick={handleToggleExpand} variant="outline" size="sm" className={cn('w-full min-h-[48px]', 'bg-slate-700/50 border-slate-600 text-slate-200', 'hover:bg-slate-600/70 hover:text-slate-50')} aria-label={isExpanded ? 'Show fewer transactions' : 'Show all transactions'}>
            {isExpanded ? (<>
