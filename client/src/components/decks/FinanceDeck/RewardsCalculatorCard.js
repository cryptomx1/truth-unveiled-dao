...
const calculateRewards = (streak, contributions, referralTier) => {
...
export const RewardsCalculatorCard = ({ className }) => {
    const [streak, setStreak] = useState([7]);
    const [contributions, setContributions] = useState([3]);
    const [referralTier, setReferralTier] = useState(1);
    ...
    const handleStreakChange = (value) => {
    ...
    const handleContributionsChange = (value) => {
        const inputStart = performance.now();
    ...
    const handleReferralTierChange = (value) => {
        const inputStart = performance.now();
        setReferralTier(parseInt(value));
        const inputTime = performance.now() - inputStart;
        if (inputTime > 50) {
    ...
    const playProjectionSummary = () => {
        if (!ttsStatus.isReady || !projection)
            return;
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-800 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Rewards Calculator">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label htmlFor="streak-slider" className="text-sm font-medium text-slate-200">
                Voting Streak
              </label>
            ...
            <Slider id="streak-slider" value={streak} onValueChange={handleStreakChange} min={1} max={30} step={1} className="w-full" aria-label={`Voting streak: ${streak[0]} days`}/>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label htmlFor="contributions-slider" className="text-sm font-medium text-slate-200">
            ...
            <Slider id="contributions-slider" value={contributions} onValueChange={handleContributionsChange} min={1} max={10} step={1} className="w-full" aria-label={`Monthly contributions: ${contributions[0]} items`}/>
            <div className="flex justify-between text-xs text-slate-400">
              <span>1 item</span>
              <span>10 items</span>
            </div>
          ...
          <div className="space-y-3">
            <label htmlFor="referral-tier-select" className="text-sm font-medium text-slate-200 block">
              Referral Tier
            </label>
            <Select value={referralTier.toString()} onValueChange={handleReferralTierChange}>
              <SelectTrigger id="referral-tier-select" className="w-full min-h-[48px] bg-slate-700/50 border-slate-600 text-slate-100" aria-label={`Current referral tier: ${referralTier}`}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="bg-slate-800 border-slate-700">
                {REFERRAL_TIERS.map((tier) => (<SelectItem key={tier.value} value={tier.value.toString()} className="text-slate-100 focus:bg-slate-700">
        ...
        {/* Projection Results */}
        {projection && (<div className="bg-slate-700/30 rounded-lg border border-slate-600/50 p-4" aria-live="polite" aria-label="Earnings projection results">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-slate-100 flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-green-400"/>
                Monthly Projection
              </h3>
              <Button onClick={playProjectionSummary} variant="ghost" size="sm" className="text-slate-400 hover:text-slate-200 p-1 min-h-[48px] min-w-[48px]" aria-label="Read projection summary aloud" disabled={ttsStatus.isPlaying}>
