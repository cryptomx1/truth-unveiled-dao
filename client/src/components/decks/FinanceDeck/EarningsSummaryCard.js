...
export const EarningsSummaryCard = ({ className }) => {
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto', 'bg-slate-800 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Civic Earnings Summary">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-slate-700/30 rounded-lg border border-slate-600/50 p-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="text-center">
              ...
              <div className={cn('text-2xl font-bold text-blue-400', isUpdating && 'animate-pulse')} aria-live="polite" aria-label={`Truth Points: ${earnings.truthPoints}`}>
            ...
            <div className="text-center">
              ...
              <div className={cn('text-2xl font-bold text-green-400', isUpdating && 'animate-pulse')} aria-live="polite" aria-label={`Contribution Credits: ${earnings.contributionCredits}`}>
        ...
        <div className="bg-slate-700/30 rounded-lg border border-slate-600/50 p-3">
          <div className="flex items-center justify-between">
            ...
            <Button onClick={handleCopyReferralCode} disabled={copyStatus === 'copying'} size="sm" variant="outline" className={cn('min-h-[48px] px-3', 'bg-slate-700 border-slate-600 text-slate-200', 'hover:bg-slate-600 hover:border-slate-500', copyStatus === 'success' && 'bg-green-600/20 border-green-500/30 text-green-300', copyStatus === 'error' && 'bg-red-600/20 border-red-500/30 text-red-300')} aria-label="Copy referral code to clipboard">
              {copyStatus === 'copying' ? (<>Copying...</>) : copyStatus === 'success' ? (<>✓ Copied</>) : copyStatus === 'error' ? (<>Error</>) : (<>
