...
export const Deck8Module3 = ({ moduleId = 'audit-compliance', restrictedContent = true }) => {
    const { isAuthenticated, cidStatus, validateCID } = useCIDAuthentication();
    if (restrictedContent && !isAuthenticated) {
        return (<CIDAuthenticationGate onValidate={validateCID} title="Audit Compliance Module" description="Access to audit compliance data requires CID authentication for regulatory compliance."/>);
    }
    const auditMetrics = {
        complianceScore: 94,
        completedAudits: 15,
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {complianceAreas.map((area, index) => (<div key={index} className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="font-medium">{area.name}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold">{area.score}%</span>
                    {area.status === 'compliant' ? (<CheckCircle className="h-4 w-4 text-green-600"/>) : (<AlertTriangle className="h-4 w-4 text-yellow-600"/>)}
                  </div>
                </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {recentAudits.map((audit) => (<div key={audit.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded">
                <div>
                  <div className="font-medium">{audit.type}</div>
                  <div className="text-sm text-gray-600 dark:text-gray-400">{audit.date}</div>
                </div>
                <div className="text-right">
                  <Badge variant={audit.status === 'Completed' ? 'default' : 'secondary'} className="mb-1">
                    {audit.status}
                  </Badge>
                  <div className="text-sm text-gray-600 dark:text-gray-400">
