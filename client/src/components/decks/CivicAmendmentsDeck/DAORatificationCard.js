...
import { Gavel, Users, Shield, Check, X, AlertTriangle, Clock, Vote, CheckCircle, XCircle, Target, Eye } from 'lucide-react';
// Mock DAO signers (5 total, need 3/5 for passage)
const MOCK_DAO_SIGNERS = [
    {
        id: 'dao_signer_001',
...
const getRatificationStatusInfo = (status) => {
    switch (status) {
        case 'pending':
            return {
...
const calculateVotingProgress = (signers, quorumThreshold) => {
...
export const DAORatificationCard = ({ className }) => {
    const [signers, setSigners] = useState(MOCK_DAO_SIGNERS);
    const [quorumThreshold, setQuorumThreshold] = useState(66);
    const [ratificationStatus, setRatificationStatus] = useState('pending');
                ...
                if (Math.random() > 0.8) {
                    setSigners(prev => {
                        ...
                        if (availableSigners.length > 0) {
                            ...
                            return prev.map(s => s.id === randomSigner.id ? {
                                ...s,
    ...
    const handleDAOVote = async (vote) => {
        ...
        setSigners(prev => prev.map(s => s.id === availableSigner.id ? {
            ...s,
            vote: vote,
            timestamp: new Date(),
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', pathBTriggered && 'animate-pulse ring-2 ring-red-500/50', ratificationStatus === 'ratifying' && 'animate-pulse ring-2 ring-amber-500/50', className)} role="region" aria-label="DAO Ratification Interface">
      {/* Status Indicator Dot */}
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', pathBTriggered ? 'bg-red-500 animate-pulse' :
            ratificationStatus === 'ratified' ? 'bg-green-500' :
                ratificationStatus === 'vetoed' ? 'bg-red-500' :
                    ratificationStatus === 'ratifying' ? 'bg-amber-500 animate-pulse' :
                        'bg-blue-500')} aria-label={`Status: ${statusInfo.label}`} title={pathBTriggered ? `${progress.didMismatchRate.toFixed(1)}% DID mismatch detected` : `Ratification status: ${statusInfo.label}`}/>
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className={cn('border-opacity-50', statusInfo.color.replace('text-', 'border-').replace('-400', '-500/30'), statusInfo.bgColor, statusInfo.color)}>
      ...
      <CardContent className="space-y-4">
        ...
        {pathBTriggered && (<div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
        ...
        <div className="space-y-2">
          ...
          <Select value={quorumThreshold.toString()} onValueChange={(value) => setQuorumThreshold(parseInt(value))} disabled={ratificationStatus !== 'pending'}>
        ...
        {/* DAO Voting Buttons */}
        {ratificationStatus === 'pending' && !userVote && (<div className="space-y-3">
            <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button onClick={() => handleDAOVote('ratify')} disabled={isProcessing} className={cn('min-h-[48px] bg-green-600 hover:bg-green-700 text-white', 'disabled:opacity-50 disabled:cursor-not-allowed')}>
              ...
              <Button onClick={() => handleDAOVote('veto')} disabled={isProcessing} className={cn('min-h-[48px] bg-red-600 hover:bg-red-700 text-white', 'disabled:opacity-50 disabled:cursor-not-allowed')}>
                {isProcessing ? (<div className="flex items-center gap-2">
                    <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>
                    Processing...
                  </div>) : (<div className="flex items-center gap-2">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="space-y-3">
            ...
            <Progress value={progress.signatureProgress} className="h-3 bg-slate-700"/>
        ...
        {userVote && (<div className={cn('rounded-lg border p-3', userVote === 'ratify' ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30')}>
            <div className="flex items-center gap-2 mb-2">
              {userVote === 'ratify' ?
                <CheckCircle className="w-4 h-4 text-green-400"/> :
                <XCircle className="w-4 h-4 text-red-400"/>}
              <span className={cn('text-sm font-medium', userVote === 'ratify' ? 'text-green-400' : 'text-red-400')}>
