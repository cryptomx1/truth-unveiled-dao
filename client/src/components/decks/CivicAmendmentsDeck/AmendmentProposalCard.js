...
import { FileEdit, Users, CheckCircle, XCircle, AlertTriangle, Clock, Send, Eye, Shield, Zap, Settings } from 'lucide-react';
...
const getAmendmentScopeInfo = (scope) => {
    switch (scope) {
        case 'local':
...
const getAmendmentIntentInfo = (intent) => {
    switch (intent) {
        case 'clarification':
...
export const AmendmentProposalCard = ({ className }) => {
    const [title, setTitle] = useState('');
    const [body, setBody] = useState('');
    const [scope, setScope] = useState('local');
    const [intent, setIntent] = useState('clarification');
    ...
    const handleSubmitProposal = async () => {
        ...
        const testCase = MOCK_VALIDATION_CASES.find(tc => title.toLowerCase().includes(tc.title.toLowerCase().split(' ')[0])) || MOCK_VALIDATION_CASES[0];
        // Validation phase
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', proposal?.status === 'submitted' && 'animate-pulse ring-2 ring-blue-500/50', className)} role="region" aria-label="Amendment Proposal Interface">
      {/* Status Indicator Dot */}
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', pathBTriggered ? 'bg-red-500 animate-pulse' : 'bg-green-500')} aria-label={pathBTriggered ? "Status: Path B Alert" : "Status: Active"} title={pathBTriggered ? `${overlapRate.toFixed(1)}% proposal overlap detected` : "Amendment system active"}/>
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          {statusInfo && (<Badge variant="outline" className={cn('border-opacity-50', statusInfo.color.replace('text-', 'border-').replace('-400', '-500/30'), statusInfo.bgColor, statusInfo.color)}>
      ...
      <CardContent className="space-y-4">
        ...
        {!proposal && (<>
            ...
            <div className="space-y-2">
              ...
              <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Enter amendment title..." className="bg-slate-800/50 border-slate-600 text-slate-200 placeholder:text-slate-400" maxLength={100}/>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="Describe the proposed amendment in detail..." className="bg-slate-800/50 border-slate-600 text-slate-200 placeholder:text-slate-400 min-h-[100px] resize-none" maxLength={500}/>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                ...
                <Select value={intent} onValueChange={(value) => setIntent(value)}>
                  <SelectTrigger className="bg-slate-800/50 border-slate-600 text-slate-200">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-slate-800 border-slate-600">
            ...
            {/* Submit Button */}
            <Button onClick={handleSubmitProposal} disabled={!title.trim() || !body.trim() || isProcessing} className={cn('w-full min-h-[48px]', 'bg-cyan-600 hover:bg-cyan-700 text-white', 'disabled:opacity-50 disabled:cursor-not-allowed')}>
        ...
        {proposal && (<div className="space-y-4">
            ...
            <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
              ...
              <div className="space-y-2 text-xs">
                ...
                {proposal.overlapDetected && (<div className="flex justify-between">
            ...
            {/* Validation Results */}
            {proposal.status === 'failed' && (<div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
                <div className="flex items-center gap-2 mb-2">
            ...
            {/* New Proposal Button */}
            <Button onClick={handleResetForm} variant="outline" className={cn('w-full min-h-[48px]', 'border-slate-600 text-slate-300 hover:bg-slate-700')}>
