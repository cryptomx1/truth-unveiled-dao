...
import { Vote, Clock, Shield, Check, X, AlertTriangle, Users, Timer, CheckCircle, XCircle } from 'lucide-react';
...
const calculateResults = (ballots) => {
...
export const ZKPVotingWindowCard = ({ className }) => {
    const [ballots, setBallots] = useState(MOCK_EXISTING_VOTES);
    const [userVote, setUserVote] = useState(null);
    const [votingStatus, setVotingStatus] = useState('open');
    const [timeRemaining, setTimeRemaining] = useState(VOTING_WINDOW_DURATION);
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', tamperDetected && 'animate-pulse ring-2 ring-red-500/50', className)} role="region" aria-label="ZKP Voting Window Interface">
      {/* Status Indicator Dot */}
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', tamperDetected ? 'bg-red-500 animate-pulse' :
            isVotingActive ? 'bg-green-500' : 'bg-slate-500')} aria-label={tamperDetected ? "Status: Tamper Alert" :
            isVotingActive ? "Status: Voting Active" : "Status: Voting Closed"} title={tamperDetected ? `${signatureMismatchRate.toFixed(1)}% signature mismatch detected` :
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className={cn('border-opacity-50', isVotingActive ? 'border-green-500/30 bg-green-500/10 text-green-400' :
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="text-center">
            <div className={cn('text-3xl font-bold mb-2', timeRemaining > 60 ? 'text-green-400' :
        ...
        {isVotingActive && !hasUserVoted && (<div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button onClick={() => handleCastVote('support')} disabled={isProcessing} className={cn('min-h-[48px] bg-green-600 hover:bg-green-700 text-white', 'disabled:opacity-50 disabled:cursor-not-allowed')}>
              ...
              <Button onClick={() => handleCastVote('oppose')} disabled={isProcessing} className={cn('min-h-[48px] bg-red-600 hover:bg-red-700 text-white', 'disabled:opacity-50 disabled:cursor-not-allowed')}>
                {isProcessing ? (<div className="flex items-center gap-2">
                    <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>
                    Processing...
                  </div>) : (<div className="flex items-center gap-2">
        ...
        {hasUserVoted && (<div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3">
            <div className="flex items-center gap-2 mb-2">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <Progress value={results.supportPercentage} className="h-2 bg-slate-700"/>
              
              <div className="flex justify-between text-xs">
                <span className="text-red-400">Oppose</span>
                <span className="text-red-400">{results.opposePercentage.toFixed(1)}%</span>
              </div>
              <Progress value={results.opposePercentage} className="h-2 bg-slate-700"/>
            </div>
