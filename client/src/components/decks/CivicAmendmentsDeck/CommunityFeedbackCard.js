...
import { MessageSquare, Hash, TrendingUp, Filter, Clock, Zap, AlertTriangle, ThumbsUp, ThumbsDown, HelpCircle, Brain, Send, Lock } from 'lucide-react';
...
const calculateSentimentStats = (comments, scopeFilter) => {
...
export const CommunityFeedbackCard = ({ className }) => {
    const [comments, setComments] = useState(MOCK_COMMENTS);
    const [newComment, setNewComment] = useState('');
    const [selectedSentiment, setSelectedSentiment] = useState('support');
    ...
    const [sentimentSkewTriggered, setSentimentSkewTriggered] = useState(false);
    const [influenceMeter, setInfluenceMeter] = useState(0);
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', className)} role="region" aria-label="Community Feedback Interface">
      {/* Status Indicator Dot */}
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', sentimentSkewTriggered ? 'bg-red-500 animate-pulse' : 'bg-green-500')} aria-label={sentimentSkewTriggered ? "Status: Sentiment Skew Alert" : "Status: Active"} title={sentimentSkewTriggered ? "Sentiment heavily skewed in one direction" : "Community feedback active"}/>
      ...
      <CardContent className="space-y-4">
        ...
        {sentimentSkewTriggered && (<div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="grid grid-cols-4 gap-2 text-center">
            {['support', 'oppose', 'question', 'insight'].map(sentiment => {
            ...
            return (<div key={sentiment} className="space-y-1">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="space-y-2">
            ...
            <div className="w-full bg-slate-700 rounded-full h-2">
              <div className="h-2 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 transition-all duration-300" style={{ width: `${influenceMeter}%` }}/>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Select value={selectedSentiment} onValueChange={(value) => setSelectedSentiment(value)}>
              <SelectTrigger className="bg-slate-800/50 border-slate-600 text-slate-200">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="bg-slate-800 border-slate-600">
                {['support', 'oppose', 'question', 'insight'].map(sentiment => {
            const info = getSentimentInfo(sentiment);
            return (<SelectItem key={sentiment} value={sentiment}>
                      <div className="flex items-center gap-2">
            ...
            <Select value={selectedScope} onValueChange={(value) => setSelectedScope(value)}>
              <SelectTrigger className="bg-slate-800/50 border-slate-600 text-slate-200">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="bg-slate-800 border-slate-600">
          ...
          <Textarea value={newComment} onChange={(e) => setNewComment(e.target.value)} placeholder="Share your feedback on this amendment..." className="bg-slate-800/50 border-slate-600 text-slate-200 placeholder:text-slate-400 min-h-[80px] resize-none" maxLength={280}/>
          
          <div className="flex justify-between items-center">
            <div className="text-xs text-slate-400">
              {newComment.length}/280 characters
            </div>
            <Button onClick={handleSubmitComment} disabled={!newComment.trim() || isSubmitting} size="sm" className="bg-cyan-600 hover:bg-cyan-700 text-white">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-[200px] w-full">
            ...
            return (<div key={comment.id} className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-3">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <div className={cn('p-1 rounded text-xs', sentimentInfo.bgColor, sentimentInfo.color)}>
                          {sentimentInfo.emoji}
                        </div>
                        <Badge variant="outline" className={cn('text-xs border-opacity-50', scopeInfo.color.replace('text-', 'border-').replace('-400', '-500/30'), scopeInfo.color.replace('text-', 'bg-').replace('-400', '-500/10'), scopeInfo.color)}>
                          {scopeInfo.label}
                        </Badge>
                      </div>
                    ...
                    <div className="text-sm text-slate-300 mb-2">
                      {comment.encrypted && comment.content.includes('*') ? (<span className="font-mono text-slate-500">{comment.content}</span>) : (comment.content)}
                    </div>
