...
export default function CertificationVerificationCard() {
    ...
    const [crossDeckSyncStatus, setCrossDeckSyncStatus] = useState("pending");
    ...
    const handleScanCertificate = () => {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                const matchedRecord = mockCertifications.find(cert => cert.certificateHash === inputHash.trim());
    ...
    return (<Card className="w-full max-w-md mx-auto bg-slate-800 border-slate-700 text-white">
      <CardHeader className="pb-4">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-300">
          ...
          <Badge variant={crossDeckSyncStatus === "synced" ? "default" : crossDeckSyncStatus === "failed" ? "destructive" : "secondary"} className={`text-xs ${crossDeckSyncStatus === "synced" ? "bg-blue-600 hover:bg-blue-700" :
          ...
          {zkpMismatch && (<span className="text-amber-400 text-xs animate-pulse">Path B Active</span>)}
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <Input value={inputHash} onChange={(e) => setInputHash(e.target.value)} placeholder="0x7f3e2d1a9b5c or scan QR" className="flex-1 bg-slate-700 border-slate-600 text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500" disabled={verificationStatus === "scanning" || verificationStatus === "verifying"}/>
            <Button onClick={handleQRScan} variant="outline" size="sm" className="border-slate-600 text-slate-300 hover:bg-slate-700" disabled={verificationStatus === "scanning" || verificationStatus === "verifying"}>
              <Scan className="h-4 w-4"/>
            </Button>
          </div>
        </div>

        {verificationStatus !== "idle" && (<div className="space-y-3">
            <div className="flex items-center gap-3 p-3 bg-slate-700 rounded-md">
              ...
              <div className="flex-1">
                <div className={`text-sm font-medium ${getStatusColor()}`}>
                  ...
                  {verificationStatus === "completed" && (verificationResult?.isValid && verificationResult?.zkpVerified ?
                "Certificate Verified" : "Verification Failed")}
                  {verificationStatus === "failed" && "Verification Failed"}
                </div>
                {verificationResult && (<div className="text-xs text-slate-400 mt-1">
                    Processing time: {verificationResult.verificationTime}ms
                  </div>)}
              </div>
            </div>

            {zkpMismatch && (<div className="flex items-center gap-2 p-2 bg-amber-900/30 border border-amber-600 rounded-md">
        ...
        {verificationResult && verificationResult.matchedRecord && (<div className="space-y-3">
        ...
        <div className="flex gap-2">
          <Button onClick={handleScanCertificate} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white" disabled={!inputHash.trim() || verificationStatus === "scanning" || verificationStatus === "verifying"}>
        ...
        {verificationHistory.length > 0 && (<>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2 max-h-32 overflow-y-auto">
                {verificationHistory.map((result, index) => (<div key={index} className="p-2 bg-slate-700 rounded-md">
                    <div className="flex items-center justify-between text-xs">
                      <code className="font-mono text-slate-400">
                        {result.certificateHash.slice(0, 12)}...
                      </code>
                      <div className="flex items-center gap-1">
                        {result.isValid && result.zkpVerified ? (<CheckCircle2 className="h-3 w-3 text-green-400"/>) : (<XCircle className="h-3 w-3 text-red-400"/>)}
        ...
        <div className="text-xs text-slate-400 space-y-1">
          ...
          {retryCount > 0 && (<div className="flex justify-between">
