...
const generateStressContributions = () => {
    const topics = ["civic-basics", "electoral-systems", "constitutional-law", "policy-law", "public-admin"];
    const statuses = ["approved", "reviewed", "submitted", "rejected", "draft"];
    const contributions = [];
...
const mockContributions = generateStressContributions();
const topicOptions = [
...
export default function KnowledgeContributionCard() {
    ...
    const [zkpValidation, setZkpValidation] = useState("pending");
    ...
    const handleSubmitContribution = () => {
        if (!newContent.trim() || !selectedTopic)
            return;
    ...
    const getDownvoteRate = (contrib) => {
        const total = contrib.upvotes + contrib.downvotes;
        return total > 0 ? Math.round((contrib.downvotes / total) * 100) : 0;
    };
    return (<Card className="w-full max-w-md mx-auto bg-slate-800 border-slate-700 text-white">
      <CardHeader className="pb-4">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-300">
          ...
          <Badge variant={zkpValidation === "validated" ? "default" : zkpValidation === "failed" ? "destructive" : "secondary"} className={`text-xs ${zkpValidation === "validated" ? "bg-blue-600 hover:bg-blue-700" :
          ...
          {pushbackTriggered && (<span className="text-amber-400 text-xs animate-pulse">Path B Active</span>)}
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-3">
          ...
          <Select value={selectedTopic} onValueChange={setSelectedTopic}>
            ...
            <SelectContent className="bg-slate-700 border-slate-600">
              {topicOptions.map((option) => (<SelectItem key={option.value} value={option.value} className="text-white hover:bg-slate-600">
                  {option.label}
                </SelectItem>))}
            </SelectContent>
          </Select>

          <Textarea value={newContent} onChange={(e) => setNewContent(e.target.value)} placeholder="Share your civic knowledge or insights..." className="bg-slate-700 border-slate-600 text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 min-h-[80px]" maxLength={500} disabled={submissionStatus === "submitting"}/>

          <Input value={newMediaURL} onChange={(e) => setNewMediaURL(e.target.value)} placeholder="Optional: Add media URL or resource link" className="bg-slate-700 border-slate-600 text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500" disabled={submissionStatus === "submitting"}/>
        ...
        <Button onClick={handleSubmitContribution} className="w-full bg-blue-600 hover:bg-blue-700 text-white" disabled={!newContent.trim() || !selectedTopic || submissionStatus === "submitting"}>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto">
            ...
            return (<div key={contrib.id} className={`p-3 bg-slate-700 rounded-md space-y-2 ${isHighDownvote ? 'border border-amber-600 bg-amber-900/10' : ''}`}>
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      {isHighDownvote && (<AlertTriangle className="h-3 w-3 text-amber-400"/>)}
                    </div>
                  ...
                  {contrib.mediaURL && (<div className="text-xs text-blue-400 truncate">
                      🔗 {contrib.mediaURL}
                    </div>)}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <button onClick={() => handleVote(contrib.id, "up")} className={`flex items-center gap-1 px-2 py-1 rounded transition-colors ${contrib.userHasVoted === "up"
                    ...
                    : "bg-slate-600 text-slate-300 hover:bg-green-600 hover:text-white"}`}>
                      ...
                      <button onClick={() => handleVote(contrib.id, "down")} className={`flex items-center gap-1 px-2 py-1 rounded transition-colors ${contrib.userHasVoted === "down"
                  ...
                  {isHighDownvote && (<div className="text-xs text-amber-400 bg-amber-900/20 p-2 rounded">
        ...
        <div className="text-xs text-slate-400 space-y-1">
          ...
          {pushbackTriggered && (<div className="flex justify-between">
