...
export default function CurriculumAssessmentCard() {
    const [selectedCurriculum, setSelectedCurriculum] = useState("civic-basics");
    const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
    const [selectedAnswer, setSelectedAnswer] = useState(null);
    const [assessmentResults, setAssessmentResults] = useState([]);
    const [assessmentStatus, setAssessmentStatus] = useState("idle");
    const [timeSpent, setTimeSpent] = useState(0);
    const [zkpValidation, setZkpValidation] = useState("verified");
    ...
    const progressPercentage = useMemo(() => totalQuestions > 0 ? (currentQuestionIndex / totalQuestions) * 100 : 0, [currentQuestionIndex, totalQuestions]);
    // Optimized handlers
    ...
    return (<Card className="w-full max-w-md mx-auto bg-slate-800 border-slate-700 text-white">
      <CardHeader className="pb-4">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-300">
          ...
          <Badge variant={zkpValidation === "verified" ? "default" : zkpValidation === "failed" ? "destructive" : "secondary"} className={`text-xs ${zkpValidation === "verified" ? "bg-blue-600 hover:bg-blue-700" :
          ...
          {retryCount > 0 && (<span className="text-amber-400 text-xs">Retries: {retryCount}</span>)}
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        {assessmentStatus === "idle" && (<>
            <div className="space-y-3">
              ...
              <select value={selectedCurriculum} onChange={(e) => setSelectedCurriculum(e.target.value)} className="w-full p-2 bg-slate-700 border border-slate-600 rounded-md text-white text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <Button onClick={handleStartAssessment} className="w-full bg-blue-600 hover:bg-blue-700 text-white" disabled={totalQuestions === 0}>
              Start Assessment
            </Button>
          </>)}
        ...
        {assessmentStatus === "active" && currentQuestion && (<>
            ...
            <div className="space-y-4">
              ...
              {currentQuestion.options && (<div className="space-y-2">
                  {currentQuestion.options.map((option, index) => (<label key={index} className={`flex items-center gap-3 p-3 rounded-md border cursor-pointer transition-colors ${selectedAnswer === index
                        ? "border-blue-500 bg-blue-600/20 text-blue-300"
                        : "border-slate-600 hover:border-slate-500 hover:bg-slate-700/50"}`}>
                      <input type="radio" name="answer" value={index} checked={selectedAnswer === index} onChange={() => setSelectedAnswer(index)} className="text-blue-500 focus:ring-blue-500"/>
                      <span className="text-sm">{option}</span>
                    </label>))}
                </div>)}
            </div>

            <Button onClick={handleAnswerSubmit} disabled={selectedAnswer === null} className="w-full bg-green-600 hover:bg-green-700 text-white disabled:opacity-50">
        ...
        {assessmentStatus === "completed" && (<>
            ...
            <div className="flex gap-2">
              <Button onClick={handleViewResults} variant="outline" className="flex-1 border-slate-600 text-slate-300 hover:bg-slate-700">
                <Eye className="h-4 w-4 mr-1"/>
                View Details
              </Button>
              <Button onClick={handleResetAssessment} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white">
        ...
        {assessmentStatus === "reviewing" && (<>
            <div className="space-y-4">
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto">
                ...
                if (!question)
                    return null;
                return (<div key={result.questionId} className="p-3 bg-slate-700 rounded-md">
                      <div className="flex items-center gap-2 mb-2">
                        {result.isCorrect ? (<CheckCircle2 className="h-4 w-4 text-green-400"/>) : (<XCircle className="h-4 w-4 text-red-400"/>)}
                      ...
                      {question.options && (<div className="text-xs text-slate-400">
                          Your answer: {question.options[result.userAnswer || 0]}
                          {!result.isCorrect && (<div className="text-green-400 mt-1">
              ...
              <Button onClick={handleResetAssessment} className="w-full bg-blue-600 hover:bg-blue-700 text-white">
