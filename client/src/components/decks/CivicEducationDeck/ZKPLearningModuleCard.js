...
import { GraduationCap, CheckCircle, XCircle, AlertTriangle, BookOpen, Award, Clock, User, Brain } from 'lucide-react';
...
export const ZKPLearningModuleCard = ({ className }) => {
    ...
    const curriculumInfo = useMemo(() => CURRICULUM_OPTIONS.find(opt => opt.value === selectedCurriculum) || CURRICULUM_OPTIONS[0], [selectedCurriculum]);
    ...
    // Optimized handlers
    const handleCurriculumSelect = useCallback((curriculum) => {
    ...
    const getValidationStatusInfo = useCallback((status) => {
        switch (status) {
            case 'verified':
    ...
    return (<Card className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', validationFailureDetected && 'ring-2 ring-red-500/50', className)} role="region" aria-label="ZKP Learning Module Interface">
      {/* Status Indicator */}
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', validationFailureDetected ? 'bg-red-500' :
            currentStage === 'certification' ? 'bg-green-500' :
                currentStage === 'studying' ? 'bg-blue-500' :
                    'bg-slate-500')} aria-label={`Status: ${currentStage.charAt(0).toUpperCase() + currentStage.slice(1)}`}/>
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className={cn('border-opacity-50', validationFailureDetected ? 'border-red-500/30 bg-red-500/10 text-red-400' :
      ...
      <CardContent className="space-y-4">
        ...
        {validationFailureDetected && (<div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
        ...
        <div className="space-y-3">
          ...
          <Select value={selectedCurriculum} onValueChange={handleCurriculumSelect} disabled={currentStage === 'studying' || currentStage === 'certification'}>
            <SelectTrigger className="bg-slate-800/50 border-slate-600 text-slate-200">
              <SelectValue />
            </SelectTrigger>
            <SelectContent className="bg-slate-800 border-slate-600">
              {CURRICULUM_OPTIONS.map((option) => (<SelectItem key={option.value} value={option.value} className="text-slate-200 hover:bg-slate-700">
        ...
        {/* Study Progress */}
        {currentStage === 'studying' && (<div className="space-y-2">
        ...
        <div className="space-y-2">
          {currentStage === 'selection' && (<Button onClick={handleStartLearning} className="w-full bg-blue-600 hover:bg-blue-700 text-white" size="sm">
              <BookOpen className="w-4 h-4 mr-2"/>
              Start Learning
            </Button>)}

          {currentStage === 'assessment' && (<Button onClick={handleCompleteAssessment} className="w-full bg-green-600 hover:bg-green-700 text-white" size="sm">
              <Award className="w-4 h-4 mr-2"/>
              Complete Assessment
            </Button>)}

          {currentStage === 'certification' && (<div className="text-center p-3 bg-green-500/10 border border-green-500/30 rounded-lg">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            return (<div key={submission.id} className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      {statusInfo.icon}
