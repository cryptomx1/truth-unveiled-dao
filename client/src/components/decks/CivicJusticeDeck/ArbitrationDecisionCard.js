...
import { Gavel, Shield, Clock, Eye, EyeOff, CheckCircle, XCircle, AlertTriangle, Archive, Lock, FileText } from 'lucide-react';
...
const getDecisionStageInfo = (stage) => {
    switch (stage) {
        case 'draft':
            return {
...
const getRulingOutcomeInfo = (outcome) => {
    switch (outcome) {
        case 'approved':
            return {
...
const calculateBiasDeviation = (rulings) => {
    if (rulings.length === 0)
        return 0;
...
export const ArbitrationDecisionCard = ({ className }) => {
    const [rulings, setRulings] = useState(MOCK_RULINGS);
    ...
    const [biasDeviation, setBiasDeviation] = useState(0);
    const [verdictRevealed, setVerdictRevealed] = useState({});
    ...
    const handleStageProgression = async (newStage, outcome) => {
    ...
    const toggleVerdictReveal = (rulingId) => {
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', biasDetected && 'animate-pulse ring-2 ring-red-500/50', className)} role="region" aria-label="Arbitration Decision Interface">
      {/* Status Indicator Dot */}
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', biasDetected ? 'bg-red-500 animate-pulse' :
            currentStage === 'archived' ? 'bg-amber-500' :
                currentStage === 'ruled' ? 'bg-green-500' :
                    currentStage === 'reviewing' ? 'bg-blue-500 animate-pulse' :
                        'bg-slate-500')} aria-label={biasDetected ? "Status: Bias Alert" :
            `Status: ${stageInfo.label}`} title={biasDetected ? `${biasDeviation.toFixed(1)}% bias deviation detected` :
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className={cn('border-opacity-50', biasDetected ? 'border-red-500/30 bg-red-500/10 text-red-400' :
            stageInfo.color.replace('text-', 'border-').replace('-400', '-500/30'), biasDetected ? 'bg-red-500/10' : stageInfo.bgColor, biasDetected ? 'text-red-400' : stageInfo.color)}>
      ...
      <CardContent className="space-y-4">
        ...
        {biasDetected && (<div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
        ...
        {/* Decision Workflow */}
        {currentStage !== 'archived' && (<div className="space-y-3">
            ...
            {currentStage === 'draft' && (<div className="space-y-3">
                <Textarea value={draftVerdict} onChange={(e) => setDraftVerdict(e.target.value)} placeholder="Draft your arbitration verdict and reasoning..." className="bg-slate-800/50 border-slate-600 text-slate-200 placeholder:text-slate-400 min-h-[100px] resize-none" maxLength={1000}/>
                ...
                }} disabled={!draftVerdict.trim() || isProcessing} size="sm" className="bg-red-600 hover:bg-red-700 text-white">
                      <div className="flex items-center gap-2">
                ...
                }} disabled={!draftVerdict.trim() || isProcessing} size="sm" className="bg-green-600 hover:bg-green-700 text-white">
                      <div className="flex items-center gap-2">
            ...
            {currentStage === 'reviewing' && (<div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                ...
                <div className="mt-3">
                  <Button onClick={() => handleStageProgression('ruled')} disabled={isProcessing} size="sm" className="bg-cyan-600 hover:bg-cyan-700 text-white w-full">
            ...
            {currentStage === 'ruled' && (<div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                ...
                <Button onClick={handleRulingSubmission} disabled={isProcessing} size="sm" className="bg-amber-600 hover:bg-amber-700 text-white w-full">
                  <div className="flex items-center gap-2">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-[250px] w-full">
            ...
            return (<div key={ruling.id} className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-3">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <div className={cn('p-1 rounded', outcomeInfo.color.includes('green') ? 'bg-green-500/20' :
                    outcomeInfo.color.includes('red') ? 'bg-red-500/20' :
                      ...
                      <div className="flex items-center gap-2">
                        <Button onClick={() => toggleVerdictReveal(ruling.id)} size="sm" variant="outline" className="border-slate-600 text-slate-400 hover:bg-slate-700 p-1 h-6 w-6">
                    ...
                    <div className={cn('text-xs p-2 rounded bg-slate-900/50 mb-2', !isRevealed && 'blur-sm select-none')}>
