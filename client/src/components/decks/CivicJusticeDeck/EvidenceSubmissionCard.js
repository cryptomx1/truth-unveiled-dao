...
import { FileText, Upload, Shield, Eye, AlertTriangle, CheckCircle, FileImage, FileAudio, File, Lock, Database } from 'lucide-react';
...
const calculateMetadataMismatchRate = (evidenceFiles) => {
    if (evidenceFiles.length === 0)
        return 0;
    const mismatchedFiles = evidenceFiles.filter(file => !file.verified).length;
    return (mismatchedFiles / evidenceFiles.length) * 100;
};
export const EvidenceSubmissionCard = ({ className }) => {
    const [evidenceFiles, setEvidenceFiles] = useState(MOCK_EVIDENCE_FILES);
    ...
    const [metadataMismatchRate, setMetadataMismatchRate] = useState(0);
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', tamperDetected && 'animate-pulse ring-2 ring-red-500/50', className)} role="region" aria-label="Evidence Submission Interface">
      {/* Hidden file input */}
      <input ref={fileInputRef} type="file" multiple accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.mp3,.wav,.mp4,.mov" onChange={handleFileInputChange} className="hidden" aria-label="Select files for evidence submission"/>

      {/* Status Indicator Dot */}
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', tamperDetected ? 'bg-red-500 animate-pulse' :
            submissionStatus === 'submitted' ? 'bg-green-500' :
                submissionStatus === 'uploading' || submissionStatus === 'wrapping' ? 'bg-amber-500 animate-pulse' :
                    'bg-blue-500')} aria-label={tamperDetected ? "Status: Tamper Alert" :
            submissionStatus === 'submitted' ? "Status: Evidence Submitted" :
                submissionStatus === 'uploading' || submissionStatus === 'wrapping' ? "Status: Processing" :
                    "Status: Ready"} title={tamperDetected ? `${metadataMismatchRate.toFixed(1)}% metadata mismatch detected` :
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className={cn('border-opacity-50', tamperDetected ? 'border-red-500/30 bg-red-500/10 text-red-400' :
      ...
      <CardContent className="space-y-4">
        ...
        {tamperDetected && (<div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="mt-3 pt-2 border-t border-slate-700">
            <div className="flex justify-between text-xs">
              ...
              <span className={cn('font-medium', metadataMismatchRate > 10 ? 'text-red-400' : 'text-green-400')}>
        ...
        <div className="space-y-3">
          ...
          <Textarea value={newDescription} onChange={(e) => setNewDescription(e.target.value)} placeholder="Describe the evidence and its relevance to the case..." className="bg-slate-800/50 border-slate-600 text-slate-200 placeholder:text-slate-400 min-h-[80px] resize-none" maxLength={500}/>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <Button onClick={triggerFileUpload} disabled={isProcessing} variant="outline" size="sm" className="border-slate-600 text-slate-300 hover:bg-slate-700">
                <div className="flex items-center gap-2">
                  <Upload className="w-3 h-3"/>
                  Select Files
                </div>
              </Button>
              <Button onClick={handleFileUpload} disabled={!newDescription.trim() || isProcessing} size="sm" className="bg-cyan-600 hover:bg-cyan-700 text-white">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-[200px] w-full">
            ...
            return (<div key={file.id} className={cn('bg-slate-800/30 rounded-lg border p-3', file.verified ? 'border-slate-700/50' : 'border-red-500/30')}>
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <div className={cn('p-1 rounded', typeInfo.bgColor, typeInfo.color)}>
                          {typeInfo.icon}
                        </div>
                      ...
                      <div className="flex items-center gap-2">
                        {file.verified ? (<CheckCircle className="w-4 h-4 text-green-400"/>) : (<AlertTriangle className="w-4 h-4 text-red-400"/>)}
