...
import { Shield, Clock, Eye, EyeOff, CheckCircle, XCircle, AlertTriangle, Archive, Gavel, Upload, Database } from 'lucide-react';
...
const calculateZKPMismatchRate = (events) => {
    if (events.length === 0)
        return 0;
...
export const JusticeAuditCard = ({ className }) => {
    ...
    const [zkpMismatchDetected, setZkpMismatchDetected] = useState(false);
    ...
    return (<Card ref={cardRef} className={cn('w-full max-w-sm mx-auto relative', 'bg-slate-900 border-slate-700/50', 'dao-card-gradient', zkpMismatchDetected && 'animate-pulse ring-2 ring-red-500/50', className)} role="region" aria-label="Justice Audit Trail Interface">
      {/* Status Indicator Dot */}
      <div className={cn('absolute top-3 right-3 w-3 h-3 rounded-full', zkpMismatchDetected ? 'bg-red-500 animate-pulse' :
            auditComplete ? 'bg-green-500' :
                'bg-blue-500 animate-pulse')} aria-label={zkpMismatchDetected ? "Status: ZKP Mismatch Alert" :
            auditComplete ? "Status: Audit Complete" :
                "Status: Audit In Progress"} title={zkpMismatchDetected ? `${zkpMismatchRate.toFixed(1)}% ZKP mismatch detected` :
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className={cn('border-opacity-50', zkpMismatchDetected ? 'border-red-500/30 bg-red-500/10 text-red-400' :
      ...
      <CardContent className="space-y-4">
        {/* ZKP Mismatch Alert */}
        {zkpMismatchDetected && (<div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
          ...
          <div className="mt-3 pt-2 border-t border-slate-700">
            <div className="flex justify-between text-xs">
              ...
              <span className={cn('font-medium', zkpMismatchDetected ? 'text-red-400' : 'text-green-400')}>
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-[280px] w-full">
            ...
            return (<div key={event.id} className={cn('bg-slate-800/30 rounded-lg border p-3', event.status === 'verified' ? 'border-slate-700/50' :
                    event.status === 'failed' ? 'border-red-500/30' :
                        'border-amber-500/30')}>
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <div className={cn('p-1 rounded', eventTypeInfo.bgColor, eventTypeInfo.color)}>
                            {eventTypeInfo.icon}
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button onClick={() => toggleEventExpansion(event.id)} size="sm" variant="outline" className="border-slate-600 text-slate-400 hover:bg-slate-700 p-1 h-6 w-6">
                      ...
                      {isExpanded && (<div className="mt-3 pt-2 border-t border-slate-700/50 space-y-2">
                          <div className="grid grid-cols-1 gap-2 text-xs">
                            ...
                            {event.outcome && (<div className="flex justify-between">
                                <span className="text-slate-500">Outcome:</span>
                                <span className={cn(event.outcome === 'approved' ? 'text-green-400' : 'text-red-400')}>
                                  {event.outcome.charAt(0).toUpperCase() + event.outcome.slice(1)}
                                </span>
                              </div>)}
                            {event.arbitratorDID && (<div className="flex justify-between">
                                <span className="text-slate-500">Arbitrator:</span>
                                <span className="text-amber-400 font-mono">{event.arbitratorDID}</span>
                              </div>)}
                          </div>
                          
                          {event.rulingReason && (<div className="mt-2 p-2 bg-slate-900/50 rounded text-xs text-slate-300">
