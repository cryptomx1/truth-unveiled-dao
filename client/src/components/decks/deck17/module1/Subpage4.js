...
export const Deck17Module1Subpage4 = ({ subpageId = 'network-configuration', networkTier = 'advanced' }) => {
    ...
    const handleNetworkAction = (action, nodeId) => {
    ...
    return (<div className="w-full space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {networkNodes.map((node) => (<Card key={node.id} className={`border cursor-pointer transition-all ${selectedNetwork === node.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700'}`} onClick={() => handleNodeSelect(node.id)}>
                ...
                handleNetworkAction('monitor', node.id);
                    ...
                    {node.status === 'Online' && (<Button size="sm" variant="ghost" onClick={(e) => {
                    e.stopPropagation();
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {networkConfigurations.map((config, index) => (<div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <Button variant="outline" className="h-20 flex flex-col gap-2" onClick={() => handleNetworkAction('add-node')}>
              <Server className="h-5 w-5"/>
              <span className="text-sm">Add Node</span>
            </Button>
            <Button variant="outline" className="h-20 flex flex-col gap-2" onClick={() => handleNetworkAction('configure-routing')}>
              <Router className="h-5 w-5"/>
              <span className="text-sm">Configure Routing</span>
            </Button>
            <Button variant="outline" className="h-20 flex flex-col gap-2" onClick={() => handleNetworkAction('bandwidth-analysis')}>
              <Wifi className="h-5 w-5"/>
              <span className="text-sm">Bandwidth Analysis</span>
            </Button>
            <Button variant="outline" className="h-20 flex flex-col gap-2" onClick={() => handleNetworkAction('security-scan')}>
