...
import { Vote, FileText, TrendingUp } from 'lucide-react';
import { useTranslation } from '../../../translation/useTranslation';
import { useLangContext } from '../../../context/LanguageContext';
export const GovernanceCard = ({ className = '', onVote }) => {
    ...
    return (<div className={`bg-slate-800 rounded-lg p-4 max-w-md mx-auto ${className}`}>
      {/* Header */}
      ...
      <div className="space-y-3">
        {proposals.map((proposal) => {
            ...
            return (<div key={proposal.id} className="p-3 bg-slate-700/30 rounded-lg">
              {/* Proposal Header */}
              ...
              <div className="mb-2">
                ...
                <div className="w-full bg-slate-600 rounded-full h-2">
                  <div className="bg-green-500 h-2 rounded-full" style={{ width: `${supportPercentage}%` }}/>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-2">
                  <button onClick={() => handleVote(proposal.id, 'support')} disabled={hasVoted} className={`px-3 py-1 rounded text-xs transition-colors ${hasVoted
                    ? 'bg-slate-600 text-slate-400 cursor-not-allowed'
                    : 'bg-green-600/20 hover:bg-green-600/40 border border-green-500/50 text-green-300'}`} aria-label={`${t('card.governance.voting')} - Support`}>
                    Support
                  </button>
                  <button onClick={() => handleVote(proposal.id, 'oppose')} disabled={hasVoted} className={`px-3 py-1 rounded text-xs transition-colors ${hasVoted
                    ? 'bg-slate-600 text-slate-400 cursor-not-allowed'
                    : 'bg-red-600/20 hover:bg-red-600/40 border border-red-500/50 text-red-300'}`} aria-label={`${t('card.governance.voting')} - Oppose`}>
                    Oppose
                  </button>
                </div>
      ...
      <div className="mt-4 flex gap-2">
        <button className="flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-lg bg-purple-600/20 hover:bg-purple-600/40 border border-purple-500/50 text-purple-300 transition-colors" aria-label={t('card.governance.results')}>
          <TrendingUp className="w-4 h-4"/>
          <span className="text-xs">{t('card.governance.results')}</span>
        </button>
        <button className="flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-lg bg-blue-600/20 hover:bg-blue-600/40 border border-blue-500/50 text-blue-300 transition-colors" aria-label="View All Proposals">
