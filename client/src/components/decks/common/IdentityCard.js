...
export const IdentityCard = ({ did = "did:civic:user_abc123", verificationLevel = 'enhanced', credentialCount = 3, className = '', onVerify }) => {
    ...
    return (<div className={`bg-slate-800 rounded-lg p-4 max-w-md mx-auto ${className}`}>
      ...
      <div className="space-y-2 mb-4">
        <button onClick={onVerify} className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-lg bg-emerald-600/20 hover:bg-emerald-600/40 border border-emerald-500/50 text-emerald-300 transition-colors" aria-label={t('aria.identity.verify')}>
          <Key className="w-4 h-4"/>
          <span className="text-sm">{t('card.identity.verify')}</span>
        </button>
        <button onClick={() => setShowCredentials(!showCredentials)} className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-lg bg-blue-600/20 hover:bg-blue-600/40 border border-blue-500/50 text-blue-300 transition-colors" aria-label={t('card.identity.credentials')}>
      ...
      {/* Credentials List */}
      {showCredentials && (<div className="mb-4 p-3 bg-slate-700/20 rounded-lg">
          <h4 className="text-sm text-slate-300 mb-2">Your Credentials</h4>
          <div className="space-y-2">
            {credentials.map((credential) => (<div key={credential.id} className="flex items-center justify-between p-2 bg-slate-600/30 rounded">
