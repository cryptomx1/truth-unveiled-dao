...
import { CheckCircle, XCircle, AlertTriangle, Shield, Zap, Hash, User } from 'lucide-react';
// Mock enforcement status data
const MOCK_ENFORCEMENT_STATUS = {
    status: 'enforced',
...
export const EnforcementStatus = ({ className }) => {
    const [statusData, setStatusData] = useState(MOCK_ENFORCEMENT_STATUS);
    ...
    return (<Card className={cn('bg-slate-800 border-slate-700 shadow-xl max-h-[600px] w-full overflow-hidden', className)} role="region" aria-label="Enforcement Status" data-testid="enforcement-status">
      ...
      <CardContent className="space-y-6">
        {/* Status Overview */}
        <div className={cn('rounded-lg p-4 border space-y-4', statusInfo.bgColor, statusInfo.borderColor)}>
          <div className="flex items-center justify-between">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button onClick={handleExecuteEnforcement} disabled={isExecuting || statusData.userRole === 'citizen'} className="bg-blue-600 hover:bg-blue-700 text-white" style={{ minHeight: '36px', minWidth: '100px' }}>
            {isExecuting ? (<div className="flex items-center space-x-2">
