...
import { Hash, Shield, CheckCircle, XCircle, Eye, Copy, Link, Zap, AlertTriangle } from 'lucide-react';
// Mock ZKP proof trail data
const MOCK_ZKP_PROOF = {
...
export const ZKPProofTrail = ({ className }) => {
    const [proofData, setProofData] = useState(MOCK_ZKP_PROOF);
    ...
    return (<Card className={cn('bg-slate-800 border-slate-700 shadow-xl max-h-[600px] w-full overflow-hidden', className)} role="region" aria-label="ZKP Proof Trail" data-testid="zkp-proof-trail">
      ...
      <CardContent className="space-y-6">
        ...
        <div className={cn('rounded-lg p-4 border space-y-3', statusInfo.bgColor, statusInfo.borderColor)}>
          <div className="flex items-center justify-between">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <Button onClick={() => handleCopy(proofData.proofHash, 'Proof Hash')} variant="ghost" size="sm" className="h-6 w-6 p-0 text-slate-400 hover:text-white">
                  {copiedField === 'Proof Hash' ? (<CheckCircle className="w-4 h-4 text-green-400"/>) : (<Copy className="w-4 h-4"/>)}
                </Button>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <Button onClick={() => handleCopy(proofData.zkpSignature, 'ZKP Signature')} variant="ghost" size="sm" className="h-6 w-6 p-0 text-slate-400 hover:text-white">
                  {copiedField === 'ZKP Signature' ? (<CheckCircle className="w-4 h-4 text-green-400"/>) : (<Copy className="w-4 h-4"/>)}
                </Button>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <Button onClick={() => handleCopy(proofData.didReference, 'DID Reference')} variant="ghost" size="sm" className="h-6 w-6 p-0 text-slate-400 hover:text-white">
                  {copiedField === 'DID Reference' ? (<CheckCircle className="w-4 h-4 text-green-400"/>) : (<Copy className="w-4 h-4"/>)}
                </Button>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <Button onClick={() => handleCopy(proofData.credentialHash, 'Credential Hash')} variant="ghost" size="sm" className="h-6 w-6 p-0 text-slate-400 hover:text-white">
                  {copiedField === 'Credential Hash' ? (<CheckCircle className="w-4 h-4 text-green-400"/>) : (<Copy className="w-4 h-4"/>)}
                </Button>
              </div>
        ...
        <div className="bg-slate-900 rounded-lg p-4 border border-slate-700 space-y-3">
          ...
          <div className="space-y-2">
            {proofData.linkedDeckModules.map((module, index) => (<div key={index} className="flex items-center justify-between">
        ...
        <div className="flex justify-between items-center">
          ...
          <Button onClick={handleValidateProof} disabled={isValidating} className="bg-indigo-600 hover:bg-indigo-700 text-white" style={{ minHeight: '36px', minWidth: '100px' }}>
            {isValidating ? (<div className="flex items-center space-x-2">
