...
import { CheckCircle, XCircle, Clock, AlertTriangle, Eye, Calendar, RotateCcw } from 'lucide-react';
// Mock appeal status data
const MOCK_APPEAL_STATUS = {
    id: 'appeal_001',
    status: 'submitted',
...
const STATUS_PROGRESSION = ['draft', 'submitted', 'under-review', 'approved'];
export const AppealStatusTracker = ({ className }) => {
    const [statusData, setStatusData] = useState(MOCK_APPEAL_STATUS);
    ...
    return (<Card className={cn('bg-slate-800 border-slate-700 shadow-xl max-h-[600px] w-full overflow-hidden', className)} role="region" aria-label="Appeal Status Tracker" data-testid="appeal-status-tracker">
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className={cn('w-10 h-10 rounded-full flex items-center justify-center', statusInfo.bgColor, statusInfo.borderColor, 'border')}>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="bg-slate-900 rounded-lg p-4 border border-slate-700 space-y-4">
          ...
          <div className="space-y-3">
            ...
            return (<div key={status} className="flex items-center space-x-3">
                  <div className={cn('w-8 h-8 rounded-full flex items-center justify-center border-2', isActive
                    ? `${statusItemInfo.bgColor} ${statusItemInfo.borderColor} border`
                    : 'bg-slate-800 border-slate-600')}>
                    <StatusItemIcon className={cn('w-4 h-4', isActive ? statusItemInfo.color : 'text-slate-500')}/>
                  </div>
                  <div className="flex-1">
                    <p className={cn('text-sm font-medium', isActive ? 'text-white' : 'text-slate-500')}>
                      {statusItemInfo.label}
                    </p>
                    <p className={cn('text-xs', isActive ? 'text-slate-300' : 'text-slate-600')}>
                      {statusItemInfo.description}
                    </p>
                  </div>
                  {isCurrent && isProgressing && (<div className="w-4 h-4 animate-spin rounded-full border-2 border-blue-400 border-t-transparent"/>)}
                </div>);
        })}
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-lg p-4 border border-slate-700 space-y-3">
          ...
          <div className="grid grid-cols-1 gap-2">
            ...
            {statusData.submittedAt && (<div className="flex justify-between items-center">
            ...
            {statusData.reviewDuration && (<div className="flex justify-between items-center">
        ...
        <div className="bg-slate-900 rounded-lg p-4 border border-slate-700 space-y-3">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center space-x-1">
                <span className={cn('text-sm', statusData.processingErrors > 2 ? 'text-red-400' :
            statusData.processingErrors > 0 ? 'text-yellow-400' : 'text-green-400')}>
                  {statusData.processingErrors}
                </span>
                {statusData.processingErrors > 2 && (<AlertTriangle className="w-4 h-4 text-red-400"/>)}
              </div>
            </div>
            
            {statusData.pushbackTriggered && (<div className="flex items-center space-x-2 p-2 bg-red-900/30 border border-red-700 rounded">
        ...
        {/* Action Buttons */}
        <div className="flex justify-between items-center">
          <Button onClick={handleReset} variant="outline" className="border-slate-600 text-slate-400 hover:bg-slate-700" style={{ minHeight: '36px' }}>
