...
export const SwipeRefactorDeck = ({ className }) => {
    ...
    return (<div className={cn('w-full max-w-md mx-auto', className)} ref={containerRef} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} role="region" aria-label="Governance Deck Swipe Interface" data-testid="swipe-refactor-deck">
      {/* Header */}
      ...
      <div className="flex justify-between items-center mb-4">
        <Button onClick={goToPrevious} disabled={currentCard === 0 || isTransitioning} variant="outline" size="sm" className="border-slate-600 text-slate-400 hover:bg-slate-700" style={{ minHeight: '48px', minWidth: '48px' }} aria-label="Previous card">
          <ChevronLeft className="w-5 h-5"/>
        </Button>

        {/* Bullet Navigation */}
        <div className="flex space-x-2" role="tablist" aria-label="Card navigation">
          {CARDS.map((_, index) => (<button key={index} onClick={() => navigateToCard(index)} disabled={isTransitioning} className={cn('w-3 h-3 rounded-full transition-all duration-200', 'focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 focus:ring-offset-slate-800', currentCard === index
                ? 'bg-blue-400 scale-110'
                : 'bg-slate-600 hover:bg-slate-500')} style={{ minHeight: '24px', minWidth: '24px' }} role="tab" aria-selected={currentCard === index} aria-label={`Go to card ${index + 1}: ${CARDS[index].title}`}/>))}
        </div>

        <Button onClick={goToNext} disabled={currentCard === CARDS.length - 1 || isTransitioning} variant="outline" size="sm" className="border-slate-600 text-slate-400 hover:bg-slate-700" style={{ minHeight: '48px', minWidth: '48px' }} aria-label="Next card">
      ...
      <div className="relative overflow-hidden">
        <div className={cn('transition-all duration-200 ease-in-out', isTransitioning ? 'opacity-75 scale-95' : 'opacity-100 scale-100')} style={{ minHeight: '600px' }}>
      ...
      <div aria-live="polite" aria-atomic="true" className="sr-only" id="swipe-announcements">
