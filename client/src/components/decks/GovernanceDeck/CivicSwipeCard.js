...
import { ChevronLeft, ChevronRight, Lock, ThumbsUp, ThumbsDown, Info } from 'lucide-react';
...
export const CivicSwipeCard = ({ className }) => {
    ...
    const handleSwipe = (direction) => {
        if (!currentProposal)
            return;
        const decision = direction === 'right' ? 'support' : 'reject';
    ...
    if (!currentProposal) {
        return (<div className="dao-card-gradient border border-[hsl(var(--dao-border))] rounded-xl p-6 max-w-sm mx-auto">
        <div className="text-center text-white">
          <p>No proposals available</p>
        </div>
      </div>);
    }
    return (<div className={cn('dao-card-gradient border border-[hsl(var(--dao-border))] rounded-xl p-6 shadow-xl hover:shadow-2xl transition-all duration-200 max-w-sm mx-auto relative overflow-hidden', className)} role="region" aria-label="Civic Proposal Voting Card" data-testid="civic-swipe-card">
      {/* Swipe Animation Overlay */}
      {isSwipeActive && (<div className={cn('absolute inset-0 z-10 flex items-center justify-center rounded-xl transition-all duration-300', swipeDirection === 'right' ? 'bg-green-500/30' : 'bg-red-500/30')}>
          <div className={cn('text-6xl font-bold', swipeDirection === 'right' ? 'text-green-400' : 'text-red-400')}>
            {swipeDirection === 'right' ? '✓' : '✗'}
          </div>
        </div>)}

      {/* Success Checkmark Overlay */}
      {showCheckmark && (<div className={cn('absolute inset-0 z-20 flex items-center justify-center rounded-xl transition-opacity duration-500', showCheckmark ? 'opacity-100' : 'opacity-0')}>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {/* TTS Toggle */}
        <TTSToggle deckId="governance" moduleId="civic-swipe" content={`Civic Vote. ${currentProposal.title}. ${currentProposal.summary}. Topic: ${currentProposal.topic}. Region: ${currentProposal.region}. ${currentProposal.urgencyLevel} priority with ${currentProposal.daysLeft} days left to vote.`} size="sm" variant="outline"/>
      </div>

      {/* Proposal Card */}
      <div ref={cardRef} className={cn('bg-white/10 rounded-lg p-4 border border-white/20 mb-6 transition-transform duration-300', isSwipeActive && swipeDirection === 'right' && 'transform translate-x-full', isSwipeActive && swipeDirection === 'left' && 'transform -translate-x-full')}>
        {/* Urgency Badge */}
        <div className="flex items-center justify-between mb-3">
          <span className={cn('px-2 py-1 rounded-full text-xs font-medium uppercase tracking-wide', getUrgencyColor(currentProposal.urgencyLevel))}>
      ...
      <div className="flex items-center justify-center space-x-8">
        {/* Reject Button */}
        <Button onClick={() => handleSwipe('left')} disabled={isSwipeActive} className="w-16 h-16 rounded-full bg-red-600/20 hover:bg-red-600/30 border-2 border-red-500/30 text-red-400 hover:text-red-300 transition-all duration-150" aria-label="Reject proposal" data-testid="reject-button">
        ...
        {/* Support Button */}
        <Button onClick={() => handleSwipe('right')} disabled={isSwipeActive} className="w-16 h-16 rounded-full bg-green-600/20 hover:bg-green-600/30 border-2 border-green-500/30 text-green-400 hover:text-green-300 transition-all duration-150" aria-label="Support proposal" data-testid="support-button">
      ...
      <div className="mt-4 pt-4 border-t border-white/20">
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center space-x-2">
            <div className={cn("w-2 h-2 rounded-full", ttsStatus.isReady ? "bg-green-400" : "bg-gray-500", ttsStatus.isPlaying ? "animate-pulse" : "")}/>
            <span className={cn("font-medium text-xs", ttsStatus.isReady ? "text-green-400" : "text-gray-500")} aria-live="polite">
