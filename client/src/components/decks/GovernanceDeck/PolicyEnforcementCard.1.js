...
import { Shield, AlertTriangle, FileText, Scale, Volume2, Clock, Crown, Users } from 'lucide-react';
// Mock policy overview data
const MOCK_POLICY_OVERVIEW = {
    'curfew': {
        id: 'policy_001',
        policyType: 'curfew',
...
const getPolicyTypeInfo = (policyType) => {
    switch (policyType) {
        case 'curfew':
            return {
...
export const PolicyOverview = ({ className }) => {
    const [selectedPolicy, setSelectedPolicy] = useState('curfew');
    const [currentPolicy, setCurrentPolicy] = useState(MOCK_POLICY_OVERVIEW.curfew);
    ...
    return (<Card className={cn('bg-slate-800 border-slate-700 shadow-xl max-h-[600px] w-full overflow-hidden', className)} role="region" aria-label="Policy Overview" data-testid="policy-overview">
