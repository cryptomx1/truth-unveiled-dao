...
export const VoteLedgerCard = ({ className }) => {
    const [voteHistory] = useState(MOCK_VOTE_HISTORY);
    ...
    const handleExportLedger = async () => {
        ...
        try {
            ...
            const csvData = [
                ...
                ...voteHistory.map(vote => `${vote.id},"${vote.proposalTitle}",${vote.decision},${vote.timestamp.toISOString()},${vote.region},${vote.category},${vote.urgencyLevel},"${vote.proposer}"`)
            ].join('\n');
    ...
    return (<div className={cn('dao-card-gradient border border-[hsl(var(--dao-border))] rounded-xl p-6 shadow-xl hover:shadow-2xl transition-all duration-200 max-w-sm mx-auto', className)} role="region" aria-label="Vote Ledger History" data-testid="vote-ledger-card">
      {/* Header Section */}
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={handleTTSToggle} disabled={!ttsStatus.isReady} className={cn('w-10 h-10 rounded-full p-0 transition-colors duration-150', ttsStatus.isReady
            ...
            : 'bg-gray-500/20 text-gray-500 cursor-not-allowed')} aria-label={ttsStatus.isPlaying ? 'Stop ledger message' : 'Play ledger message'} data-testid="tts-toggle-button">
      ...
      <div className="bg-white/5 rounded-lg border border-white/10 mb-4 max-h-64 overflow-y-auto">
        <div className="p-4">
          ...
          <div className="space-y-3">
            {voteHistory.map((vote) => (<div key={vote.id} className="bg-white/10 rounded-lg p-3 border border-white/20">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center space-x-2 ml-2">
                    {vote.decision === 'support' ? (<CheckCircle className="w-4 h-4 text-green-400"/>) : (<XCircle className="w-4 h-4 text-red-400"/>)}
                  </div>
                </div>
      ...
      <div className="bg-white/10 rounded-lg p-4 border border-white/20">
        <Button onClick={handleExportLedger} disabled={isExporting} className={cn('w-full flex items-center justify-center space-x-2 transition-colors duration-150', isExporting
            ? 'bg-gray-500/20 text-gray-500 cursor-not-allowed'
            : 'bg-blue-600 hover:bg-blue-700 text-white')} aria-label="Download vote ledger" data-testid="export-button">
          {isExporting ? (<>
