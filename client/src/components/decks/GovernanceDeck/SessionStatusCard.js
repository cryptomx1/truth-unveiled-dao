...
export const SessionStatusCard = ({ className }) => {
    ...
    const playTTSMessage = async (message) => {
    ...
    return (<div className={cn('bg-slate-800 border border-slate-700 rounded-xl p-4 shadow-xl hover:shadow-2xl transition-all duration-200 max-w-sm mx-auto', className)} role="region" aria-label="Session Status Information" data-testid="session-status-card">
      {/* Header Section */}
      <div className="flex items-center justify-between mb-4">
        ...
        {/* TTS Button */}
        <Button onClick={handleTTSToggle} disabled={!ttsStatus.isReady} className={cn('w-8 h-8 rounded-full p-0 transition-colors duration-150', ttsStatus.isReady
            ? 'bg-slate-700 hover:bg-slate-600 text-slate-100'
            : 'bg-slate-600 text-slate-500 cursor-not-allowed')} aria-label={ttsStatus.isPlaying ? 'Stop status message' : 'Play status message'} data-testid="tts-toggle-button">
          {ttsStatus.isPlaying ? (<VolumeX className="w-3 h-3"/>) : (<Volume2 className="w-3 h-3"/>)}
        </Button>
      </div>
      ...
      <div className={cn('bg-slate-700 rounded-lg p-4 mb-4', statusConfig.bgColor)}>
        ...
        <div className="w-full bg-slate-600 rounded-full h-2 mb-2">
          <div className="bg-green-500 h-2 rounded-full transition-all duration-300" style={{ width: `${(session.votedProposals / session.totalProposals) * 100}%` }}/>
        </div>
      ...
      <div className="bg-slate-700 rounded-lg p-4 mb-4">
        <div className="text-center">
          ...
          <div className={cn('text-lg font-bold mb-1', session.status === 'closing_soon' ? 'text-amber-400' :
            session.status === 'closed' ? 'text-red-500' : 'text-blue-500')} aria-live="polite" aria-label={`Session timer: ${formatTimeRemaining(timeRemaining)}`}>
      ...
      <div className="mt-4 pt-4 border-t border-slate-700">
        <div className="flex items-center justify-between text-xs">
          ...
          <div className="flex items-center space-x-2">
            <div className={cn("w-2 h-2 rounded-full", ttsStatus.isReady ? "bg-green-500" : "bg-slate-500", ttsStatus.isPlaying ? "animate-pulse" : "")}/>
            <span className={cn("font-medium", ttsStatus.isReady ? "text-green-500" : "text-slate-500")} aria-live="polite">
