...
import { Scale, AlertTriangle, Send, FileText, User, MessageSquare } from 'lucide-react';
...
export const AppealForm = ({ className }) => {
    ...
    return (<Card className={cn('bg-slate-800 border-slate-700 shadow-xl max-h-[600px] w-full overflow-hidden', className)} role="region" aria-label="Appeal Form" data-testid="appeal-form">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <span className={cn('text-xs', characterCount > 450 ? 'text-red-400' : characterCount > 400 ? 'text-yellow-400' : 'text-slate-400')}>
              {characterCount}/500
            </span>
          </div>
          <Textarea id="justification" value={formData.justification} onChange={(e) => handleJustificationChange(e.target.value)} placeholder="Provide detailed justification for your appeal..." className="bg-slate-900 border-slate-700 text-white placeholder:text-slate-500 min-h-[100px] resize-none" maxLength={500} aria-describedby="character-count"/>
        ...
        <div className="flex justify-end">
          <Button onClick={handleSubmit} disabled={!isFormValid || isSubmitting} className="bg-purple-600 hover:bg-purple-700 text-white" style={{ minHeight: '40px', minWidth: '120px' }}>
