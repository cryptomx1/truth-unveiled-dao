...
export default function LedgerStreamVisualizer() {
    ...
    const loadLedgerData = useCallback(async () => {
        ...
        try {
            ...
            const formattedEntries = latest50.map((entry) => ({
    ...
    const applyFilters = useCallback(() => {
        ...
        if (filters.timelineFilter.trim()) {
            ...
            filtered = filtered.filter(entry => entry.credentialId.toLowerCase().includes(query) ||
    ...
    const determineCredentialType = (credentialHash) => {
        // Simple heuristic based on hash patterns
        if (credentialHash.includes('identity'))
            return 'Identity';
        if (credentialHash.includes('voting'))
            return 'Voting';
        if (credentialHash.includes('governance'))
            return 'Governance';
        if (credentialHash.includes('role'))
            return 'Role';
        if (credentialHash.includes('vault'))
            return 'Vault';
        return 'Generic';
    };
    // Determine user role from DID
    const determineUserRole = (did) => {
        if (did.includes('governor') || did.includes('admin'))
            return 'Governor';
        if (did.includes('moderator') || did.includes('verifier'))
            return 'Moderator';
        if (did.includes('citizen') || did.includes('user'))
            return 'Citizen';
        return 'Unknown';
    };
    // Trigger Path B fallback
    const triggerPathB = (reason) => {
        const pathBEntry = {
    ...
    return (<div className="w-full max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      ...
      <div className="mb-6 p-4 bg-slate-700 border border-slate-600 rounded-md">
        ...
        <div className="space-y-2 text-xs">
          ...
          {newEntryIds.size > 0 && (<div className="flex justify-between">
      ...
      <div className="mb-6 space-y-4">
        ...
        <div>
          ...
          <select id="roleFilter" value={filters.roleFilter} onChange={(e) => setFilters(prev => ({ ...prev, roleFilter: e.target.value }))} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 text-white rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
        ...
        <div>
          ...
          <select id="statusFilter" value={filters.statusFilter} onChange={(e) => setFilters(prev => ({ ...prev, statusFilter: e.target.value }))} className="w-full px-3 py-2 bg-slate-700 border border-slate-600 text-white rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
        ...
        <div>
          ...
          <input id="timelineFilter" type="text" value={filters.timelineFilter} onChange={(e) => setFilters(prev => ({ ...prev, timelineFilter: e.target.value }))} placeholder="Search by credential ID, CID, status..." className="w-full px-3 py-2 bg-slate-700 border border-slate-600 text-white placeholder-slate-400 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"/>
        </div>

        {/* Control Buttons */}
        <div className="grid grid-cols-2 gap-2">
          <button onClick={() => setAutoRefresh(!autoRefresh)} className={`py-2 px-3 font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 ${autoRefresh
            ? 'bg-green-600 hover:bg-green-700 text-white focus:ring-green-500'
            : 'bg-slate-600 hover:bg-slate-700 text-slate-300 focus:ring-slate-500'}`} style={{ minHeight: '48px' }}>
            Auto Refresh {autoRefresh ? 'ON' : 'OFF'}
          </button>
          
          <button onClick={refreshLedgerData} className="py-2 px-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800" style={{ minHeight: '48px' }}>
            Refresh Now
          </button>
          
          <button onClick={handleExportBundle} className="py-2 px-3 bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-md transition-colors duration-200 focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-slate-800 col-span-2" style={{ minHeight: '48px' }}>
            Export Bundle (.json)
          </button>
        </div>
      </div>
      ...
      <div className="bg-slate-700 border border-slate-600 rounded-md">
        ...
        <div className="max-h-96 overflow-y-auto">
          {isLoading ? (<div className="p-6 text-center">
              <div className="animate-spin w-6 h-6 border-2 border-blue-500 border-t-transparent rounded-full mx-auto mb-2"></div>
              <div className="text-sm text-slate-400">Loading ledger stream...</div>
            </div>) : filteredEntries.length > 0 ? (<div className="divide-y divide-slate-600">
              {filteredEntries.slice().reverse().map((entry) => {
                ...
                return (<div key={entry.syncId} className={`p-3 hover:bg-slate-600 transition-all duration-300 ${isNewEntry ? 'bg-blue-900/20 animate-pulse' : ''}`} role="row" aria-label={`Ledger entry for ${entry.credentialType} credential with ${statusDisplay.status} status`}>
                    ...
                    {isNewEntry && (<div className="mt-2 p-2 bg-blue-900/20 border border-blue-700 rounded">
      ...
      {/* Export Modal */}
      {showExportModal && exportBundle && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-slate-800 border border-slate-700 rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              <button onClick={() => setShowExportModal(false)} className="py-2 px-4 bg-slate-600 hover:bg-slate-700 text-white rounded-md transition-colors duration-200">
                Cancel
              </button>
              
              <button onClick={downloadExportBundle} className="py-2 px-4 bg-green-600 hover:bg-green-700 text-white rounded-md transition-colors duration-200">
                Download JSON
              </button>
            </div>
          </div>
