...
export default function SocialCohesionCard() {
    ...
    const [actionType, setActionType] = useState("helped");
    const [isRecording, setIsRecording] = useState(false);
    const [showPathB, setShowPathB] = useState(false);
    const [disengagementAlert, setDisengagementAlert] = useState(false);
    ...
    const handleRecordInteraction = async () => {
        ...
        const trustImpact = actionType === "helped" ? Math.floor(Math.random() * 5) + 6 :
    ...
    const getActionDescription = (action, name) => {
        switch (action) {
            case "helped":
    ...
    const calculateBondStrength = (reputation, trustActions) => {
    ...
    return (<Card className="w-full max-w-md mx-auto h-[600px] bg-slate-800 border-slate-700 text-white">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={selectedConnection} onValueChange={setSelectedConnection}>
            ...
            <SelectContent className="bg-slate-700 border-slate-600">
              {connections.map(connection => (<SelectItem key={connection.id} value={connection.id} className="text-white hover:bg-slate-600">
        ...
        {/* Connection Details */}
        {selectedConnectionData && (<div className="space-y-2">
        ...
        {/* Record Interaction */}
        <div className="space-y-2">
          <Button onClick={handleRecordInteraction} disabled={!canRecord} className="w-full h-10 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600" aria-label="Record interaction">
            {isRecording ? (<>
          ...
          {!canRecord && !isRecording && (<div className="text-xs text-amber-400 text-center" role="alert" aria-live="polite">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-32">
            ...
            return (<div key={interaction.id} className="p-2 rounded-md bg-slate-700/50 border border-slate-600">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1">
            {referrals.slice(0, 2).map(referral => (<div key={referral.id} className="p-2 rounded-md bg-slate-700/30 border border-slate-600">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-300">{referral.serviceType}</span>
                  <Badge className={`text-xs ${referral.status === "completed" ? "bg-green-600" :
                referral.status === "accepted" ? "bg-blue-600" :
                    referral.status === "pending" ? "bg-yellow-600" : "bg-red-600"} text-white`}>
                    {referral.status}
                  </Badge>
                </div>
        ...
        {/* Disengagement Alert */}
        {disengagementAlert && (<div className="p-2 bg-red-900/50 border border-red-600 rounded-md animate-pulse" role="alert" aria-live="polite">
            <div className="flex items-center gap-2">
