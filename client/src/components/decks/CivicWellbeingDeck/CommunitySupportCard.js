...
import { HandHeart, Home, Utensils, Car, Zap, Heart, Clock, AlertTriangle, Sparkles, Hash, UserCheck } from "lucide-react";
...
export default function CommunitySupportCard() {
    ...
    const [communityAgents, setCommunityAgents] = useState(mockCommunityAgents);
    ...
    const handleSubmitRequest = async () => {
        ...
        const priority = selectedCategory === "mental_health" ? "high" :
        ...
        if (matchChance > 0.3) {
            setTimeout(async () => {
                const availableAgents = communityAgents.filter(agent => agent.availability && agent.specialties.includes(selectedCategory));
                if (availableAgents.length > 0) {
                    const selectedAgent = availableAgents[Math.floor(Math.random() * availableAgents.length)];
                    setSupportRequests(prev => prev.map(req => req.id === newRequest.id ? {
                        ...req,
                        status: "matched",
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <Textarea value={requestDescription} onChange={(e) => setRequestDescription(e.target.value)} placeholder="Please describe what kind of support you need..." className="bg-slate-700 border-slate-600 text-white resize-none h-16" maxLength={300}/>
          ...
          {/* Submit Button */}
          <Button onClick={handleSubmitRequest} disabled={!canSubmit} className="w-full h-10 bg-purple-600 hover:bg-purple-700 disabled:bg-slate-600" aria-label="Submit support request">
            {isSubmitting ? (<>
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-40">
            <div className="space-y-2 pr-2">
              {supportRequests.map((request) => (<div key={request.id} className="p-2 rounded-md bg-slate-700/50 border border-slate-600">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                  ...
                  <div className="mt-1 text-xs text-slate-300">
                    ...
                    {request.agentName && (<div className="flex items-center gap-2 mt-1">
                        <UserCheck className="h-3 w-3 text-green-400"/>
                        <span className="text-green-400">Agent: {request.agentName}</span>
                        {request.estimatedTime && (<span className="text-slate-400">• ETA: {request.estimatedTime}</span>)}
                      </div>)}
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1">
            {communityAgents.slice(0, 2).map(agent => (<div key={agent.id} className="p-2 rounded-md bg-slate-700/30 border border-slate-600">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
        ...
        {/* Unmatched Alert */}
        {unmatchedAlert && (<div className="p-2 bg-red-900/50 border border-red-600 rounded-md animate-pulse" role="alert" aria-live="polite">
            <div className="flex items-center gap-2">
