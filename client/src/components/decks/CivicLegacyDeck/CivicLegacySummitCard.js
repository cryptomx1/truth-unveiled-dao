...
const CivicLegacySummitCard = () => {
    const [summitEntries] = useState([
        {
            id: 'summit_001',
    ...
    const [communityVoices] = useState([
        {
            id: 'voice_001',
    ...
    const [preservationFile, setPreservationFile] = useState({
        title: '',
        fileType: 'document',
        sourceLinks: '',
        civicUseTag: ''
    ...
    const generateZKPHash = (title, fileType) => {
    ...
    const handlePreservationSubmit = () => {
        if (!preservationFile.title.trim() || !preservationFile.civicUseTag.trim())
            return;
    ...
    return (<Card className="w-full max-w-sm mx-auto bg-slate-800 border-slate-700 text-white">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          {showPushbackAlert && (<div className="flex items-center gap-1 animate-pulse">
        ...
        {/* Pushback Alert */}
        {unverifiableRate >= 30 && (<div className="bg-red-900/50 border border-red-700 rounded-lg p-2 mt-2 animate-pulse">
      ...
      <CardContent className="space-y-4" role="main" aria-live="polite">
        ...
        <div className="bg-slate-900 rounded-lg p-3">
          ...
          <ScrollArea className="h-32">
            <div className="space-y-2">
              {summitEntries.map((entry) => (<div key={entry.id} className="bg-slate-800 rounded-lg p-2 border border-slate-700">
                  <div className="flex items-start justify-between mb-1">
                    <div className="flex items-center gap-2">
                      {getSourceIcon(entry.source)}
                      <div className="text-xs font-medium text-white truncate">{entry.title}</div>
                    </div>
                    {entry.verified ? (<CheckCircle className="w-3 h-3 text-green-400 flex-shrink-0"/>) : (<XCircle className="w-3 h-3 text-red-400 flex-shrink-0"/>)}
                  </div>
        ...
        <div className="bg-slate-900 rounded-lg p-3">
          ...
          <div className="relative">
            ...
            <div className="flex items-center justify-between mt-2">
              <Button variant="ghost" size="sm" onClick={() => navigateVoice('prev')} className="text-slate-400 hover:text-white hover:bg-slate-700 p-1" style={{ minHeight: '32px', minWidth: '32px' }}>
                <ChevronLeft className="w-3 h-3"/>
              </Button>
              
              <div className="flex gap-1">
                {communityVoices.map((_, index) => (<div key={index} className={`w-1.5 h-1.5 rounded-full transition-colors ${index === currentVoiceIndex ? 'bg-blue-400' : 'bg-slate-600'}`}/>))}
              </div>
              
              <Button variant="ghost" size="sm" onClick={() => navigateVoice('next')} className="text-slate-400 hover:text-white hover:bg-slate-700 p-1" style={{ minHeight: '32px', minWidth: '32px' }}>
        ...
        <div className="bg-slate-900 rounded-lg p-3">
          ...
          <div className="space-y-3">
            <Input placeholder="Preservation title (max 100 chars)" value={preservationFile.title} onChange={(e) => setPreservationFile(prev => ({ ...prev, title: e.target.value.slice(0, 100) }))} className="bg-slate-800 border-slate-600 text-white placeholder-slate-400" maxLength={100}/>
            
            <div className="grid grid-cols-2 gap-2">
              <Select value={preservationFile.fileType} onValueChange={(value) => setPreservationFile(prev => ({ ...prev, fileType: value }))}>
              ...
              <Input placeholder="Civic use tag" value={preservationFile.civicUseTag} onChange={(e) => setPreservationFile(prev => ({ ...prev, civicUseTag: e.target.value.slice(0, 50) }))} className="bg-slate-800 border-slate-600 text-white placeholder-slate-400" maxLength={50}/>
            </div>
            
            <Textarea placeholder="Source links and references (optional)" value={preservationFile.sourceLinks} onChange={(e) => setPreservationFile(prev => ({ ...prev, sourceLinks: e.target.value.slice(0, 200) }))} className="bg-slate-800 border-slate-600 text-white placeholder-slate-400 min-h-[60px]" maxLength={200}/>
            
            <div className="flex justify-between items-center">
              <span className="text-xs text-slate-400">
                {preservationFile.title.length}/100 chars
              </span>
              <Button onClick={handlePreservationSubmit} disabled={!preservationFile.title.trim() || !preservationFile.civicUseTag.trim() || isSubmitting} className="bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-1 text-sm flex items-center gap-2" style={{ minHeight: '40px' }}>
