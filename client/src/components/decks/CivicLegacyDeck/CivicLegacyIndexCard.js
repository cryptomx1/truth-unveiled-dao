...
import { Search, Filter, Users, FileText, Mic, Image, AlertTriangle, CheckCircle, XCircle, Hash, User, MapPin, Archive } from 'lucide-react';
const CivicLegacyIndexCard = () => {
    ...
    const [filteredEntries, setFilteredEntries] = useState(legacyEntries);
    ...
    useEffect(() => {
        ...
        if (searchQuery.trim()) {
            filtered = filtered.filter(entry => entry.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    return (<Card className="w-full max-w-sm mx-auto bg-slate-800 border-slate-700 text-white">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          {unverifiableRate > 20 && (<div className="flex items-center gap-1">
      ...
      <CardContent className="space-y-4" role="main" aria-live="polite">
        ...
        <div className="space-y-3">
          <div className="relative">
            ...
            <Input placeholder="Search entries, contributors, tags..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-slate-900 border-slate-600 text-white placeholder-slate-400 pl-10"/>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={clearFilters} variant="outline" size="sm" className="bg-slate-700 border-slate-600 text-slate-300 hover:bg-slate-600" style={{ minHeight: '40px' }}>
        ...
        <ScrollArea className="h-64">
          <div className="space-y-3">
            {filteredEntries.map((entry) => (<div key={entry.id} className="bg-slate-900 rounded-lg p-3 border border-slate-700 cursor-pointer hover:bg-slate-800 transition-colors" onClick={() => setSelectedEntry(selectedEntry?.id === entry.id ? null : entry)} role="button" tabIndex={0} onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    setSelectedEntry(selectedEntry?.id === entry.id ? null : entry);
                }
            }} aria-expanded={selectedEntry?.id === entry.id} aria-label={`Legacy entry: ${entry.title}`}>
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center gap-1">
                    {entry.verified ? (<CheckCircle className="w-4 h-4 text-green-400"/>) : (<XCircle className="w-4 h-4 text-red-400"/>)}
                ...
                {selectedEntry?.id === entry.id && (<div className="mt-3 pt-3 border-t border-slate-700 space-y-2">
                    ...
                    {entry.tags.length > 0 && (<div className="flex flex-wrap gap-1 mt-2">
                        {entry.tags.map((tag, index) => (<Badge key={index} variant="outline" className="text-xs px-1 py-0 bg-slate-800 border-slate-600 text-slate-400">
                            {tag}
                          </Badge>))}
                      </div>)}
                    
                    {entry.riskLevel !== 'none' && (<div className="flex items-center gap-2 text-xs">
