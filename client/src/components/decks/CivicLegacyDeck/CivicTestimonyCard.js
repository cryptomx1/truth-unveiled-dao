...
import { FileText, Mic, Play, Pause, AlertTriangle, Users, MapPin, CheckCircle, XCircle, Hash, User } from 'lucide-react';
const CivicTestimonyCard = () => {
    ...
    const generateZKPHash = (contributorDID, content) => {
    ...
    const handlePlayTestimony = (testimony) => {
    ...
    return (<Card className="w-full max-w-sm mx-auto bg-slate-800 border-slate-700 text-white">
      <CardHeader className="pb-3">
        ...
        {unverifiableRate > 25 && (<div className="bg-red-900/50 border border-red-700 rounded-lg p-2 mt-2 animate-pulse">
      ...
      <CardContent className="space-y-4" role="main" aria-live="polite">
        ...
        <div className="bg-slate-900 rounded-lg p-3 space-y-3">
          ...
          <Input placeholder="Testimony title (max 100 chars)" value={newTestimony.title} onChange={(e) => setNewTestimony(prev => ({ ...prev, title: e.target.value.slice(0, 100) }))} className="bg-slate-800 border-slate-600 text-white placeholder-slate-400" maxLength={100}/>
          
          <Textarea placeholder="Share your civic testimony (max 500 chars)" value={newTestimony.content} onChange={(e) => setNewTestimony(prev => ({ ...prev, content: e.target.value.slice(0, 500) }))} className="bg-slate-800 border-slate-600 text-white placeholder-slate-400 min-h-[80px]" maxLength={500}/>
          
          <div className="grid grid-cols-2 gap-2">
            <Select value={newTestimony.format} onValueChange={(value) => setNewTestimony(prev => ({ ...prev, format: value }))}>
            ...
            <Select value={newTestimony.theme} onValueChange={(value) => setNewTestimony(prev => ({ ...prev, theme: value }))}>
              <SelectTrigger className="bg-slate-800 border-slate-600 text-white">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="bg-slate-800 border-slate-600">
          ...
          <Select value={newTestimony.emotion} onValueChange={(value) => setNewTestimony(prev => ({ ...prev, emotion: value }))}>
            <SelectTrigger className="bg-slate-800 border-slate-600 text-white">
              <SelectValue />
            </SelectTrigger>
            <SelectContent className="bg-slate-800 border-slate-600">
          ...
          <div className="flex justify-between items-center pt-2">
            ...
            <Button onClick={handleTestimonySubmit} disabled={!newTestimony.title.trim() || !newTestimony.content.trim() || isSubmitting} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1 text-sm" style={{ minHeight: '48px' }}>
        ...
        <ScrollArea className="h-64">
          <div className="space-y-3">
            {testimonies.map((testimony) => (<div key={testimony.id} className="bg-slate-900 rounded-lg p-3 border border-slate-700 cursor-pointer hover:bg-slate-800 transition-colors" onClick={() => setSelectedTestimony(selectedTestimony?.id === testimony.id ? null : testimony)} role="button" tabIndex={0} onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    setSelectedTestimony(selectedTestimony?.id === testimony.id ? null : testimony);
                }
            }} aria-expanded={selectedTestimony?.id === testimony.id} aria-label={`Testimony: ${testimony.title}`}>
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center gap-1">
                    {testimony.verified ? (<CheckCircle className="w-4 h-4 text-green-400"/>) : (<XCircle className="w-4 h-4 text-red-400"/>)}
                  </div>
                </div>
                ...
                {selectedTestimony?.id === testimony.id && (<div className="mt-3 pt-3 border-t border-slate-700 space-y-2">
                ...
                }} className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 text-xs flex items-center gap-1" style={{ minHeight: '32px' }}>
