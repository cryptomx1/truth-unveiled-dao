...
import { Archive, FileText, Mic, Image, Quote, Clock, AlertTriangle, Sparkles, Hash, BookOpen, Shield, Users } from "lucide-react";
const mockCivicContributions = [
    {
        id: "contrib_001",
...
export default function CivicMemoryVaultCard() {
    const [contributions, setContributions] = useState(mockCivicContributions);
    ...
    const [unverifiableAlert, setUnverifiableAlert] = useState(false);
    ...
    const generateZKPHash = (contributorDID) => {
    ...
    const handleSubmitContribution = async () => {
        ...
        if (progressChance > 0.3) {
            setTimeout(() => {
                setContributions(prev => prev.map(c => c.id === newContrib.id ? {
                    ...c,
                    lifecycle: "verified",
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <Input value={newContribution.title} onChange={(e) => setNewContribution(prev => ({ ...prev, title: e.target.value }))} placeholder="Brief title for your contribution..." className="bg-slate-700 border-slate-600 text-white h-8" maxLength={100}/>
          ...
          <div className="flex gap-2">
            <div className="flex-1">
              ...
              <Select value={newContribution.contentType} onValueChange={(value) => setNewContribution(prev => ({ ...prev, contentType: value }))}>
            ...
            <div className="flex-1">
              ...
              <Select value={newContribution.filterType} onValueChange={(value) => setNewContribution(prev => ({ ...prev, filterType: value }))}>
          ...
          <div className="space-y-2">
            ...
            <Textarea value={newContribution.content} onChange={(e) => setNewContribution(prev => ({ ...prev, content: e.target.value }))} placeholder="Describe your civic contribution or memory..." className="bg-slate-700 border-slate-600 text-white resize-none h-16" maxLength={500}/>
            <div className="text-xs text-slate-400 text-right">
          ...
          {/* Submit Button */}
          <Button onClick={handleSubmitContribution} disabled={!canSubmit} className="w-full h-10 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600" aria-label="Save to civic memory vault">
            {isSubmitting ? (<>
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-40">
            <div className="space-y-2 pr-2">
              {filteredContributions.map((contribution) => (<div key={contribution.id} className="p-2 rounded-md bg-slate-700/50 border border-slate-600">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                    ...
                    {contribution.isVerifiable && (<Shield className="h-3 w-3 text-green-400"/>)}
                  </div>
        ...
        {/* Unverifiable Alert */}
        {unverifiableAlert && (<div className="p-2 bg-red-900/50 border border-red-600 rounded-md animate-pulse" role="alert" aria-live="polite">
            <div className="flex items-center gap-2">
