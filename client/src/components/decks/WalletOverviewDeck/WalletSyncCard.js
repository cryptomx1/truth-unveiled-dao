...
export const WalletSyncCard = ({ className }) => {
    ...
    return (<div className={cn('dao-card-gradient border border-[hsl(var(--dao-border))] rounded-xl p-6 shadow-xl hover:shadow-2xl transition-all duration-200 max-w-sm mx-auto', className)} role="region" aria-label="Wallet Synchronization Status" data-testid="wallet-sync-card">
      {/* Header Section */}
      <div className="flex items-center justify-between mb-6">
        ...
        {/* TTS Button */}
        <Button onClick={handleTTSToggle} disabled={!ttsStatus.isReady} className={cn('w-10 h-10 rounded-full p-0 transition-colors duration-150', ttsStatus.isReady
            ? 'bg-white/10 hover:bg-white/20 text-white'
            : 'bg-gray-500/20 text-gray-500 cursor-not-allowed')} aria-label={ttsStatus.isPlaying ? 'Stop security message' : 'Play security message'} data-testid="tts-toggle-button">
          {ttsStatus.isPlaying ? (<VolumeX className="w-4 h-4"/>) : (<Volume2 className="w-4 h-4"/>)}
        </Button>
      </div>
      ...
      <div className="bg-white/10 rounded-lg p-4 border border-white/20 mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="relative">
              <div className={cn("w-3 h-3 rounded-full transition-all duration-300", syncStatus.isSynced
            ? "bg-green-400 animate-pulse"
            : "bg-yellow-400")}/>
              {!syncStatus.isSynced && (<div className="absolute inset-0 w-3 h-3 bg-yellow-400 rounded-full animate-ping opacity-75"/>)}
            </div>
            <span className={cn("text-sm font-medium", syncStatus.isSynced ? "text-green-400" : "text-yellow-400")} aria-live="polite">
        ...
        {!syncStatus.isSynced && (<div className="mt-2">
            <div className="w-full bg-gray-700 rounded-full h-2 mb-2">
              <div className="bg-yellow-400 h-2 rounded-full transition-all duration-300" style={{ width: `${syncStatus.syncProgress}%` }}/>
            </div>
      ...
      <div className="bg-white/10 rounded-lg p-4 border border-white/20 mb-4">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={handleManualSync} disabled={syncStatus.isSynced || isRefreshing} className={cn('px-4 py-2 text-sm font-medium transition-colors duration-150', syncStatus.isSynced || isRefreshing
            ? 'bg-gray-500/20 text-gray-500 cursor-not-allowed'
            : 'bg-blue-600 hover:bg-blue-700 text-white')} aria-label="Manually sync wallet" data-testid="manual-sync-button">
            {isRefreshing ? (<>
      ...
      <div className="bg-white/5 rounded-lg p-3 border border-white/10">
        <div className="flex items-center justify-center space-x-2 text-center">
          <div className="flex items-center space-x-2">
            {syncStatus.isSynced ? (<Wifi className="w-4 h-4 text-green-400"/>) : (<WifiOff className="w-4 h-4 text-yellow-400"/>)}
