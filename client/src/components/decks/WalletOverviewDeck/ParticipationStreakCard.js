...
export const ParticipationStreakCard = ({ streak = MOCK_STREAK, className }) => {
    ...
    return (<div className={cn('dao-card-gradient border border-[hsl(var(--dao-border))] rounded-xl p-6 shadow-xl hover:shadow-2xl transition-all duration-200 max-w-sm mx-auto', className)} role="region" aria-label="Participation Streak Information" data-testid="participation-streak-card">
      {/* Header Section */}
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={handleTTSToggle} disabled={!ttsStatus.isReady} className={cn('w-10 h-10 rounded-full p-0 transition-colors duration-150', ttsStatus.isReady
            ...
            : 'bg-gray-500/20 text-gray-500 cursor-not-allowed')} aria-label={ttsStatus.isPlaying ? 'Stop motivation message' : 'Play motivation message'} data-testid="tts-toggle-button">
      ...
      <div className="text-center mb-6">
        <div className="relative inline-flex items-center justify-center mb-3">
          ...
          <div className="relative bg-green-500/20 backdrop-blur-sm rounded-full w-20 h-20 flex items-center justify-center border-2 border-green-400/30">
            <span className="text-3xl font-bold text-green-400" aria-live="polite" aria-label={`Current streak: ${streak.currentStreak} days`}>
