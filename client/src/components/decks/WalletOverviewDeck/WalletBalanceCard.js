...
const CopyButton = ({ targetText, label, size = 'sm', className, onSuccess, onError }) => {
    ...
    return (<Button onClick={handleCopy} disabled={copyState === 'copying'} className={cn('flex items-center space-x-1 transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-blue-400/50', getButtonStyles(), sizeClasses, className)} aria-label={`Copy ${label} to clipboard`} data-testid={`copy-${label.toLowerCase().replace(/\s+/g, '-')}-button`}>
      {getButtonContent()}
    </Button>);
};
const WalletBalanceCard = React.memo(({ balance = MOCK_BALANCE, onCopySuccess, onCopyError, className }) => {
    const [viewMode, setViewMode] = useState('tp');
    ...
    return (<div className={cn('dao-card-gradient border border-[hsl(var(--dao-border))] rounded-xl p-6 shadow-xl hover:shadow-2xl transition-all duration-200 min-w-[300px] max-w-[460px] h-auto overflow-y-auto', className)} role="region" aria-label="Civic Wallet Balance" data-testid="wallet-balance-card">
      {/* Header Section */}
      <div className="flex items-center justify-between mb-6">
        ...
        {/* Token Toggle */}
        <Button onClick={toggleViewMode} className="flex items-center space-x-2 bg-white/10 hover:bg-white/20 text-white border border-white/20" aria-label={`Switch to ${viewMode === 'tp' ? 'Contribution Credits' : 'Truth Points'}`} aria-pressed={viewMode === 'cc'}>
      ...
      <div className="bg-white/10 rounded-lg p-6 border border-white/20 mb-4">
        <div className="text-center">
          <div className="text-3xl font-bold text-white mb-2" aria-live="polite" aria-label={`Current ${currentToken} balance: ${formatNumber(currentBalance)}`}>
          ...
          {/* Token Badge */}
          <div className={cn("inline-flex items-center px-3 py-1 rounded-full text-xs font-medium", viewMode === 'tp'
            ? "bg-blue-500/20 text-blue-400"
            : "bg-purple-500/20 text-purple-400")}>
            <span className={cn("w-2 h-2 rounded-full mr-2", viewMode === 'tp' ? "bg-blue-400" : "bg-purple-400")}/>
            {currentTokenSymbol}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/10 rounded-lg p-4 border border-white/20 mb-4">
        <label className="block text-sm font-medium text-white mb-2" htmlFor="tp-address">
          Truth Points Address
        </label>
        <div className="flex items-center justify-between">
          <span className="font-mono text-sm text-white truncate flex-1 mr-2" id="tp-address" title={balance.tpAddress}>
            {balance.tpAddress.substring(0, 20)}...{balance.tpAddress.substring(balance.tpAddress.length - 6)}
          </span>
          <CopyButton targetText={balance.tpAddress} label="Copy" onSuccess={() => handleCopySuccess(balance.tpAddress)} onError={handleCopyError}/>
        </div>
      </div>
      ...
      <div className="bg-white/10 rounded-lg p-4 border border-white/20 mb-4">
        ...
        <div className="bg-black/30 rounded p-3 border border-green-400/30">
          <span className="font-mono text-xs text-green-400 break-all" title="Zero-Knowledge Proof Hash">
            {balance.zkpHash}
          </span>
        </div>
      </div>
      ...
      <div className="mt-6 pt-4 border-t border-white/20">
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center space-x-2">
            <div className={cn("w-2 h-2 bg-green-400 rounded-full", ttsStatus.isPlaying ? "animate-pulse" : "", !ttsStatus.isReady ? "bg-gray-500" : "")}/>
            <span className={cn("font-medium", ttsStatus.isReady ? "text-green-400" : "text-gray-500")} aria-live="polite">
