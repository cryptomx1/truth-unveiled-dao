...
export const CivicMemoryIndex = () => {
    const { t } = useTranslation();
    const { language } = useLangContext();
    const [memoryEntries] = useState(mockMemoryEntries);
    ...
    const [glowingEntries, setGlowingEntries] = useState(new Set());
    ...
    const handleReplayAction = (entry) => {
        ...
        try {
            ...
            if ('speechSynthesis' in window) {
                ...
                const utterance = new SpeechSynthesisUtterance(t('tts.memory.ready'));
                utterance.rate = 0.9;
    ...
    const handleTrustVote = (voteType, zkpHash) => {
        if (selectedEntry) {
    ...
    return (<div className="max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6">
      {/* Header */}
      ...
      <div className="mb-4 p-3 bg-slate-700/50 rounded-lg">
        <div className="flex items-center justify-between">
          ...
          <span className={`text-sm font-medium ${sessionSource === 'DID' ? 'text-blue-400' :
      ...
      <div className="mb-4 p-3 bg-slate-700/30 rounded-lg border border-slate-600">
        ...
        <div className="space-y-2">
          <div className="flex gap-2">
            <button onClick={() => setShowTracePanel(true)} className="flex-1 flex items-center justify-center gap-1 px-2 py-1 bg-purple-600/20 hover:bg-purple-600/30 border border-purple-500/30 text-purple-300 text-xs rounded transition-colors">
              <GitBranch className="w-3 h-3"/>
              Trace Panel
            </button>
            <button onClick={() => setShowImpactGraph(!showImpactGraph)} className={`flex-1 flex items-center justify-center gap-1 px-2 py-1 border text-xs rounded transition-colors ${showImpactGraph
          ...
          <button onClick={handleZKPExport} disabled={getStats().totalEvents === 0} className="w-full flex items-center justify-center gap-1 px-2 py-1 bg-green-600/20 hover:bg-green-600/30 disabled:bg-slate-600/20 disabled:text-slate-500 border border-green-500/30 disabled:border-slate-500/30 text-green-300 text-xs rounded transition-colors" title="📄 View Zero-Knowledge Proof of Trace">
            📄 Export ZKP Proof
          </button>
        </div>
        ...
        <div className="flex gap-2 mt-2">
          <button onClick={() => setShowTrustVote(!showTrustVote)} className={`flex-1 flex items-center justify-center gap-1 px-2 py-1 border text-xs rounded transition-colors ${showTrustVote
            ? 'bg-orange-600/30 border-orange-500/50 text-orange-300'
            : 'bg-slate-600/20 hover:bg-slate-600/30 border-slate-500/30 text-slate-300'}`}>
            💬 Trust Vote
          </button>
          <button onClick={() => setShowImpactAnalyzer(!showImpactAnalyzer)} className={`flex-1 flex items-center justify-center gap-1 px-2 py-1 border text-xs rounded transition-colors ${showImpactAnalyzer
      ...
      {/* Impact Graph */}
      {showImpactGraph && (<div className="mb-4">
          <DeckImpactGraph selectedEvent={selectedEntry ? {
      ...
      {showTrustVote && selectedEntry && (<div className="mb-4">
          <TrustVoteCard memoryAction={{
                id: selectedEntry.id,
                action: selectedEntry.action,
                value: selectedEntry.value,
      ...
      {showImpactAnalyzer && (<div className="mb-4">
          <FeedbackImpactAnalyzerCard />
        </div>)}

      {/* Memory Timeline */}
      <div className="space-y-3">
        {memoryEntries.map((entry) => (<div key={entry.id} className={`p-4 rounded-lg border transition-all duration-200 ${selectedEntry?.id === entry.id
                ? 'bg-slate-700 border-blue-500'
                : 'bg-slate-700/50 border-slate-600 hover:border-slate-500'} ${glowingEntries.has(entry.id)
            ...
            {/* Replay Button */}
            <button onClick={() => handleReplayAction(entry)} onFocus={() => setSelectedEntry(entry)} className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800" aria-label={`${t('action.replay')}: ${entry.action} ${entry.value}`}>
      ...
      {/* Truth Trace Panel */}
      <TruthTracePanel isOpen={showTracePanel} onClose={() => setShowTracePanel(false)} selectedEvent={selectedEntry ? {
            actionId: selectedEntry.id,
            timestamp: selectedEntry.timestamp.getTime(),
            sourceDeck: 'CivicMemoryIndex',
            moduleId: 'replay_action',
