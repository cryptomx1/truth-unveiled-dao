...
export const ProtocolValidator = ({ onValidationComplete }) => {
    ...
    return (<div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`text-xs px-2 py-1 rounded flex items-center gap-1 ${isValidating ? 'bg-amber-900 text-amber-200' :
      ...
      <div className="space-y-2">
        {validationResults.map((result, index) => (<div key={index} className="flex items-center justify-between p-2 bg-slate-900 rounded">
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${result.status === 'PASSED' ? 'bg-green-400' :
