...
export const ValidatorThrottle = ({ onValidationComplete }) => {
    const [validationResults, setValidationResults] = useState([]);
    const [isValidating, setIsValidating] = useState(false);
    const [latencyTest, setLatencyTest] = useState(0);
    useEffect(() => {
    ...
    return (<div className="bg-slate-800 border border-slate-700 rounded-lg p-4 max-w-md mx-auto">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-medium text-slate-200">
          🔐 Phase III-A Validator
        </h3>
        <div className={`text-xs px-2 py-1 rounded ${isValidating ? 'bg-amber-900 text-amber-200' :
      ...
      <div className="mb-4 p-3 bg-slate-900 rounded border border-slate-600">
        <div className="flex justify-between text-xs text-slate-300 mb-2">
          ...
          <span className={latencyStatus === 'PASSED' ? 'text-green-400' :
      ...
      <div className="space-y-2">
        {validationResults.map((result, index) => (<div key={index} className="flex items-center justify-between text-xs">
            <span className="text-slate-300">
              Step {result.step}/6: {result.message.split(':')[0].replace(/[✅❌]/g, '').trim()}
            </span>
            <div className={`px-2 py-1 rounded text-xs ${result.status === 'PASSED' ? 'bg-green-900 text-green-200' :
      ...
      {allPassed && !isValidating && (<div className="mt-4 p-3 bg-green-900 border border-green-700 rounded">
