...
import { MapPin, Search, FileText, Vote, AlertCircle, Wifi, Bell, X } from 'lucide-react';
import { getDistrictByZIP } from '../../agents/LegiSyncAgent';
import { fetchBillsByDistrict, fetchBillsByZIP, checkAPIHealth } from '../../services/LegiFeedService';
import { evaluatePushRules, getMockTrustLevel, getMockStreakLevel, formatAlertForARIA } from '../../agents/PushRuleEngine';
import { FeedbackZoneEngine } from '../feedback/FeedbackZoneEngine';
...
export const GovMapEngine = ({ className = "" }) => {
    ...
    const alertAriaRef = useRef(null);
    ...
    const handleDistrictClick = async (district) => {
        ...
        try {
            ...
            const activeCount = districtBills.filter(b => b.status === 'Active' || b.status === 'In Committee' || b.status === 'Passed House').length;
        ...
        if (zip.length === 5) {
            ...
            if (district) {
                ...
                try {
                    ...
                    setBills(zipBills);
                    setActiveCount(zipBills.filter(b => b.status === 'Active' || b.status === 'In Committee').length);
    ...
    return (<div className={`max-w-4xl mx-auto bg-slate-800 border border-slate-700 rounded-lg p-6 ${className}`}>
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          {pushAlerts.length > 0 && (<button onClick={() => setShowAlerts(!showAlerts)} className="relative flex items-center gap-1 text-orange-400 hover:text-orange-300 transition-colors" aria-label={`${pushAlerts.length} civic alerts available`}>
      ...
      <div className="mb-6">
        ...
        <input id="zip-search" type="text" value={searchZIP} onChange={(e) => handleZIPSearch(e.target.value)} placeholder="Enter 5-digit ZIP code for live data..." className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" maxLength={5}/>
      </div>
      ...
      <div className="mb-6 p-4 bg-slate-700/30 rounded-lg border border-slate-600">
        ...
        <div className="relative bg-slate-900 rounded-lg overflow-hidden" style={{ height: '240px' }}>
          <svg width="100%" height="100%" viewBox="0 0 400 240" className="absolute inset-0">
            {DISTRICT_MAP.map((district) => (<g key={district.id}>
                <rect x={district.coordinates.x} y={district.coordinates.y} width={district.coordinates.width} height={district.coordinates.height} fill={selectedDistrict?.id === district.id ? district.color : `${district.color}40`} stroke={district.color} strokeWidth="2" className="cursor-pointer transition-all duration-200 hover:opacity-80" onClick={() => handleDistrictClick(district)} role="button" tabIndex={0} aria-label={`Select ${district.name} for legislative information`}/>
                <text x={district.coordinates.x + district.coordinates.width / 2} y={district.coordinates.y + district.coordinates.height / 2} textAnchor="middle" dominantBaseline="middle" fill="white" fontSize="12" fontWeight="bold" className="pointer-events-none">
                  {district.id}
                </text>
              </g>))}
          </svg>
        ...
        <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
          {DISTRICT_MAP.map((district) => (<div key={district.id} className="flex items-center gap-1">
              <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: district.color }}/>
      ...
      {selectedDistrict && (<div className="mb-6 p-4 bg-slate-700/50 rounded-lg border border-slate-600">
      ...
      {/* Push Alerts Overlay */}
      {showAlerts && pushAlerts.length > 0 && (<div className="mb-6 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-white">Civic Alerts</h3>
            <button onClick={() => setShowAlerts(false)} className="text-slate-400 hover:text-white transition-colors" aria-label="Close alerts">
              <X className="w-5 h-5"/>
            </button>
          </div>
          
          {pushAlerts.map((alert, index) => (<div key={index} className={`p-4 border rounded-lg ${getAlertPriorityColor(alert.priority)}`}>
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-xs opacity-75">
                    ...
                    {alert.actionRequired && (<span className="px-2 py-1 bg-slate-800 rounded">Action Required</span>)}
                  </div>
                </div>
                <button onClick={() => dismissAlert(index)} className="ml-2 text-slate-400 hover:text-white transition-colors" aria-label="Dismiss alert">
                  <X className="w-4 h-4"/>
                </button>
              </div>
            </div>))}
      ...
      {/* Error Message */}
      {error && (<div className="mb-6 p-4 bg-red-900/20 border border-red-700/50 rounded-lg">
      ...
      {bills.length > 0 && (<div className="space-y-3">
          ...
          {bills.map((bill) => (<div key={bill.id} className="p-4 bg-slate-700/50 border border-slate-600 rounded-lg hover:border-slate-500 transition-colors">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  {bill.bill_number && (<div className="mt-1 text-xs text-blue-400">
              ...
              <div className="mt-4">
                <FeedbackZoneEngine context={{
                    ...
                    status: bill.status
                }} userTier="Citizen" onFeedbackSubmitted={(feedback) => {
      ...
      {/* No Bills Message */}
      {selectedDistrict && bills.length === 0 && !loading && !error && (<div className="text-center py-8 text-slate-400">
      ...
      <div ref={alertAriaRef} aria-live="assertive" aria-atomic="true" className="sr-only"/>
    </div>);
};
