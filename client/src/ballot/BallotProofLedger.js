 ...
 */
import { ZKVoteTokenIssuer } from './ZKVoteTokenIssuer';
// Main Ballot Proof Ledger class
export class BallotProofLedger {
    static instance;
    ledgerEntries = new Map();
    ballotIndex = new Map(); // ballotId -> tokenIds
    timestampIndex = new Map(); // date -> tokenIds
    weightIndex = new Map(); // weight -> tokenIds
    ledgerPosition = 0;
    rejectHistory = [];
    constructor() {
    ...
    generateVoteDigest(encryptedVote, proofSignature) {
    ...
    queryLedger(query) {
        ...
        let candidates = Array.from(this.ledgerEntries.values());
        // Filter by ballot ID
        if (query.ballotId) {
        ...
        if (query.voteWeightBucket) {
            candidates = candidates.filter(entry => entry.voteWeight >= query.voteWeightBucket.min &&
    ...
    getTally(ballotId) {
        ...
        entries.forEach(entry => {
            ...
            if (entry.timestamp < firstVote)
                firstVote = entry.timestamp;
            if (entry.timestamp > lastVote)
                lastVote = entry.timestamp;
        });
    ...
    // Mock vote choice extraction from encrypted vote
    extractMockVoteChoice(encryptedVote) {
    ...
    exportOutcomeLedgerBundle(ballotId) {
    ...
    getLedgerStatistics() {
        ...
        const recentEntries = entries.filter(entry => new Date(entry.timestamp).getTime() > oneHourAgo).length;
        // Weight distribution
        const weightDistribution = {};
        entries.forEach(entry => {
    ...
    getBallotList() {
        ...
        for (const [ballotId, tokenIds] of this.ballotIndex.entries()) {
            ...
            const totalWeight = entries.reduce((sum, entry) => sum + entry.voteWeight, 0);
            const lastActivity = entries.reduce((latest, entry) => entry.timestamp > latest ? entry.timestamp : latest, entries[0].timestamp);
    ...
    createMockBallotAggregation(ballotId, voteCount = 10) {
        const choices = ['Support', 'Oppose', 'Abstain'];
        const voteBreakdown = {};
        const weightedTotals = {};
        const weights = [];
...
export const recordVoteToken = async (zkToken) => {
    const ledger = BallotProofLedger.getInstance();
    return await ledger.recordVoteToken(zkToken);
};
export const getBallotTally = (ballotId) => {
    const ledger = BallotProofLedger.getInstance();
    return ledger.getTally(ballotId);
};
export const queryBallotLedger = (query) => {
    const ledger = BallotProofLedger.getInstance();
    return ledger.queryLedger(query);
};
export const exportBallotBundle = (ballotId) => {
    const ledger = BallotProofLedger.getInstance();
    return ledger.exportOutcomeLedgerBundle(ballotId);
};
export const createMockBallotAggregation = (ballotId, voteCount) => {
    const ledger = BallotProofLedger.getInstance();
