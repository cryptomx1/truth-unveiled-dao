...
export class BallotEligibilityVerifier {
    ...
    verifyEligibilityFromBundle(bundle, context) {
        ...
        try {
            ...
            if (!overallValid) {
                const reasons = [];
                if (epochStatus !== 'valid')
                    reasons.push(`epoch ${epochStatus}`);
                if (integrityStatus !== 'verified')
                    reasons.push(`integrity ${integrityStatus}`);
                if (!tierValid)
                    reasons.push(`tier insufficient (${bundle.tier} < ${context.minimumTier})`);
                if (!trustValid)
                    reasons.push(`trust insufficient (${bundle.trustScore} < ${context.minimumTrustScore})`);
                reason = reasons.join(', ');
            }
            const result = {
    ...
    verifyEligibilityFromVault(userHash, context) {
        const validationTimestamp = new Date().toISOString();
        try {
            ...
            if (!overallValid) {
                const reasons = [];
                if (!tierValid)
                    reasons.push(`tier insufficient (${civicProfile.tier} < ${context.minimumTier})`);
                if (!trustValid)
                    reasons.push(`trust insufficient (${civicProfile.trustScore} < ${context.minimumTrustScore})`);
                reason = reasons.join(', ');
            }
            const result = {
                cidDigest: userHash,
    ...
    verifyEligibilityFromUpload(uploadData, context) {
        try {
    ...
    validateEpoch(epoch, toleranceDays) {
        try {
    ...
    validateTierRequirement(userTier, minimumTier) {
        return TIER_HIERARCHY[userTier] >= TIER_HIERARCHY[minimumTier];
    }
    // Derive civic profile from vault entries
    deriveCivicProfileFromVault(entries) {
        ...
        const reputationScore = Math.round((missionCompletions * 10) +
    ...
    logEligibilityResult(result, ballotId) {
    ...
    createMockZKPBundle(userHash, tier, trustScore, daysAgo = 0) {
        ...
        for (let i = 0; i < capsuleCount; i++) {
            const mockCapsule = {
                ...
                metadata: {
                    eventType: ['mission_completion', 'vote_cast', 'feedback_submitted', 'identity_verified'][Math.floor(Math.random() * 4)],
                    userTier: tier,
                    trustScore,
        ...
        const vaultMetadata = {
            ...
            reputationScore: Math.round((proofCapsules.filter(c => c.metadata.eventType === 'mission_completion').length * 10) +
                (trustScore * 0.5) +
    ...
    getVerificationByCID(cidDigest) {
    ...
    getVerificationStatistics() {
        ...
        const recentActivity = verifications.filter(v => new Date(v.validationTimestamp).getTime() > oneDayAgo).length;
        return {
...
export const verifyBallotEligibility = (bundle, context) => {
    const verifier = BallotEligibilityVerifier.getInstance();
    return verifier.verifyEligibilityFromBundle(bundle, context);
};
export const verifyBallotEligibilityFromVault = (userHash, context) => {
    const verifier = BallotEligibilityVerifier.getInstance();
    return verifier.verifyEligibilityFromVault(userHash, context);
};
export const verifyBallotEligibilityFromUpload = (uploadData, context) => {
    const verifier = BallotEligibilityVerifier.getInstance();
    return verifier.verifyEligibilityFromUpload(uploadData, context);
};
export const createMockZKPBundle = (userHash, tier, trustScore, daysAgo) => {
    const verifier = BallotEligibilityVerifier.getInstance();
