...
export class ReputationIndexEngine {
    credentialSources;
    maxCalculationTime;
    constructor(sources = ['vault', 'proposals', 'votes', 'referrals'], maxTime = 250) {
     ...
     */
    async calculateReputationScore(cid) {
        ...
        try {
            ...
            const credentialAges = credentials.map(cred => (Date.now() - new Date(cred.issuedAt).getTime()) / (1000 * 60 * 60 * 24));
            const averageAge = credentialAges.reduce((sum, age) => sum + age, 0) / credentialAges.length;
            const oldestCredential = credentials.reduce((oldest, cred) => new Date(cred.issuedAt) < new Date(oldest.issuedAt) ? cred : oldest);
            const newestCredential = credentials.reduce((newest, cred) => new Date(cred.issuedAt) > new Date(newest.issuedAt) ? cred : newest);
     ...
     */
    async fetchCredentialHistory(cid) {
        const credentials = [];
        ...
        return uniqueCredentials.sort((a, b) => new Date(b.issuedAt).getTime() - new Date(a.issuedAt).getTime());
    }
    /**
     * Fetch credentials from a specific source
     * Mock implementation for demonstration
     */
    async fetchFromSource(source, cid) {
     ...
     */
    determineTier(score) {
        if (score >= TIER_THRESHOLDS[TruthTokenRole.COMMANDER])
            return TruthTokenRole.COMMANDER;
        if (score >= TIER_THRESHOLDS[TruthTokenRole.GOVERNOR])
            return TruthTokenRole.GOVERNOR;
        if (score >= TIER_THRESHOLDS[TruthTokenRole.MODERATOR])
            return TruthTokenRole.MODERATOR;
        if (score >= TIER_THRESHOLDS[TruthTokenRole.CONTRIBUTOR])
            return TruthTokenRole.CONTRIBUTOR;
        return TruthTokenRole.CITIZEN;
    }
    /**
     * Calculate progress to next tier (0-100%)
     */
    calculateTierProgress(score, currentTier) {
        ...
        const nextTier = tierValues[currentIndex + 1];
     ...
     */
    aggregateByField(credentials, field) {
     ...
     */
    async batchCalculateScores(cids) {
        const results = [];
     ...
     */
    static getTierStatistics(scores) {
    ...
    static validateScore(score) {
        ...
        if (score.totalScore < 0 || score.baseScore < 0)
            return false;
        if (score.decayPercentage < 0 || score.decayPercentage > 100)
            return false;
        if (score.tierProgress < 0 || score.tierProgress > 100)
            return false;
        if (!Object.values(TruthTokenRole).includes(score.tier))
            return false;
        return true;
    }
}
 ...
 */
export async function calculateQuickReputation(cid) {
 ...
 */
export function qualifiesForTier(score, tier) {
    return score >= TIER_THRESHOLDS[tier];
}
/**
 * Calculate points needed for next tier
 */
export function pointsToNextTier(currentScore, currentTier) {
    const tierValues = Object.values(TruthTokenRole);
    const currentIndex = tierValues.indexOf(currentTier);
    if (currentIndex === tierValues.length - 1) {
