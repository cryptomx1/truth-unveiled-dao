...
const ZKPLedgerTest = () => {
    ...
    const handleAddTestEntry = () => {
        ...
        const success = zkpProofLedger.addStakeEntry(testZKP, testDID, 'Contributor', randomPillar, undefined, amount);
        if (success) {
            refreshLedgerData();
            toast({
                title: "Entry Added",
    ...
    return (<div className="min-h-screen bg-background p-6">
      <div className="max-w-6xl mx-auto space-y-6">
            ...
            <CardContent>
              ...
              <div className="mt-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {Object.entries(ledgerStats.pillarDistribution).map(([pillar, amount]) => (<div key={pillar} className="flex justify-between text-sm">
                      <span>{pillar}:</span>
        ...
        <Card>
          ...
          <CardContent>
            {recentEntries.length > 0 ? (<div className="space-y-3">
                {recentEntries.map((entry) => (<div key={entry.id} className="border rounded-lg p-4 space-y-2">
        ...
        {integrityStatus && (<Card>
            ...
            <CardContent>
              <div className={`p-4 rounded-lg ${integrityStatus.valid ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'}`}>
                ...
                {!integrityStatus.valid && integrityStatus.invalidEntries.length > 0 && (<div className="text-sm text-red-600 dark:text-red-400">
                    Invalid entries: {integrityStatus.invalidEntries.join(', ')}
                  </div>)}
                
                {integrityStatus.valid && (<div className="text-sm text-green-600 dark:text-green-400">
