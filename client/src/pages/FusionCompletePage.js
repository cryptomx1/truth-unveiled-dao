...
import { GenesisBadgeExport } from '@/services/GenesisBadgeExport';
...
const FusionCompletePage = () => {
    const [genesisBadgeData, setGenesisBadgeData] = useState(null);
    ...
    useEffect(() => {
        ...
        const initializeBadgeData = () => {
            ...
            FusionCheckpointEmitter.emitFusionInitiated(mockBadgeData.did, mockBadgeData.civicTier);
        };
        initializeBadgeData();
    }, []);
    const handleBadgeReady = (badgeData) => {
        setBadgeReady(true);
        // Emit badge generated checkpoint
        FusionCheckpointEmitter.emitBadgeGenerated(badgeData.did, badgeData.badgeId, badgeData.zkpHash);
        console.log('🏆 Genesis Badge ready for export');
    };
    const handleDownloadBadge = async () => {
        if (!genesisBadgeData || exportInProgress)
            return;
        ...
        try {
            ...
            FusionCheckpointEmitter.emitExportComplete(genesisBadgeData.did, genesisBadgeData.badgeId, `genesis-badge-${genesisBadgeData.badgeId}.genesis.json`);
    ...
    return (<div className="container mx-auto px-4 py-8 space-y-8">
      ...
      <TruthGenesisBadge badgeData={genesisBadgeData} onExportReady={handleBadgeReady}/>

      {/* Fusion Result */}
      {fusionResult && (<Card className={`border-2 ${fusionResult.success ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : 'border-red-500 bg-red-50 dark:bg-red-900/20'}`}>
          <CardHeader>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Button onClick={handleDownloadBadge} disabled={!badgeReady || exportInProgress} size="lg" className="min-h-[48px] bg-purple-600 hover:bg-purple-700">
            {exportInProgress ? (<div className="flex items-center gap-2">
                <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"/>
                Downloading...
              </div>) : ('📥 Download Badge')}
          </Button>
          
          <Button onClick={handleReturnToDeck} variant="outline" size="lg" className="min-h-[48px]">
            🏠 Return to Deck
          </Button>
          
          <Button onClick={handleClaimNextPillar} variant="outline" size="lg" className="min-h-[48px]">
            🚀 Claim Next Pillar
          </Button>

          <Button onClick={handleViewFusionHistory} variant="outline" size="lg" className="min-h-[48px]">
            📖 View History
          </Button>
        </div>
