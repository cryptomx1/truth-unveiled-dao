    ...
    return (<div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 dark:from-slate-900 dark:to-slate-800 py-8">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          {fusionStages.map((stage, index) => (<Card key={stage.id} className={`cursor-pointer transition-all hover:shadow-lg ${stage.status === 'locked' ? 'opacity-60' : ''}`} onClick={() => handleStageNavigation(stage)}>
              <CardHeader>
              ...
              <CardContent>
                ...
                {stage.tpRequired && (<div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span>TP Requirement:</span>
                      <span className="font-semibold">{stage.tpRequired} TP</span>
                    </div>
                    {stage.userTp && (<Progress value={(stage.userTp / stage.tpRequired) * 100} className="h-2"/>)}
                  </div>)}
                
                <Button className="w-full mt-4" variant={stage.status === 'active' ? 'default' : 'outline'} disabled={stage.status === 'locked'}>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {activeAudits.length > 0 ? (activeAudits.map((audit, index) => (<div key={index} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                    <div className="flex items-center gap-3">
                      <div className={`w-3 h-3 rounded-full ${audit.hashReproduced && audit.zkpVerification ? 'bg-green-500' : 'bg-amber-500'}`}/>
                      <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {audit.anomalies.length > 0 && (<Badge variant="destructive">{audit.anomalies.length} Issues</Badge>)}
                    </div>
