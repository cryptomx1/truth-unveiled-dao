...
export const TokenDisplayTest = () => {
    const [, setLocation] = useLocation();
    const [currentDemo, setCurrentDemo] = useState('single');
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <Card className="bg-slate-800/90 border-slate-700">
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" onClick={() => setLocation('/command')} className="border-slate-600 text-slate-300 hover:bg-slate-700">
        ...
        <Card className="bg-slate-800/90 border-slate-700">
          <CardContent className="pt-6">
            <div className="flex gap-4 justify-center">
              <Button variant={currentDemo === 'single' ? 'default' : 'outline'} onClick={() => setCurrentDemo('single')} className="flex items-center gap-2">
                <Eye className="w-4 h-4"/>
                Single Token Demo
              </Button>
              <Button variant={currentDemo === 'gallery' ? 'default' : 'outline'} onClick={() => setCurrentDemo('gallery')} className="flex items-center gap-2">
                <Grid className="w-4 h-4"/>
                Gallery Demo
              </Button>
              <Button variant={currentDemo === 'preview' ? 'default' : 'outline'} onClick={() => setCurrentDemo('preview')} className="flex items-center gap-2">
        ...
        {/* Single Token Demo */}
        {currentDemo === 'single' && (<div className="space-y-6">
            <Card className="bg-slate-800/90 border-slate-700">
              <CardHeader>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8 justify-items-center">
                  {mockTokens.map((token, index) => (<div key={index} className="text-center space-y-4">
                      <RotatingTokenDisplay frontCID={token.frontCID} backCID={token.backCID} guardian={token.guardian} isGenesis={token.isGenesis} size={250}/>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2 justify-center">
                          ...
                          {token.isGenesis && (<Badge className="bg-gradient-to-r from-purple-600 to-blue-600">
                              GENESIS
                            </Badge>)}
                        </div>
              ...
              <CardContent>
                <div className="flex gap-8 justify-center items-end">
                  <div className="text-center">
                    <RotatingTokenDisplay frontCID={mockTokens[0].frontCID} backCID={mockTokens[0].backCID} guardian={mockTokens[0].guardian} isGenesis={mockTokens[0].isGenesis} size={150}/>
                    <p className="text-slate-300 text-sm mt-2">150px</p>
                  </div>
                  
                  <div className="text-center">
                    <RotatingTokenDisplay frontCID={mockTokens[1].frontCID} backCID={mockTokens[1].backCID} guardian={mockTokens[1].guardian} size={200}/>
                    <p className="text-slate-300 text-sm mt-2">200px</p>
                  </div>
                  
                  <div className="text-center">
                    <RotatingTokenDisplay frontCID={mockTokens[2].frontCID} backCID={mockTokens[2].backCID} guardian={mockTokens[2].guardian} size={300}/>
        ...
        {/* Gallery Demo */}
        {currentDemo === 'gallery' && (<TruthCoinsGallery showGenesis={true} size="medium" viewMode="grid"/>)}

        {/* Module Preview Demo */}
        {currentDemo === 'preview' && (<DeckModulePreview modules={[]} showTokenRewards={true}/>)}
