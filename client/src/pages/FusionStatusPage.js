...
export const FusionStatusPage = () => {
    const { cid } = useParams();
    const [fusionRecord, setFusionRecord] = useState(null);
    ...
    const loadFusionRecord = async (recordCid) => {
        setLoading(true);
    ...
    const calculateProgress = () => {
        ...
        const completedSteps = fusionRecord.auditTrail.filter(entry => entry.status === 'completed').length;
        return (completedSteps / totalSteps) * 100;
    };
    if (loading) {
        return (<div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 dark:from-slate-900 dark:to-slate-800 py-8">
    ...
    if (!fusionRecord) {
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="text-center py-16">
            ...
            <Button onClick={() => window.location.href = '/fusion/dashboard'} className="mt-4">
              Return to Dashboard
            </Button>
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <Button variant="outline" onClick={() => window.location.href = '/fusion/dashboard'}>
              ← Back to Dashboard
            </Button>
            <Button onClick={refreshRecord} disabled={refreshing} variant="outline">
        ...
        {/* Metadata */}
        {fusionRecord.metadata && (<Card className="mb-8">
            <CardHeader>
              <CardTitle>Record Metadata</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {fusionRecord.metadata.inputTp && (<div className="text-center p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
                ...
                {fusionRecord.metadata.outputTokens && (<div className="text-center p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
                ...
                {fusionRecord.metadata.pillarsCompleted && (<div className="text-center p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {fusionRecord.auditTrail.map((entry, index) => (<div key={index} className="flex items-start gap-4 p-4 border rounded-lg">
