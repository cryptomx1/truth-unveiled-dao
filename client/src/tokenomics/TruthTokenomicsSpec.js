 ...
 */
export function canAccessTier(cid, tokenBalance, repScore, stakedAmount = 0) {
 ...
 */
export function calculateAccessibleTier(cid, tokenBalance, repScore, stakedAmount = 0) {
 ...
 */
export function getUnlockableCIDTiers(tokenRole) {
 ...
 */
export function getRewardMultiplier(tokenRole) {
    const tierSpec = TruthTokenomicsTiers.find(tier => tier.tier === tokenRole);
 ...
 */
export function getUserTokenomicProfile(cid) {
 ...
 */
export function getTruthPointsToNextTier(currentTier) {
...
export const TruthTokenomicsUtils = {
     ...
     */
    getAllTiers: () => Object.values(TruthTokenRole),
    /**
     * Gets all tier specifications
     */
    getAllTierSpecs: () => [...TruthTokenomicsTiers],
    /**
     * Gets mock user profiles for testing
     */
    getMockProfiles: () => ({ ...mockUserProfiles }),
    /**
     * Adds a mock user profile for testing
     */
    addMockProfile: (cid, profile) => {
     ...
     */
    simulateTierProgression: (cid, truthPointsGained, reputationGained) => {
        ...
        const newTier = calculateAccessibleTier(cid, newBalance, newReputation, profile.stakedAmount || 0);
        return {
            oldTier,
            newTier,
 ...
 */
export function createTokenomicsCIDIntegration(cid) {
