...
class Deck10FeedbackSync {
    ...
    initializeFeedbackSync() {
        ...
        if (storedFeedback) {
            this.feedbackHistory = JSON.parse(storedFeedback).map((item) => ({
                ...item,
        ...
        if (storedAlignment) {
            this.daoAlignmentData = JSON.parse(storedAlignment).map((item) => ({
                ...item,
    ...
    syncFeedbackWithAlignment() {
        let syncedEntries = 0;
        const highDissonanceReps = [];
        // Update alignment data based on recent feedback
        this.daoAlignmentData.forEach(repData => {
            const recentFeedback = this.feedbackHistory.filter(feedback => feedback.representativeId === repData.representativeId &&
                feedback.timestamp > new Date(Date.now() - 24 * 60 * 60 * 1000) // Last 24 hours
            );
            if (recentFeedback.length > 0) {
                // Calculate weighted sentiment from feedback
                const avgFeedbackSentiment = recentFeedback.reduce((sum, feedback) => sum + feedback.sentiment, 0) / recentFeedback.length;
                // Update constituent sentiment based on feedback
                repData.constituentSentiment = Math.round((repData.constituentSentiment * 0.7) + (avgFeedbackSentiment + 100) * 0.5 * 0.3);
     ...
     */
    getFeedbackSummary(representativeId) {
        ...
        const recentFeedback = feedback.filter(entry => entry.timestamp > new Date(Date.now() - 24 * 60 * 60 * 1000));
        return {
     ...
     */
    getSyncStatus() {
        const dissonanceScores = this.daoAlignmentData.map(rep => Math.abs(rep.constituentSentiment - rep.publicVotingRecord));
