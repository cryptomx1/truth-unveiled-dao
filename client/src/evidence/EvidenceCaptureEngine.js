...
export class EvidenceCaptureEngine {
    ...
    captureMissionCompletion(userId, userTier, trustScore, missionId, missionTitle, deckId, deckCategory, sourceTraceHash) {
        const context = {
            userId,
            userTier,
            trustScore,
    ...
    captureCivicInteraction(userId, userTier, trustScore, interactionType, deckId, sourceTraceHash, additionalData = {}) {
        const context = {
            userId,
            userTier,
            trustScore,
    ...
    verifyProofCapsule(capsule) {
        const errors = [];
        // Check integrity hash
        const expectedIntegrityHash = this.generateIntegrityHash(capsule.evidenceDigest, capsule.userHash, capsule.timestamp);
    ...
    generateIntegrityHash(evidenceDigest, userHash, timestamp) {
    ...
    generateMockIpfsHash(eventId) {
        const hash = this.simpleHash(eventId);
        return `Qm${hash}${Math.random().toString(36).substring(2, 10)}`;
    }
    // Determine completion status
    determineCompletionStatus(context) {
        if (context.userTier === 'Administrator')
            return 'verified';
    ...
    determineValidationLevel(context) {
        if (context.userTier === 'Administrator')
            return 'dao_verified';
        if (context.userTier === 'Governor')
            return 'civic_grade';
        if (context.userTier === 'Moderator')
            return 'enhanced';
        return 'basic';
    }
    // Generate mock witnesses
    generateMockWitnesses(userTier) {
        const witnesses = [];
    ...
    formatInteractionTitle(interactionType) {
        const titleMap = {
    ...
    // Infer deck category
    inferDeckCategory(deckId) {
        if (deckId.includes('wallet'))
            return 'wallet';
        if (deckId.includes('identity'))
            return 'identity';
        if (deckId.includes('governance'))
            return 'governance';
        if (deckId.includes('consensus'))
            return 'consensus';
        if (deckId.includes('feedback'))
            return 'feedback';
        if (deckId.includes('education'))
            return 'education';
        return 'general';
    }
    // Validate evidence digest format
    validateEvidenceDigest(digest) {
        return digest.startsWith('evidence-') && digest.length >= 17;
    }
    // Get proof capsule by ID
    getProofCapsule(eventId) {
        return this.captureHistory.get(eventId) || null;
    }
    // Get all proof capsules for user
    getUserProofCapsules(userHash, limit) {
    ...
    getCapsulesByEventType(eventType, limit) {
    ...
    getCapsulesByMission(missionId, limit) {
        const filtered = Array.from(this.captureHistory.values())
    ...
    getCaptureStatistics() {
        ...
        const recentActivity = capsules.filter(capsule => new Date(capsule.timestamp).getTime() > oneDayAgo).length;
        return {
