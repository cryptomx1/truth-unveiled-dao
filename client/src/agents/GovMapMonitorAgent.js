...
class GovMapMonitorAgent {
        ...
        try {
            ...
            if (!isOnline) {
                ...
                if (LLMAgentCore.isEnabled()) {
                    try {
                        const llmResponse = await LLMAgentCore.classifyAPIError(status.errorMessage, endpoint.name);
    ...
    async performFullHealthCheck() {
        ...
        const checkPromises = this.endpoints.map(endpoint => this.checkEndpoint(endpoint).catch(error => ({
            endpoint: endpoint.name,
            status: 'offline',
        ...
        const healthStatuses = results.map(result => result.status === 'fulfilled' ? result.value : result.reason);
