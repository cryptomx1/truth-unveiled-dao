    ...
    async initializeAllAgents() {
        ...
        try {
            ...
            if (typeof window !== 'undefined') {
                window.civicAgentSystem = this.agentSystem;
                window.ttsEngine = ttsAgent;
                window.rippleCampaignEngine = campaignEngine;
                window.llmPromptEmitter = llmEmitter;
                window.truthCoinClaimEngine = claimEngine;
                window.genesisFusionEngine = this.agentSystem.genesisFusionEngine;
                window.trustFeedbackEngine = trustEngine;
                window.trustSentimentAggregator = sentimentAggregator;
                window.trustSentimentMonitor = sentimentMonitor;
                window.feedbackOrchestrationEngine = orchestrationEngine;
                window.productionSimulationEngine = simulationEngine;
                window.zkpMintTriggerAgent = mintTriggerAgent;
                window.trustFusionOrchestrator = fusionOrchestrator;
            }
        }
        catch (error) {
