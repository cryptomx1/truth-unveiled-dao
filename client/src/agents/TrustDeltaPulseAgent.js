...
export class TrustDeltaPulseAgent {
    ...
    async sweepDeckVolatility(deckId, history) {
    ...
    determineThreshold(deltaChange) {
        if (deltaChange >= this.THRESHOLDS.critical)
            return 'critical';
        if (deltaChange >= this.THRESHOLDS.high)
            return 'high';
        if (deltaChange >= this.THRESHOLDS.moderate)
            return 'moderate';
        return 'low';
    }
    identifyTriggerEvents(deckId, volatility, delta) {
        const events = [];
        // Governance-specific triggers
        if (deckId === 'governance_feedback') {
            if (delta > 20)
                events.push('policy_vote_surge');
            if (volatility > 30)
                events.push('representative_action_spike');
        }
        // Wallet/Identity triggers
        if (deckId === 'wallet_overview' || deckId === 'civic_identity') {
            if (delta > 15)
                events.push('citizen_engagement_shift');
            if (volatility > 25)
                events.push('trust_verification_activity');
        }
        // Treasury triggers
        if (deckId === 'treasury_management') {
            if (delta > 18)
                events.push('truthpoint_allocation_change');
            if (volatility > 35)
                events.push('dao_reward_distribution');
        }
        // General triggers
        if (delta > 25)
            events.push('critical_trust_delta');
        if (volatility > 40)
            events.push('system_wide_volatility');
        return events;
    }
    async processSweepResults(detections) {
    ...
    calculateAggregateRisk(detections) {
        if (detections.length === 0)
            return 0;
    ...
    async transmitDAOBroadcast(broadcast) {
        try {
...
if (typeof window !== 'undefined') {
    window.trustDeltaPulseAgent = TrustDeltaPulseAgent.getInstance();
}
