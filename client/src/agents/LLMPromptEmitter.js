...
export class LLMPromptEmitter {
    ...
    async generateCivicMessage(request) {
    ...
    async generateWithGPT4o(request) {
...
Respond with JSON in this format: {"message": "generated message", "confidence": 0.85}`;
    ...
    generateWithFallback(request) {
        const templates = this.fallbackTemplates.filter(t => t.topic === request.topic && t.tone === request.tone);
    ...
    async testGeneration(topic, tone) {
