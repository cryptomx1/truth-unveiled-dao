...
const ClaudeGuardAgent = () => {
    ...
    return (<div className="hidden" role="region" aria-label="Claude Guard Agent">
      ...
      {process.env.NODE_ENV === 'development' && (<button onClick={triggerHallucinationAlert} className="fixed bottom-4 right-4 p-2 bg-red-500 text-white text-xs opacity-20 hover:opacity-100" aria-label="Test hallucination detection">
          Test Alert
        </button>)}
    </div>);
};
