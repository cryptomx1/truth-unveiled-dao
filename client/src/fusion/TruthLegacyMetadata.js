 ...
 */
import { TruthCoinPillar, getTruthCoinsProfile } from '../tokenomics/TruthCoinsIntegration';
 ...
 */
const legacyMetadataRegistry = {};
/**
 * Mock pillar achievement records for development
 */
const mockPillarAchievements = [
    {
        cid: 'commander-hash',
 ...
 */
export function recordLegacyFusion(fusionDetails) {
 ...
 */
export function getLegacyMetadata(cid) {
 ...
 */
export function exportLegacyMetadata(cid) {
 ...
 */
export function generateLegacyHash(fusionDetails) {
 ...
 */
export function createVaultStateSnapshot(cid) {
 ...
 */
export function createTruthCoinSnapshot(cid) {
 ...
 */
export function recordTruthPointsBurn(burnRecord) {
 ...
 */
export function getTruthPointsBurnHistory(cid) {
 ...
 */
export function getPillarAchievementHistory(cid) {
 ...
 */
function generateExportHash(legacyEntry) {
 ...
 */
function getAccessibleRoutesForTier(tier) {
    const routeMap = {
 ...
 */
export function getAllLegacyMetadata() {
...
export function getGlobalLegacyStats() {
    ...
    const genesisRecords = allEntries.filter(entry => entry.fusionDetails.truthCoinSnapshot.ownsGenesis).length;
    const totalExports = allEntries.reduce((sum, entry) => sum + entry.exportMetadata.exportCount, 0);
    const crossForkCompatible = allEntries.filter(entry => entry.legacyPreservation.crossForkVerifiable).length;
    const daoTransferEligible = allEntries.filter(entry => entry.legacyPreservation.daoTransferEligible).length;
 ...
 */
export function validateLegacyMetadata(cid) {
    const entry = legacyMetadataRegistry[cid];
    if (!entry) {
        return false;
    }
...
export const LegacyMetadataUtils = {
     ...
     */
    createMockLegacyMetadata: (cid) => {
        const mockFusionEvent = {
