 ...
 */
import { registerVote } from '../dao/DAOVoteEngine';
...
class MockBlockchainSubmitter {
    ...
    static validateChainPackage(chainPackage) {
        const errors = [];
        try {
    ...
    static estimateProcessingTime(chainPackage) {
...
export class ZKPProofRegistrar {
    ...
    async submitProofToDAO(chainExportResult, progressCallback) {
    ...
    updateProgress(submissionId, progress) {
    ...
    async validateProofPackage(chainPackage) {
        try {
    ...
    estimateSubmissionCost(chainPackage) {
        try {
